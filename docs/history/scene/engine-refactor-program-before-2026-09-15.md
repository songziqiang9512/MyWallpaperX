# 引擎工程旧计划与实验记录（截至 2026-09-15）

<!-- document-role: historical-evidence -->

> **历史证据 — 非现役入口**
>
> 保存基点：`a7863c3e0bf5c2d7f134c7378aff1d13424a5c10`。正文保留原计划与实验叙事，仅调整相对链接及旧角色标记；其中的“现在”、M0–M6 顺序与执行命令均属于当时，不是现役指令。
>
> 当前工程顺序与后继设计：[重构执行档案](../../scene/engine-refactor-program.md)。能力现状：[能力台账](../../scene/semantics/coverage-ledger.md)。本文件保存 completion 唤醒回归、validation memo 撤回、daemon 迁移、编译和解码消融等独有历史证据，不证明后续 HEAD 的性能或验收。

---

# Scene 引擎重构工程计划（Engine Refactor Program）

<!-- 原记录角色：active-plan；现已由文件头的 historical-evidence 撤权。 -->
