...
| 自动门（当前增量） | interpretation format 29，签名 App、Metal 双帧；十二类 strict backend/ordered chain、Puppet、B0 property、dynamic text、provider/file override、Particle 与 stock TEX resolver 合同；435 项 Scene 测试为 433 通过、2 跳过；签名构建和严格验签通过 | 45 样本完整快照与固定 13 样本 PASS 是 `678a052` 历史门；当前本机固定门仅 6/13 样本可用且矩阵仍为 v25；六样本 image 82/82、particle 11/13、无 stock 资源诊断，但整门不成立；preview 不设绝对阈值，仍没有 Windows WE 同配置录屏差异门 |
