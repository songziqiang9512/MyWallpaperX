...
- `2938612768` execution contract not satisfied + passthrough 激活证据缺失——**2026-09-25 取证定性：执行健康、剩余为矩阵期望漂移**。带 observation 复跑（`--no-execution-observations` 会使 executor 证据族全灭，28 项失败中该族是运行模式伪影）：executor claimed=22 encoded=22 failures=0 localFallbacks=0；`utilityNamedTargetGapCount: 0`——79719606 排除本样本时的真实缺口（层 1513 命名绑定未成功）已被后续批次修复（1513 现在 planned 名单内）。剩余五族漂移：①utility named_* 计数增长（consumers 7→9、target_planned 2→6、binding_planned 2→9，gaps 1→0）；②13 个 authored effect graph 计数器期望对应已退役插桩（`authoredEffectGraph*Count` 行 app 源码零存在，actual 恒 None≠0）；③passthrough activation/next-frame 证据 missing/malformed（succeeded IDs 交集收缩的约束源）；④runtime evidence effect_graph 计数+sha256 快照漂移；⑤text script binding 计数/层 ID 漂移。**收尾配方**：先解决③（查 passthrough 证据 emission 的 missing/malformed 具体层），再手改 full matrix 本样本条目（succeeded IDs/utility 计数/effect_graph sha/text bindings）+ wrapper override remove 补 13 个计数器键 + base_matrix_sha256 同步，复跑到绿后走 79719606 同款验证。刷新器护栏现状：all 域要求全绿报告、retire 域要求失败仅限计数器类，本样本两者皆不满足，需先消③或扩 retire 流程。
