...
- **P2 视觉保真缺口（2026-09-18 用户观察）**：粒子集群样本中**流星/拖尾效果的样式与官方不符**（用户实机观察）。已知线索：集群样本含 eventfollow trail 子粒子链（如 2974757317 的 matrix_code→matrix_trail，type=eventfollow）。下一批诊断：对比实际渲染与官方预览，定位 trail 粒子的样式差异来源（sprite 形态/运动轨迹/生命周期/发射密度），再按公共 owner 修复。
