...
- **现役开放项（按序，各含下一步）**：
  ...
  5. **SceneScript host 对象模型面已补齐（2026-09-18 批 A）**：`thisObject` 对 `.layer`/`.text` 属性 = 层自身对象（同一 JS 对象、写入走既有 mutation journal）、层句柄 `getChildren()`、own-layer 句柄补 `alpha`/`color` 写与 `getAnimation`（官方 `ILayer extends IObject`）。验证=host harness 新断言全绿 + 注册门 `test_scene_property_vector_script` 20/20（含反例先行：临时移除自身层 `getAnimation` 即 FAIL）+ 隔离真实回放（`3788066613` 三次 PASS，异常从 `not a function` 推进到 authored `sortLayer` 拒绝）。证据见 [E-2026-09-18-SCENESCRIPT-OBJECT-MODEL-LAYER-OBJECT](semantics/runtime-evidence-current.md#e-2026-09-18-scenescript-object-model-layer-object)。
     **批 B 实现方案（2026-09-18 精化，替换早前草案的 DTO 管线设计）**：探明三件事。①**根因在上游准入**：脚本候选路径只收 `.effectConstant`/layer/text/particle 目标，`.effectVisibility` 目标的绑定根本不会成为程序绑定（`SceneScriptQuickJSProgramCandidate` 无该 case；`SceneScriptScalarProgram` 候选循环同样 `default: continue`）——这就是 8 样本 0 次派发的根因，`SceneScriptMediaEventMutations`/标量 dispatch 机器本身是现成的。②**发布可复用标量值通道，无需新 DTO**：`SceneSurfaceEvaluationTransaction.evaluate` 直接吃 `sceneScriptValues: [SceneDynamicTarget: SceneDynamicValue]`，`.effectVisibility` 是合法 target，且 effect-activation 消费者现成（user-property 直通布尔已走通）。③C 侧按"暂存-直通"最小实现：新 API `mwx_scene_quickjs_owner_configure_effect_visibility_target(layer_id, effect_index)`（配置时在 object_handle 上定义可写 `visible`：getter 返回暂存值或 true，setter 仅收布尔并暂存），`update_primitive_callback` 尾部若 result==OK 且有暂存则 `*output = staged`（读后清），commit/discard 亦清。**实现清单**：候选准入（scalar bool 程序收 `.effectVisibility`）→ Swift 三运行时调 configure → `valuePublishingTargets` 收纳 → C 暂存/直通 → 夹具（事件帧发布 bool、非事件帧不发布）→ 双执行核对（`SceneInitialMediaEffectVisibilityProjection` 的 load 期预标记与实时值一致性；初始事件语义确认后再评估退役）。**边界**：仅 `.effectVisibility` 目标；`thisObject.solid` 仍为显式边界。
