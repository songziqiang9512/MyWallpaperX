...
- **批 G（2026-09-18，归因已完成，待实现的设计批）**：`3122339805` layer 478「Eyes」tint（`478#effect#480`）从未执行。**机制定案**：478 的父层 144「Eyes Window」作者 `visible: false` 且 `displayScriptOwnership.visible == true`（脚本拥有可见性，`applyUserProperties` 按 `hideeyeswindow` 开关显示窗口）；启动期路由准入 `SceneResolvedMaterialExecutionCapabilityAdmission.executionSourceRoute` 用 `SceneLayerVisibility.visibleLayerIDs(in: descriptor)`（snapshot=nil）沿父链判可见性，脚本拥有可见性的层在无快照时回退作者值 false → 整链判死 → `execution-route-layer-hidden` 永久拒绝；运行期 effective 值 `hideeyeswindow=false`（窗口应显示），批 F 已让 144 的 owner 在帧 0 派发并把 144 点亮（`input=false output=true`，窗口可见部分已恢复），但路由是启动冻结的，tint 仍死。**候选修法（Option A）**：snapshot==nil 且 `ownership.visible == true` 时启动可见性按"乐观可见"处理（作者值只是种子；已准入路由的每帧行为仍由 per-frame stage activation 按真实快照把关，隐藏期行为不变，脚本点亮后路由可恢复）。**影响面**：启发式普查（1-2 层父链）**20 个样本**存在此类链（`2902406982`/`2938612768`/`2959875782`/`3122339805`/`3211615441`/`3238423642`/`3396722575`/`3448845950`/`3470948192`/`3509243656`/`3601964477`/`3610154602`/`3612199597`/`3612795410`/`3662790108`/`3665307769`/`3768229922`/`3768724269`/`3779026256`/`3788066613`）。**验收门**：需独立设计审查（与 initially-inactive-passthrough 机制的衔接、per-frame 隐藏强制在全部 route 形态上成立的证明）；实现后 20 样本全量定向回放+逐个 triage 新暴露面；与 X-Ray 条件内容 P3（队列现役项 1）同族，可共用关闭门。
