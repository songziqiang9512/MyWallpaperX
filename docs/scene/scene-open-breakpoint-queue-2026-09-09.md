...
2026-09-13 PreparedProduct / Puppet GeometryProduct **本切片闭合**。静态 bind pose 与动画 pose 共享 `SceneGeometryProduct`；mesh 保留原始像素位置和 UV，层级 transform、作者 origin/scale、pivot、parallax 与 camera VP 只合成一次 world MVP。无 effect 时 mesh 直接进入唯一 compositor；有 effect 时 atlas 先按自身 mapped extent 进入普通 graph，graph-final 纹理再由同一 mesh 采样。旧 coverage 归一化与大纹理、逐帧重组 target、尺寸/budget/cache 补丁、`.puppet` layer-source identity 和 passthrough 已删除。当前签名 Debug App 的 fresh 四样本定向回放为 **4/4 strict PASS**；`3665307769 / 3780119725 / 3264246690 / 3238423642` 的稳定帧已逐张检查，角色组合、世界位置、大小、头部完整性和代表性颜色合成均恢复，现场为 `/private/tmp/mwx-puppet-world-final-20260913-v3`。`3780119725:279` 的 package 引用 mesh 但实际未包含该文件，因此仅这个“作者资源确实缺失”边界允许局部普通纹理降级；非法路径及其他读取、解析、验证、分配或 encode 失败不能把 atlas 当组合输出。命名依赖目前只表达完整栅格层源，Puppet provider 在 plan compilation 阶段明确拒绝，不通过重新压平补齐。完整身份、hash 和未验证边界见 [coverage ledger](semantics/coverage-ledger.md)与[当前运行证据](semantics/runtime-evidence-current.md#e-2026-09-13-puppet-world-geometry)。
...
- **2026-09-12 视觉质量断点第一切片（历史，Puppet 部分已由 2026-09-13 世界空间绘制取代）**：维护者当时报告"很多样本纹理像被压缩再拉大；木偶动画层边缘锯齿与摩尔纹"。该切片为直接图片上传和当时的 `ScenePuppetMeshRecomposer` 中间纹理生成 mip，checkpoint 与两样本结构回放通过，但没有取得清晰度验收。现役 Puppet GeometryProduct 不再创建 mesh recompose 中间纹理，因此本段的 Puppet 路径、待复核状态和后继建议均已退役；直接图片的 mip 修复仍保留，authored `nomip`/`halfmip` 状态位接入仍是独立次级项。

- **2026-09-12 视觉质量断点第二切片（历史，已由 2026-09-13 世界空间绘制取代）**：该切片曾用尺寸 ceiling 和更高预算缓解 coverage 压平欠采样，但仍保留错误的中间纹理架构；这些补丁已随本页顶部的 GeometryProduct 闭环删除，不再代表现役实现或建议方案。

- **2026-09-12 视觉质量断点第三切片（历史，已退役）**：该切片证明 coverage×scale 压平对 `3780119725:21` 需要约 478 MiB、预算内只有 0.535 密度，并定位到 `3264246690` 头部修复引入的 coverage 归一化路径。它曾以 256 MiB/8192 ceiling 缓解其他层，但仍无法满足清晰度。2026-09-13 已删除这条压平路径和全部尺寸/budget 补丁，改为 atlas graph → raw/deformed mesh world draw；本条只保留根因 provenance，不再是待办或实现建议。维护者指示的不做全量样本验证继续有效。
