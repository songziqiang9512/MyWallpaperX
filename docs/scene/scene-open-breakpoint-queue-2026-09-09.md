...
- **当前复跑**：`3792249095` 的 `degraded-layer-source-passthrough` 是唯一 route operation，六个其它 accepted layer 均有 GPU/compositor/next-frame；`3448845950` 的 layer 1475 仍记录 `material-generic-owner-revoked`。这两项应作为 owner-local 修复，不能并入 B2 代表结论。
- **进一步归因**：`3448845950` 的 layer 1475 不是普通 provider 缺失：其 authored effect 是两个 material pass + 一个 `copy`/combine command，内部 FBO `_rt_FullCompoBuffer1/2` 形成 persistent feedback；当前日志为 `authoredNodes=3/materialNodes=0/copyNodes=0/rejectedNodes=3`。下一批应在现有 `SceneAuthoredEffectRenderPlanner`、`SceneGraphNodeScheduler`/`SceneGraphCommandRuntime`、`SceneOffscreenTextureAllocationCache+SharedPair` 与 `ScenePreservedChannelHistory` 上补有界 feedback graph admission，保留 command order、FBO、previous-state、generation/completion；不得放宽 generic-owner veto。`3792249095` layer 254 的 `crt_scan_line` 则是同层 self-composite binding，`refraction` 是独立 sampler-schema 断点，分别归 B2/B4。
