...
- **2026-09-12 视觉质量断点第二切片：木偶"先压后拉"根因闭合（用户复查后定位）**：mipmap 修复后维护者确认模糊仍在——取证将根因从采样质量转向**捕获分辨率**：① `phase=puppet-source-published layer=21 texture=2912x3840 logical=7701x10154`——木偶 mesh coverage 远超层 quad，`ScenePuppetMeshRecomposer.targetDimensions` 按 coverage 像素预算（128MB 全局共享）把 3780119725 人物压到 0.38× 分辨率，每帧 `ScenePuppetPlaybackState` 渲染 target 同样被压（动画人物持续模糊）；② effect 链捕获用 authored 面尺寸（3874×2000），显示时再乘 scale 1.24 拉伸。修复：**捕获/重合成尺寸一律钳制到层在 canvas 上的显示 extent（renderSize × max(1,|scale|)，`displayExtentCeiling`）**——recompose 与 playback target 只降分辨率不改 coverage UV/几何；`recomposeByteBudget` 128→256MB；recomposer/pool 的 4096 维度上限升到 8192（Apple GPU 16384，字节预算仍约束总量）；preflight 的非 solid 捕获 extent 取 max(authored, 世界矩阵列长=canvas 显示宽高，128px 向上量化)。实测：L21 源 2912×3840→**4791×2473**（显示 extent 命中）、3665307769 L153 4667×4672→**3992×1974**、L71 高度钳到 2006；两样本严格 PASS。验证：checkpoint build、puppet/pool/focused 模块 ALL OK；全 corpus 5 路复验随后登记。**最终视觉确认仍需维护者复核**（分辨率账面已 1:1，剩余差异若有则属纹理源/相机链）。

- **2026-09-12 视觉质量断点第三切片：coverage×scale 密度语义 + 方向修正（维护者裁决）**：维护者确认 3665307769 摩尔纹已解决，但 3780119725/3264246690 人物仍模糊，并指出①根因首次出现于 3264246690 头部缺失修复（coverage-ledger L127：coverage 改为"作者 size∪bind-pose 并集、origin 居中"逻辑画幅）；②连续加预算不是方向，压平中间纹理本身就是设计缺陷。据此把上一版的 authored-based ceiling **替换**（非叠加）为 coverage×|authored scale|（合成器实际按 coverage 画幅铺四边形，这才是真实显示密度），预算/维度上限不膨胀（256MB/8192）。实测：**3264246690 L389 = 2552×2183 精确 1:1、3665307769 L71/L153 全 coverage 1:1**，三样本严格 PASS、puppet 4 模块 ALL OK；3780119725 L21（coverage 7701×10155×scale≈9528×12561）压平需 478MB、预算内 0.535 密度——**登记为下一主切片：世界空间蒙皮直绘**（官方 §10 行为：mesh 每帧按世界 MVP 直绘层工作目标，与粒子同构，天然 1:1、约 53MB 视口有界；删除压平中间层而非加预算）。维护者指示：不再每次全量样本验证，验证按最小定向集执行。
