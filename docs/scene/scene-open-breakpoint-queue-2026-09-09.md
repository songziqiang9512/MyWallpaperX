...
- **P1 下一批选定（2026-09-18，type-error 集群归因）**：scenescript type-error ×4（layer:alpha/visibility 子簇）的异常消息已捕获——
  ...
  **公共根因 = SceneScript 对象模型缺口**：脚本引用的跨脚本 `shared` 状态、按引用的文本层对象、texture animation API 接收者，
  在 VM typed host bridge 中未暴露 → null/undefined → TypeError → 脚本失效回退 current-frame-lower-priority。
  下一批 = 按 §8.4 扩展 VM 对象模型面（named layer 引用 / texture animation API / shared 状态），以失败脚本为证据逐 API 验收。
  同簇其余：effectConstant ×2（3448845950/3747492842）、visibility ×1（3788066613）待同法归因。
