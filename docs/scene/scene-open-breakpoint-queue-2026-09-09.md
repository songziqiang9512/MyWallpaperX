...
- **新候选批 G（2026-09-18，视频层效果路由启动期死亡）**：`3122339805` layer 478「Eyes」（作者默认可见的 video/image 层）的 tint 效果从未执行过：启动期 `capability-admission … execution-route-layer-hidden`，之后永不恢复；recovery 观察门无法清除它，因为视频层不产生 graph-execution（"下一帧成功"不可观测）。基线静默（PASS 掩盖）、批 F 后编码为 degraded passthrough（FAIL 可见）。下一步：归因"启动期为何判 hidden"（video provider 未就绪被当作 layer-hidden？）与路由恢复语义，为视频源效果建立可恢复的 admission；同时评估观察门对 video-provider 支撑源的恢复语义（与时间门控层同属 gate-scope 候选）。
