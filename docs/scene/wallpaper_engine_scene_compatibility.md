...
SceneScript、粒子、Bloom/HDR、Timeline、Puppet Warp 都是重要能力方向，但官方没有给出它们在 Workshop 热门壁纸中的覆盖率统计。具体项目仍应按隔离样本结构化 census、主构图影响和可验证性排序；MyWallpaperX 当前只实现 bounded named-target、property fallback、受限 PNG/JPEG property source、官方 Effect 行中的 14 个 `L3` 子集、exact Workshop Shadow/stock Opacity/stock Shake/Water Waves/Water Flow/gradient-color/layer-Bloom 等受限 profile、八个 strict graph backend 的 all-supported ordered chain、9 个确定性 built-in 粒子 key 和 Sprite Trail 等边界，不能据此宣称完整 sceneTexture、Scene、Effect、Particle 或 Wallpaper Engine 兼容。新增 Water Waves 与 Water Flow 仍只是 exact `L3 executed-degraded`，不是通用 authored shader 或像素等价。样本封面可作为固定画布上的主构图/色调方向性门，WaifuX SceneBake MP4 可补充动态方向性参考，但二者都不能替代 WE 同配置动态或像素 golden；当前精确等级统一查 [覆盖台账](semantics/coverage-ledger.md)。
