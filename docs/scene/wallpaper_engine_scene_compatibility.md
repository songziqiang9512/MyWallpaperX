...
SceneScript、粒子、Bloom/HDR、Timeline、Puppet Warp 都是重要能力方向，但官方没有给出它们在 Workshop 热门壁纸中的覆盖率统计。具体项目仍应按隔离样本结构化 census、主构图影响和可验证性排序；MyWallpaperX 当前只实现 bounded named-target、property fallback、受限 PNG/JPEG property source、官方 Effect 行中的 12 个 `L3` 子集、exact Workshop Shadow/stock Opacity/gradient-color/layer-Bloom 等受限 profile、五个 strict graph backend 的 all-supported ordered chain、9 个确定性 built-in 粒子 key 和 Sprite Trail 等边界，不能据此宣称完整 sceneTexture、Scene、Effect、Particle 或 Wallpaper Engine 兼容。当前精确等级统一查 [覆盖台账](semantics/coverage-ledger.md)。
