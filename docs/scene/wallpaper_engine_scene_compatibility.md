...
SceneScript、粒子、Bloom/HDR、Timeline、Puppet Warp 都是重要能力方向，但官方没有给出它们在 Workshop 热门壁纸中的覆盖率统计。具体项目仍应按隔离样本结构化 census、主构图影响和可验证性排序；MyWallpaperX 当前只实现 bounded named-target、property fallback、受限 PNG/JPEG property source、官方 Effect 行中的 18 个 `L3` 子集、含 stock Radial God Rays 在内的十四类 strict graph backend、受限 Timeline/audio、stock 粒子 TEX 与 Sprite Trail 等边界，不能据此宣称完整 sceneTexture、Scene、Effect、Particle 或 Wallpaper Engine 兼容。新增 profile 仍只是 exact `L3 executed-degraded`，不是通用 authored shader 或像素等价。样本封面可作为固定画布上的主构图/色调方向性门，WaifuX SceneBake MP4 可补充动态方向性参考，但二者都不能替代 WE 同配置动态或像素 golden；当前精确等级、运行门与 App 身份统一查 [覆盖台账](semantics/coverage-ledger.md) 和 [运行证据索引](semantics/runtime-evidence-index.md)。
