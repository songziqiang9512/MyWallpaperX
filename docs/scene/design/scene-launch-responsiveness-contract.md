...
Scene 详情的齿轮次级菜单提供“播放诊断”入口。打开诊断面板只显示说明或已存在的结果，初始状态为 `idle`，不开始诊断；只有用户点击“开始诊断”才可创建诊断请求。

详情先显示本地元数据。缺少作者昵称或链接时可异步补充公开作品资料，沿用统一 Steam 获取链的请求取消与身份检查；不得借元数据补全触发 Scene 文件解析或 preparation。
