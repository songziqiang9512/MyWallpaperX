...
15. **唯一下一项仍是 R4 旧 owner 迁移，而非转样本或进 R5**：按当前已被 unified Program/executor 覆盖的下一 capability family 比对 dedicated probe、backend case、planner/profile/hash/path selector、renderer dispatch 与旧 telemetry；每波同提交接管并撤权，以跨样本正反门证明没有像素 owner 漂移。样本 ID/layer/path/hash 只用于证据，绝不选择算法；普通波次优先定向门，只有共享风险无法排除、矩阵合同变更或里程碑才运行 fixed13/full45。
