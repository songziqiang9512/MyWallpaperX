...
> 建立日期：2026-08-03
>
> 文档状态：现役执行计划；更新至 2026-08-10。R4-A 四类 partial recovery 已删除，R4-C 的 rejected-authored-graph legacy neutralization 切片已落地；R4 仍为 `partial`，R5 为 `not_started`。
>
...
- **架构判断**：新代码已朝“样本声明需求 -> typed capability -> 公共调度/资源 -> 公共执行/合成”收敛，capability 现在直接来自 raw authored graph admission，审计的新链没有按 sample/workshop/path/hash 选择可见算法；effect classification 只在资源加载后投影为 telemetry family，new capability 不再以它作为 admission authority。现役静态合同已禁止 rejected authored graph 回落到 `SceneEffectRuntimePlanner` 的 legacy effect plan/inputs，也已删除显式 prefix/suffix/isolation recovery；没有 authored graph 的层仍保留旧 planner，完整 accepted graph 仍可由旧 whole-chain renderer 执行。严格独立准入的 ImageBlend source preparation 与 typed named dependency 仍按各自合同和独立 telemetry 作为 R4-4 前的产品 owner；它们不提升 authored-chain admission，也不恢复 unsupported sibling。产品整体因此尚未形成唯一能力池，也不能表述为已与官方架构对齐。
- **阶段边界**：当前停在 R4，不进入 R5。逐帧 CPU 热点、owner matrix/lifecycle、独立 authored-shader 产品 owner 撤权、统一材质 pass 方向回归、Program-compatible Tint/Film Grain、九段authored Program、Light Shafts当前正例、Water/Shimmer stage-local像素证据、bounded orthographic Scene Camera Shake、stock standalone Blend，以及Simple Audio Bars ordinary 64-band、无child/dependency composition默认32-band与relocated 16-band + zero-distortion Fisheye正例的owner migration已闭合；三个旧Simple专用profile已有Program正证据后，其专用planner/profile/pipeline/renderer产品owner也已删除。四类 partial recovery 现已归零；真正剩余的R4产品面包括旧whole-chain root、无图 legacy planner GPU authority、Light Shafts main-pass bypass及script/profile行为selector。GraphExecutor 可接的 typed pair leaf 仅为 Shift Hue、增强版 Audio Bars、Workshop Gradient、Workshop Shadow、Shake 和 zero-distortion Fisheye；Blend、Film Grain、Depth 等完整 accepted chain 仍属于旧 whole-chain 路径，不得按 typed leaf 记账。后续从本页最末checkpoint与第9节继续逐项撤权。可疑语义依次查现役文档、Ghidra官方客户端、参考实现并按需要交叉验证。
...
3. **R4-2 普通 material 能力族**：按 color/tint/opacity/transform/shift-hue/film-grain/pulse、动态uniform/provider、blend/depth/cull/alpha-writing等共享语义扩binder/executor；每个已证子集必须把默认产品owner转给Program并避免双写。尚未迁移的明确形态只能在没有 authored graph 时保留旧 planner，或在整个 graph 完整 accepted 时继续由旧 whole-chain renderer执行；rejected graph 不得据此回落到 `SceneEffectRuntimePlanner`。整个family完成时同提交删除对应旧产品owner、fallback dispatch与只服务旧路径的probe/backend。只有 Shift Hue、增强版 Audio Bars、Workshop Gradient、Workshop Shadow、Shake、zero-distortion Fisheye 六类 `supportsUnifiedPairLeaf` 可作为GraphExecutor typed leaf保留在inventory，不能把Blend、Film Grain、Depth等旧whole-chain backend误计为leaf，也不能为了case归零重写一遍。
4. **R4-3 graph resource 与多 pass**：四类 omission/rebase recovery 已归零；继续闭合 logical target、copy/swap、persistent/history、clear/function/condition、compose pair、hazard、resize/reparse/reset generation，再迁移 blur/bloom/local-contrast/water/cursor/depth/xray/shine/godrays 等族；完成后删除 old chain renderer。
...
截至2026-08-10，五个有限波次的现役状态是：**R4-A 已完成**，四种 partial recovery 与 recovery-only 产品代码/测试归零；**R4-B 待完成**，仍须撤销 product compositor 到 `renderLegacyAuthoredChain`/whole-chain renderer 的入口；**R4-C partial**，rejected authored graph 已中和普通层与 utility 的 `SceneEffectRuntimePlanner` legacy effect plan/inputs，但无 authored graph 的 planner GPU authority仍保留；**R4-4 仍未完成**，严格 ImageBlend source preparation 与 typed named dependency 仍是未迁移的独立 owner；**R4-D 待完成**，仍须撤销 quad Light Shafts 在 `notMigrated` 时直写 main pass 的 fallback；**R4-E 待完成**，仍须归零 script/profile 行为 selector并核对单一 owner/telemetry。R4结束后，R5再删除已经不可达的whole-chain surface、frame batch/target、standalone call、`fallbackGraph`/`.legacyContract`、旧observation和telemetry；不得反过来把这些删除项解释成新的像素迁移。
