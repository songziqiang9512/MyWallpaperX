...
> 建立日期：2026-08-03
>
> 文档状态：现役执行计划；更新至 2026-08-11。R4-A 四类 partial recovery 已删除，R4-C 的 rejected-authored-graph legacy neutralization 切片已落地，R4-B1 pair chain 与 R4-B2 exact X-Ray identity-output 已闭环；R4 仍为 `partial`，R5 为 `not_started`。
>
...
截至2026-08-11：**R4-A 已完成**；**R4-B1 pair chain与R4-B2 exact X-Ray identity-output已完成并独立提交**；**R4-C partial**；**R4-4/R4-D/R4-E未完成**。multi-pass/logical-target和其他X-Ray topology尚未迁移，whole-chain compositor入口、无图legacy planner、ImageBlend/typed dependency、Light Shafts fallback与script/profile selector仍可达。R4结束后R5只删除已经不可达的surface/adapter/observation与重复telemetry；发现行为迁移必须退回R4。
