...
> 建立日期：2026-07-22
>
> 最近更新：2026-07-25（实现基线 `678a052`；interpretation v28；十一类 strict effect backend、Puppet bind-pose/静态 MDAT/严格单 clip MDLA CPU LBS、strict depth-one static/default-static/eventspawn/natural-eventdeath/eventfollow child、持续/混合/duration child emitter、root child aggregate budget、有限 static origin translation，以及非音频 turbulent velocity 是当前执行子集；程序化 built-in registry 已扩到 20-key。BC1/2/3 颜色纹理在单 image 16M 像素预算内走 CPU premultiply，超预算或多 image 载荷走 GPU premultiply；跨 image sprite 当前只裁出 authored 首帧作静态 fallback，不执行完整动画。REFRACT 粒子材质 fail closed。当前完整 45 样本门 45/45、particle 101/131；固定 13 样本门 13/13、particle 19/27；static-origin 定向门 1/1、particle 17/19）
>
...
- `project.json` / entry 同名 `gifscene.json` 识别、PKGV 索引、受控缓存解包、路径校验和 interpretation format 28；v25 在 v24 static attachment frame 之上增加 authored Puppet animation layer 声明，v26 在 material pass 上增加 `alphawriting` render state 与 `usershadervalues` 绑定声明，v27 在 layer 上增加作者 `brightness` 颜色乘数并由 image/solid 通道消费，v28 在 text layer 上增加作者 `anchor`（Screen anchor）并折进 layer 平移；
...
- text 以 authored `pointsize * 300 / 72` 换算官方 300 DPI point raster、处理 vector padding、包内字体、系统字体别名和缺失字体诊断；direct user property 的 content/point-size/color 已按变化 layer 异步重栅格化；
...
- 签名 Debug App、隔离 sample root/HOME、Metal ready/after 双帧、语义合同和 stop 后 surface=0；`678a052` 的完整快照门 `.codex/scene-static-origin-full45-20260725/report.json` 为 **45/45**、particle **101/131**，报告/矩阵 SHA-256 为 `8d578f2b8bb39e8f62fb0a64cfc1e0f6fd2018b578752afd3a555d8fb141e6a0` / `b005da924cfefbcd08410d298f8795af67086f62c7df95ca802988fcfabea38e`；45 门 strict 为 stage 91、chain 15、failed 0。固定回归门 `.codex/scene-static-origin-fixed13-20260725/report.json` 为 **13/13**、particle **19/27**、strict stage 24、chain 2、failed 0，报告/矩阵 SHA-256 为 `6537c945963e2005d9354113036602e57357a57ece44769e68aef3291252b07d` / `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c`；static-origin 定向门 `.codex/scene-static-origin-targeted-20260725/report.json` 为 **1/1**、particle **17/19**，报告/矩阵 SHA-256 为 `febd6e14feba2bb74251d1f513d89be0baa72084f6ddf8bfdf89f0791b9c3a4f` / `4f68c1655eec246c6a94093c82a098bbe03a161997b1aec46175d9612e169c9e`。当前三门 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `8661db034136a4030592f6e2f3190b08de4c448a`、可执行文件 SHA-256 `1ddea5e09f45e45828b5e1003debb56e92fcb82cfef4f0428e7cde45ac70d004`；运行前后签名通过、sample root residue 0。完整 Scene suite 为 **393 项：390 通过、3 跳过**，代码健康为 446 Swift files、44 locked legacy files、400-line limit。完整聚合缺口见 [运行证据索引](semantics/runtime-evidence-index.md)。
...
- **S4（进行中）**：strict depth-one static/default-static/eventspawn/natural-eventdeath/eventfollow 已复用 fixed step + deterministic event queue + child owner/budget 执行，持续/混合/duration child emitter、root child aggregate budget、有限 static origin translation 与 20-key built-in registry 已闭合；隔离 45 样本 census 已确认 static/default-static 119 条，其中 14 条 identity strict declaration 与 `snowstormfog` 有限 origin declaration 已执行，其余 104 条 non-identity static 仍先被 nested child 等 profile 阻断，collision/delete、event CP/value inheritance 当前均无真实定义。两条 event scale probe 仍分别被 world-space root 与 world-space Rope Trail root 阻断，当前不扩执行口径；下一步裁决 nested ownership、递归深度和跨层/递归 total budget。rope/rope trail 仍需独立 geometry/material renderer，audio response 仍需 injectable audio snapshot；前置未完成时只补 IR/fixture，不提前接产品执行；
