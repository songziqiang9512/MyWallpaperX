...
- stock texture resolver 读取随 App 构建的 catalog，保留样本本地资源优先级，把精确 `particle/...`、`materials/...` 或显式 `assets/.../*.tex` identity 映射到同路径 TEX；311/311 official TEX path 与 298/298 sidecar path 自动门通过。当前只有 Particle material slot 0 是真实 consumer；固定门现有六样本重跑 image 82/82、particle 11/13 且无 stock 资源诊断；sidecar、LUT、normal、多槽、Effect/preset/scene preview 与逐资产 TEX metadata 不因 resolver 存在而升级；
