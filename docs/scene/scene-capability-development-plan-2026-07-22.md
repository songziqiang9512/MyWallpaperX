...
- `project.json` / entry 同名 `gifscene.json` 识别、PKGV 索引、受控缓存解包、路径校验和 interpretation format 27；v25 在 v24 static attachment frame 之上增加 authored Puppet animation layer 声明，v26 在 material pass 上增加 `alphawriting` render state 与 `usershadervalues` 绑定声明，v27 在 layer 上增加作者 `brightness` 颜色乘数并由 image/solid 通道消费；
