...
> 建立日期：2026-07-22
>
> 最近更新：2026-07-24（当前实现基线 `1766c76`；interpretation v22；B0 direct dynamic text、B2 同帧 copy/swap/受限 history、Precise Blur interleave/legacy compose 与 exact stock Shake 已通过合同门；六个 strict backend 是当前 GPU 执行子集。Water Waves legacy inline 现只接受 owner layer 唯一可见 Effect，mixed/repeated declarations fail closed；当前固定 13 样本门 13/13，上一构建完整 26 样本快照 26/26）
>
...
- 签名 Debug App、隔离 sample root/HOME、Metal ready/after 双帧、语义合同和 stop 后 surface=0；上一构建的完整快照门 `.codex/scene-shake-20260724/full26-final/report.json` 为 **26/26**，报告 SHA-256 `112f3d50a63f2dd20002ca2c2f1d177fc86abaf82510f5dd85c7e5b8c0822f65`，仓库矩阵 SHA-256 为 `9a44707e9c84069ce3dc5ffa8c93c001c5fc54cf4b02eba89e53a3daeb8cfb75`。当前 `1766c76` 签名 App 的固定回归门 `.codex/scene-waterwaves-ordered-chain-fixed13-final-20260724/report.json` 为 **13/13**，报告 SHA-256 `6c67dd17a544f512daa8b4d00588840ed90495050a4696815dd047f22181637f`，矩阵 SHA-256 `049465cfb68eafd5e317348a1a697e12bbcc4d43ac39f4a466895f9f87dc62e6`。26 门为 strict stage 30、failed 0、chain 3、Shake 3、legacy blocked 16、route-only 64；固定 13 门继续保护 stage 14、真实 chain 1、Opacity 4、Workshop Shadow 1、failed 0，并锁定五个 Water Waves 正反样本的 legacy count。当前提交相关测试 **40/40**；上一实现基线完整 Scene suite 为 **297 项，其中 294 项通过、3 项跳过**。当前固定门 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `d37ecaceedea6b82daadeac90e17159c63d93e73`、可执行文件 SHA-256 `95b667f4fc6a54865b1a78f207af9258d16a1dfa0785fd68ded09b5630792896`，benchmark staged App 运行前后签名均验证。完整聚合缺口见 [运行证据索引](semantics/runtime-evidence-index.md)。
...
direct text 三类 target 与 B1 generation/stale cancellation 已由 `1762743` 完成首个闭环，同帧 copy/swap command foundation 已由 `f1c6a10` 完成，`dcedc2e` 已完成显式 unique FBO 的受限 history seed/clear，`ebf44a9` 已完成 Precise Blur 两种 material-command interleave，`4f13daf` 已完成 exact legacy compose 归一化，`e505a9e` 已用 exact stock Shake 让未修改样本新增 6 stage、3 条 chain，`3194ac5` 已建立非阻断 preview 并排图和同样本分项证据，`1766c76` 已撤销 mixed/repeated Water Waves 的错误 inline 执行。下一切片优先为 Water Waves 建立 authored-chain backend 或补齐其前序 Blend/Water Flow 等完整链，不再扩大 effect-name inline；随后继续按未修改样本新增 stage、解除完整 effect chain、同一样本拼图/分项改善和实现成本重排 history、SceneScript、shader 与 provider 路线。以后所有属性能力继续要求 compiler target、活动 consumer、per-surface snapshot、原子失败/fallback 和 surface/window identity 同批验收。
