...
> 建立日期：2026-07-22
>
> 最近更新：2026-07-25（实现基线 `f02f41d`；interpretation v25；十一类 strict effect backend、Puppet bind-pose/静态 MDAT/严格单 clip MDLA CPU LBS，以及 strict depth-one eventspawn/natural-eventdeath particle child 是当前执行子集；`particle/fire/fire1` 已把程序化 built-in registry 扩到 14-key。BC1/2/3 颜色纹理在单 image 16M 像素预算内走 CPU premultiply，超预算或多 image 载荷走 GPU premultiply；跨 image sprite 当前只裁出 authored 首帧作静态 fallback，不执行完整动画。REFRACT 粒子材质 fail closed。当前完整 45 样本快照门 45/45、particle 86/131；固定 13 样本门 13/13、particle 18/27）
>
...
- 签名 Debug App、隔离 sample root/HOME、Metal ready/after 双帧、语义合同和 stop 后 surface=0；当前完整快照门 `.codex/scene-fire1-full45-20260725/report.json` 为 **45/45**、particle **86/131**，报告/矩阵 SHA-256 为 `71544e52623942818da44a85fc805f3281368f8fd7ce489d28acf895e7030126` / `ad68f6bd889a6f8152a27e60aad2aaff55ccc7ccf9a924c0551f24e64d02357c`；45 门 strict 为 stage 91、chain 15、Water Flow 10、Water Waves 11、Shake 24、Opacity 8、Local Contrast 2、failed 0、route-only 113，image `449/451`、text `244/244`、solid `140/140`。固定回归门 `.codex/scene-fire1-fixed13-20260725/report.json` 为 **13/13**、particle **18/27**，报告/矩阵 SHA-256 为 `c21c2a13c4754019578129a04e7be07df1b9e1c2c694396e0b26fab87b40c992` / `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c`；固定门保护 strict stage 24、chain 2、Local Contrast 2、Water Flow 1、Water Waves 6、Shake 1、Opacity 4、Workshop Shadow 1、failed 0。两门 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `8400b69a9dfb69aee168f6a8f57200a3da2ac94e`、可执行文件 SHA-256 `c20abc4f14ef80d7291b6ccb98ed3bc28d3db65e8d6859f4f5f17ad5eec91dd6`；13/45 个样本均 interpretation v25、退出 surface `1 -> 0`、样本根 residue 0。完整 Scene suite 为 **372 项：369 通过、3 跳过**，代码健康为 441 Swift files。完整聚合缺口见 [运行证据索引](semantics/runtime-evidence-index.md)。
...
- **S4（进行中）**：strict depth-one eventspawn/natural-eventdeath 已复用 fixed step + deterministic event queue + child owner/budget 执行，`rosepetals`/`beam_1` 已补入高频 built-in texture；粒子下一步扩 static/follow/collision/delete、event transform/inheritance、depth/total budget。rope/rope trail 仍需独立 geometry/material renderer，audio response 仍需 injectable audio snapshot；前置未完成时只补 IR/fixture，不提前接产品执行；
...
`8bac86e`/`dd85dcf` 已关闭 Puppet 图集重组、预算内 BC 颜色 premultiply、REFRACT fail-closed；`49ee89a` 关闭静态 MDAT attachment；`ca6d841`/`56f92a2`/`2be2b44`/`f1ee79b` 关闭三来源 MDLA IR、full TRS、rig/weights 和严格单 clip fixed-step CPU LBS；`c654571` 以 `rosepetals`/`beam_1` 严格 key关闭高命中 built-in 批次；`18d0056` 关闭超预算/多 image BC1/2/3 的 GPU premultiply 与静态 authored 首帧 fallback；`f02f41d` 以 `particle/fire/fire1` 的低能量火焰遮罩把 registry 扩到 14-key，使 `3769364482` 两层 torch 与 `3757555836` 一层 candle 从 unavailable 进入受限执行。按样本收益与公共依赖排序的下一批候选：

1. **`2998757800` 多余/缺失粒子与全局比例/裁切**：先把 preview、当前输出和逐层声明对齐，区分粒子资源/child、坐标空间、camera cover 与 layer transform 根因；
2. 只在完整 fingerprint 与共享依赖成立时补对应粒子、空间或构图合同，不按样本 ID 调比例。
