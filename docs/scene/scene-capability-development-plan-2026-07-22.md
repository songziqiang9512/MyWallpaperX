...
> 建立日期：2026-07-22
>
> 最近更新：2026-07-25（实现基线 `dd85dcf`；interpretation v23；十一类 strict effect backend + puppet bind-pose mesh 是当前 GPU 执行子集；BC1/2/3 颜色纹理在单帧 16M 像素预算内按 premultiplied 合同解码，REFRACT 粒子材质 fail closed。当前完整 45 样本快照门 45/45、particle 79/131；固定 13 样本门 13/13、particle 13/27）
>
...
- `project.json` / entry 同名 `gifscene.json` 识别、PKGV 索引、受控缓存解包、路径校验和 interpretation format 23；v23 在 v22 的 exact stock Opacity、ShaderContract、provider/resource metadata、authored graph 与 direct text binding program 合同之上加入 layer `puppetMeshPath`；
...
- **puppet bind-pose mesh 渲染**（`8bac86e`）：解析 MDLV0021/0023 mesh block（已核验 stride 80/84、position@0、UV 在尾部 8 字节、uint16 三角形、max index 必须等于 vertexCount-1），加载时把 puppet 层的 UV 图集一次性重组为 bind pose 纹理，下游 mask/effect/blend 无感消费；`3769688830` 的 7 个 puppet 层（含 stride 84 的 skinned base）全部重组成功，Ahri 主体从图集碎块恢复为正确构图。warp 动画、骨骼播放、MDAT attachment 变换仍未实现并 fail closed；该能力为 `executed-degraded`，不宣称官方 mesh/skinning 像素等价；
...
- 签名 Debug App、隔离 sample root/HOME、Metal ready/after 双帧、语义合同和 stop 后 surface=0；当前完整快照门 `.codex/scene-puppet-full45-20260725/results-v2/report.json` 为 **45/45**、particle **79/131**，报告/矩阵 SHA-256 为 `18206614218b51e34cbf9f7f11e6ac395542329c4d43bdcafc69747a7de0328b` / `02d04479329b597b214342aa1a440d929d2e86aa5037df58c6a6488ae186595b`；45 门 strict 为 stage 91、chain 15、Water Flow 10、Water Waves 11、Shake 24、Opacity 8、Local Contrast 2、failed 0、route-only 113，image `449/451`、text `244/244`、solid `140/140`。固定回归门 `.codex/scene-puppet-fixed13-20260725/final-v2/report.json` 为 **13/13**、particle **13/27**，报告/矩阵 SHA-256 为 `9de444ea578870d620e0d01902e50ca7110b6ecc9974fe16f564a9d41e45bc7e` / `cd178b01bd6c34e1602b57068b2c3f9093b4e43e7f09dc77ce293c6cbe7469e6`；固定门保护 strict stage 24、chain 2、Local Contrast 2、Water Flow 1、Water Waves 6、Shake 1、Opacity 4、Workshop Shadow 1、failed 0。particle 由 84/131 与 15/27 分别降至 79/131 与 13/27，是 REFRACT 修正的正确口径变化（折射雨/玻璃/星层不再画白块）。`3769688830` 定向门报告 SHA-256 为 `fecc135d11a5e7f7c27ef1d381714f162adc4908262beacd6e793b7553f11918`。两门签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `50db8b27546c3d6c2a1adff4a45991f61a94c7cf`、可执行文件 SHA-256 `973f43b1ef58267c5678a1e6478c38a0cefd5d893b0267243b7a430a01dfcc7d`。preview 方向性视觉证据基线仍为 `3194ac5`。完整 Scene suite 为 **415 项：412 通过、3 跳过**。完整聚合缺口见 [运行证据索引](semantics/runtime-evidence-index.md)。
...
- puppet 已执行 bind-pose mesh 重组，但 **MDLS 骨骼、MDLA 动画、MDAT attachment 变换只解析边界不消费**：挂在 attachment 点的子层仍按父层中心定位（`3769688830` 的 ahriarm/ahriorb/water droplets 有可见偏差），animationlayers 声明的 warp 动画不播放（bind pose 静止）；
...
固定 13 样本门 PASS 不能解释成 Wallpaper Engine 视觉兼容率：它只证明矩阵声明的解析、GPU 完成、非黑画面、能力计数和释放门通过。`3769688830` 的主体构图已由 puppet bind pose 恢复，但 attachment 定位、warp 动画、樱花/光束粒子与折射雨仍缺；`2131872317` 的烟花 eventdeath child 爆炸、`3750813609` 的动态时钟、完整 Clouds/Blur、world-space 雨滴溅射等差距不变。
...
13. Puppet `.mdl`（MDLV0021/0023）没有官方公开格式。当前 mesh block 合同（marker 9 字节、首个 `MDLS` 为搜索上界、`u32 vertexBytes` + 顶点 + `u32 indexBytes` + uint16 三角形、position@0、UV 在 stride 尾部 8 字节、stride 80/84、`max(index)==vertexCount-1` 唯一判定）来源是**第三方播放器解释 + 真实 Workshop 资产交叉验证**，不是官方规则；`MDLS`（骨骼）、`MDLA`（动画）、`MDAT0001`（attachment：u16 id + name\0 + 列主序 4x4 bind 矩阵）已定位但未消费，任何超出已验证形状的数据必须 fail closed。
...
`8bac86e`/`dd85dcf` 已关闭 `3769688830` 暴露的三个公共合成缺口（puppet 图集重组、BC 颜色 premultiply、REFRACT fail-closed），45/13 两门已按新口径刷新。按样本收益与公共依赖排序的下一批候选：

1. **puppet MDAT attachment 定位**：`3769688830` 的 ahriarm/ahriorb/droplets 挂点偏差是 bind pose 后最大的可见残差；MDAT bind 矩阵已解析出布局，需要真实样本对照确定 child origin 与 attachment 的组合语义后进入受限执行；
2. **puppet MDLA 动画**：呼吸/尾巴摆动是该类样本的核心动态；先建 IR fixture 再接 fixed-step 播放；
3. **粒子 child/event death 最小闭环**（`2131872317`/`3768903841` 烟花爆炸）与高命中 built-in 纹理（`rosepetals`、`beam_1`）继续按官方 Children 语义推进；
4. **超预算 BC 容器的正确合成**：多帧/超大 BC 载荷当前保持直通（straight-alpha matte 边界已记录）；候选方案是 GPU compute premultiply 或 fragment 侧 per-texture straight-alpha 标记；
5. `3769364482` 的 Fire effect、299 多余粒子与全局比例/裁切按隔离样本收益随后推进。
