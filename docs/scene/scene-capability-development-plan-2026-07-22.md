...
> 建立日期：2026-07-22
>
> 最近更新：2026-07-26（实现基线 `678a052` 加当前 stock TEX resolver 批次；interpretation v29；十一类 strict effect backend、Puppet bind-pose/静态 MDAT/严格单 clip MDLA CPU LBS、strict depth-one particle child、持续/混合/duration child emitter、root child aggregate budget、有限 static origin translation，以及非音频 turbulent velocity 是当前执行子集。2.8.42 官方安装中观察到的 311 项 stock TEX 与 298 项 sidecar 已按原相对路径全部建立占位，资产身份库存状态完整；其中 Particle slot 0 可消费 164 项 TEX。当前 TEX 均封装同一 16x16 占位，sidecar 只建身份且不进入 runtime，22-key 程序纹理改为 bundle 缺失回退。库存完整不表示所有 consumer 或 TEX metadata 兼容。BC1/2/3 与跨 image sprite 边界不变，REFRACT 粒子材质 fail closed。历史完整 45 样本门 45/45、particle 101/131；历史固定 13 样本门 13/13、particle 19/27；当前本机固定门仅 6/13 样本可用且矩阵仍为 v25，不能写成新基线 PASS）
>
...
- stock texture resolver 读取随 App 构建的 catalog，保留样本本地资源优先级，把精确 `particle/...`、`materials/...` 或显式 `assets/.../*.tex` identity 映射到同路径 TEX；311/311 official TEX path 与 298/298 sidecar path 自动门通过，已知 stock 纹理资产路径占位已全部准备。素材作者只需按 catalog 原路径替换，不改变命名；后续每个 runtime consumer 必须复用这些 identity，并以替换文件被实际读取为该能力的资源链验收门。当前只有 Particle material slot 0 是真实 consumer；固定门现有六样本重跑 image 82/82、particle 11/13 且无 stock 资源诊断；sidecar、LUT、normal、多槽、Effect/preset/scene preview 与逐资产 TEX metadata 不因库存完整或 resolver 存在而升级；
...
- **S4（进行中）**：strict depth-one static/default-static/eventspawn/natural-eventdeath/eventfollow 已复用 fixed step + deterministic event queue + child owner/budget 执行，持续/混合/duration child emitter、root child aggregate budget、有限 static origin translation 已闭合；Particle slot 0 现按 exact catalog identity 消费 164 项 bundle TEX，22-key 程序 registry 只作 bundle 缺失回退。隔离 45 样本 census 的 static/default-static 119 条和 nested/transform/budget 边界不变。下一步仍裁决 nested ownership、递归深度和跨层/递归 total budget；占位 TEX/sidecar 则按独立素材制作流程逐项替换并补 codec、尺寸、通道、mip、atlas、像素门。rope/rope trail 与 audio response 前置未完成时不提前接产品执行；
...
既有 Puppet、BC、child、22-key 程序纹理与 `678a052` static-origin 批次边界不变。当前 stock resolver 进一步把 catalog 311/311 official path 接到同路径 TEX，Particle slot 0 可消费其中 164 项，先前 `debris1`/`fire2`/`hose_4` 的 78 次资源缺口因此转为占位素材质量问题；固定门现有六样本已由签名 App 复核资源链。按样本收益与公共依赖排序的下一批候选：
