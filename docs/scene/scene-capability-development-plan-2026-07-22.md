...
> 建立日期：2026-07-22
>
> 最近更新：2026-07-25（实现基线 `4a17ee6`；interpretation v25；十一类 strict effect backend、Puppet bind-pose/静态 MDAT/严格单 clip MDLA CPU LBS、strict depth-one eventspawn/natural-eventdeath child，以及非音频 turbulent velocity 是当前执行子集；`particle/light/light_shafts_0` 已把程序化 built-in registry 扩到 15-key。BC1/2/3 颜色纹理在单 image 16M 像素预算内走 CPU premultiply，超预算或多 image 载荷走 GPU premultiply；跨 image sprite 当前只裁出 authored 首帧作静态 fallback，不执行完整动画。REFRACT 粒子材质 fail closed。`a5a951f` 的完整 45 样本快照门 45/45、particle 91/131；`4a17ee6` 的固定 13 样本门 13/13、particle 18/27）
>
...
- 签名 Debug App、隔离 sample root/HOME、Metal ready/after 双帧、语义合同和 stop 后 surface=0；`a5a951f` 的完整快照门 `.codex/scene-lightshafts0-full45-20260725/report.json` 为 **45/45**、particle **91/131**，报告/矩阵 SHA-256 为 `44d299f0c6757ae46e59e9486bbf5ee423f7dc908d9fa6ddbae72b8c90578c38` / `b005da924cfefbcd08410d298f8795af67086f62c7df95ca802988fcfabea38e`；45 门 strict 为 stage 91、chain 15、failed 0。`4a17ee6` 的固定回归门 `.codex/scene-turbulent-velocity-final-fixed13-20260725/report.json` 为 **13/13**、particle **18/27**，报告/矩阵 SHA-256 为 `18ea2ca3ae8a65dcb1f9ad94d6bba1afb418b6aec5a944274756b71416f8abfe` / `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c`；定向门 `.codex/scene-turbulent-velocity-final-targeted-20260725/report.json` 为 **5/5**，报告 SHA-256 `d86694accc534e83fba4f83375864ed705b1f6cc49b7a9f48966a9543e2a40c2`。当前固定/定向 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `f3e387c64312ab85cbe1a163c745931148a60085`、可执行文件 SHA-256 `11b78046f9907c069dbdf28d0dfecdd6314a4ae0e4aece349dcc773ba21daa6d`；完整 Scene suite 为 **374 项：371 通过、3 跳过**，代码健康为 441 Swift files。完整聚合缺口见 [运行证据索引](semantics/runtime-evidence-index.md)。
