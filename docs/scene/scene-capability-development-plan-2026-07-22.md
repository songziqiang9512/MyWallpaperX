...
> 建立日期：2026-07-22
>
> 最近更新：2026-07-25（实现基线 `a5a951f`；interpretation v25；十一类 strict effect backend、Puppet bind-pose/静态 MDAT/严格单 clip MDLA CPU LBS，以及 strict depth-one eventspawn/natural-eventdeath particle child 是当前执行子集；`particle/light/light_shafts_0` 已把程序化 built-in registry 扩到 15-key。BC1/2/3 颜色纹理在单 image 16M 像素预算内走 CPU premultiply，超预算或多 image 载荷走 GPU premultiply；跨 image sprite 当前只裁出 authored 首帧作静态 fallback，不执行完整动画。REFRACT 粒子材质 fail closed。当前完整 45 样本快照门 45/45、particle 91/131；固定 13 样本门 13/13、particle 18/27）
>
...
- 签名 Debug App、隔离 sample root/HOME、Metal ready/after 双帧、语义合同和 stop 后 surface=0；当前完整快照门 `.codex/scene-lightshafts0-full45-20260725/report.json` 为 **45/45**、particle **91/131**，报告/矩阵 SHA-256 为 `44d299f0c6757ae46e59e9486bbf5ee423f7dc908d9fa6ddbae72b8c90578c38` / `b005da924cfefbcd08410d298f8795af67086f62c7df95ca802988fcfabea38e`；45 门 strict 为 stage 91、chain 15、Water Flow 10、Water Waves 11、Shake 24、Opacity 8、Local Contrast 2、failed 0、route-only 113，image `449/451`、text `244/244`、solid `140/140`。固定回归门 `.codex/scene-lightshafts0-fixed13-r2-20260725/report.json` 为 **13/13**、particle **18/27**，报告/矩阵 SHA-256 为 `23f1c3da560646d499dd77624a6d955d7eb80f6ffc43701c2a074c608e5810f2` / `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c`；固定门保护 strict stage 24、chain 2、Local Contrast 2、Water Flow 1、Water Waves 6、Shake 1、Opacity 4、Workshop Shadow 1、failed 0。两门 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `2ee48e8fc3d9380c4b4f2d1f1083b5b7805d8457`、可执行文件 SHA-256 `3cb6e4df0aadf25c4693a87a84cd8363dca2ea1577f214eca7afde447a9f0bc1`；13/45 个样本均 interpretation v25、退出 surface `1 -> 0`、样本根 residue 0。完整 Scene suite 为 **372 项：369 通过、3 跳过**，代码健康为 441 Swift files。完整聚合缺口见 [运行证据索引](semantics/runtime-evidence-index.md)。
...
`8bac86e`/`dd85dcf` 已关闭 Puppet 图集重组、预算内 BC 颜色 premultiply、REFRACT fail-closed；`49ee89a` 关闭静态 MDAT attachment；`ca6d841`/`56f92a2`/`2be2b44`/`f1ee79b` 关闭三来源 MDLA IR、full TRS、rig/weights 和严格单 clip fixed-step CPU LBS；`c654571` 以 `rosepetals`/`beam_1` 严格 key关闭高命中 built-in 批次；`18d0056` 关闭超预算/多 image BC1/2/3 的 GPU premultiply 与静态 authored 首帧 fallback；`f02f41d` 以 `particle/fire/fire1` 的低能量火焰遮罩把 registry 扩到 14-key；`a5a951f` 再以 `particle/light/light_shafts_0` 的低能量双光束遮罩扩到 15-key，使五个真实层从 unavailable 进入受限执行。按样本收益与公共依赖排序的下一批候选：

1. **Flare 粒子公共执行合同**：`2998757800` 的同一 Flare 仍缺 `particle/halo_3`、`particle/fog/fog3`、`particle/flare/flare_1`，并依赖 turbulent velocity、`eventfollow` 和非瞬时 child emitter；先建立这些共享 emitter/operator/event 合同，再决定纹理批次，不能只补三张纹理把计数做绿；
...
| S4.6 Light Shafts 0 built-in 粒子纹理 | `a5a951f` | `particle/light/light_shafts_0` 128×128 低能量双光束确定性预乘遮罩；五个真实层进入受限执行，21 样本 census 的 `builtInTextureUnavailable` 从 11 降至 9，完整门升至 `91/131`；程序遮罩不等于官方资产，Flare 的 turbulent/eventfollow/非瞬时 child 合同仍缺 |
