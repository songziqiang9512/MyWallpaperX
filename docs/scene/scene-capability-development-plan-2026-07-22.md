...
> 建立日期：2026-07-22
>
> 最近更新：2026-07-25（实现基线 `899704b`；interpretation v25；十一类 strict effect backend、Puppet bind-pose/静态 MDAT/严格单 clip MDLA CPU LBS、strict depth-one static/default-static/eventspawn/natural-eventdeath/eventfollow child、持续/混合/duration child emitter、root child aggregate budget，以及非音频 turbulent velocity 是当前执行子集；程序化 built-in registry 已扩到 18-key。BC1/2/3 颜色纹理在单 image 16M 像素预算内走 CPU premultiply，超预算或多 image 载荷走 GPU premultiply；跨 image sprite 当前只裁出 authored 首帧作静态 fallback，不执行完整动画。REFRACT 粒子材质 fail closed。`a5a951f` 的完整 45 样本快照门 45/45、particle 91/131；`899704b` 的固定 13 样本门 13/13、particle 18/27，static child 定向门 2/2）
>
...
- 签名 Debug App、隔离 sample root/HOME、Metal ready/after 双帧、语义合同和 stop 后 surface=0；`a5a951f` 的完整快照门 `.codex/scene-lightshafts0-full45-20260725/report.json` 为 **45/45**、particle **91/131**，报告/矩阵 SHA-256 为 `44d299f0c6757ae46e59e9486bbf5ee423f7dc908d9fa6ddbae72b8c90578c38` / `b005da924cfefbcd08410d298f8795af67086f62c7df95ca802988fcfabea38e`；45 门 strict 为 stage 91、chain 15、failed 0。`899704b` 的固定回归门 `.codex/scene-static-child-final-fixed13-20260725/report.json` 为 **13/13**、particle **18/27**、strict stage 24、chain 2、failed 0，报告/矩阵 SHA-256 为 `8466740278b5c194d0bf57f8d4dc825615e7af1f8742750a94a6b359af647ea6` / `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c`；static child 定向门 `.codex/scene-static-child-final-targeted-20260725/report.json` 为 **2/2**，报告 SHA-256 `7c3189d27c21919a99e7046a435901ceae57dd24edc57c627e1f3e1ddd4c5f29`。当前固定/定向 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `d20e9f264f657fece37107a314ff6647787ee138`、可执行文件 SHA-256 `271a7e69b471af4e36e16da1f9570e74d2a63372f59e4b3ad01cd8850d993862`；完整 Scene suite 为 **376 项：373 通过、3 跳过**，代码健康为 443 Swift files、44 locked legacy files、400-line limit。完整聚合缺口见 [运行证据索引](semantics/runtime-evidence-index.md)。
...
- **S4（进行中）**：strict depth-one static/default-static/eventspawn/natural-eventdeath/eventfollow 已复用 fixed step + deterministic event queue + child owner/budget 执行，持续/混合/duration child emitter、root child aggregate budget 与 Flare 三张 built-in 纹理已闭合；隔离 45 样本 census 已确认 static/default-static 119 条、当前 identity strict 子集 14 条，collision/delete、event CP/value inheritance 当前均无真实定义，非 identity event scale 只有两条。下一步先补 event scale，再按 nested child 等公共前置依赖处理 105 条 non-identity static，并补跨层/递归 total budget。rope/rope trail 仍需独立 geometry/material renderer，audio response 仍需 injectable audio snapshot；前置未完成时只补 IR/fixture，不提前接产品执行；
...
1. **Event child scale transform**：census 只有 `2998757800` eventspawn scale `0.2` 与 `3743305891` eventfollow scale `2.5` 两条非 identity event transform；先建立 child-local scale 合同、identity/default 正例和 offset/angles/非法值反例，再跑两样本可见门；
2. **Static transform / nested child 前置链**：119 条 static/default-static 中 14 条已执行；其余 105 条集中在 `2938612768`、`2974757317`、`3088601835`，前两个 matrix-code target 还受 nested child 阻断。先裁决 transform 与 nested budget/ownership 公共顺序，不为单样本放宽；
3. **`2998757800` 构图边界**：Flare 三纹理已使 layer 260 从 unavailable 进入 `64x64 additive` 执行，particle `8/15 -> 9/15`；`fog3` 以 2/255 峰值避免 continuous smoke 累积过曝。右下亮边仍与 `fog1` 无关，不提交 fog 调暗或样本 ID 比例特判；后续必须用 layer/camera 合同或合法 Windows WE 输出继续裁决。

`f4173ea`/`7d53c10` 已关闭 strict depth-one eventspawn/natural-eventdeath 最小闭环，`928acca` 已关闭严格 Event Follow owner，`2f897bc` 已关闭持续/混合/duration child emitter 与 root aggregate budget，`899704b` 已关闭 strict static/default-static child；`3750813609` 与 `3766387484` 恢复第二层金黄色叶片，`2131872317` 延迟签名 App 门可见两簇烟花。census 中 collision/delete、inherit-value-from-event 与非空 child CP mapping 均为 0；non-identity static/event transform、nested child、跨层/递归总预算与 Windows golden 仍未完成。`3768903841` 的 5-image/140-frame sprite 仍只有静态首帧 fallback，完整跨-image animation 继续 fail closed，不得将本批写成 sprite animation 兼容。
