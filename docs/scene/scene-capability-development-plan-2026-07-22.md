...
- 签名 Debug App、隔离 sample root/HOME、Metal ready/after 双帧、语义合同和 stop 后 surface=0；`a5a951f` 的完整快照门 `.codex/scene-lightshafts0-full45-20260725/report.json` 为 **45/45**、particle **91/131**，报告/矩阵 SHA-256 为 `44d299f0c6757ae46e59e9486bbf5ee423f7dc908d9fa6ddbae72b8c90578c38` / `b005da924cfefbcd08410d298f8795af67086f62c7df95ca802988fcfabea38e`；45 门 strict 为 stage 91、chain 15、failed 0。`928acca` 的固定回归门 `.codex/scene-eventfollow-final-fixed13-20260725/report.json` 为 **13/13**、particle **18/27**，报告/矩阵 SHA-256 为 `97fb8b32c3ce54f8c71ab0f3d6b1ed05f73b8db87acce9b7b225b2b6c542b5ea` / `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c`；Flare 定向门 `.codex/scene-eventfollow-final-flare-20260725/report.json` 为 **1/1**，报告 SHA-256 `f9dfee453014d13381f5c8c89d8887d10dd6925a606bf4c91426461b7843f0e0`，particle 仍为 `8/15`。当前固定/定向 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `b7b3017bfca4ed726ac79bbcb40b6443e7103422`、可执行文件 SHA-256 `b2e78d3d579f69a6781fada6480abfbc8da00d8cffa7d85e6d558d84ecce81b7`；完整 Scene suite 为 **375 项：372 通过、3 跳过**，代码健康为 441 Swift files。完整聚合缺口见 [运行证据索引](semantics/runtime-evidence-index.md)。
...
- **S4（进行中）**：strict depth-one eventspawn/natural-eventdeath/eventfollow 已复用 fixed step + deterministic event queue + child owner/budget 执行，`rosepetals`/`beam_1` 已补入高频 built-in texture；粒子下一步扩非瞬时 child emitter、static/collision/delete、event transform/CP/value inheritance、depth/total budget。rope/rope trail 仍需独立 geometry/material renderer，audio response 仍需 injectable audio snapshot；前置未完成时只补 IR/fixture，不提前接产品执行；
...
`8bac86e`/`dd85dcf` 已关闭 Puppet 图集重组、预算内 BC 颜色 premultiply、REFRACT fail-closed；`49ee89a` 关闭静态 MDAT attachment；`ca6d841`/`56f92a2`/`2be2b44`/`f1ee79b` 关闭三来源 MDLA IR、full TRS、rig/weights 和严格单 clip fixed-step CPU LBS；`c654571`/`f02f41d`/`a5a951f` 将 built-in registry 扩到 15-key；`18d0056` 关闭超预算/多 image BC1/2/3 的 GPU premultiply 与静态 authored 首帧 fallback；`4a17ee6` 关闭非音频 Turbulent Velocity Random 的受限执行，audio 继续 fail closed；`928acca` 关闭 identity/no-CP/瞬时 Sprite child 的 Event Follow owner、逐帧位置更新与父死亡回收。按样本收益与公共依赖排序的下一批候选：

1. **Flare 粒子公共执行合同**：`2998757800` 的 non-audio turbulent velocity 已执行，strict Event Follow owner 已由合成生命周期门闭合；同一 Flare 仍缺 `particle/halo_3`、`particle/fog/fog3`、`particle/flare/flare_1` 和非瞬时 child emitter。下一步建立持续 rate/duration child emitter 的生命周期与总预算合同，最后才决定纹理批次，不能只补三张纹理把计数做绿；
...
`f4173ea`/`7d53c10` 已关闭 strict depth-one eventspawn/natural-eventdeath 最小闭环，`928acca` 已关闭严格瞬时 Event Follow owner；`2131872317` 延迟签名 App 门可见两簇烟花。非瞬时 emitter、static/collision/delete/nested child 与 Windows golden 仍未完成。`3768903841` 的 5-image/140-frame sprite 仍只有静态首帧 fallback，完整跨-image animation 继续 fail closed，不得将本批写成 sprite animation 兼容。
