...
- stock texture resolver 读取随 App 构建的 catalog，保留样本本地资源优先级，把精确 `particle/...`、`materials/...` 或显式 `assets/.../*.tex` identity 映射到同路径 PNG；311/311 official path 自动门通过。当前只有 Particle material slot 0 是真实 consumer，`3757555836` 的真实签名 App 门含 `debris1` 的 particle 7/7、纹理均为 16x16；LUT、normal、多槽、Effect/preset/scene preview 与 TEX metadata 不因 resolver 存在而升级；
...
- **S4（进行中）**：strict depth-one static/default-static/eventspawn/natural-eventdeath/eventfollow 已复用 fixed step + deterministic event queue + child owner/budget 执行，持续/混合/duration child emitter、root child aggregate budget、有限 static origin translation 已闭合；Particle slot 0 现按 exact catalog identity 消费 164 项 bundle PNG，22-key 程序 registry 只作 bundle 缺失回退。隔离 45 样本 census 的 static/default-static 119 条和 nested/transform/budget 边界不变。下一步仍裁决 nested ownership、递归深度和跨层/递归 total budget；占位 PNG 则按独立素材制作流程逐项替换并补尺寸/通道/atlas/像素门。rope/rope trail 与 audio response 前置未完成时不提前接产品执行；
...
既有 Puppet、BC、child、22-key 程序纹理与 `678a052` static-origin 批次边界不变。当前 stock resolver 进一步把 catalog 311/311 official path 接到同路径 PNG，Particle slot 0 可消费其中 164 项，先前 `debris1`/`fire2`/`hose_4` 的 78 次资源缺口因此转为占位素材质量问题；`3757555836` 已证明未修改真实样本中的 `debris1` 进入签名 App runtime。按样本收益与公共依赖排序的下一批候选：
