...
- 签名 Debug App、隔离 sample root/HOME、Metal ready/after 双帧、语义合同和 stop 后 surface=0；`a5a951f` 的完整快照门 `.codex/scene-lightshafts0-full45-20260725/report.json` 为 **45/45**、particle **91/131**，报告/矩阵 SHA-256 为 `44d299f0c6757ae46e59e9486bbf5ee423f7dc908d9fa6ddbae72b8c90578c38` / `b005da924cfefbcd08410d298f8795af67086f62c7df95ca802988fcfabea38e`；45 门 strict 为 stage 91、chain 15、failed 0。`9748a8c` 的固定回归门 `.codex/scene-flare-three-builtins-fixed13-20260725/report.json` 为 **13/13**、particle **18/27**，报告/矩阵 SHA-256 为 `adee4dad08e8322d25edcbc2fdd8589bf266d713fbb3d68af3a4df727e3b2543` / `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c`；Flare 定向门 `.codex/scene-flare-three-builtins-calibrated-20260725/report.json` 为 **1/1**，报告 SHA-256 `dc4e965636f12d3ef68da072258649839d47188ed9947db1b8eca0d6a37b05af`，particle 为 `9/15`。当前固定/定向 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `b541e2f73a68bfce74f16bc5129873fff3c5d339`、可执行文件 SHA-256 `767d4d9ca3358e3984bf1c3c53f66ee1a5a1cdba849f4b05c03a8e3f0c811fd1`；完整 Scene suite 为 **376 项：373 通过、3 跳过**，代码健康为 442 Swift files。完整聚合缺口见 [运行证据索引](semantics/runtime-evidence-index.md)。
...
- **S4（进行中）**：strict depth-one eventspawn/natural-eventdeath/eventfollow 已复用 fixed step + deterministic event queue + child owner/budget 执行，持续/混合/duration child emitter、root child aggregate budget 与 Flare 三张 built-in 纹理已闭合；粒子下一步先重做 static/collision/delete 与 event transform/CP/value inheritance 的真实样本 census，再按公共命中选择执行批次，并补跨层/递归 total budget。rope/rope trail 仍需独立 geometry/material renderer，audio response 仍需 injectable audio snapshot；前置未完成时只补 IR/fixture，不提前接产品执行；
...
1. **Particle 剩余事件/空间组件 census**：先在隔离 45 样本资源图中统计 static/collision/delete、event transform/CP/value inheritance 的真实定义与阻断链，按共同依赖和可见命中选择下一项；没有样本/fixture 证据前不预建 runtime 分支；
2. **`2998757800` 构图边界**：Flare 三纹理已使 layer 260 从 unavailable 进入 `64x64 additive` 执行，particle `8/15 -> 9/15`；`fog3` 以 2/255 峰值避免 continuous smoke 累积过曝。右下亮边仍与 `fog1` 无关，不提交 fog 调暗或样本 ID 比例特判；后续必须用 layer/camera 合同或合法 Windows WE 输出继续裁决。
