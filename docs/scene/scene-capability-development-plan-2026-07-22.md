...
> 建立日期：2026-07-22
>
> 最近更新：2026-07-25（实现基线 `8a27089`；interpretation v25；十一类 strict effect backend、Puppet bind-pose/静态 MDAT/严格单 clip MDLA CPU LBS、strict depth-one static/default-static/eventspawn/natural-eventdeath/eventfollow child、持续/混合/duration child emitter、root child aggregate budget，以及非音频 turbulent velocity 是当前执行子集；程序化 built-in registry 已扩到 20-key。BC1/2/3 颜色纹理在单 image 16M 像素预算内走 CPU premultiply，超预算或多 image 载荷走 GPU premultiply；跨 image sprite 当前只裁出 authored 首帧作静态 fallback，不执行完整动画。REFRACT 粒子材质 fail closed。当前完整 45 样本门 45/45、particle 101/131；固定 13 样本门 13/13、particle 19/27；Snow/Smoke 定向门 1/1、particle 17/19）
>
...
- 作者包内 2D sprite 粒子的 definition/resource graph、sphere/box、rate/burst/prewarm、常见 initializer/operator、sequence/random frame、additive/translucent、朝向、静态 override、CPU simulation 与 Metal instancing；现对 `particle/drop`、`particle/chromaticdot`、`particle/fire/fire1`、`fog1/fog3`、`leaves7/8`、`nature/snow`、`light_shafts_0/6`、`light/flare_1`、`lightning3`、`halo/halo_2/halo_3/halo_4`、`ripple_single`、`rosepetals`、`beam_1`、`smoke/smoke2` 二十个精确 key 提供项目自行生成的确定性预乘纹理，并支持按速度方向和作者 length/min/max stretch 绘制 Sprite Trail；child particle 的 static/default-static/eventspawn/natural-eventdeath/eventfollow、持续/混合/duration emitter 与 root aggregate budget 已进入受限执行；static 只接受 identity/no-CP/probability=1 的 strict depth-one 子集并在 root origin 创建一次；这些程序纹理是 `executed-degraded`，不是官方纹理副本或像素等价实现；
...
- 签名 Debug App、隔离 sample root/HOME、Metal ready/after 双帧、语义合同和 stop 后 surface=0；`8a27089` 的完整快照门 `.codex/scene-snow-smoke-full45-20260725/report.json` 为 **45/45**、particle **101/131**，报告/矩阵 SHA-256 为 `6118c02a07439661b873d415b4eab9371944bc4b89389c8e2578ee2cbed3dd87` / `b005da924cfefbcd08410d298f8795af67086f62c7df95ca802988fcfabea38e`；45 门 strict 为 stage 91、chain 15、failed 0。固定回归门 `.codex/scene-snow-smoke-fixed13-v2-20260725/report.json` 为 **13/13**、particle **19/27**、strict stage 24、chain 2、failed 0，报告/矩阵 SHA-256 为 `0b73d8aec67702cd1108f6af8d123b13ce506ff11f8bcbcd14165b29a90ecdd5` / `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c`；Snow/Smoke 定向门 `.codex/scene-snow-smoke-targeted-v3-20260725/report.json` 为 **1/1**、particle **17/19**，报告/矩阵 SHA-256 为 `f10103a142a5420692b52675440638f8a19bb954865151fa96f10e2a3d100c51` / `4f68c1655eec246c6a94093c82a098bbe03a161997b1aec46175d9612e169c9e`。当前三门 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `3ee30758506229d5d0a531d76389c863dad9c686`、可执行文件 SHA-256 `7f50e7f9ff995e0be2df4b96a05fd68c0ba98b4ca37764ab6315d9d0f3b05325`；运行前后签名通过、sample root residue 0。完整 Scene suite 为 **376 项：373 通过、3 跳过**，代码健康为 443 Swift files、44 locked legacy files、400-line limit。完整聚合缺口见 [运行证据索引](semantics/runtime-evidence-index.md)。
...
- **S4（进行中）**：strict depth-one static/default-static/eventspawn/natural-eventdeath/eventfollow 已复用 fixed step + deterministic event queue + child owner/budget 执行，持续/混合/duration child emitter、root child aggregate budget 与 20-key built-in registry 已闭合；隔离 45 样本 census 已确认 static/default-static 119 条、当前 identity strict 子集 14 条，collision/delete、event CP/value inheritance 当前均无真实定义。两条 event scale probe 仍分别被 world-space root 与 world-space Rope Trail root 阻断，当前不扩执行口径；下一步先补 `3088601835` 的有限 static origin translation，再按 nested child 等公共前置依赖处理其余 non-identity static，并补跨层/递归 total budget。rope/rope trail 仍需独立 geometry/material renderer，audio response 仍需 injectable audio snapshot；前置未完成时只补 IR/fixture，不提前接产品执行；
...
`8bac86e`/`dd85dcf` 已关闭 Puppet 图集重组、预算内 BC 颜色 premultiply、REFRACT fail-closed；`49ee89a` 关闭静态 MDAT attachment；`ca6d841`/`56f92a2`/`2be2b44`/`f1ee79b` 关闭三来源 MDLA IR、full TRS、rig/weights 和严格单 clip fixed-step CPU LBS；`c654571`/`f02f41d`/`a5a951f` 将 built-in registry 扩到 15-key；`18d0056` 关闭超预算/多 image BC1/2/3 的 GPU premultiply 与静态 authored 首帧 fallback；`4a17ee6` 关闭非音频 Turbulent Velocity Random 的受限执行，audio 继续 fail closed；`928acca` 关闭 identity/no-CP Sprite child 的 Event Follow owner、逐帧位置更新与父死亡回收；`2f897bc` 关闭持续/混合/duration child emitter、每 system 1,024 粒子与 root child runtime 64 system/65,536 capacity aggregate budget；`9748a8c` 将 registry 扩到 18-key 并恢复 Flare layer 260；`899704b` 关闭 identity/no-CP/probability=1 的显式与 legacy default static child；`8a27089` 将 registry 扩到 20-key，使 `3088601835` 的 Snow layers `513/534` 进入执行，并解除 `snowstormfog` 的 `smoke2` 资源阻断。按样本收益与公共依赖排序的下一批候选：

1. **Strict static origin translation**：`3088601835` 的两条 root Snow system 已从 unavailable 进入 `64x64 additive` 执行，样本 particle 达 `17/19`；其 static child `snowstormfog` 资源已完整，只剩 authored origin `0 650 0` 被 `unsupportedTransformOrControlPoint` 阻断。先只接受有限 origin translation，保留 scale/angles/CP/非法值反例和 depth-one/budget 边界，再跑同样本可见门；
2. **Nested static / aggregate budget 前置链**：119 条 static/default-static 中 14 条已执行；其余主要集中在 `2938612768`、`2974757317`、`3088601835`，前两个 matrix-code target 仍受 nested child 阻断。static origin 闭合后再裁决 nested ownership、递归深度和跨层总预算，不为单样本放宽；
3. **Event child scale 与 `2998757800` 构图边界**：`2998757800` eventspawn scale `0.2` 的父系统是 world-space，`3743305891` eventfollow scale `2.5` 的父系统是 world-space Rope Trail；probe 没有增加真实执行层，必须等各自 root 前置闭合后再接 scale。Flare layer 260 已执行且右下亮边仍与 `fog1` 无关，不提交 fog 调暗或样本 ID 比例特判。

`f4173ea`/`7d53c10` 已关闭 strict depth-one eventspawn/natural-eventdeath 最小闭环，`928acca` 已关闭严格 Event Follow owner，`2f897bc` 已关闭持续/混合/duration child emitter 与 root aggregate budget，`899704b` 已关闭 strict static/default-static child；`8a27089` 新增的 Snow/Smoke 程序纹理不是官方资产或 WE pixel parity，Smoke child 当前也未执行。census 中 collision/delete、inherit-value-from-event 与非空 child CP mapping 均为 0；non-identity static/event transform、nested child、跨层/递归总预算与 Windows golden 仍未完成。`3768903841` 的 5-image/140-frame sprite 仍只有静态首帧 fallback，完整跨-image animation 继续 fail closed，不得将本批写成 sprite animation 兼容。
