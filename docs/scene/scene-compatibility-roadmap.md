...
截至 2026-08-16，步骤 1–5 与步骤 7 的两个 Fast correctness atom 已闭合到产品 worker 切片：one-pass Light Shafts 和 optional-texture Opacity present/absent 成员均已批准；固定 glslang/SPIRV-Cross arm64 source artifacts、licenses、Developer ID `CodeSignOnCopy` 与签名 manifest 已进入 App。cache miss 时产品在独立 helper 进程中执行公共 normalization、link、SPIR-V、reflection、MSL、ABI/loop/texture/color 合同与原子发布，source/diagnostic/artifact/resident/wall budget 均有门；同一 key 的失败在本次 launch 内负缓存，下一 variant/owner 可继续。全新空缓存的 Light Shafts template/active key 现场生成 artifact，目标 node 唯一 frontend owner 是 `genericCompilerArtifact`，并闭合 GPU/publication/terminal compositor/next-frame；坏摘要切回 `boundedSwift` 后同一可见链和 ROI 保持。该定向 `--sample-id` 因旧整样本 matrix 计数和同包相邻 typed visual fallback 仍为 NON-PASS，不能冒充 Fast Suite PASS。步骤 6 的公共 effect-local failure 仍只覆盖 launch-time frontend/shader-preparation/color-contract 的 exact current-pair leaf；multi-node/FBO、sampler/uniform/texture/resource/target/state/hazard/generation/runtime encode 继续硬拒绝。当前首断点转为：把 Metal library/pipeline 首次编译从首帧 graph preparation 前移到 launch preparation，取得真实 App helper crash/timeout 后重启/重试证据，再逐类收窄可在 encode 前证明原子安全的 Program/renderer failure。bounded owner 撤权仍属于步骤 8，不能由产品 worker、两个正例或局部回滚提前宣告。
