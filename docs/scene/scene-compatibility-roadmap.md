...
当前段位：V4 已有 single-surface pointer/cursor/authored-transform，以及 pass-owned typed Vec2/audio 与 Vec3/颜色模块 → MaterialProgram 的 bounded 纵向原子；精确 consumer、数量、route、证据与剩余缺口只查运行输入专项表和运行证据索引。V4 完成门尚未成立，不能进入 V5。
