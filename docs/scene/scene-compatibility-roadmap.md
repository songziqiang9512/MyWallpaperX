...
路线采用有限 `PreparedProduct` 模型：纹理、世界空间几何、模拟状态、异步 provider 和多 pass graph 都是统一 compositor 的输入产品，而不是平行主链。Puppet 的长期路线是 `GeometryProduct` 世界空间绘制；中间纹理只在明确的 effect capture 边界使用。当前代码尚未达到可见验收，TextureProduct 回退保持为唯一稳定路径。任何新增兼容 profile 必须有静态预算、局部失败半径和退役条件，不能按样本或层身份扩展专用路径。
>
