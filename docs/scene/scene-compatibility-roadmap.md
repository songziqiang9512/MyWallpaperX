...
截至 2026-08-16，步骤 1–3 与步骤 7 的两个 Fast correctness atom 已闭合：one-pass Light Shafts 和 optional-texture Opacity present/absent 成员均已批准，并有 generic Program、GPU/compositor/next-frame、预登记 ROI 与坏 artifact 局部回滚证据。variant compilation 已先解析 generic artifact：命中时不再调用 bounded Swift frontend，missing/rejected 时才以 typed reason 调用一次 bounded fallback；现在 route 环境变量缺省时也进入 `prefer-generic`，显式 `observe-only` 仍只导出请求，显式 `disable-generic` 以 typed `route-disabled` 回滚，未知状态失败关闭。这闭合的是默认 artifact lookup 与已命中的步骤 4–5 切片，不是产品内已有 compiler worker，也不是 `generic-only`。步骤 6 的公共 effect-local 失败切片现覆盖 launch-time `material-variant-envelope-frontend` 与 `material-variant-envelope-shader-preparation`：只有无 dependency/FBO/blocker、单 material node 且 current-in/current-out 成对的 leaf effect 才以全帧 GPU copy 保留 previous current，并继续执行后续 suffix；该 effect 记录 typed failed/fallback，graph transaction 只在 copy/publication/compositor 成功时完成。派生真实内容已从批次前 preparation 失败导致整层无执行，恢复为 `Program → fallback → Program`、GraphExecutor `7/7/7` GPU 完成及 terminal compositor/next-frame，且末段截图与既有 `.35` suffix 基线同 hash。multi-node/FBO、sampler/uniform/texture/resource/target/state/hazard/generation/runtime encode 等失败继续硬拒绝，不被伪装成成功。当前首断点仍在步骤 4–6：把外部预生成 artifact 收敛为可重启、受预算约束的产品 preparation worker/cache，并把其余可在 encode 前证明原子安全的 Program/renderer claimed draw 失败逐类收窄到最小安全单元；已向 command buffer 追加工作、无法 effect-local 回滚的 runtime encode 失败不进入这条软化路径。bounded owner 撤权仍属于步骤 8，不能由默认 artifact lookup、两个正例、一次 owner 优先级修正或两个 launch-time leaf fallback 提前宣告。
