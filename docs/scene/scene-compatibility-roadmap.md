...
截至 2026-08-16，步骤 1–5 与步骤 7 的两个 Fast correctness atom 已闭合到产品 worker 切片：one-pass Light Shafts 和 optional-texture Opacity present/absent 成员均已批准；固定 glslang/SPIRV-Cross arm64 source artifacts、licenses、Developer ID `CodeSignOnCopy` 与签名 manifest 已进入 App。cache miss 时产品在独立 helper 进程中执行公共 normalization、link、SPIR-V、reflection、MSL、ABI/loop/texture/color 合同与原子发布，source/diagnostic/artifact/resident/wall budget 均有门；同一 key 的失败在本次 launch 内负缓存，下一 variant/owner 可继续。GraphExecutor 现又从完整 launch envelope 取得 shader、render state、attachment format、sample count、write mask 与 device，使用 frame PassEncoder 的同一物理 key 在 launch preparation 预热 Metal library/pipeline；首帧只消费 ready/failed cache，不再首次同步编译。全新空缓存的 `3747492842` 实测 5 个计划归并为 3 个物理 pipeline 并全部预热成功，首帧三次命中时 compilation attempts 仍为 3，随后闭合 GPU/publication/terminal compositor/next-frame；自洽摘要但非法 Metal source 的临时反例在 launch 形成 1 个 typed `library-compilation` 负缓存，frame 明确消费该失败且 attempts 不增加，恢复原 cache 后同一可见链复现。真实 App helper 生命周期门也已闭合：同团队签名的隔离 fault shim 只对首次 stage-link 注入 timeout 或 signal 9；每种首次运行都由 `source=spawn` 产生精确 typed failure，同 key 的再次请求由 `source=launch-result-cache` 返回而不重复 spawn，独立 active request 仍发布并让 layer 168 完成 GPU/publication/compositor/next-frame；下一次 App 启动则重新 spawn 原失败 key 并成功发布。四份定向 `--sample-id` 报告因旧整样本 matrix 计数和同包相邻 typed visual fallback 仍为 NON-PASS，不能冒充 Fast Suite PASS。步骤 6 的公共 effect-local failure 已覆盖 launch-time frontend/shader-preparation/color-contract/static uniform-schema、exact single-current-pair leaf 的 Metal library/vertex function/fragment function/pipeline compilation 四类 pre-encode failure、producer/target 全部可证且无 control 的多 dynamic value contributor 歧义，以及同一 leaf 在 frame-time 只由 `uniform/dynamicUniformBindingInvalid` 标识的 dynamic value 缺失、source mismatch、类型编码失败和 authored fallback 不一致；它们把 previous current 复制为该 stage 输出、继续 suffix，并保留 exact effect identity、typed failure 与聚合 fallback。multi-node/FBO、static/host `uniformBindingInvalid`、零 contributor、缺失或 target 不匹配的 producer、任意 contributor control、`uniformControlUnproven`、frame/resource/target/state/hazard/generation 与 runtime encode 继续硬拒绝。当前首断点转为审查仍在 encode 前发生的 control policy failure；bounded owner 撤权仍属于步骤 8，不能由产品 worker、pipeline warmup、helper 重试、两个正例或局部回滚提前宣告。
