...
截至 2026-08-16，步骤 1–5 与步骤 7 已闭合到产品 worker 切片：one-pass Light Shafts 与 optional-texture Opacity present/absent 两个 correctness atom 已批准；固定 glslang/SPIRV-Cross helper、独立进程预算、launch result cache、exact Metal pipeline warmup/negative cache 及 helper timeout/signal 后同启动隔离和新启动重试均有产品证据。步骤 6 的公共 effect-local failure 已覆盖 launch-time frontend/shader-preparation/color/static-uniform-schema、四类 Metal pre-encode failure、producer/target 全可证且没有 script attachment 的多 dynamic value contributor 歧义、唯一已证 value contributor 附带单个未证明 script attachment、唯一 value contributor 且没有 script attachment但对应 producer 完全缺席、多个 value contributor 中至少一个 producer 完全缺席、零个已证明 value contributor 但有 authored fallback 与单个未证明 script attachment、frame-time 精确 `dynamicUniformBindingInvalid` / `staticUniformBindingInvalid`、作者 declaration 与 host-owned field 的 `hostUniformDeclarationConflict`、同一 reflected field 的 alias/field 双 declaration `uniformDeclarationConflict`，以及 launch-time authored sampler annotation/declaration 无法形成 schema 的 `authoredSamplerSchemaInvalid`；它们只把 previous current 复制为失败 stage 输出、继续 suffix 并保留 typed effect identity，不执行、解释或把未证明 script 预判成 control/value producer，也不在多个 owner 中选择值或伪造缺失 producer/动态值/static/host值/texture。multi-node/FBO、`activeUniformSchemaMissing`、真正的 `hostUniformBindingInvalid`、launch-time schema `uniformBindingInvalid`、同 user-property key 但 target mismatch、无 authored fallback 的零 contributor、重复/混合 script attachment、已知 media restart attachment 缺其 Timeline value owner、frame/resource/target/state/hazard/generation/runtime encode 继续硬拒绝。`reachable-sampler-schema` 的 frame/product 重复 owner 已撤销：frame selection 只能消费同一 launch envelope 冻结的 reachable samplers 与 compiled variants，不能在 frame 首次解析 source schema 或编译新 key；`RuntimeCatalog` 的 source reachability 暂时只服务 launch resource-demand discovery，不取得 Program/产品执行权。旧 compound `activeSamplerSchemaInvalid` 已从当前产品 writer 删除：binding order、duplicate slot、slot/name identity、internal target、active `[PASS]` 与 cross-variant divergence 现在分别以 `samplerBindingOrderInvalid`、`samplerBindingDuplicateSlot`、`samplerBindingIdentityMismatch`、`samplerInternalTargetUnsupported`、`activePassUnsupported` 与 `samplerVariantSchemaDivergence` 硬拒绝，且都不在 visual passthrough allowlist；frame 在构造 binding 字典前先完成同一共享验证，duplicate slot 不再触发进程级 trap。该阶段仍在 V0，冻结时的下一门是以既有隔离 authored `formatcombo` 候选确认 `samplerVariantSchemaDivergence` 是否为真实首断点；该历史表述已由后文 Chromatic visible completion 取代。active pass/internal target 保持关闭并留给 V1 ordered graph/target；bounded owner 撤权仍属于步骤 8，不能由 worker、pipeline warmup、helper 重试、两个 Fast atom或局部回滚提前宣告。
...
随后闭合的 non-host uniform declaration provenance 原子把 defensive active-schema 缺席与作者 alias ownership 冲突分开：Finalizer 对 `variant.activeUniforms[field.name] == nil` 返回 `activeUniformSchemaMissing` 并继续硬拒绝；只有同一 reflected field 同时匹配多个不同 authored key 时返回 `uniformDeclarationConflict`，exact single-current-pair leaf 才以 `material-finalizer-uniform-declaration-conflict` 复制 previous current并继续suffix，不选择 `alpha`、`u_Alpha` 或任一动态 owner。派生真实内容 `9000000620` 从批次前 `uniformBindingInvalid`、claimed/encoded/GPU 全 0 与无 compositor/next-frame 的黑窗恢复为 `Program → failed effect-local passthrough → Program`，frame 0/next-frame 六条 transaction 全部 GPU completed 且末段由 compositor 消费；`9000000621` two-node 反例在公共 `graphBlocked` 门 accepted/claimed/encoded/GPU 均为 0 并保持全黑。删除 `u_Alpha` alias、只保留 `alpha=0.8` 的 `9000000622` 为 targeted 1/1 PASS、fallback 归零、三个普通 Program 恢复；正门相对回滚只在目标 bbox 改变 65,792 个像素，证明 passthrough 保留 previous current 而非任选 declaration。正门按严格 typed CPU failure 聚合为 NON-PASS，只达到 `slice-visible`；route 仍为 `prefer-generic`，没有 Fast Suite、官方对照、owner migration 或 parity-release。其后 authored sampler schema 原子见下段；`activeUniformSchemaMissing` 仍未放宽，也未进入 V1。

随后闭合的 authored sampler schema provenance 原子新增 `authoredSamplerSchemaInvalid`，只承接作者 sampler annotation/declaration 或 `formatcombo` 无法形成 launch schema 的失败；typed variant-cache construction 保留该 provenance，不再由 failable initializer 吞掉原因。只有 exact single-current-pair leaf 才以 `material-variant-envelope-sampler-schema` 复制 previous current 并继续 suffix，不选择、伪造或绑定 texture。派生真实内容 `9000000630` 从批次前 accepted/claimed/encoded/GPU 全 0、无 compositor/next-frame 与黑窗恢复为 `Program → failed effect-local passthrough → Program`，frame 0/next-frame 六条 transaction 全部 GPU completed 且末段由 compositor 消费；`9000000631` two-node 反例在更早公共 `graphBlocked` 门 accepted/claimed/encoded/GPU 均为 0，harness 另锁相同 typed reason 的 multi-node 不取得产品输出。删除未知 `mode` 的 `9000000632` 为 targeted 1/1 PASS、fallback 归零、三个普通 Program 恢复；正门相对回滚只在目标 bbox 改变 65,792 个像素，证明 passthrough 保留 previous current 而非执行未知 sampler mode。正门按严格 typed CPU failure 与整层 exact-backend断言聚合为 NON-PASS，只达到 `slice-visible`；route 仍为 `prefer-generic`，没有 Fast Suite、官方对照、owner migration 或 parity-release。active `[PASS]`、internal target、binding order/slot/name、cross-variant divergence、frame/resource/target/state/hazard/generation/runtime encode 继续以 `activeSamplerSchemaInvalid` 或其他 invariant 硬拒绝。其后的 launch-envelope owner 收敛见下段；本原子的局部视觉边界未被扩大。

随后完成的 launch-envelope owner 收敛撤销了 frame/product 对 source schema 与 variant compiler 的重复所有权：`resolveSelection` 只匹配 `precompileLaunchEnvelope` 已冻结的 reachable sampler 集合和 ready compiled entry，launch/frame implicit framebuffer identity 不同、无法唯一匹配或出现未预编译 key 时以 `identityInvariant` 硬拒绝且 compiler/cache 计数不增长；旧 Finalizer/TextureResolver 的隐式 cache overload 已删除。真实 `3767460992` 批次前后保持同一组 22 个 ordinary Program subjects、7 个 accepted/succeeded layer、52 个成功 transaction、GPU completion、terminal compositor 与 next-frame 集合，ready/after 均非黑且有运动；截图因动态相位不同不作像素等价。该 targeted 1/1 PASS 只是 owner consolidation/no-regression，不是新增视觉兼容、Fast/full、官方一致、owner migration-complete 或 parity-release。该包冻结时的下一首断点是拆分剩余 compound `activeSamplerSchemaInvalid` provenance；后续现役结果见能力台账与运行证据索引，不能把这里的历史下一门读成当前事实。

随后闭合的 sampler binding provenance 原子把 compound `activeSamplerSchemaInvalid` 拆成六个稳定失败码并从产品 writer 移除旧码：binding order/duplicate slot/slot-name identity 在 invariant phase 硬拒绝，internal target/active pass/cross-variant divergence 在 preparation phase 硬拒绝；纯容器尺寸与 slot 范围防御仍使用 `identityInvariant`。六个新码继续统一映射 `material-variant-envelope-invariant`，不在 visual passthrough allowlist。共享 binding 验证同时覆盖 launch compile、frame selection 与 texture resolver，确保 duplicate slot 在 `Dictionary(uniqueKeysWithValues:)` 前返回 typed failure。真实 `3767460992` 保持同一 22 个 Program disposition、52 个稳定 transaction、7 层 GPU/compositor/next-frame 集合，targeted 1/1 PASS 且非黑有运动；这只证明 provenance refactor no-regression，不新增视觉能力。route 仍为 `prefer-generic`；该包冻结时仍在 V0，下一门曾是核实隔离 authored `formatcombo` 候选，现已由后文 Chromatic visible completion 封存。active pass/internal target 属于 V1 ordered graph/target 候选，不能因已能精确诊断而提前准入。

V0 首个可见完成门随后由真实 `2902406982` 资源树派生、无 SceneScript attachment 的 Chromatic visual atom 闭合。公共 frontend 只增加 scope-aware global reference、same-slot RGB channel reconstruction、唯一 typed 浮点 texture-coordinate identifier 的 `.xy`、结构可证乘除链到 scalar 的 `.x`，并把 `g_ModelViewProjectionMatrixInverse` 作为 typed host uniform 编码；没有 sample/layer/path/hash 产品分派。正门 descriptor `6201#effect#1373` 形成 Program `e14575b5...bab2`、CPU `encoded-output`、GPU completion、publication generation `2/4`、terminal compositor 与 next-frame；预登记 ROI 的 22/22 帧均满足 `R < 0 < G < B`，after 位移为 `-30/+30/+60 px`。author-disabled control 不执行 effect；`AUDIOPROCESSING=0` 的合法 combo 因尚未证明的 `float2 - float4` additive coordinate 在 Metal preparation 失败，只对当前 effect 做 previous-current passthrough，保留 base、GPU/compositor/next-frame。这满足 V0 的 `slice-visible` 完成门并把当前主线推进到 V1，但 route 仍为 `prefer-generic`；它不是 Fast Suite、`generic-only`、owner migration、官方客户端 parity 或 release 证据。来源轴只使用 `authored-corpus-observation`、`MyWallpaperX-current-evidence` 与 `MyWallpaperX-strategy`。

以下 1-8 项保留为 V0 首批执行记录，不再构成当前待办；V1 的首断点必须从能力台账与运行证据索引重新选择一个公共 graph/FBO correctness atom：
