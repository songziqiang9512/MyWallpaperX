...
首轮评估确认的方向仍成立：优先建设通用 runtime，而不是增加按样本命中的 effect 分支。截至 `c654571`，当前增量已包含 authored graph、typed texture registry/property fallback、ShaderContract、B0 binding program/per-surface snapshot、direct dynamic text、十一类 strict backend、ordered strict chain、同帧 copy/swap、受限 history、Precise Blur interleave/legacy compose、exact Shake/Foliage Sway/Water Ripple/Water Waves/Water Flow/X-Ray、13 个精确登记的 built-in 粒子纹理、strict depth-one eventspawn/natural-eventdeath child、Puppet bind-pose/静态 attachment/严格单 clip MDLA LBS、BC1/2/3 premultiplied 解码与 REFRACT 粒子 fail-closed。Timeline、SceneScript、system/media text producer、Texture Variants、通用/effectful/nested provider、真实 persistent/history、generic compose、其余 effect 和完整 particle/Puppet 生命周期仍是主要缺口。精确当前基线只在 [总覆盖台账](semantics/coverage-ledger.md) 维护。
...
| 自动门（当前增量） | interpretation format 29，签名 App、Metal 双帧；十二类 strict backend/ordered chain、Puppet、B0 property、dynamic text、provider/file override、Particle 与 stock PNG resolver 合同；435 项 Scene 测试为 433 通过、2 跳过；无 timeout、stop 后 surface=0 | 45 样本完整快照与固定 13 样本 PASS 是 `678a052` 历史门；当前本机固定门仅 6/13 样本可用且矩阵仍为 v25，最新数据以 [运行证据索引](semantics/runtime-evidence-index.md) 为准；preview 不设绝对阈值，仍没有 Windows WE 同配置录屏差异门 |
...
21 个历史样本中有 18 个声明 particle，共 60 个 root layer、55 个可达 definition 和 77 个 child 引用。当前 renderer 已有作者包内 2D sprite、13 个精确 built-in key、常见 emitter/initializer/operator、Sprite Trail、sprite sheet、静态 override，以及 strict depth-one event child 子集；固定 13 样本门加载 18/27，完整 45 样本门加载 83/131（REFRACT fail-closed 口径），其中 `3724289844` 为 5/5、`3750813609` 为 7/9，`3765760121` 的默认隐藏 particle 保持不创建。`3768903841` layer 264 的 eventspawn child 已在真实缓存门执行；`2131872317` layer 529/832 的 natural-eventdeath trail burst 已执行，4.5 秒签名 App 截图可见两簇烟花。layer 1375 的复杂 child 仍 fail closed；world-space、rope、static/follow/nested child、动态 control point/override、collision/delete 和音频驱动也未执行。不能用这一严格子集概括成完整支持。
