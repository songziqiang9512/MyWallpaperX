...
21 个样本中有 18 个声明 particle，共 60 个 particle layer，按默认可见性与父链计算有 54 个有效层。当前 renderer 已有作者包内 2D sprite 主链，但正式 11 样本中 `3742133044`、`3750813609` 仅各加载 1 个粒子 layer，`3765760121` 的默认隐藏 particle 保持不创建。样本还声明 box/sphere emitter、sprite trail、rope trail、child graph、音频驱动和 Wallpaper Engine 内建资源；这些高级语义仍需逐类补齐，不能用“已有粒子”概括成完整支持。
...
5. 在主构图和上述三条基础能力稳定后，再逐类提高 waterflow/ripple、depth parallax、godrays/glitter、puppet、音频和复杂 offscreen pass 的视觉一致性。
