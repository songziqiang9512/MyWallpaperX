...
- **M5.2 最小 daemon（同二进制模式，契约 §2 修订）** ⬜ 交接撤回 `868bbfe3` 原型：EOF 不经 Host.stop、固定延迟冒充排空、`firstFramePresented` 实为提交计数且无请求归属、未消费命令静默丢弃。原型没有主 App 消费者，撤回只删除未接入的显式 CLI 模式，不改变普通 Scene 播放。重做门：请求身份绑定、真实 present、版本/未消费命令拒绝、EOF/shutdown teardown 与 GPU drain 反例；安全门通过后再进 M5.3。历史 smoke 只证明进程运行，不能追认契约完成。
