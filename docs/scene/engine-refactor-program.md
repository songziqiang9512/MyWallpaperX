...
- **E2a 静态边界地图与归属设计（2026-09-26 勘察定稿，勘察报告存本机证据缓存 `docs/scene/evidence/20260926-e2-control-boundary/`）：**
  - **现状差距（按严重度）**：①"当前活动壁纸"四份并行真值（Manager `currentWallpaper`、Engine `currentWallpaper/currentContentPath/currentPlaybackContentKind`、SceneDaemonClient `activeIntent/activeRecordID`、Steam 下载卡片 `isRecordCurrentlyPlaying` 派生判定）互不同步；②epoch 已有但引擎私有（`WallpaperEngine.playbackIntentEpoch` 只守卫 daemon 崩溃回放）且 Manager/Coordinator 不知道；③提交点不一致——video 在引擎确认前写真值（`+WallpaperApplication.swift:31-32`）、web 在 host 启动前清 Manager 真值（coordinator :443-444）、scene 在 `.launched` 才清（:523-524），三种 runtime 三个停止时机（通知观察者/launch 内部/applyWallpaper 内部）；④失败无"保留旧输出"保证（web stop-then-start 无回滚；video 失败递归重放完整切换事务）；⑤切换意图基本不在命令通道——video/web/静态直调引擎或 NSWorkspace，仅 `.loadScene` 走 multiplexer；`.web` 引擎类别注册缺席、video handler 回手调 Manager（switchNext）、引擎反向向 multiplexer 发命令（`+PlaybackControl.swift:15`）三处依赖倒置。
  - **归属设计（裁决）**：不新增协调器（§3.1 既定）。**requested 归 WallpaperManager**——它是唯一持久化选择权威，切换入口唯一写点；**prepared/visible 归各 runtime adapter 作为执行事实**——Scene 现有 `pendingIntent→.launched` 是范式（pending=requested 的 runtime 投影、`.launched`=commit），Engine 侧 `currentWallpaper` 等降格为执行事实镜像（UI 显示投影，§3.1 语言）；**stopped 归命令通道的 Manager 回收**——`.stop` 必须同步清 Manager 真值（现状问题 10：Steam 详情页停止后真值不回收）。**epoch 产品化**：`playbackIntentEpoch` 从引擎私有提升为 Manager 持有、随每次 requested 递增、向下注入各 adapter（Scene 已有 `sessionGeneration` 对齐机制可映射）。
  - **事务形态裁决**：Scene 的 prepare-then-commit（旧壁纸在 prepared 阶段继续播放、`.launched` 才 commit+retire previous、失败保留旧输出）推广为唯一切换事务形态；video 的"真值先行"与 web 的"stop-then-start 无回滚"按此收敛。video→video 同 daemon 换 play 命令保留短路优化（:233-239），但真值提交同样延后到 daemon confirmed。
  - **迁移切片（每步独立可验证，未列完不启动后续）**：E2a-1 epoch 产品化（Manager 持有+注入，先只读注入不改提交点——回归门为现有 multiplexer/daemon wiring 测试）；E2a-2 `.stop` 回收 Manager 真值（修问题 10，行为门=Steam 停止后卡片判定与 Manager 一致）；E2a-3 video 切换提交点延后（真值写从 :31-32 移到 daemon confirmed 回调——行为门=失败回退不再递归重放整事务）；E2a-4 web stop-then-start 改 prepare-then-commit（行为门=web 启动失败旧输出保留）；E2a-5 切换意图全部上命令通道（video/web/静态的启动停止经 multiplexer，修三处依赖倒置——`PlaybackEngineKind.web` 注册、switchNext 去回手、引擎不再自发命令）。E2b/c/d 在此之后按卡执行。
...
- **候选 1 已执行（2026-09-26，清单 437a1788 落地）：**死簇三处已删（`SceneGraphNodeScheduler.swift`、`SceneGraphCommandRuntime.swift`、`makeCommandRuntime()`），layout 两行、telemetry 源清单+计数断言、pool 源清单、表格测试四场景块与两测试方法同步删减（copy/swap GPU 行为由活路径门覆盖：encoder `copyReadbackMatches`、execution-state swap 映射断言、executor 准入反门 `swap-target-descriptor-incompatible`）；−552 行；5 模块测试 + source_sets/verify/code_health + checkpoint 构建全绿，只读终审五项 PASS。候选 2 已复核执行（2026-09-26）：普查 5 文件中 **4 个过时**——`SceneDirectDrawLayerRenderer`（`drawQuadLayer` 被 SceneMetalRenderer:799 调用）、`SceneScriptLayerWorldTransformPublication`（`publishLayerWorldTransformsIfNeeded` 被 HandleBridge:796 调用）、`SceneAuthoredShaderPreparation+Support`（`failure` 同类型裸调用）、`SteamWorkshopSceneDownloadRecord+Scene`（`isSceneLaunchable` 被 Launchability 扩展使用）全部保留；仅 `SceneEffectMaskSemantics.swift`（29 行，`declaresMask` 零引用）真死已删（−31 行，layout/gates 同步）。**教训：文件名级 grep 不足以判死——扩展文件成员可不点名使用，删除判定必须成员级核查。** 候选 3 部分执行（2026-09-26）：XG 探针残留（XG1/XGATE 四块硬编码 layer 1315/813 的 DEBUG NSLog，约 40 行）已删——终审附注登记的 BindingCompilation:520 残留清零；warmup 报告器复核为**活功能**（启动管线预热 + 严格 opt-in 诊断打印）不可删；其余 ~240 行"launch 报告器"因普查证据目录仅存 README、无机器清单无法精确识别，按成员级核查标准登记为需重新识别后再删。
...
**AS1 身份矩阵与回归保护口径（同日续）：**后续功能开发期间，AS1 的用途从「找消融点」转为**回归保护**：同一 `--no-execution-observations` 口径、同一 30 s warmup＋≥60 s 窗口、同一 `phase=performance`／`performance-stages-frames`／`performance-gpu` 解析，用来发现新能力是否重新引入整图序列化、重复 pass 切分或新增整图拷贝。测量对桌面负载的敏感度必须如实分层：本批三次运行的 `cpu_frame_p50`（5.956／5.889／5.918 ms）与 `gpu_frame_p50`（7.566／7.530／7.588 ms）在噪声内稳定，而 `actual_present_p99`（16.667／25.0／25.0 ms）与 `present_over_1_5_ratio`（0.18%／1.05%／0.87%）随共享桌面负载摆动，保持「只作呈现参考、不进回归判定」；会话早期的 run1–run2 曾出现 18–23% 噪声与一次 5099.992 ms 呈现停顿，已按同一规则作废。**尚未验收：30 FPS 节能档、混合刷新率双屏、M1 或最低支持设备、低电量模式、冷启动／缓存热启动各 5 次、功耗与 wakeups、30 min 长稳，以及 Release 等价构建（现为 `-O` Debug）。** 这些是继续性能结论前必须先补的身份维度。

**HEAD 回归保护复测与 AS3 解阻塞（2026-09-25）：**2938612768 matrix 转绿（82330d2f/934d31fa）后按冻结口径复测三次（`-O` 命令行覆盖、warmup 30 s、窗口 63.5 s）：`cpu_frame_p50` 5.964→5.342（−10.4%，观察非配对）、present over-budget 0.57%→0.0%、GPU 预算零违例、`gpu_frame_p50` +3.4% 门内（p95 负载敏感 2/3 抬升）；census 确定性 +1/+1/+1（主 24／offscreen 79／effect capture 6）。**每帧 6 次整纹理 blit（35.5 MB/帧，基线 0）次日归因闭合（2026-09-26）：D2b''（50672fda）使 239/657/1509 以激活直通执行后，每帧为 previous-current 在 pair ping-pong 成员间 blit（3 层 × 2 成员）。** 成员互斥语义核验（同日）：跨成员别名发布会使下一阶段采样与写入同一纹理（渲染冒险）、同成员调度无法预知运行时直通——**AS3 对该项记"有证据不实施"（机制固有成本），census 增量作为新基线事实登记**；E1c 前提失效担忧解除（rehydration 零命中）。AS3 本样本可删对象仍为零，下一动作回身份矩阵补齐。详见[证据](semantics/runtime-evidence-current.md#e-2026-09-25-as1-regression-protection-rerun)、[归因](semantics/runtime-evidence-current.md#e-2026-09-26-as3-copy-attribution)与[裁决](semantics/runtime-evidence-current.md#e-2026-09-26-as3-alias-verdict)。
...
**当前结果（2026-09-16）：**已补最小 GPU 侧观测：`SceneGPUCensus` ＋ `ScenePerformanceCounterHub` 的帧路径普查计数器（主 pass 创建、offscreen pass 四分类、整纹理拷贝 pass／字节、framebuffer snapshot、named graph publication，以及字节量未知格式的 unmeasured 计数），由 Debug runner 按窗口输出 `phase=performance-gpu`，benchmark 严格解析并校验「四类 offscreen 之和 = offscreen 总数」「子集不超过总数」「frames > 0」，非法即整项性能证据 unavailable；旧日志无此行仍可解析，不静默降级既有证据。重 graph `2938612768` 三次运行（CDHash `9160d747…`、30 s warmup、63.5 s 窗口）每帧恰好 **23 个主 pass（22 次切分）＋78 个 offscreen pass（resolved material 51 ＋ graph resource source capture 22 ＋ initialization 0 ＋ offscreen effect capture 5）＋128 draw**，而 blit 拷贝、framebuffer snapshot、named graph publication、depth 全为 **0**；`gpu_frame_p50` 7.566／7.530／7.588 ms（噪声 0.8%），`cpu_frame_p50` 与无普查的 AS1 门（5.958 ms）不可区分。**结论：本样本没有可删的整图 copy／capture，AS3 首选项在此无对象；22 次切分与 22 个渲染式 source capture 1:1，是「读取已累积合成」的固有代价。** 但该样本 matrix 为 NON-PASS 且 `graphOutputPublicationCopies=0`（样本声明 named_texture_references），**尚不能证明这 22 个 capture 属于最终产品路由**，故本批**不选择消融点、不实施 AS3 改动**：普查按回归保护口径冻结，等路由稳定与 AS1 身份矩阵（30 FPS 档、双屏混合刷新、M1／低内存、Release 等价构建、含 blend／refraction／history 的样本）补齐后再定。详见[证据](semantics/runtime-evidence-current.md#e-2026-09-16-as3-gpu-operation-census)。
