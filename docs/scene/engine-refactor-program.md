...
- **M0.3 设置容器拆公共层 + FPS 档** 🔶 cbd96207：文件迁移/FPS 档已落地，公共边界未完成。**修正版方案已定稿（2026-09-14）**：不用协议倒置（MainActor 隔离陷阱致 conformance 诊断不可读），改用闭包注入——`Shared/Settings/AppSettingsActions` 结构体持 20 个动作闭包 + `AppSettingsPanelDependency` ObservableObject 持 settings 值，`AppKitSettingsView` init 接收 dependency 而非 WallpaperManager。实施 defer 至下批（1500 行视图重构 + 60 处引用 + 17 方法映射需独立批次），脚手架已验证后撤回。FPS 档已在 cbd96207 落地（UserDefaults + 命令层下发）。
