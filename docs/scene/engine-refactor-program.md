...
**HEAD 回归保护复测与 AS3 解阻塞（2026-09-25）：**2938612768 matrix 转绿（82330d2f/934d31fa）后按冻结口径复测三次（`-O` 命令行覆盖、warmup 30 s、窗口 63.5 s）：`cpu_frame_p50` 5.964→5.342（−10.4%，观察非配对）、present over-budget 0.57%→0.0%、GPU 预算零违例、`gpu_frame_p50` +3.4% 门内（p95 负载敏感 2/3 抬升）；census 确定性 +1/+1/+1（主 24／offscreen 79／effect capture 6）。**关键发现：每帧 6 次整纹理 blit（35.5 MB/帧，基线 0）——history rehydration 拷贝回归（Executor+Validation），E1c 当年"无 history closure"前提失效**；AS3 的"无对象"裁决更新为：路由已稳定，6 次 rehydration 拷贝 + 22 次 source capture 构成 AS3 候选面，rehydration 拷贝为首个可证伪候选（先证消费者与更廉价的 history 保持）。详见[证据](semantics/runtime-evidence-current.md#e-2026-09-25-as1-regression-protection-rerun)。
