...
- **候选 1 复核与删除清单（2026-09-26，待下一批机械执行）：**`SceneGraphNodeScheduler.encode` 零产品调用今日复核成立；死簇=`SceneGraphNodeScheduler.swift`＋`SceneGraphCommandRuntime.swift`＋`SceneGraphRenderTargetTable.makeCommandRuntime()`。触达面：`scene_source_layout.json:454/458` 两行；`test_scene_performance_telemetry.py:96` 与 `test_scene_offscreen_texture_pool.py:41` 的源清单条目；`test_scene_graph_render_target_table.py` 需重构（删 `scheduledBytes` helper 与 scheduler/copy/swap/incompatible 四场景块、`test_scheduler_interleaves…`/`test_copy_blits…` 两测试方法及 result 键——copy/swap GPU 行为已由活路径 `SceneGraphResourcePassEncoder` 的执行器测试覆盖）；`test_scene_graph_texture_publication.py:123` 与 `test_scene_material_copy_history_rendering.py:40` 是独立本地桩不受影响。候选 2/3 的今日复核未做，执行时先复核再删。
