...
- **M4.2 Phase 4 资源缓存** 🔶：executor reset 热缓存子项已落地——reset 只推进命令世代以拒绝旧 PreparedPass，保留由完整 `MetalCompileStateKey`（shader 语义/render state/attachment/sample/write mask/device）寻址的不可变 PSO 与负缓存；相同内容在 reset 后不再重新 `makeLibrary`/创建 PSO。MTLLibrary 复用子项亦已落地：PassEncoder 按完整 `metalSource` 缓存 library 与编译失败，launch warmup 的不同 PSO 变体并发请求同一源时只允许一个编译者，其余等待并消费同一结果。其余：colorBlend pipeline 启动预热、纹理 decode 去重、上传私有 queue 合并、纹理缓存字节上限（接预算档）、userPropertyTextures 字符串键→整数 ID。
