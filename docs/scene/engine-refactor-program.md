...
**HEAD 回归保护复测与 AS3 解阻塞（2026-09-25）：**2938612768 matrix 转绿（82330d2f/934d31fa）后按冻结口径复测三次（`-O` 命令行覆盖、warmup 30 s、窗口 63.5 s）：`cpu_frame_p50` 5.964→5.342（−10.4%，观察非配对）、present over-budget 0.57%→0.0%、GPU 预算零违例、`gpu_frame_p50` +3.4% 门内（p95 负载敏感 2/3 抬升）；census 确定性 +1/+1/+1（主 24／offscreen 79／effect capture 6）。**每帧 6 次整纹理 blit（35.5 MB/帧，基线 0）次日归因闭合（2026-09-26）：D2b''（50672fda）使 239/657/1509 以激活直通执行后，每帧为 previous-current 在 pair ping-pong 成员间 blit（3 层 × 2 成员）——有消费者，非无消费者拷贝；消融候选=跨成员免拷贝别名发布（同成员分支已有 `rewrappedForGraphIdentity` 先例，需核验 pair 池成员互斥语义）；E1c 前提失效担忧解除（rehydration 零命中）。** 详见[证据](semantics/runtime-evidence-current.md#e-2026-09-25-as1-regression-protection-rerun)与[归因条目](semantics/runtime-evidence-current.md#e-2026-09-26-as3-copy-attribution)。
