...
| C02 Relative offset | CP 可相对系统 origin 设置 offset。 | `L3` | [DEF] [SIM] [T-DEF] [T-SIM] [T-RUN] | static definition offset 与 static/absolute-Timeline instance position 只在 root Sphere/Box emitter 的 local origin 相加；snapshot 缺值时回退 static。没有 parent/world/child/operator consumer 或 Windows 数值门。 | 多 emitter、parent/space 转换和 Windows fixed-seed 位置 golden。 |
...
- 等级分布为 `L0 16 / L1 39 / L2 25 / L3 91 / L4 0`；该数字按表内 171 个能力行机械重计并取代已漂移的旧汇总，本批把 E03 从仅识别名称的 `L1` 升为有 typed dependency、静态 alpha emission map、正反自动门和库存预览资产运行证据的有界 `L3`。`L3` 主要集中在 Sphere/Box/静态 plain-image Layer Image、常见 exponent-biased random initializer、基础 movement/change/lifetime Alpha/Size/Position oscillation、非音频 Turbulence、Sprite/Sprite Trail/strict RopeTrail、strict static/event child、已测试的静态 override 及其 General gates、TEX sampler/mip/atlas aspect、两种 blend 和 strict REFRACT 双槽子集。
...
- 开发批次应优先消除公共断点：raw TEX frame axes/sidecar nominal geometry、author allow gates、absolute Timeline root-emitter Control Point position 与静态 plain-image Layer Image 已闭合；粒子音频响应仍因公式证据不足保持 fail closed，下一步优先补有合法 consumer 的 Control Point angle/space 子集，或为 Layer Image 增加有证据的动态/text/puppet provider，再补剩余 Children/Event、Collision 和 Rope，最后扩展动态 world-space、material/lighting。逐样本 hardcode、把 unsupported 静默回退成 Sprite/translucent、或把程序纹理称为官方资产，都不允许升级等级。
