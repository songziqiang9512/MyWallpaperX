...
> Scene 实现基线：`f02f41d`；child/event 子集由 `f4173ea`、`7d53c10` 实现，`4e64232` 补齐延迟截图证据入口，`c654571` 增加 `rosepetals`/`beam_1`，`f02f41d` 增加 `particle/fire/fire1`。固定 13 样本门 particle 为 `18/27`，完整 45 样本门为 `86/131`；当前两层运行门、签名 App 身份及聚合缺口统一见 [运行证据索引](runtime-evidence-index.md)。仍不是通用 Particle System。
...
| M17 Built-in texture identity | `particle/...` key 指向 Wallpaper Engine 内置粒子资产。 | `L1` | [TEX] [AST] [T-AST] | 任意 key 可识别为 built-in reference，但只支持十四个精确枚举。 | 建立合法官方 assets 来源与版本化 key registry。 |
| M18 Fourteen generated built-ins | 当前十四个高命中 key 可生成确定性替代纹理。 | `L3` | [TEX] [BUILTIN] [T-TEX] [T-RUN] | 程序图形不是官方资产；`chromaticdot` 是白色软点，`halo_4` 近似小型发光弹头，`rosepetals` 是单花瓣遮罩，`beam_1` 是水平软光束，`particle/fire/fire1` 是低能量火焰遮罩；均只保证受控 shape family 和安全 alpha。 | 每个 key 先以样本自带 preview 验证明显外观，再与合法 Windows WE 输出做尺寸/通道/pixel 差异门。 |
...
- 当前固定矩阵的“可见粒子 18/27”、完整矩阵的“86/131”、`3724289844` 的“5/5”与 `3750813609` 的“7/9”只是样本运行门，不是上述 171 项的兼容率；world-space fail-closed 也不能计为可播放。
