...
| M06 Normal map | normal texture 参与 lighting/refraction 的表面方向。 | `L3` | [AST] [REFR] [RUN] [GPU] [ROPE-TRAIL] [T-GPU] [T-REFRACT-PIXEL] [T-ROPE] [T-RUN] | 在该 strict REFRACT profile 内，normal 只接受 format 0/4，两者统一消费 A/G；format 8 RG88 只可用于 color/albedo slot 并由 `colorEncoding=.luminanceAlpha` 解释，format 5 / BC5 继续 fail closed。slot 1 请求 typed `.normal` 并跳过颜色预乘；项目自建门已锁定 single-image CPU BC3 decode、multi-image native BC3 分别与 byte-equivalent format-0 RGBA 位移一致，且有 G/A swapped、neutral normal 负对照。strict RopeTrail REFRACT 复用该背景/normal 路径并有切线方向与 UV span GPU 门。 | Lighting/Rope 独立 profile、官方/Windows DXT5n 恢复公式与 normal/pixel golden；若 normal policy 增加 swizzle/解包，取消当前同文件物理复用。 |
...
| M09 Refraction | 粒子可采样背景并产生折射。 | `L3` | [AST] [REFR] [SNAP] [RUN] [GPU] [T-AST] [T-GPU] [T-REFRACT-PIXEL] [T-RUN] | `e698c18` 只接受 pass 0、`genericparticle`、`REFRACT=1`、两槽、无 user binding、no-depth/no-cull、translucent/additive 的静态 profile；`d432d4d5` 让 slot 0/1 分别请求 `.straightAlbedo` / `.normal`。按作者 batch 顺序捕获此前 framebuffer，消费 amount/overbright 与 normal/albedo animation；自建 GPU 门已闭合项目内部 packed-normal storage equivalence 与 translucent/additive coverage 单次作用，`3770444459` 三个 water-impact 层另有 8 秒非空活动门。全分辨率 BGRA snapshot 上限 64 MiB，超限、未知 combo/state/format、缺槽均 fail closed；这是 clean-room 近似，无 Windows 像素等价。 | Windows 固定背景/法线/amount golden、官方 DXT5n 恢复公式、5K/多 batch 带宽预算、resize/多屏/8K fail-closed 门。 |
