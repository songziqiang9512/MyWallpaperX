...
> 当前结论：MyWallpaperX 已有可见的 2D Sprite 粒子子集，并执行五种 General author instance-override gate、root direct User Property 的 `alpha/size/lifetime/rate/speed/count/brightness/colorn`、相同七个 scalar 字段的 absolute Timeline、非音频 Turbulent Velocity Random、非音频 Turbulence operator、bounded classic Vortex、最多 64 粒子的 zero-separation Boids alignment/cohesion、zero-min centered Box、常见 Random initializer exponent、lifetime-normalized Alpha/Size/Position oscillation、TEX filter/address/mip sampler、单 sequence sidecar nominal frame aspect 与 raw TEX frame-axis 像素比例回退、严格的 child 层级 ≤ 2（depth-one static/default-static/`eventspawn` / natural-`eventdeath` / `eventfollow`，加 depth-two 仅 event 触发的 nested child）、有限 static origin translation、按 authored identity 准入的 root Sphere/Box emitter Control Point 0...7、有限非负且有序的 emitter 初始 speed range、有限三轴 Directions 与 Sphere `-1/0/1` Sign、绝对 Timeline 驱动的 root-emitter local Control Point position、depth-one static child 的有限 raw parent Control Point position copy、精确 pointer-lock CP 1...7 驱动的 bounded local Control Point Force、持续/混合/duration child emitter、静态可逆 layer world frame 下的 General/Movement/Renderer world-space、严格 `genericparticle` `REFRACT=1` 双纹理背景折射、root/child 共用且允许省略 `length` 的 bounded Sprite Trail、严格单 renderer/Screen 的 bounded Rope（含 subdivision、UV scale/equal-lifetime smoothing/scrolling 与 strict child 隔离拓扑）和有界 RopeTrail、静态同空间 plain image alpha bitmap 的 Layer Image emitter、仅 rate 的 Sphere/Box Random periodic active/delay 窗口，以及无 REFRACT/Lighting/Cutout/user binding 的单 screen Sprite `cullmode=normal` 子集；它仍不是通用 Particle System，尤其没有 dynamic override 的 SceneScript、冲突来源、direct color、relative/Combined、child inheritance 或存量粒子追溯更新，也没有 classic Vortex 的 audio/CP/center-force/Windows 数值公式或 `vortex_v2` ring、Boids separation/speed clamp/大群 spatial index、standalone initial delay、periodic burst/maximum-per-period、Layer Image 的 text/puppet source、Control Point angles/previous pointer/adjusted-space parent copy/cross-space/其他 consumer、event child transform、collision/delete event、动态 root world-space Rope、animated/multiple-renderer Rope、精确 ribbon join、通用 RopeTrail subdivision/UV、Audio Response、Lighting 和通用 Particle Material。
> 粒子落地提交分别是 eventspawn/death `f4173ea`/`7d53c10`、eventfollow `928acca`、持续 child/root aggregate budget `2f897bc`、static/default-static `899704b`、有限 static origin `678a052`、strict depth-two nested child/per-depth 预算 `b86db59`、非音频 turbulent velocity `4a17ee6`、strict REFRACT `e698c18`、REFRACT BC3 purpose `8b06538d`、static world-space `54a6ebc`、非音频 Turbulence operator `7b1d36f`、bounded classic Vortex `b6a1667e`、TEX sampler `d19e76b`、分布/offset/lifetime Alpha/Size oscillation `d792296`、实际 mip 链 `f4e8a0c`、lifetime-normalized Position oscillation `5911049`、strict RopeTrail `aa68bb8`、bounded Random periodic `69731707`、bounded raw parent Control Point copy `40768dfc`、bounded emitter Control Point identity `c6fc9c24`、bounded emitter speed `938630a0`、bounded emitter shape `ce16de08`、bounded pointer Control Point Force `482ed21a`、bounded basic Rope `9a098351`、bounded Rope subdivision/UV/strict-child `319214c5`、Sprite Trail omitted-length `36b9775a`、bounded Boids `1fd08811` 与 root dynamic instance override `32e3a928`。最近完整 45 样本门仍是 `f161de8a` 的 particle `129/130`，唯一 root 缺口为显式空 renderer；它早于 Rope/Sprite Trail/Boids/dynamic override/classic Vortex 五批粒子提交，不能作为这些能力的正门。本批另跑 official stock `previewvortex0`、粒子全套、simulator 聚焦组与签名 App，没有重跑 fixed13/full45。仍不是通用 Particle System。
...
| E12 One per frame | 作者 flag 可将本次发射限制为每帧最多一个。 | `L3` | [DEF] [SIM] [T-DEF] [T-SIM] | 当前 wire 准入 raw bit 2：stock corpus 中 13 个 Sphere/Box `flags=2` emitter 全部服务 Rope，未发现 `flags=1` 正例；执行中的“帧”仍是 1/60 simulation step，不一定等于 WE render frame。 | 不同 render FPS 下与官方计数核验；公开文档没有给出私有 wire bit。 |
...
| O16 Vortex | 围绕中心/CP 产生旋转场。 | `L3` | [DEF] [PAR] [VORTEX] [SIM-VORTEX] [SUP] [T-DEF] [T-SIM] | classic v1 只准 raw flags 0/1、无 audio/blend/CP/未知字段且显式 finite ordered distance/speed；省略 axis 默认 `(0,0,1)`，flags 0 用三维中心距离，flags 1 用无限轴径向距离，按右手方向施加项目自有 fixed-step 切向加速度，负 speed 反向，静态 speed override 服从 General gate。9 份 official stock classic corpus 中 `previewvortex0` 已有隔离多相位正门；`vortex_v2` 保持独立 unsupported。 | Windows fixed-seed 数值/轨迹 golden；classic audio/CP/center force、默认省略 distance/speed 与 `vortex_v2` ring/angles。 |
...
| O29 Audio-modulated operator | Vortex/Turbulence 等 operator 可读取频谱调制。 | `L1` | [DEF] [PAR] [SUP] [T-DEF] | 声明与 emitter/initializer 共用同一类型并保真保存；启用后进入 `audioResponseIgnored`，classic Vortex 另同时记录 `vortexUnsupported` 并零执行。当前 45 样本唯一 classic Vortex 命中 `2419444134` 正是 audio profile，因此不被 O16 的非音频 `L3` 准入。 | 同 E15：缺求值公式证据；需官方 audio formula 与真实频谱轨迹门。 |
...
| CH03 Static child | 在 particle system authored local origin 创建一次 child system。 | `L3` | [DEF] [PAR] [CHILD] [CHILD-EXPAND] [CHILD-SUPPORT] [RUN] [T-DEF] [T-RUN] | 显式 `type=static` 与 legacy 缺失 type 都只创建一次 strict child；允许有限 origin translation、depth-one exact raw parent CP position copy，以及 Sphere/Box + Sprite/Sprite Trail/bounded Rope。Sprite Trail child 共享 omitted-length `1` 默认与 malformed fail-closed；`3770444459` 的 water-impact droplets 在 layers 239/245/248 均为真实正例。Rope child 按 system 独立建链，支持 raw bit-2 one-per-frame、birth-time world origin 和跨 child 8,192 段聚合预算；parent CP mapping 只有零 transform/flags、无任何已知 consumer 或未知组件时才作为 unused marker 忽略。仍要求零 angles、单位 scale、probability=1、非音频且纹理可加载。depth-two static、adjusted/dynamic/active/malformed CP copy、非法 origin、angles/scale、非单位 probability、其他 unsupported renderer、缺资源继续 fail closed。 | visibility/stop teardown、adjusted/angle copy、depth-two static 与 animated Rope child 语义样本。 |
...
| IV06 Speed | 缩放粒子速度来源。 | `L3` | [DEF] [SIM] [DYN-OVERRIDE] [T-SIM] [T-BIND] | dynamic value 影响新生 initial velocity，并对当前步非音频 Turbulence 力使用同一倍率；不会重新缩放存量 velocity，其他 operator 是否消费仍未定义。 | live emitter/initializer/operator 组合与 Windows 轨迹门。 |
...
- 等级分布为 `L0 12 / L1 37 / L2 21 / L3 101 / L4 0`；该数字按表内 171 个能力行机械重计。本批不改变等级分布；R02、R13 与 CH03 在既有 `L3/L2/L3` 内扩大到通过正反门的 Sprite Trail omitted-length root/child 公共合同，malformed 值继续失败关闭。`L3` 主要集中在 Sphere/Box/静态 plain-image Layer Image、仅 rate 的 Random periodic、有界 emitter speed/shape、常见 exponent-biased random initializer、基础 movement/change/lifetime Alpha/Size/Position oscillation、非音频 Turbulence、Sprite/Sprite Trail/bounded Rope/strict RopeTrail、strict static/event child、root emitter CP identity/position、depth-one static raw parent CP position copy、pointer-lock CP/force exact 子集、已测试的静态 override 及其 General gates、TEX sampler/mip/atlas aspect、两种 blend 和 strict REFRACT 双槽子集。
- 最近完整门的 `129/130` 只是 root layer 样本运行证据，不是上述 171 项的兼容率；唯一缺口是应继续失败关闭的显式空 renderer。完整门另有 14 个初始 REFRACT root batch，`2131872317` 的 event-death child REFRACT 由延迟 runtime 门计证；`319214c5` 的 Rope child/subdivision 与 `36b9775a` 的 Sprite Trail omitted-length 扩展都晚于该完整门，只能由各自独立真实/stock 门证明。未准入 Rope 形态、动态 world-space、depth-three、音频/Boids/child transform 或其他 fail-closed declaration 仍不能计为可播放。
...
- 开发批次应优先消除公共断点：raw TEX frame axes/sidecar nominal geometry、author allow gates、root-emitter Control Point authored identity/absolute Timeline position、bounded emitter speed/shape、depth-one static raw parent copy、pointer-lock CP 的 bounded local Force consumer、静态 plain-image Layer Image、仅 rate Random periodic 与 bounded Rope subdivision/UV/strict-child 已闭合；粒子音频响应仍因公式证据不足保持 fail closed，下一步优先扩展有合法 consumer 的 Control Point adjusted/angle/space 子集、standalone emitter delay，或为 Layer Image 增加有证据的动态/text/puppet provider，再补剩余 Children/Event、Collision 与 Rope ribbon/RopeTrail，高阶最后扩展动态 world-space、material/lighting。逐样本 hardcode、把 unsupported 静默回退成 Sprite/translucent、或把程序纹理称为官方资产，都不允许升级等级。
