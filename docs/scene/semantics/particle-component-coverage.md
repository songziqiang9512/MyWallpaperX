...
| M06 Normal map | normal texture 参与 lighting/refraction 的表面方向。 | `L3` | [AST] [REFR] [RUN] [GPU] [ROPE-TRAIL] [T-GPU] [T-ROPE] [T-RUN] | strict REFRACT 按 RG88 luminance-alpha 或 RGBA alpha/green 解码；strict RopeTrail REFRACT 复用该背景/normal 路径并有切线方向与 UV span GPU 门。Lighting、Rope 与 Windows normal 数值未覆盖。 | Lighting/Rope 独立 profile、Windows normal/pixel golden。 |
