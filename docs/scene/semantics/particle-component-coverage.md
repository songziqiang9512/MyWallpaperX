...
| R03 Rope | 连续连接存活粒子，并维持 subdivision 与 UV continuity。 | `L3` | [DEF] [PAR] [ROPE] [RUN] [GPU] [T-BASIC-ROPE] [T-RUN] | strict 单 renderer/Screen/static-texture 子集按单调 particle ID 的出生顺序连接相邻 live 粒子，2...512 authored maxcount，连续归一化 UV 覆盖 `0...1`；重复 ID、非有限值、零长段与超预算失败关闭。端点 size/color 当前取段均值；subdivision、UV modifiers、animated texture、world-space、siblings/malformed/未知字段全部拒绝。官方 stock basic Rope 隔离视觉门为 `L3` 运行证据，不是 Windows parity。 | 端点插值、曲线 subdivision、UV scale/smoothing/scroll、死亡拓扑、pause/seek/reset 与 Windows trajectory/pixel golden。 |
...
| R11 Rope segments/subdivision | Rope 用 segments/subdivision 控制曲线细分。 | `L1` | [DEF] [PAR] [ROPE] [ROPE-TRAIL] [T-DEF] [T-BASIC-ROPE] [T-ROPE] | basic Rope 只接受 subdivision 缺失或显式 0，`segments` 一律拒绝；strict RopeTrail 消费 1...8 的 `segments`。Rope/RopeTrail 非零 subdivision 仍没有 geometry consumer。 | Rope/RopeTrail subdivision、GPU buffer 上限和 Windows 曲线门。 |
| R12 Rope UV scale/smoothing/scroll | Rope 保持连续 UV，并支持 scale、平滑和滚动。 | `L3` | [DEF] [PAR] [ROPE] [ROPE-TRAIL] [T-BASIC-ROPE] [T-ROPE] [T-RUN] | basic Rope 为 N 个 live 节点生成 N−1 段，UV 严格连续覆盖 `0...1`；`uvscale`、启用的 smoothing/scroll 与 animated texture 继续拒绝。strict RopeTrail 仍拒绝全部 UV modifier；`uvsmoothing` 是否属于 RopeTrail 的公开合同也未确认。 | 时间驱动 UV pixel fixture、scale/smoothing/scroll 数值合同与 Windows sampling golden。 |
| R13 Renderer selection failure | 没有可执行 renderer 时应 fail-closed 并给出可定位诊断。 | `L2` | [RUN] [T-DEF] [T-BASIC-ROPE] [T-ROPE] [T-RUN] | strict Rope 对 maxcount<2/超预算、advanced fields、animated TEX、world-space、multiple/malformed sibling 有独立 `ropeRendererUnsupported` 门；strict RopeTrail 对 missing/invalid length、超预算、advanced fields、animated TEX、multiple/malformed sibling、非数组/`null` renderer 容器也保持 fail-closed。显式空 `renderer: []` 沿 `missingSpriteRenderer` 拒绝；unknown material state 或 custom/missing shader 同样不执行。 | 为每个新增 renderer/shader family 建立联合准入与负向门。 |
...
- 等级分布为 `L0 12 / L1 38 / L2 21 / L3 100 / L4 0`；该数字按表内 171 个能力行机械重计。本批把 O11 与 C04 从 `L1` 升为 exact pointer-lock CP 驱动的 bounded local Control Point Force `L3`；E05 Directions/Sphere Sign、E13 speed、C01/E14 authored-ID 与 C06/C07 raw parent copy 边界不变。`L3` 主要集中在 Sphere/Box/静态 plain-image Layer Image、仅 rate 的 Random periodic、有界 emitter speed/shape、常见 exponent-biased random initializer、基础 movement/change/lifetime Alpha/Size/Position oscillation、非音频 Turbulence、Sprite/Sprite Trail/strict RopeTrail、strict static/event child、root emitter CP identity/position、depth-one static raw parent CP position copy、pointer-lock CP/force exact 子集、已测试的静态 override 及其 General gates、TEX sampler/mip/atlas aspect、两种 blend 和 strict REFRACT 双槽子集。
