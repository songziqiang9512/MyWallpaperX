...
| M17 Built-in texture identity | `particle/...` key 指向 Wallpaper Engine 内置粒子资产。 | `L2` | [TEX] [AST] [STOCK-ASSETS] [STOCK-RESOLVER] [T-AST] [T-RUN] [T-STOCK] | 2.8.42 / build 23967692 的 919 个播放候选路径已在 `SceneStockAssets.bundle` 建立物理文件，其中 223 TEX、198 sidecar，`assets/materials/particle` TEX 164 项；样本本地纹理优先，缺失时直接把 `.tex`/无扩展名引用映射到官方相对路径，不依赖 catalog。223/223 TEX 直查门通过；TEX payload 已是真实素材（格式/尺寸/mip 逐资产各异），定向 runtime 门加载 `debris1` 为 1024x128 R8 spritesheet 整张纹理。sidecar 仍不进入 runtime，序列帧未逐帧播放，逐资产官方通道/mip/atlas/像素 parity 未证明，因此只到 wired。 | 接通 sidecar metadata 与 spritesheet 序列帧 consumer，补通道、mip、atlas、颜色空间和 Windows pixel 门后再升级。 |
