...
- 等级分布为 `L0 12 / L1 37 / L2 21 / L3 101 / L4 0`；该数字按表内 171 个能力行机械重计。本批把 R11 从只识别 Rope subdivision 的 `L1` 升为 bounded root/child geometry consumer 的 `L3`；E12、R03、R12、CH03、X04 与 X08 保持 `L3` 但扩大到实际通过正反门的 bit-2 emitter、UV 与 child topology 边界。`L3` 主要集中在 Sphere/Box/静态 plain-image Layer Image、仅 rate 的 Random periodic、有界 emitter speed/shape、常见 exponent-biased random initializer、基础 movement/change/lifetime Alpha/Size/Position oscillation、非音频 Turbulence、Sprite/Sprite Trail/bounded Rope/strict RopeTrail、strict static/event child、root emitter CP identity/position、depth-one static raw parent CP position copy、pointer-lock CP/force exact 子集、已测试的静态 override 及其 General gates、TEX sampler/mip/atlas aspect、两种 blend 和 strict REFRACT 双槽子集。
- 最近完整门的 `129/130` 只是 root layer 样本运行证据，不是上述 171 项的兼容率；唯一缺口是应继续失败关闭的显式空 renderer。完整门另有 14 个初始 REFRACT root batch，`2131872317` 的 event-death child REFRACT 由延迟 runtime 门计证；`319214c5` 的 Rope child/subdivision 扩展晚于该完整门，只能由本批独立真实/stock 门证明。未准入 Rope 形态、动态 world-space、depth-three、音频/Boids/child transform 或其他 fail-closed declaration 仍不能计为可播放。
