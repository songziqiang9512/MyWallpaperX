...
| 3D model layer | `L3 bounded / S4 two real static slices` | bounded MDLV0023 format15/u16单mesh进入launch-prepared geometry、straight-albedo、独立UV/sampler的显式emissive-alpha mask、authored transform、reverse-Z depth与现役main pass/compositor；direct model material link以typed source fact发布，精确小写 `color/alpha` 优先于同材质中的编辑器默认 `Color/Alpha`，`LIGHTING=0` 保留base albedo/tint而只关闭场景光；`tintfront/tintback/tintwexponent`、tint-mask alpha及scripted back tint由共享材质/QuickJS owner消费；真实`3437487219`的Earth/Cloud双壳可见且角度脚本按degree API发布为内部radian；真实native-perspective `3509243656`的scene Eye/Center/Up/FOV与省略layer override已进入同一camera frame，可选mask采样失败只关闭channel；[E-V4-STATIC-MODEL-BASE-COMPOSITION](runtime-evidence-index.md#e-v4-static-model-base-composition) | display SceneScript/shared orbit、8K straight-alpha、UInt32/V16、runtime-provider model、node hierarchy、多mesh/material、normal/PBR、HDR bloom、完整stock model shader、asset camera/skeleton/animation/physics与官方pixel golden |
