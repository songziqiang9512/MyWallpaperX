...
| nested/effectful provider | `L3 bounded` | primary `_a`、backward、acyclic、hidden image provider closure 已闭合两级 provider 链：provider 可先由普通 Program/GraphExecutor 执行 effect，再把 graph final 复制到独立 named reservation；中间 provider 不持有 compositor，最终可见 consumer 才持有唯一 compositor | secondary、forward/cycle、child、非 image、multi-dependency/reference、history/resize/device-loss 与更深混合链 |
