...
| TEX common decode | `L3` | [`SceneTextureLoader.swift`](../../../MyWallpaperX/Core/SteamWorkshopScene/Resources/Textures/SceneTextureLoader.swift)、[E-INGEST](../../history/scene/runtime-evidence-index.md#e-ingest) | 全容器/format/mip/color-space 边界 |
| resource identity and missing diagnostics | `L3` | [`SceneResourceReferenceIndex.swift`](../../../MyWallpaperX/Core/SteamWorkshopScene/Resources/Assets/SceneResourceReferenceIndex.swift)、[E-INGEST](../../history/scene/runtime-evidence-index.md#e-ingest) | 统一 VFS、alias/case 规则与依赖版本 |
...
| light object | `L3 bounded / S3 point direct-model executed + existing standalone spot` | scene ambient/skylight与directional/point/spot共用按当前作者render order和canonical visible set选取的**总计4灯**typed snapshot；directional/spot既有direct static-model diffuse consumer保留，point的`color/intensity/radius/world position`现由同一consumer和Metal ABI消费。真实`3662790108`逐帧建立1个point snapshot并让14个`receivesLighting`静态模型draw取得Metal completion；spot/directional color仍可经条件型typed provider与真实`WEColor.normalizeColor`发布，既有standalone spot保持自身窄合同。见[当前点光证据](runtime-evidence-current.md#e-2026-09-20-point-light-static-model) | point当前只消费authored fallback intensity/radius与现有world frame/color，动态intensity live target、shadow/PBR、tube、lit 2D material、HDR/tone-map/volume、官方固定输入像素对照均未闭合；`3287715210`虽有point声明但没有static-model consumer。不得把本direct-model slice外推为通用lighting或整样本通过 |
