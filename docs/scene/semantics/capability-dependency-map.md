...
| Runtime | 前置依赖 | 最小闭环 |
|---|---|---|
| Timeline | D2 + D3 | lossless IR、绝对 scene-time evaluator 与 typed writes 已完成，现役 Workshop **48/48 authored host** 有受限 consumer：effect constant 23、layer alpha 5、bounded relative layer transform 9、root particle scalar override 7、bounded text `maxwidth` 2，以及唯一 default 2D camera Combined `origin↔zoom` 两成员。camera origin 不再重复计入 layer transform；两成员共用 owner clock，经同一 per-surface transaction 原子写入 projection、particle camera 与 pointer projection。multiple path、3D/未知 camera、坏组、越界值和 generic Combined 均失败关闭；tangent 只保真不消费、插值走线性，wrap-loop、event crossing、particle relative/colorn/child/存量追溯仍待推进 |
