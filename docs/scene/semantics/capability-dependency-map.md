...
| history/ping-pong | exact Cursor Ripple strict topology与普通Motion Blur `material → material → copy → unique history/FBO → material`分别为bounded `L3` consumer；后者已闭合initial seed/COW、submission pin、completion/publication/compositor/next-frame，并证明executor invalidation重新seed、同输入与same-identity/different-authored-state scene re-launch及surface stop/relaunch隔离新runtime/pool、正式pause保留同runtime/pool且resume继续COW | 其他authored topology/consumer、logical swap、其他different-Scene topology/provider/format/SceneScript/particle state、resize、seek/discontinuity、device loss、多surface、epoch rollback和memory budget通用门；有界Motion Blur static atom不证明动态效果或官方parity |
