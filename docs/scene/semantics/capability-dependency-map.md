...
| named/effect/history target identity | `_a` 子集 `L3`、`_b` `L2`；exact Cursor Ripple strict history target与普通Motion Blur material/copy/unique-history graph各有bounded `L3` consumer，后者已闭合initial/COW、executor invalidation、同输入及same-identity/different-authored-state scene re-launch、surface stop/relaunch与正式pause/resume生命周期 | primary/secondary/history不混用；继续补其他authored history topology/consumer、logical swap、其他不同Scene topology/provider/format/state、resize/seek/device loss、多surface与预算门 |
