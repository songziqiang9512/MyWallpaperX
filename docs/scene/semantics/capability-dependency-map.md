...
| media state/properties/timeline/thumbnail | thumbnail `L3 bounded`；其他 media state/property/timeline `L0` | current/previous generation、replacement pending last-ready、clear/decode-failure 作者 fallback 与严格 transition profile 已闭合；仍缺通用事件队列、metadata/status/timeline snapshot、旧 decode 主动取消与 live producer |
...
| upload/cancel/teardown | PNG/JPEG property 子集；embedded video 有按帧 publication、pause/rebuild/stop 状态合同和临时文件清理；media cover 有串行最长边 256 decode、requested-generation stale completion 拒绝、pending last-ready、clear/decode-failure 最终 fallback、per-surface pause-safe transition 与 rebuild 不重播 | 主动取消旧 thumbnail decode、device loss、live producer start/pause/stop、多 surface/hot-plug、budget 与真实 lifecycle 门 |
