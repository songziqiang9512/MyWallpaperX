...
| particle breadth | D2 + D3 + D4 + D5 + D8 + D9 | 静态 plain-image Layer Image 已复用 typed object dependency、base image texture 与 simulator；仅 rate root Sphere/Box 的 Random periodic 已复用 fixed-step simulation、独立确定性 schedule RNG 与 fail-closed diagnostics。standalone delay、periodic burst/max-per-period、动态/text/puppet Layer Image、control point angle/space consumer、child/event、rope、audio、collision 仍待闭合 |
