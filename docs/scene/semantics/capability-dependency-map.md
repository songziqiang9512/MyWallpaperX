...
| media state/properties/timeline/thumbnail | thumbnail `L3 bounded`；其他 media state/property/timeline `L0` | current/previous generation 与严格 transition profile 已同代；仍缺通用事件队列、metadata/status/timeline snapshot、旧 decode 主动取消与 live producer |
...
| upload/cancel/teardown | PNG/JPEG property 子集；embedded video 有按帧 publication、pause/rebuild/stop 状态合同和临时文件清理；media cover 有串行最长边 256 decode、requested-generation stale completion 拒绝、per-surface pause-safe transition 与 rebuild 不重播 | 主动取消旧 thumbnail decode、device loss、live producer start/pause/stop/clear、多 surface/hot-plug、budget 与真实 lifecycle 门 |
...
| particle breadth | D2 + D3 + D4 + D5 + D8 + D9 | 静态 plain-image Layer Image 已复用 typed object dependency、base image texture 与 simulator；仅 rate root Sphere/Box 的 Random periodic 已复用 fixed-step simulation、独立确定性 schedule RNG 与 fail-closed diagnostics；root Sphere/Box emitter 已按 authored CP identity 0...7 准入并拒绝缺失/重复/越界 declaration，emitter speed 已建立 finite/nonnegative/ordered/max-only 准入与无效范围零发射，Directions 已建立 scalar/完整 finite 三轴 multiplier 准入，Sphere Sign 只准官方 `-1/0/1`，depth-one static child 的 exact raw parent CP position copy 已复用 typed instance override 与 child simulator；exact pointer-lock CP 1...7 现复用 surface-local current pointer、layer model/camera 逆投影与 operator 顺序，驱动 bounded local Control Point Force。standalone delay、periodic burst/max-per-period、动态/text/puppet Layer Image、control point angle/adjusted/world/cross-space、previous pointer 与其他 consumer、event/nested CP copy、force falloff、rope、audio、collision，以及 emitter speed/shape/force 的 WE RNG/分布/轨迹 golden 仍待闭合 |
| audio/media | D4 + D5 | audio 侧已闭合 consumer-driven 16/32/64 host input、既有 effect consumers 与两个 native 64-band consumers；embedded MP4 image-layer 已有受限 SceneClock/provider generation/lifecycle；current/previous cover generation 与一个 strict authored transition profile 已闭合 bounded consumer。通用 SceneScript bridge/Sound/particle audio、live system media producer、event ordering、metadata/status/timeline 与其他 transition 仍未闭合 |
