...
`8474ace` 后全量 Scene 测试共 **223 项、222 项通过、1 项跳过**；`test_scene_shader_contract.py` 覆盖 source/include/annotation/declaration、identity 与失败关闭，benchmark 合同同时拒绝缺字段、source/hash 不一致、空 diagnostic 和 host built-in 带 stage 等结构残缺。
