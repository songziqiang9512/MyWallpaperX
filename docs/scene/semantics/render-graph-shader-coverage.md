...
`b541867` 的 scheduler 验证全局 effect/node/target identity、layer source -> effect output 连续性和最终输出，并先为整链建立 plan/table、预算与 LRU 候选，成功后一次提交。GPU 在同一 offscreen command buffer 依序执行五个 fixed material-only backend。`f1c6a10` 在同一 target identity/table 合同旁建立严格的同帧 copy/swap plan/runtime，但尚未把 command 与 material node 交错调度；compose/history/condition/function 仍不执行，也不让下表任何 generic primitive 升级到 `L3`。
...
| ordered pass / material ordinal | `L2` | definition pass 顺序、definition index、material ordinal 与 instance pass index 分离 | ordered strict scheduler 可循环执行已注册 stage；独立 command runtime 可顺序执行同帧 copy/swap | 两个 Blur、Local Contrast、Workshop Shadow 与 stock Opacity 均要求完整固定顺序；copy/swap 不消耗 material ordinal | [graph planner](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredEffectRenderPlanner.swift)、[chain planner](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredEffectExecutionChain.swift)、[graph command runtime](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneGraphCommandRuntime.swift)、[E-GRAPH-COMMAND](runtime-evidence-index.md#e-graph-command) | 五个 material backend 与 command runtime 尚未形成通用交错执行循环 | 接 material-command interleave、compose/history，并继续以 read/write hazard 验证命令顺序 |
