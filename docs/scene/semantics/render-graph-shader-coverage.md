...
>
> 结论：当前 graph 已有较完整的保真 IR，ShaderContract v1 已安全保留 authored source/include/annotation/declaration；Blur Precise、stock Blur、stock Local Contrast 与 exact Workshop `shadow_____________` 有严格、失败关闭的 Metal 执行器。`b541867` 以 synthetic GPU 门建立 ordered strict scheduler、allocation 与最终合成原子性，`809b75e` 再取得真实 `Blur Precise -> Shadow` 集成正门；该真实链尚无逐 stage compositor 像素 golden。当前仍没有 preprocessor/translation/compile、通用 authored command-graph executor 或通用 shader executor。因此本页没有任何 `L4` 项。
...
当前代码基线为 `809b75e`，interpretation wire schema 仍为 v20；v20 在 v19 ShaderContract 基础上增加 strict Local Contrast `strength` live-binding 合同。ordered strict effect-chain 与 exact Workshop Shadow 沿用 v20 数据，Shadow 的 alpha/color/border/offset 仍是静态、无 user binding 的 profile 常量；该版本号只说明缓存与运行时能交换已验证数据，不代表 generic shader uniform 已完成。

## 2. 三条实现通道

| 通道 | 当前含义 | 当前事实 |
|---|---|---|
| IR | 读取并保真保存 definition/material/shader source contract，再编译资源身份、节点、slot 和 blocker | [SceneEffectDefinition.swift](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneEffectDefinition.swift)、[SceneAuthoredEffectRenderPlan.swift](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredEffectRenderPlan.swift)、[SceneShaderContract.swift](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneShaderContract.swift)、[SceneAuthoredEffectRenderPlanner.swift](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredEffectRenderPlanner.swift) |
...
| `condition` | `L2` | FBO/pass/bind 各位置保留 raw `SceneJSONValue` 并产生显式 blocker | 无表达式环境或逐帧 evaluator | 四个 strict profile 要求所有 condition 为 nil | [definition IR](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneEffectDefinition.swift)、[planner blocker](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredEffectRenderPlanner.swift)、[graph 测试](../../../script/tests/test_scene_effect_render_graph.py) | 没有 AST、类型转换、combo/runtime variable 绑定或分支资源分析 | 从合法 fixture 建最小 condition AST/evaluator，并验证 false 分支不分配 RT/不占 material ordinal |
| `function` | `L2` | definition 顶层 functions 原样保留并阻断执行 | 无 live function executor 或 reset hook | 四个 strict profile 只接受 functions 缺失 | [definition loader](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneEffectDefinition.swift)、[保真测试](../../../script/tests/test_scene_effect_definition.py)、[planner blocker](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredEffectRenderPlanner.swift) | Fluid reset 等函数不会运行 | 先确认 function schema、调用时机和资源副作用，再实现 resize/reset/initialization fixture |
| extent (`scale`/`fit`/absolute/input) | `L2` | 规范化为 `input`、`scale`、`fit`、`absolute`、`unsupported` | pools 能按请求尺寸分配/限额，但不解释完整 authored extent | precise 只接受 input extent；stock Blur 与 Local Contrast 接受各自 exact scale=4 FBO；Workshop Shadow 要求零 authored FBO | [extent resolver](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredEffectRenderPlanner+Resolution.swift)、[E-EFFECT-IR](runtime-evidence-index.md#e-effect-ir)、[Local Contrast 证据](runtime-evidence-index.md#e-effect-local-contrast) | authored input/scale-4 只在三个 FBO profile 内执行，Shadow 只复用 synthetic full-size input/output；generic extent 仍未执行 | 定义基于 source/effect/canvas 的尺寸算法，逐类测试 fit/fixed/scale、rounding 和 resize |
| `format` | `L2` | 已识别 `r8`、`rg88`、`r16f`、`rg1616f`、`rgba8888`、`rgba_backbuffer`，未知值阻断 | target table 精确映射 `rgba_backbuffer -> .bgra8Unorm` 与 `rgba8888 -> .rgba8Unorm`；synthetic input/output 保持 BGRA，两者按 4 B/px 计费 | strict Blur 执行 `rgba_backbuffer`；stock Local Contrast 的两个 quarter target 执行 `rgba8888` | [format gate](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredEffectRenderPlanner+Resolution.swift)、[allocation table](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneGraphRenderTargetTable.swift)、[Local Contrast renderer](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneLocalContrastRenderer.swift)、[E-EFFECT-RT](runtime-evidence-index.md#e-effect-rt) | R/RG/float 仍只是 graph 可结构化；RGBA8888 也只在 exact Local Contrast profile 执行 | 逐项验证其他格式的通道、精度、filter/write 和 byte budget，并接入统一 scheduler |
...
|---|---|---|---|---|---|---|---|
| GLSL-like source | `L1` | ShaderContract 安全保留完整 UTF-8 source、raw SHA-256、stage path/kind 与 canonical identity | 无 preprocessor、translator 或 compiler | strict Blur 按固定 profile 调用手写 MSL；Local Contrast 与 Workshop Shadow 额外消费 exact identity/canonical/stage/raw/source hash 作准入，但仍不编译 source | [contract](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneShaderContract.swift)、[loader](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneShaderContractLoader.swift)、[Shadow fingerprint gate](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredWorkshopShadowPlanner.swift)、[E-SHADER-CONTRACT](runtime-evidence-index.md#e-shader-contract) | fingerprint match 只证明 source 身份；当前没有 authored WE GLSL 进入 Metal compiler | 建 directive/token frontend、source map 与 typed stage contract |
...
| `combo` / permutation | `L2` | material 与 instance combo 字典合并，key/value 保留 | 无按 combo 编译/缓存 shader variant 的通用系统 | 两个 Blur planner 精确白名单 combo、默认值和方向；Local Contrast 接受 KERNEL0/GREYSCALE0/MASK0；Workshop Shadow 只接受 `BLENDMODE=0`/`MASK=0` | [material resolver](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredMaterialResolver.swift)、[Shadow combo gate](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredWorkshopShadowPlanner.swift)、[E-EFFECT-WORKSHOP-SHADOW](runtime-evidence-index.md#e-effect-workshop-shadow) | strict profile 只消费白名单值；mode 0 尚无官方 Windows 像素 oracle，也没有 authored permutation/annotation 联动 | 建 normalized variant key，并由 annotation、实际 slot readiness 与 preprocessor 共同决定 permutation |
| shader annotation | `L1` | marker、行号、raw JSON 与结构化 JSON value 已 loss-preserving 保存 | resolver 不消费 typed shader default 或 optional-slot annotation | Local Contrast 缺失 Gaussian scale 时使用 exact stock profile adapter 的 `scale=(1,1)`，不是 typed annotation consumer | [contract loader](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneShaderContractLoader.swift)、[合同测试](../../../script/tests/test_scene_shader_contract.py)、[E-SHADER-CONTRACT](runtime-evidence-index.md#e-shader-contract) | 固定 profile 缺省值不等于理解 default/mode/combo/paint/UI metadata；属性面板不能由未类型化数据自动生成 | 建 typed schema，分别输出 runtime default、combo dependency 和 editor-only metadata |
| uniform / `constantshadervalues` | `L2` | number/vector/string/binding raw value 与 components 被保存；material/instance override 合并；v20 可持久化 exact Local Contrast strength target | 手写 pipelines 有固定 Swift/MSL uniform structs，没有按 name/type 通用上传 | Blur 读取固定 constants；Local Contrast strength 可从 snapshot live 消费；Workshop Shadow 只接受无 user binding 的静态 alpha/black color/border/offset | [value parser](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneDocument.swift)、[binding program](../../../MyWallpaperX/Core/SteamWorkshopScene/ScenePropertyBindingProgram.swift)、[Shadow renderer](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneWorkshopShadowRenderer.swift)、[E-EFFECT-WORKSHOP-SHADOW](runtime-evidence-index.md#e-effect-workshop-shadow) | 这些是固定字段读取；Shadow 未新增 live target，通用 name/type/upload/binding 不存在 | 下一切片让 stock Opacity `alpha` 同批进入 compiler target、per-surface snapshot 与 GPU consumer；未知/重复/type mismatch 失败关闭 |
| include / header | `L1` | include raw token、relative path 与行号已保存并做 root/symlink escape 校验 | 无 include expansion、dependency graph、cycle detection 或 cache invalidation | 手写 MSL 的 `<metal_stdlib>` 与 WE header 无关 | [contract loader](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneShaderContractLoader.swift)、[E-SHADER-CONTRACT](runtime-evidence-index.md#e-shader-contract) | 引用被保留不等于 common/header/blending helper 已解析或加载 | 在只读 VFS 上实现 include resolver，记录 dependency hash、cycle、missing 与 source map |
| vertex attribute | `L1` | authored attribute declaration、type/name/array shape 与行号已保存 | 只有 MyWallpaperX 自有 quad/particle vertex layout | strict Blur 使用自有 `vertex_id` quad，不消费 authored attribute | [contract loader](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneShaderContractLoader.swift)、[E-SHADER-CONTRACT](runtime-evidence-index.md#e-shader-contract) | declaration 保存不等于 semantic/location 或实际 vertex layout 已映射 | 建 typed stage frontend，再映射 2D quad/model/particle vertex sources |
| varying / fragment output | `L1` | authored varying declaration、type/name/array shape 与行号已保存；fragment output 尚未形成 typed contract | 只有手写 MSL stage IO | strict Blur 使用自有 varyings | [contract loader](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneShaderContractLoader.swift)、[E-SHADER-CONTRACT](runtime-evidence-index.md#e-shader-contract) | 无跨 stage 类型/location 校验，也无 authored fragment output/颜色空间合同 | 增加 stage-link validation、interpolation 与 output format/alpha contract |
...
| render state (blend/depth/write/cull) | `L2` | 四项按 material pass 保存并进入 resolved node | Metal pipeline state 可表达这些状态，但没有通用 authored state translator/cache | 四个 strict executor 只接受 `normal/disabled/disabled/nocull`；其他值拒绝 | [material IR](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAssetCatalog.swift)、[E-EFFECT-IR](runtime-evidence-index.md#e-effect-ir)、[Shadow gate](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredWorkshopShadowPlanner.swift) | 固定 profile 默认状态不代表 authored state translator 或枚举兼容 | 建显式 enum 与 Metal descriptor 映射，先覆盖 alpha convention，再逐枚举做 source/destination 像素门 |
| MyWallpaperX 手写 MSL profile（非 WE 语言语义） | `L3` | 由 effect/runtime plan 转成固定 Swift uniform struct | Metal compiler 可编译 repo 内 MSL | Blur Precise、stock Blur、stock Local Contrast 与 exact Workshop Shadow 有 strict path；其他 legacy effect 多为 bounded approximation | [Gaussian pipeline](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneGaussianBlurPipeline.swift)、[standard pipeline](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneStandardBlurPipeline.swift)、[Local Contrast pipeline](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneLocalContrastPipeline.swift)、[Shadow pipeline](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneWorkshopShadowPipeline.swift)、[E-EFFECT-WORKSHOP-SHADOW](runtime-evidence-index.md#e-effect-workshop-shadow) | 只说明当前自有 backend 可交付；fingerprint 准入不能给 WE source/macro/annotation primitive 升级，Shadow mode 0 与 Local Contrast 均缺 Windows WE golden | 保留 strict backend 作为 generic executor 的 oracle/fallback，并为每个 profile 记录输入合同和 parity 预算 |
...
5. **Stock Opacity `MASK=0` + live alpha（当前主线）**：新增 exact single-pass profile，并让 alpha 经既有 binding program/per-surface snapshot 被 GPU 消费；同批覆盖 premultiplied RGBA、ordered chain、invalid/mixed rebuild、same surface/window 与 unsupported stage 原子失败。
6. **Provider Core（并行线）**：为最终 texture candidate 建显式 dynamic generation、metadata 与 cancellation，使 slot readiness、physical/mapped metadata 和 graph invalidation 共用同一代际合同。
...
9. **按效果扩面**：优先选择能复用已闭合 graph/shader primitive 的 effect；Motion Blur 用于 copy/history，Refraction 用于 compose，Fluid 用于 swap/condition/function/format 压力测试。不得用显示名或视觉近似绕开 authored contract。

## 7. 本轮验证

`809b75e` 后全量 Scene 测试共 **258 项、257 项通过、1 项跳过**。除既有 ShaderContract、graph-target、Local Contrast 与 synthetic chain 门外，Workshop Shadow planner/GPU tests 覆盖 exact fingerprint、零 authored FBO、combo/state/静态常量、premultiplied alpha、offset/border、ordered prior-stage input 与失败不泄漏。`b541867` 阶段的 251/250/1 只保留为 scheduler 历史证据；当前测试仍不是 Windows WE 像素 golden。

正式隔离矩阵 `.codex/scene-workshop-shadow-final13-20260723-1540/report.json` 为 **13/13**：四个 strict backend 合计 10 stage、1 条真实 multi-effect chain、Workshop Shadow 1、failed 0、legacy blocked 2、route-only 34。`3724289844` 为 stages 4/chains 1/shadow 1、succeeded `[20,28,36]`、blocked/failed 为空；`2938612768` mixed chains 保持 stages/chains 0。这同时给出真实正门与 unsupported mixed 负门，完整边界见 [E-EFFECT-WORKSHOP-SHADOW](runtime-evidence-index.md#e-effect-workshop-shadow) 和 [E-EFFECT-CHAIN](runtime-evidence-index.md#e-effect-chain)。

本轮签名 App 身份为 version `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `e4c25d85cb1b878019ec6a7b9e334fd6caaef098`，可执行文件 SHA-256 `bd02f455de2e5e11cbc365dabee5063afa4c32ab28d959abcd8c8bb77746a8c0`。这些证据只把四个 strict backend 的 ordered all-supported chain 子集记为 `L3`；不得据此升级 unsupported mixed graph、generic graph primitive、官方 Shadow 或 authored shader execution。
