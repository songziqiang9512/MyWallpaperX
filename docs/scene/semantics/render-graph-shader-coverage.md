...
>
> 结论：当前 graph 已有较完整的保真 IR，ShaderContract v1 已安全保留 authored source/include/annotation/declaration，且 Blur Precise 与 stock Blur 默认 profile 有严格、失败关闭的 Metal 执行器；当前没有 preprocessor/translation/compile、通用 authored graph executor或通用 shader executor。因此本页没有任何 `L4` 项。
...
- [运行证据索引](runtime-evidence-index.md)：IR、ShaderContract、strict Blur 与 GPU 证据包；
...
| `format` | `L2` | 已识别 `r8`、`rg88`、`r16f`、`rg1616f`、`rgba8888`、`rgba_backbuffer`，未知值阻断 | target table 精确映射 `rgba_backbuffer -> .bgra8Unorm` 与 `rgba8888 -> .rgba8Unorm`；synthetic input/output 保持 BGRA，两者按 4 B/px 计费 | strict Blur 只执行 `rgba_backbuffer`；RGBA8888 尚无 executor | [format gate](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredEffectRenderPlanner+Resolution.swift)、[allocation table](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneGraphRenderTargetTable.swift)、[E-EFFECT-RT](runtime-evidence-index.md#e-effect-rt) | R/RG/float 仍只是 graph 可结构化；RGBA8888 分配成功也不等于 Local Contrast 已执行 | 先闭合 strict Local Contrast RGBA pipeline，再逐项验证其他格式的通道、精度、filter/write 和 byte budget |
...
|---|---|---|---|---|---|---|---|
| GLSL-like source | `L1` | ShaderContract 安全保留完整 UTF-8 source、raw SHA-256、stage path/kind 与 canonical identity | 无 preprocessor、translator 或 compiler | strict Blur 只匹配 shader path/material profile 后调用手写 MSL，尚不消费 ShaderContract | [contract](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneShaderContract.swift)、[loader](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneShaderContractLoader.swift)、[E-SHADER-CONTRACT](runtime-evidence-index.md#e-shader-contract) | source 进入 IR 不等于执行 GLSL；当前没有 authored WE GLSL 进入 Metal compiler | 建 directive/token frontend、source map 与 typed stage contract |
...
| shader annotation | `L1` | marker、行号、raw JSON 与结构化 JSON value 已 loss-preserving 保存 | resolver 不消费 typed shader default 或 optional-slot annotation | 无 strict annotation consumer | [contract loader](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneShaderContractLoader.swift)、[合同测试](../../../script/tests/test_scene_shader_contract.py)、[E-SHADER-CONTRACT](runtime-evidence-index.md#e-shader-contract) | 保存 annotation 不等于理解 default/mode/combo/paint/UI metadata；属性面板不能由未类型化数据自动生成 | 建 typed schema，分别输出 runtime default、combo dependency 和 editor-only metadata |
...
2. **strict Local Contrast + Graph resource executor**：先以 `2902406982` layers `167/177` 闭合官方默认 4-pass 单效果 profile，`2938612768` mixed chains 保持 fail closed；随后让统一 scheduler 执行 target/bind/copy/swap、extent/format/clear/UV，并加入 read/write hazard、budget、resize/reset/device-loss 测试。compose、condition、function 继续失败关闭，直到各自合同有 fixture。
...
`228cdde` 后全量 Scene 测试共 **225 项、224 项通过、1 项跳过**；`test_scene_shader_contract.py` 覆盖 source/include/annotation/declaration、identity 与失败关闭，benchmark 合同同时拒绝缺字段、source/hash 不一致、空 diagnostic 和 host built-in 带 stage 等结构残缺。新增 plan/table/pool 门覆盖 RGBA8888 preservation/allocation、旧 BGRA framebuffer、精确预算与 format cache 原子替换。

隔离正式矩阵 `.codex/scene-shader-contract-final13-v2-20260723/report.json` 为 **13/13**，记录 173 contracts（143 authored + 30 host built-in）、286 stages、0 diagnostics；原始 source 与 IR 的 include/annotation/declaration 数均为 `155/1523/2660`。现有 graph-target 正负矩阵和 strict Blur GPU 像素门继续通过。完整 App 身份与边界见 [E-SHADER-CONTRACT](runtime-evidence-index.md#e-shader-contract)；不得用 source contract 把任何 authored shader 或其他 graph-built effect升级为 executed。
