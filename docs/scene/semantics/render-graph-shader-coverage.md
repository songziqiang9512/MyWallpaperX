...
`b541867` 的 scheduler 验证全局 effect/node/target identity、layer source -> effect output 连续性和最终输出，并先为整链建立 plan/table、预算与 LRU 候选，成功后一次提交。GPU 在同一 offscreen command buffer 依序执行：首段使用原 layer uniforms/mask/UV/alpha，后续段使用 neutral uniforms 与 empty masks，只在全链成功后把最终 stage 合成到主画面。`b8842d8` 让第五个 fixed material-only backend 进入这条路径；它仍不执行 copy/swap/compose/history/condition/function，也不让下表任何 generic primitive 升级到 `L3`。

| 语义项 | 等级 | IR | generic runtime | strict executor | 代码/测试证据 | 执行边界 | 下一门 |
|---|---|---|---|---|---|---|---|
| `EffectDefinition` | `L2` | JSON5 loader 保留 metadata、dependencies、FBO、ordered pass、functions/gizmos、extra fields 和 unknown paths | 没有按任意 definition 驱动 GPU 的执行循环 | 两个 Blur、exact stock Local Contrast、exact Workshop Shadow 与 exact stock Opacity 从同一 authored plan 进入执行，未知字段/blocker 一律拒绝 | [定义 loader](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneEffectDefinition.swift)、[定义测试](../../../script/tests/test_scene_effect_definition.py)、[E-EFFECT-IR](runtime-evidence-index.md#e-effect-ir)、[E-EFFECT-OPACITY](runtime-evidence-index.md#e-effect-opacity) | 五个完整 strict profile 不代表通用 definition 已执行；该 primitive 仍停在 IR | 建立 graph executor capability registry；每种 node/resource semantic 显式声明并逐项放行 |
| ordered pass / material ordinal | `L2` | definition pass 顺序、definition index、material ordinal 与 instance pass index 分离 | ordered strict scheduler 可循环执行已注册 stage；没有任意 node/command list 执行循环 | 两个 Blur、Local Contrast、Workshop Shadow 与 stock Opacity 均要求完整固定顺序，且整条 supported chain 按作者 effect 顺序执行；copy/swap 不消耗 material ordinal | [graph planner](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredEffectRenderPlanner.swift)、[chain planner](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredEffectExecutionChain.swift)、[graph 测试](../../../script/tests/test_scene_effect_render_graph.py)、[E-EFFECT-CHAIN](runtime-evidence-index.md#e-effect-chain) | 只有五个固定 material-only 序列及其全支持组合进入 GPU；generic command scheduler 不存在 | 逐项接 copy/swap/compose/history，并以 read/write hazard 验证通用命令顺序 |
...
| uniform / `constantshadervalues` | `L2` | number/vector/string/binding raw value 与 components 被保存；material/instance override 合并；v21 可持久化 exact Local Contrast strength 与 stock Opacity alpha target | 手写 pipelines 有固定 Swift/MSL uniform structs，没有按 name/type 通用上传 | Blur 读取固定 constants；Local Contrast strength 与 stock Opacity direct alpha 可从 snapshot live 消费；Workshop Shadow 只接受静态常量 | [value parser](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneDocument.swift)、[binding program](../../../MyWallpaperX/Core/SteamWorkshopScene/ScenePropertyBindingProgram.swift)、[Opacity renderer](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneOpacityRenderer.swift)、[E-EFFECT-OPACITY](runtime-evidence-index.md#e-effect-opacity) | 这些是固定字段读取；SceneScript/通用 name/type/upload/binding 不存在 | 新 uniform 必须同批有 compiler target、per-surface snapshot、GPU consumer 和 unknown/duplicate/type mismatch 失败门 |
...
| render state (blend/depth/write/cull) | `L2` | 四项按 material pass 保存并进入 resolved node | Metal pipeline state 可表达这些状态，但没有通用 authored state translator/cache | 五个 strict executor 只接受各自精确白名单状态；其他值拒绝 | [material IR](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAssetCatalog.swift)、[E-EFFECT-IR](runtime-evidence-index.md#e-effect-ir)、[Opacity gate](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredOpacityPlanner.swift) | 固定 profile 默认状态不代表 authored state translator 或枚举兼容 | 建显式 enum 与 Metal descriptor 映射，先覆盖 alpha convention，再逐枚举做 source/destination 像素门 |
| MyWallpaperX 手写 MSL profile（非 WE 语言语义） | `L3` | 由 effect/runtime plan 转成固定 Swift uniform struct | Metal compiler 可编译 repo 内 MSL | Blur Precise、stock Blur、stock Local Contrast、exact Workshop Shadow 与 exact stock Opacity 有 strict path；其他 legacy effect 多为 bounded approximation | [Gaussian pipeline](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneGaussianBlurPipeline.swift)、[Local Contrast pipeline](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneLocalContrastPipeline.swift)、[Opacity pipeline](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneOpacityPipeline.swift)、[E-EFFECT-OPACITY](runtime-evidence-index.md#e-effect-opacity) | 只说明当前自有 backend 可交付；fingerprint 准入不能给 WE source/macro/annotation primitive 升级，各 profile 均缺完整 Windows WE golden | 保留 strict backend 作为 generic executor 的 oracle/fallback，并为每个 profile 记录输入合同和 parity 预算 |
...
正式隔离矩阵 `.codex/scene-opacity-final13-20260723-1730/report.json` 为 **13/13**：五个 strict backend 合计 14 stage、1 条真实 multi-effect chain、Opacity 4、Workshop Shadow 1、failed 0、legacy blocked 2、route-only 30。`2902406982:[365,372,647,664]` 是 stock Opacity direct-binding 正门；`2938612768:[165,454,626,629,924]` 因 SceneScript 保持 Opacity/stages 0。这同时给出真实正门与 fail-closed 负门，完整边界见 [E-EFFECT-OPACITY](runtime-evidence-index.md#e-effect-opacity) 和 [E-EFFECT-CHAIN](runtime-evidence-index.md#e-effect-chain)。

本轮签名 App 身份为 version `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `6e70e547f61dcc6821009a4ebbd97af156fb5916`，可执行文件 SHA-256 `0cbaff3bd989e5cb9f8807236fd270feec19ca1f18548c122ca3ccb74033774a`。这些证据只把五个 strict backend 的 ordered all-supported chain 子集记为 `L3`；不得据此升级 unsupported mixed graph、generic graph primitive、SceneScript、官方 Shadow 或 authored shader execution。
