...
| FBO declaration | `L2` | effect-scoped identity、extent、format、unique、clear、UV、condition 均进入 `RenderTarget` | 有 offscreen pool/named layer target pool，但不是 effect-FBO 通用 allocator | precise 分配 1 个 full RT；standard 分配 2 个 quarter RT，并验证不 alias | [graph IR](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredEffectRenderPlan.swift)、[E-EFFECT-IR](runtime-evidence-index.md#e-effect-ir)、[strict GPU 证据](runtime-evidence-index.md#e-effect-blur) | strict Blur 只执行两个固定 allocation profile；通用 FBO 仍未执行 | 建立 effect-instance resource table、lifetime plan、budget、resize/reset/device-loss 合同 |
| `target` | `L2` | 区分 layer source、effect output、effect-scoped framebuffer 和 unresolved；同名 FBO 不跨 effect 共享 | named layer target pool 是独立 provider 设施，尚未与 authored effect target 统一 | strict Blur 把声明 target 映射到固定 full/quarter Metal textures | [target identity](../../../MyWallpaperX/Core/SteamWorkshopScene/SceneAuthoredEffectRenderPlan.swift)、[graph 测试](../../../script/tests/test_scene_effect_render_graph.py)、[E-EFFECT-IR](runtime-evidence-index.md#e-effect-ir) | 非 Blur authored target 不会分配或发布；named layer target 不能冒充 effect FBO/history | 统一 resource identity 与 frame registry，保留 wallpaper/screen/layer/effect/variant/name 作用域 |
