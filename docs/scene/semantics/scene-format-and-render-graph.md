...
- `b9bdbd5e` 把 `8b06538d` 只服务 strict REFRACT 的用途合同扩展到共享 `SceneTextureLoader`：PNG/JPEG、embedded/raw format 0 与 BC1/2/3 都按 `premultipliedColor` / `preservedChannels` 分流，cache identity 包含用途，全部 30 个 Effect 辅助纹理调用显式传入 purpose。exact Blend 的覆盖图继续预乘；其余 29 个 mask/flow/phase/normal/noise/gradient/X-Ray 消费者保留通道。X-Ray 对同一 property source 同时维护普通颜色与 preserved-channel 两种身份，避免 halo/blend 的 alpha 被二次参与 RGB。自动门覆盖典型 straight RGBA PNG、BC 双用途/cache 隔离、raw color premultiply/extent，以及 production X-Ray authored/property fallback；三样本定向门见 [E-EFFECT-TEXTURE-PURPOSE](runtime-evidence-index.md#e-effect-texture-purpose)。非典型或超大图片的 alpha=0 隐藏 RGB、BC5 编码，以及尚未携带 physical/mapped metadata 的辅助纹理 UV/sampler 仍未闭合。
