...
当整条effect route精确为`.unclaimed`时，当前产品还有一条与上述effect graph transaction分离的source-stage故障隔离路径。它只在不存在已授权/可执行typed dependency、source-copy、frame target或final-alpha，style中性，exact static-file或current-media publication atom完整，且有限authored quad与viewport有正面积交集时，才把原始source按作者几何直接交给普通final layer compositor。coverage/opacity资源默认阻断；已证例外是exact stock Pulse单pass在`PULSEALPHA=0`时，以及严格stock identity准入的Water Waves单pass在`TIMEOFFSET/PERSPECTIVE/DUALWAVES`缺省/0时。Pulse可选mask在alpha分支关闭时不改变source alpha；Water Waves slot-1 mask只是被跳过effect的effect-local位移输入，不是独立opacity/final-alpha law，本降级不声称effect输出与raw source逐像素coverage恒等。`PULSEALPHA=1`有无loaded mask、非法pass/path/combo、非stock Water Waves identity或combo及任一typed Opacity entry仍阻断。visible authored dependency edge还会同时阻断static-file provider与consumer，hidden/self不阻断；唯一X-Ray consumer端豁免由current stock与exact historical stock两条公共资源身份授权。每条身份都把完整typed definition variant、唯一typed material pass、root-stage canonical identity及VFS sourceGraph dependency/唯一stock `common_blending` include原子闭合，digest只校验登记资源内容而不按sample/layer/hash选择算法；historical身份不进入current execution planner，只允许consumer显示自己的base source。实例还必须只有一个真实存在的非self provider，全部跨层输入精确等于slot-1 provider composite；provider端仍一律阻断。current-media保留先前独立审查的authority。该路径不执行effect、不发布`effectOutput`、不消费authored dependency，也不把任何已执行前缀当作可恢复输出；记录`degraded-layer-source-passthrough`并强制benchmark为NON-PASS。因此strict chain的整链原子失败合同没有改变；fresh最终源码运行仍为 **0/2 NON-PASS**，`2241938645:68`只证明基础source目视高度接近官方构图，`1636394814:195`只证明historical X-Ray consumer基础source恢复但裁切/效果仍不等价，`1636394814:24`继续作为跨层provider负向sentinel。
...
8. strict chain 遇到 unsupported stage 时整链保持可诊断失败关闭，不合成任何前段结果；当前source-stage降级只输出精确source并保持NON-PASS，不发布effect或partial-prefix成功；exact Pulse的`PULSEALPHA=0`局部mask正门、`PULSEALPHA=1`/Opacity负门、strict-identity Water Waves effect-local位移mask正门、current/exact-historical双资源身份闭合且provider存在的X-Ray consumer端provider-input豁免、historical无execution owner及provider端阻断，以及visible跨层static provider+consumer双端阻断必须同时保留；通用 graph 后续若采用 passthrough，也不得改变后续 effect 顺序；
