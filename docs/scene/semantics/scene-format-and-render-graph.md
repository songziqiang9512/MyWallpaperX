...
- `b9bdbd5e` 把 `8b06538d` 只服务 strict REFRACT 的用途合同扩展到共享 `SceneTextureLoader`：PNG/JPEG、embedded/raw format 0 与 BC1/2/3 都按 `premultipliedColor` / `preservedChannels` 分流，cache identity 包含用途，全部 31 个 Effect 辅助纹理调用显式传入 purpose。exact Blend 的覆盖图继续预乘；其余 30 个 mask/flow/phase/normal/noise/gradient/X-Ray 消费者保留通道。`226ea5f7` 进一步让 direct/embedded data image 只在原尺寸、无 decode 映射、integer packed、明确非预乘 alpha 或无 alpha（skip）且 32-bit byte order 受控的 RGB(A/X) provider 可归一为 RGBA 时直通；其余表示失败关闭，不再用反预乘猜测原通道。自动门覆盖 direct/embedded Water Flow 逐字节读回、Water Ripple normal/mask GPU 消费、BC 双用途/cache 隔离、raw color premultiply/extent 及 production X-Ray fallback；定向门见 [E-EFFECT-TEXTURE-PURPOSE](runtime-evidence-index.md#e-effect-texture-purpose)。受支持 straight-RGBA decoder/source 集合、Windows 通道 golden、BC5 编码，以及尚未携带 physical/mapped metadata 的辅助纹理 UV/sampler 仍未闭合。
