...
>
> 格式与 graph 实现基线、当前 45 样本完整快照门与固定 13 样本回归门、签名 App 身份及聚合缺口统一见 [运行证据索引](./runtime-evidence-current.md)；本表不复制基线 commit，避免随代码演进失真。文内出现的 commit 号是该项能力的**历史落地提交**，不是当前基线。
...
历史 **v22** 时点的 material resolver 与 frame registry 仍是两份分离的选择合同，尚无 material candidate -> frame selection 桥；该批只增加 direct text binding/generation，不改变 ShaderContract 或 strict adapter。现役 bounded Template/Program/GraphExecutor 已建立受限同代 resource selection 与执行桥，但 ordinary authored material 的通用 provider/backend 仍未闭合；当前事实统一见 §12 和[运行证据索引](./runtime-evidence-current.md)，不能从 v22 叙事推断现役缺口或升级 generic shader/provider 能力。
...
- **TEX V5 writer/consumer 与动态采样状态**（2.8.42 官方客户端静态路径）：`resourcecompiler64.exe` 的同一策略入口按类型读取 `format`、`nointerpolation`、`clampuvs`、`nomip` / `halfmip`、image/sprite sequence、crop/resize 与 `slice3d`；`nomip` 会转换成内部“生成 mip”的反向状态，字段名不能直接映射成 container bit。writer 固定组织 `TEXV0005 -> TEXI0001 -> TEXB0004`，仅有序列元数据时追加 `TEXS0003`。64 位主程序 V5 consumer 走 chunk loop 并按 TEXI/TEXB/TEXS marker 分派，V4 走旧式直接 handler。运行中的 `nointerpolation/clampuvs` 另由 image-layer property 注册，支持 bool 与 `{value: ...}`，分别写入 filter/address 状态；两个 descriptor 的直接 callback 均为 null。resource preparation 把 TEX subtype 与 layer state 折叠为 sampler policy：`nointerpolation` 选择 D3D11 point，否则为 min/mag/mip linear；`clampuvs` 选择 U/V/W clamp，否则为 wrap。完整状态字及额外位进入 sampler cache key，cache miss 才创建，随后按 material slot 绑定；其他 sampler 位仍有额外分支。动态 property 写入如何保证 resource reprepare、mip/边缘像素、Metal 等价与 Windows sampling golden 仍未闭合。
  2026-09-13 对官方默认工程的 87 份 `.tex-json` 做配对复核，其中 86 份有可解析 TEX：47 份 `nomip:true` 全部存为单 mip，另外 39 份全部存为多 mip，且两组 flags 均不支持一个 `nomip` 位映射。10 份 `srgb:true` 配对全部使用 flags `0x12`，无 `srgb` 的配对没有 bit `0x10`；该位不能改称 `halfmip`。`halfmip` 在 `assets/shaders/declarations.json` 中是导入能力，但这 87 份持久 sidecar 为 0 命中，现有证据不足以识别任何 TEX V5 runtime bit。项目以编译结果中的 mip level 序列为运行时权威，不另造 authoring flag 状态：单级 TEX 经尺寸归一化仍必须单级，多级 TEX 必须完整保留，否则局部失败；直接图片没有 TEX 权威时才生成完整链。
...
- `b9bdbd5e` / `226ea5f7` 把 `8b06538d` 只服务 strict REFRACT 的用途合同扩展到共享 `SceneTextureLoader` 并让 data image 严格 fail closed；`d432d4d5` 再把 cache/upload identity 穷举为 `premultipliedColor`、`straightAlbedo`、generic `preservedChannels`、`mask`、`noise`、`flow`、`phase`、`normal`，`b623f421` 新增 Depth Parallax 专用 `depth`。现役专用Effect辅助纹理调用已收敛为29个并显式传入purpose：mask 15、noise 6、flow 1、phase 1、normal 1、depth 1、generic preserved 2、X-Ray straight albedo 1、premultiplied Blend color 1；Water Flow不再计入专用调用。非预乘用途各自 cache 稳定、跨 purpose identity 分离；preserved/data用途仍按严格 source-channel policy 上传，direct/embedded data image只在原尺寸、无 decode 映射、integer packed、明确非预乘 alpha 或无 alpha（skip）且32-bit byte order受控的RGB(A/X) provider可归一为RGBA时直通。`.straightAlbedo`另只允许effective TEXB0003、完整mip/payload与decoded extent证明JPEG、authored physical=mapped且CGImage无alpha的单图按既有4096预算等比缩放；authored padding、TEXB0004 metadata伪装、带alpha、格式冲突与其他data purpose不继承该例外。`378e68e7`曾为旧Water Flow flow/phase与Standard Blur mask建立共享`SceneTextureCandidate`；现役Water Flow已删除专用loader/slot owner，作者flow与exact stock phase改由普通material asset catalog/publication及Program消费；`de5f01b6` 又接入 static plain base image 与 bounded distinct REFRACT normal；`d87de042` 再让同一 static base candidate 在 direct、inline、offscreen 与 authored-effect 路由都经过最终原子校验；`b623f421` 复用同一原子携带 slot 1 R8 depth。尺寸模型明确区分 header extent、首 mip/encoded physical、实际 Metal texture 与 mapped content：raw/BC 仍要求首 mip对齐 header；已验证 TEXB0003 embedded PNG/JPEG 可为 encoded=decoded=mapped、同时不同于 header，且全部 mip/payload/format 必须逐项对账。有效多级embedded color TEX是编译后的完整采样产品，即使首mip超过loose/single-image的4096规范化边界，也保留首级及全部作者mip；单级超限embedded输入继续按原有bounded decode规范化并保持单级，不能把该例外扩展到data purpose或不完整mip链。上述opaque JPEG缩放候选以实际Metal extent作为physical/mapped并使用identity UV。candidate physical 始终记录实际 Metal texture；base 只消费 static single-image non-sprite、identity RGBA8 linear-clamp candidate，但可作为已支持 Effect/offscreen 的 base 输入；REFRACT normal 只消费 distinct static single-image non-sprite format 0/4，Depth Parallax 只消费 R8 `.depth` 与 axis-aligned mapped UV，其他形态保持 legacy 或失败关闭。自动门覆盖Water Flow stock phase publication、作者loose覆盖与普通Program运行，另覆盖Standard Blur padding、fractional premultiplied base、opaque JPEG straight-albedo预算缩放及padded/mislabeled负例、effectful base 正负 GPU split、mapped REFRACT normal、Depth mapped R8、错误 purpose/identity/UV/sampler/Metal contract、BC/raw purpose 与 particle coverage；定向门见 [E-EFFECT-TEXTURE-PURPOSE](../../history/scene/runtime-evidence-index.md#e-effect-texture-purpose)、[E-TEXTURE-CANDIDATE](../../history/scene/runtime-evidence-index.md#e-texture-candidate)、[E-V1-XRAY-OPAQUE-JPEG-ASSET-PUBLICATION](../../history/scene/runtime-evidence-index.md#e-v1-xray-opaque-jpeg-asset-publication)、[E-V1-WATER-FLOW-GRAPH-OWNER](../../history/scene/runtime-evidence-index.md#e-v1-water-flow-graph-owner)、[E-EFFECT-DEPTH-PARALLAX](../../history/scene/runtime-evidence-index.md#e-effect-depth-parallax) 与 [E-PARTICLE](../../history/scene/runtime-evidence-index.md#e-particle)。format 5 / BC5、effectful/nested/child provider、generic Effect/material DXT5n、Windows golden、动态 generation/cancel、sprite rotation、clamp-border 与通用 material slot 仍未闭合。
- **Puppet MDL mesh block**（`8bac86e`及2026-09-14版本扩展）：MDLV0014/0016/0017/0021/0023，marker 9 字节；首个 `MDLS` 偏移为 mesh 搜索上界。块形状为 `u32 vertexBytes` + 顶点 + `u32 indexBytes` + uint16 三角形索引；0014/0016只接受已核验stride 52，0017/0021/0023只接受80或84，position是块内偏移0的3个float，四bone index/weight位于`stride - 40/-24`，UV是stride尾部8字节。判定条件 `max(index) == vertexCount - 1` 对同一version的候选stride数学互斥，天然唯一。加载时保留原始 position/UV 并建立 `SceneGeometryProduct` 的 immutable bind-pose buffer；动画用三份 persistent vertex buffer 写入 LBS 后的原始 mesh 像素位置。GeometryProduct在generation preparation声明`exactSamplingTexture` source extent；有effect时graph input必须保持atlas实际首mip尺寸，不能继承普通layer的2048 standard缩放。最终mesh经唯一world MVP直绘主target并采样atlas或graph-final atlas；不建立coverage/bind-pose重组纹理，也不发布为Puppet layer source。exact target超出现役pool硬边界时只局部拒绝该graph unit，不增加预算、不降采样、不回退atlas quad。真实`2797913147`证明一个无MDLA/MDAT的`MDLV0014 + MDLS0002` bind/script形状；它复用同一world geometry链，不授权未知0014动画或attachment。
- **Puppet 脚本骨骼与平移约束**（2026-09-10）：公开 interactive Puppet 示例确立 dense **0-based** 骨骼 identity，root=0、未知名称=-1；名称和数字 world/local API 解析到同一 catalog，parent 使用真实 MDLS parent。MDLS 的 `se/re`、`t`、`ts/tf/ti/tm` 在 load 保留为 typed spring/rigid、translation、stiffness/friction/inertia/max-distance；非有限、负值和布尔冒充数字拒绝。唯一 Puppet playback 持有速度与前一 authored target，先以 Scene delta 更新平移约束，再接收当前 frame 的脚本 local journal；持续 setter 覆盖当帧位置并清速度，release 不再写入时 spring 回到 authored pose。速度、pose 与 revision 一并纳入现有 commit/discard。未知旋转/重力/IK 模式只诊断对应 bone 并保留合法 rig/动画，不扩大为整个模型失败。官方公开 [交互教程](https://docs.wallpaperengine.io/en/scene/puppet-warp/interactive.html) 与 [约束参数](https://docs.wallpaperengine.io/en/scene/puppet-warp/boneconstraints.html) 定义行为方向；当前闭式阻尼积分不宣称官方数值等价。可见证据由 [B9 回弹闭环](runtime-evidence-current.md#e-2026-09-10-b9-spring-closure) 拥有。
- **MDLS / MDAT attachment**（`49ee89a`及2026-09-13 current-pose后继）：受限 reader 只接受已验证的 `MDLV0023 + MDLS0004 + MDAT0001`。MDAT 条目为 `u16 boneIndex + name\0 + 列主序 4x4 attachment-local matrix`，reader保留bone identity与local matrix。bind/current model frame均由对应bone world乘attachment local，再用 `F * M * F`（`F = diag(1,-1,1,1)`）转为Scene frame；child transform顺序固定为 `parentWorld * attachmentSceneCurrent * childLocal`。animation/physics的pre-script pose供layer/bone/cursor snapshot，脚本完成后的final pose同时供skinning、attachment frame、world resolver和encode。非法bounds/count/parent/bone/name/matrix失败关闭整组；current frame局部缺失时使用该attachment已验证bind frame，parent model无同名attachment或frame非16个有限float则保留普通parent transform并诊断。
- **MDLS skin / MDLA 动画**（`2be2b44`、`f1ee79b`、`0892e74b`及2026-09-14版本扩展）：无动画的受限reader另接受已验证的`MDLV0014 + MDLS0002` 52-byte bind/script rig，并在出现未知MDLA/MDAT时失败关闭。动画reader只接受已验证的 `MDLV0016 + MDLS0002 + MDLA0003`、`MDLV0017 + MDLS0002 + MDLA0004`、`MDLV0019 + MDLS0002 + MDLA0005` 或 `MDLV0023 + MDLS0004 + MDLA0006` version pair。52/80/84-byte record的四bone index/weight均按`stride - 40/-24`读取，每顶点权重必须有限、非负且和为1。MDLA每个bone track保存`frameCount + 1`个完整TRS；局部矩阵顺序为`T * Rz * Ry * Rx * S`，层级world后用`animatedWorld * inverse(bindWorld)`做normalized four-weight LBS。当前消费`loop/mirror/single` interval、blend=1且无blend-in/out的单clip或受限layered组合，绑定visibility通过typed per-frame snapshot激活。MDLA0003 per-bone auxiliary scalar只严格验证并丢弃；完整冲突权重、非1 blend、point-property attachment、旋转/重力/IK constraint尚未执行；受限平移physics见下一条。

- **2026-09-02 straight-albedo 现役更正**：上方JPEG-only缩放边界已被共享candidate正反门扩为source-proven opaque embedded JPEG或RGB PNG；仍要求effective TEXB0003、完整逐mip magic/decoded extent、authored physical=mapped且CGImage无alpha。带alpha PNG、padding、TEXB0004、格式/extent冲突及其他data purpose不继承；实际Metal extent仍同时作为physical/mapped并使用identity UV。见[E-V4-OPAQUE-PNG-STRAIGHT-ALBEDO-AND-ACTIVE-READINESS](../../history/scene/runtime-evidence-index.md#e-v4-opaque-png-straight-albedo-and-active-readiness)。

## 12. 当前 MyWallpaperX 映射

| 层级 | 当前状态 | 下一合同 |
|---|---|---|
| Scene/object IR | format 25 继承 v24 attachment bind frame，并增加 authored Puppet `animationlayers` 的 id/animation/additive/blend/blend-in/out/time/rate/static-or-bound visibility；runtime另外从同一MDAT/pose生成current attachment frame，受限layered animation与typed visibility已消费 | 保持 raw/typed 双层合同；不外推完整冲突mixing/权重、point-property attachment、其他MDAT/version或任意SceneScript visibility |
