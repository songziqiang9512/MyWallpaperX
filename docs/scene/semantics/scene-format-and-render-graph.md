...
- `b9bdbd5e` / `226ea5f7` 把 `8b06538d` 只服务 strict REFRACT 的用途合同扩展到共享 `SceneTextureLoader` 并让 data image 严格 fail closed；`d432d4d5` 再把 cache/upload identity 穷举为 `premultipliedColor`、`straightAlbedo`、generic `preservedChannels`、`mask`、`noise`、`flow`、`phase`、`normal`。全部 32 个 Effect 辅助纹理调用显式传入 purpose：mask 16、noise 6、flow 2、phase 2、normal 2、generic preserved 3、premultiplied Blend color 1；此前手列 loader 的门漏掉 Standard Blur，当前 glob 全部 production helper。七类非预乘语义各自 cache 稳定、跨 purpose identity 分离，但当前都按严格 source-channel policy 上传；direct/embedded data image 仍只在原尺寸、无 decode 映射、integer packed、明确非预乘 alpha 或无 alpha（skip）且 32-bit byte order 受控的 RGB(A/X) provider 可归一为 RGBA 时直通。自动门覆盖 Water Flow 逐字节、Water Ripple normal/mask GPU、BC semantic cache、raw color premultiply/extent、production X-Ray fallback，以及 strict REFRACT 的 format-4 CPU/native BC 与 byte-equivalent format-0 RGBA 位移一致和 coverage 单次作用；定向门见 [E-EFFECT-TEXTURE-PURPOSE](runtime-evidence-index.md#e-effect-texture-purpose) 与 [E-PARTICLE](runtime-evidence-index.md#e-particle)。format 5 / BC5 在当前 strict REFRACT 仍拒绝；受支持 straight-RGBA decoder/source、generic Effect/material DXT5n、Windows 通道/法线 golden，以及尚未携带 physical/mapped metadata 的辅助纹理 UV/sampler 仍未闭合。
