...
| dependency | graph 已结构化区分固定 `previous`、effect-scoped RT 和 copy/swap；strict Blur 已消费 effect target table，并闭合 effect/plan/extent/format cache、跨 effect 隔离、原子替换与 reset；table 已分配 BGRA backbuffer 和 RGBA8888 FBO；bounded frame registry 按另一命名空间处理 named target、property-authored fallback 和受限 PNG/JPEG property source | ShaderContract v1 与 Local Contrast 格式前置已完成；下一步 strict Local Contrast，再接显式 dynamic generation、system/media/video/variant/effectful/nested source及 generic compose/history scheduler |
