...
| live values | format 25 继承 binding program；layer alpha、solid color、direct text、Local Contrast/Opacity consumer 已执行；Timeline 已对 layer alpha 与 effect constant 建立受限 typed target/evaluator，部分 transform/relative 等声明继续 fail closed；Puppet animation visibility 只接受无 property binding 的静态值；通用 SceneScript 未接入 | 下一 target 继续同批接 compiler、consumer、fallback 与 identity 门；Timeline 的精确子项以 [覆盖台账 §6.1](coverage-ledger.md#61-timeline-与-scenescript) 为准 |
