...
| live values | format 25 继承 binding program；layer alpha、solid color、direct text、Local Contrast/Opacity consumer 已执行；Timeline 的 **48/48** 现役 authored host 已进入受限 typed target/绝对时钟/作者 Bézier evaluator，覆盖 effect constant、layer alpha、bounded relative transform、root particle scalar、text width 与单一 default 2D camera Combined；Puppet animation visibility 只接受无 property binding 的静态值；通用 SceneScript 未接入 | generic Combined、其他 relative/target、multiple path/3D camera、wrap-loop smoothing 与 event crossing 继续 fail closed；Timeline 精确子项以 [覆盖台账 §6.1](coverage-ledger.md#61-timeline-与-scenescript) 为准 |
