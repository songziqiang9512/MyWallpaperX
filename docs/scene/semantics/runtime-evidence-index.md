...
| 最新合同门 | interpretation v29；完整 Scene suite 407 项：404 通过、3 跳过；代码健康 447 Swift files、44 locked legacy files、400-line limit |
...
- 类别判据：15 个 stock 字体的类别由本机对只读安装目录的 CoreText 实测得出，不按字体名猜——emoji 看 `traitColorGlyphs` 与 COLR/sbix/CBDT 表（仅 TwemojiMozilla 命中）；mono 看 fixed-pitch trait 或数字与字母 advance 是否一致（Monofur-PK7og、RobotoMono-Regular、Segment7Standard、opensticks 带 trait，CursedTimerUlil-Aznm 无 trait 但数字与字母 advance 全等）；NotoSans-Regular 数字等宽而字母不等宽，归 sans；其余 8 个归 display。近似目标按类别取 Menlo / Noto Sans / Apple Color Emoji / Helvetica，家族缺失时统一退 Helvetica。
- 自动门：[test_scene_text_rendering.py](../../../script/tests/test_scene_text_rendering.py)。四个 stock 类别各报 `stockApproximation` + `stockFontApproximated:<category>`；形态断言用 CoreText symbolic traits 实测而不是比名字——`fonts/Segment7Standard.otf` 落到等宽（Menlo-Regular）、`fonts/TwemojiMozilla.ttf` 保住 color glyph（AppleColorEmoji）、`fonts/NotoSans-Regular.ttf` 命中同名本机家族（NotoSans）、`fonts/Alcubierre.otf` 为非等宽（Helvetica）。`systemfont_consolas` 断言 `monospace == true` 且 postScript 不含 Helvetica（本机得 Menlo-Regular）。**三个负例**：包内存在同名 `fonts/Atami-Regular.otf` 时必须回到 `embedded`（stock 不能抢包内文件）；`../Alcubierre.otf` 仍报 `unsafeFontPath`（stock 识别不给越界路径开后门）；表外 `systemfont_notarealalias` 报 `systemAliasUnknown`，而表内 8 个名字都不得报该诊断。
- 边界：本项目**不搬运官方字体文件**，stock 引用只是按实测类别做形态近似——字面轮廓、advance、kerning 与官方字形都不同，像素不可能等价，只保证"等宽仍等宽、emoji 仍彩色"这一层形态特征和可诊断性。`systemfont_segoe`/`systemfont_calibri` 在本机既无对应家族又无等宽/彩色特征，落到 Helvetica 后与通用 sans 结果相同，只能靠诊断区分。类别判据来自本机只读探针而非仓库内自动门（仓库里不能放官方字体做 fixture）；`weight`/`italic`/CJK 缺字回退链与 Windows 逐像素对照仍缺，见 [E-TEXT](#e-text) 的同一条边界。
