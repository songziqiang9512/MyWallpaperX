...
| 固定回归门 | 同一签名 App 的 `.codex/scene-lightshafts0-fixed13-r2-20260725/report.json` 为 13/13；报告 SHA-256 `23f1c3da560646d499dd77624a6d955d7eb80f6ffc43701c2a074c608e5810f2`，矩阵 SHA-256 `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c` |
| 最新合同门 | interpretation v25；完整 Scene suite 372 项：369 通过、3 跳过；代码健康 441 Swift files、44 locked legacy files、400-line limit |
...
| Particle runtime | `f4173ea`/`7d53c10` 的 strict depth-one eventspawn/natural-eventdeath child 已由真实缓存、离屏 Metal 和 `2131872317` 延迟桌面截图验证；`c654571` 的 `rosepetals`/`beam_1`、`f02f41d` 的 `particle/fire/fire1` 与 `a5a951f` 的 `particle/light/light_shafts_0` 已由真实样本、15-key Metal 门及两层矩阵验证；见 E-PARTICLE |
...
- 运行门：`c654571` 将 registry 扩到 13-key，21 样本 asset census 的 `builtInTextureUnavailable` 从 15 降至 13；`f02f41d` 以 `particle/fire/fire1` 扩到 14-key，使 census 降至 11；`a5a951f` 再以 `particle/light/light_shafts_0` 扩到 15-key，使 census 降至 9。Light Shafts 0 定向报告 `.codex/scene-lightshafts0-targeted-20260725/report.json` 为 5/5，SHA-256 `56044c4c24484bd7bb103d6ffdd87c56b0d970b86ff4fb0ed671b34df4528671`：`2131872317:3180`、`2998757800:38618`、`3028090166:139`、`3088601835:579`、`3769761761:156` 均进入 loaded 集合；固定门维持 `18/27`，完整门新增五层至 `91/131`。`2998757800` 的同一 Flare 仍缺 `halo_3`、`fog3`、`flare_1`，并依赖 turbulent velocity、`eventfollow` 和非瞬时 child emitter；`.codex/scene-fog1-zero-299-20260725/report.json` 的透明 `fog1` 隔离门 SHA-256 为 `8d8d6f736a1ebde5828bb635cfc1f896cf7b3b5f5fccf86e6ce7131e438914d9`，右下亮边仍未改变，因此不提交 fog 调暗修复。此前 Fire 与 `c654571` 定向报告继续保留为阶段证据。默认隐藏的 `3742133044:189` 与 `3765760121:161` 仍不创建。真实缓存门中 `3768903841` layer 264 执行 eventspawn child；`2131872317` layer 529 在 fixed 60 Hz 第 233 帧生成 1,024 个 eventdeath trail instance，离屏 Metal 覆盖超过 1,000 像素，二者均不再报对应 layer 的 `childSystemsUnsupported`。
...
- 边界：当前仅接受 depth-one、identity transform、无 control-point mapping、probability 合法、child system limit 1...512、instantaneous emitter 和 Sprite/Sprite Trail renderer；每 child system 最多 1,024 粒子。`rosepetals` 是 64×64 单花瓣近似，`beam_1` 是 128×128 水平软光束近似，`particle/fire/fire1` 是 128×128 低能量火焰遮罩，`particle/light/light_shafts_0` 是 128×128 低能量双光束遮罩，四者都不是官方资产或像素等价实现。`3769364482` 的 turbulence 与 torch ember child 仍未支持；`2131872317` layer 1375 的缺纹理/复杂 profile 继续 `childSystemsUnsupported`，collision/delete/static/follow/nested child、event value inheritance、跨空间 transform、总预算和 Windows WE pixel golden 均未完成。
