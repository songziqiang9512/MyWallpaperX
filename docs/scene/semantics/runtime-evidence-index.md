...
| 项目 | 当前证据 |
|---|---|
| 当前完整快照门 | `.codex/scene-fire1-full45-20260725/report.json` 为 45/45；报告 SHA-256 `71544e52623942818da44a85fc805f3281368f8fd7ce489d28acf895e7030126`，仓库矩阵 `script/scene_wallpaper_full_sample_matrix.json` SHA-256 `ad68f6bd889a6f8152a27e60aad2aaff55ccc7ccf9a924c0551f24e64d02357c` |
| 固定回归门 | 同一签名 App 的 `.codex/scene-fire1-fixed13-20260725/report.json` 为 13/13；报告 SHA-256 `c21c2a13c4754019578129a04e7be07df1b9e1c2c694396e0b26fab87b40c992`，矩阵 SHA-256 `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c` |
...
| Particle runtime | `f4173ea`/`7d53c10` 的 strict depth-one eventspawn/natural-eventdeath child 已由真实缓存、离屏 Metal 和 `2131872317` 延迟桌面截图验证；`c654571` 的 `rosepetals`/`beam_1` 与 `f02f41d` 的 `particle/fire/fire1` 已由真实样本、14-key Metal 门及两层矩阵验证；见 E-PARTICLE |
...
- 运行门：`c654571` 将 registry 扩到 13-key，21 样本 asset census 的 `builtInTextureUnavailable` 从 15 降至 13；`f02f41d` 再以 `particle/fire/fire1` 扩到 14-key，使 census 降至 11。Fire 定向报告 `.codex/scene-fire1-targeted-v2-20260725/report.json` 为 3/3，SHA-256 `8bc040d7146da3ee8b60d1cfbc6f095393f6c066a8fdb086dd79b97b8b6b6561`：`3769364482:446/453` 从 `1/3` 升至 `3/3`，`3757555836:98` 从 `6/7` 升至 `7/7`，`2998757800` 因同一 Flare 仍缺 `halo_3`、`fog3`、`flare_1` 而保持不变。固定门维持 `18/27`，完整门新增三层至 `86/131`。此前 `c654571` 的定向报告 `.codex/scene-builtin-textures-targeted-20260725/report.json` 为 2/2，SHA-256 `9aedf1d47eff68d9d4800de2918fb9f41673f101c59c3502bb3723521b5d800f`。默认隐藏的 `3742133044:189` 与 `3765760121:161` 仍不创建。真实缓存门中 `3768903841` layer 264 执行 eventspawn child；`2131872317` layer 529 在 fixed 60 Hz 第 233 帧生成 1,024 个 eventdeath trail instance，离屏 Metal 覆盖超过 1,000 像素，二者均不再报对应 layer 的 `childSystemsUnsupported`。
...
- 边界：当前仅接受 depth-one、identity transform、无 control-point mapping、probability 合法、child system limit 1...512、instantaneous emitter 和 Sprite/Sprite Trail renderer；每 child system 最多 1,024 粒子。`rosepetals` 是 64×64 单花瓣近似，`beam_1` 是 128×128 水平软光束近似，`particle/fire/fire1` 是 128×128 低能量火焰遮罩，三者都不是官方资产或像素等价实现。`3769364482` 的 turbulence 与 torch ember child 仍未支持；`2131872317` layer 1375 的缺纹理/复杂 profile 继续 `childSystemsUnsupported`，collision/delete/static/follow/nested child、event value inheritance、跨空间 transform、总预算和 Windows WE pixel golden 均未完成。
