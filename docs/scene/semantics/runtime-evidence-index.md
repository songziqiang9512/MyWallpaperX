...
| 固定回归门 | `2f897bc` 签名 App 的 `.codex/scene-continuous-child-final-fixed13-20260725/report.json` 为 13/13；报告 SHA-256 `19135b84d74082f0cdcd5474157e0d1cbc8324379d8d706ae089236ad2d8433e`，矩阵 SHA-256 `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c` |
...
| 签名 App | 当前固定门与 continuous-child Flare 定向门均为 `2.0.8 (268)`，Team `H9QWU9XN8R`，CDHash `fe2036eabd4d4093340dd372565a1d95e98c1f72`，executable SHA-256 `3ebc20173bb8a942765078c16eb0dc288b9843145f974596eab8c2cc8779f308`；benchmark staged App 运行前后签名均验证。45 样本完整快照仍是 `a5a951f` 阶段证据，未冒充当前二进制全量门 |
| executable SHA-256 | `3ebc20173bb8a942765078c16eb0dc288b9843145f974596eab8c2cc8779f308` |
...
- 运行门：`c654571`/`f02f41d`/`a5a951f` 依次把 registry 扩到 15-key，21 样本 asset census 的 `builtInTextureUnavailable` 从 15 降至 9；完整快照维持 particle `91/131`。`4a17ee6` 的五样本 turbulent 定向门继续作为对应阶段证据。`2f897bc` 的当前源码固定门 `.codex/scene-continuous-child-final-fixed13-20260725/report.json` 为 13/13、particle `18/27`，SHA-256 `19135b84d74082f0cdcd5474157e0d1cbc8324379d8d706ae089236ad2d8433e`；Flare 定向门 `.codex/scene-continuous-child-final-flare-20260725/report.json` 为 1/1、particle `8/15`，SHA-256 `3ca3c4919b3ed2d5aa6142775e27a809090a49bc0cd5ba9b9008ce832726b2e5`。两门 sample-root residue 0。合成门锁定 continuous、instantaneous+rate、有限/无限 duration、完成回收、Event Follow parent-death 回收与 64-system/65,536-capacity aggregate budget；真实 Flare profile 中 `Flare_Flame`/`Flare_Smoke` 为无限 continuous，`Flare_Sparks` 为 instantaneous 1 + rate 10，三者均 accepted，audio child 继续 fail closed。`2998757800` Flare 当前只剩 `particle/halo_3`、`particle/fog/fog3`、`particle/light/flare_1` 三张缺失 built-in 纹理，仍为 `8/15`；`.codex/scene-fog1-zero-299-20260725/report.json` 仍证明右下亮边与 `fog1` 无关。默认隐藏的 `3742133044:189` 与 `3765760121:161` 仍不创建。真实缓存门中 `3768903841` layer 264 执行 eventspawn child；`2131872317` layer 529 在 fixed 60 Hz 第 233 帧生成 1,024 个 eventdeath trail instance，离屏 Metal 覆盖超过 1,000 像素。
