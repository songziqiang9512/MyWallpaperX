...
| 项目 | 当前证据 |
|---|---|
| 当前完整快照门 | `.codex/scene-lightshafts0-full45-20260725/report.json` 为 45/45；报告 SHA-256 `44d299f0c6757ae46e59e9486bbf5ee423f7dc908d9fa6ddbae72b8c90578c38`，仓库矩阵 `script/scene_wallpaper_full_sample_matrix.json` SHA-256 `b005da924cfefbcd08410d298f8795af67086f62c7df95ca802988fcfabea38e` |
| 固定回归门 | `899704b` 签名 App 的 `.codex/scene-static-child-final-fixed13-20260725/report.json` 为 13/13、particle `18/27`、strict stage 24、chain 2、failed 0；报告 SHA-256 `8466740278b5c194d0bf57f8d4dc825615e7af1f8742750a94a6b359af647ea6`，矩阵 SHA-256 `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c` |
| Static child 定向门 | `.codex/scene-static-child-final-targeted-20260725/report.json` 为 2/2；报告 SHA-256 `7c3189d27c21919a99e7046a435901ceae57dd24edc57c627e1f3e1ddd4c5f29`；`3750813609` 与 `3766387484` 的第二层金黄色叶片可见 |
| 最新合同门 | interpretation v25；完整 Scene suite 376 项：373 通过、3 跳过；代码健康 443 Swift files、44 locked legacy files、400-line limit |
...
| 签名 App | 当前固定门与 static child 定向门均为 `2.0.8 (268)`，Team `H9QWU9XN8R`，CDHash `d20e9f264f657fece37107a314ff6647787ee138`，executable SHA-256 `271a7e69b471af4e36e16da1f9570e74d2a63372f59e4b3ad01cd8850d993862`；benchmark staged App 运行前后签名均验证。45 样本完整快照仍是 `a5a951f` 阶段证据，未冒充当前二进制全量门 |
| executable SHA-256 | `271a7e69b471af4e36e16da1f9570e74d2a63372f59e4b3ad01cd8850d993862` |
| 样本边界 | 真实 Workshop root 只读；报告均来自隔离 sample root 与临时 HOME |

当前真实目录有 46 个数字目录；其中 `3770500543` 缺少 package，source manifest 明确跳过，45 个可运行副本在隔离 root/HOME 下完成正式快照。`a5a951f` 完整门加载 image `449/451`、text `244/244`、solid `140/140`，particle 为 `91/131`（`8bac86e` 的 REFRACT fail-closed 与当时 15-key registry 口径）；strict graph 为 91 stage、15 条 multi-effect chain、Water Flow 10、Water Waves 11、Shake 24、Opacity 8、Local Contrast 2、0 failed、113 个 route-only effect。`899704b` 固定 13 样本门单独保护 particle `18/27`、24 stage、2 条 chain、Water Flow 1、Water Waves 6、Shake 1、Workshop Shadow 1、Opacity 4、Local Contrast 2、0 failed；两门互补，均不证明完整兼容或 WE parity，也不把旧 45 门冒充当前二进制全量门。
...
- 运行门：`c654571`/`f02f41d`/`a5a951f` 依次把 registry 扩到 15-key，`9748a8c` 再扩到 18-key，21 样本 asset census 的 `builtInTextureUnavailable` 从 15 降至 9、再降至 6；旧完整快照维持 particle `91/131`。`4a17ee6` 的五样本 turbulent 定向门继续作为对应阶段证据。当前源码固定门 `.codex/scene-static-child-final-fixed13-20260725/report.json` 为 13/13、particle `18/27`、strict stage 24、chain 2、failed 0，SHA-256 `8466740278b5c194d0bf57f8d4dc825615e7af1f8742750a94a6b359af647ea6`；static child 定向门 `.codex/scene-static-child-final-targeted-20260725/report.json` 为 2/2，SHA-256 `7c3189d27c21919a99e7046a435901ceae57dd24edc57c627e1f3e1ddd4c5f29`。两门 sample-root residue 0。合成门锁定显式/legacy default static 单次创建、transform/probability fail-closed、continuous、instantaneous+rate、有限/无限 duration、完成回收、Event Follow parent-death 回收与 64-system/65,536-capacity aggregate budget；`3750813609` 与 `3766387484` 第二层金黄色叶片恢复。真实 Flare profile 中 `Flare_Flame`/`Flare_Smoke` 为无限 continuous，`Flare_Sparks` 为 instantaneous 1 + rate 10，三者均 accepted，audio child 继续 fail closed。Flare 三纹理让 layer 260 进入 `64x64 additive` 执行，初版持续烟雾累积过曝经逐纹理隔离后将 `fog3` 峰值锁到约 2/255；ready/after 均保留背景细节。程序纹理不是官方资产，也不是 WE pixel parity；`.codex/scene-fog1-zero-299-20260725/report.json` 仍证明右下亮边与 `fog1` 无关。默认隐藏的 `3742133044:189` 与 `3765760121:161` 仍不创建。真实缓存门中 `3768903841` layer 264 执行 eventspawn child；`2131872317` layer 529 在 fixed 60 Hz 第 233 帧生成 1,024 个 eventdeath trail instance，离屏 Metal 覆盖超过 1,000 像素。隔离 45 样本 census 记录 static/default-static 119 条（显式 63、缺失 type 56），当前 strict identity 子集 14 条、覆盖 9 个样本；collision/delete、inherit-value-from-event、非空 child CP mapping 均为 0，非 identity event transform 只有 `2998757800` scale 0.2 与 `3743305891` scale 2.5 两条。
...
- 边界：Turbulent Velocity 仅执行非音频 profile，使用项目自建确定性 gradient noise 消费 forward/right/up、phase、scale、time 和 speed range；它不是 WE 数值或像素等价实现，audio profile 继续 `unsupportedInitializer` + `audioResponseIgnored`。Child 仍仅接受 depth-one、identity transform、无 control-point mapping、static probability=1、event probability 合法、child system limit 1...512、Sphere/Box、有限合法 schedule、非 audio profile、已知 emitter flag 和 Sprite/Sprite Trail renderer；Event Follow 以 parent ID 更新 origin 并随 parent death 回收，static/default-static 只在 root origin 创建一次且不消费 event `nextSeed`。每 child system 最多 1,024 粒子，每 root child runtime 最多 64 systems/65,536 capacity。18 个项目自建程序纹理均不是官方资产。`3769364482` 的 torch turbulence 已执行，但 ember child 仍有其他 profile/资源边界；`2131872317` layer 1375 的缺纹理/复杂 profile 继续 `childSystemsUnsupported`，collision/delete/nested child、event value/CP inheritance、非 identity transform、跨层/递归总预算和 Windows WE pixel golden 均未完成。
