...
| 固定回归门 | `4a17ee6` 签名 App 的 `.codex/scene-turbulent-velocity-final-fixed13-20260725/report.json` 为 13/13；报告 SHA-256 `18ea2ca3ae8a65dcb1f9ad94d6bba1afb418b6aec5a944274756b71416f8abfe`，矩阵 SHA-256 `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c` |
| 最新合同门 | interpretation v25；完整 Scene suite 374 项：371 通过、3 跳过；代码健康 441 Swift files、44 locked legacy files、400-line limit |
...
| Particle runtime | `f4173ea`/`7d53c10` 的 strict depth-one eventspawn/natural-eventdeath child 已由真实缓存、离屏 Metal 和 `2131872317` 延迟桌面截图验证；`c654571`/`f02f41d`/`a5a951f` 的 15-key built-in registry 已由真实样本及两层矩阵验证；`4a17ee6` 的非音频 turbulent velocity 有 seed/time/scale/audio 正反门与 5 个真实样本定向门；见 E-PARTICLE |
...
| 签名 App | 当前固定门与 turbulent 定向门均为 `2.0.8 (268)`，Team `H9QWU9XN8R`，CDHash `f3e387c64312ab85cbe1a163c745931148a60085`，executable SHA-256 `11b78046f9907c069dbdf28d0dfecdd6314a4ae0e4aece349dcc773ba21daa6d`；benchmark staged App 运行前后签名均验证。45 样本完整快照仍是 `a5a951f` 阶段证据，未冒充当前二进制全量门 |
| executable SHA-256 | `11b78046f9907c069dbdf28d0dfecdd6314a4ae0e4aece349dcc773ba21daa6d` |
...
- 运行门：`c654571`/`f02f41d`/`a5a951f` 依次把 registry 扩到 15-key，21 样本 asset census 的 `builtInTextureUnavailable` 从 15 降至 9；完整快照维持 particle `91/131`。`4a17ee6` 的定向报告 `.codex/scene-turbulent-velocity-final-targeted-20260725/report.json` 为 5/5，SHA-256 `d86694accc534e83fba4f83375864ed705b1f6cc49b7a9f48966a9543e2a40c2`，覆盖 leaves `2470144420`、Flare `2998757800`、temple leaves `3750813609`、torch `3769364482` 与 ember `3769761761`；五者 sample-root residue 0，非音频 `turbulentvelocityrandom` 不再产生 `unsupportedInitializer`。当前源码固定门 `.codex/scene-turbulent-velocity-final-fixed13-20260725/report.json` 为 13/13、particle `18/27`。`2998757800` Flare 仍缺 `halo_3`、`fog3`、`flare_1`、`eventfollow` 与非瞬时 child emitter；`.codex/scene-fog1-zero-299-20260725/report.json` 仍证明右下亮边与 `fog1` 无关。默认隐藏的 `3742133044:189` 与 `3765760121:161` 仍不创建。真实缓存门中 `3768903841` layer 264 执行 eventspawn child；`2131872317` layer 529 在 fixed 60 Hz 第 233 帧生成 1,024 个 eventdeath trail instance，离屏 Metal 覆盖超过 1,000 像素。
...
- 边界：Turbulent Velocity 仅执行非音频 profile，使用项目自建确定性 gradient noise 消费 forward/right/up、phase、scale、time 和 speed range；它不是 WE 数值或像素等价实现，audio profile 继续 `unsupportedInitializer` + `audioResponseIgnored`。Child 仍仅接受 depth-one、identity transform、无 control-point mapping、probability 合法、child system limit 1...512、instantaneous emitter 和 Sprite/Sprite Trail renderer；每 child system 最多 1,024 粒子。四类程序纹理均不是官方资产。`3769364482` 的 torch turbulence 已执行，但 ember child 仍未支持；`2131872317` layer 1375 的缺纹理/复杂 profile 继续 `childSystemsUnsupported`，collision/delete/static/follow/nested child、event value inheritance、跨空间 transform、总预算和 Windows WE pixel golden 均未完成。
