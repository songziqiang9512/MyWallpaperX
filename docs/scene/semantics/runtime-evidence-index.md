...
| 固定回归门 | `928acca` 签名 App 的 `.codex/scene-eventfollow-final-fixed13-20260725/report.json` 为 13/13；报告 SHA-256 `97fb8b32c3ce54f8c71ab0f3d6b1ed05f73b8db87acce9b7b225b2b6c542b5ea`，矩阵 SHA-256 `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c` |
| 最新合同门 | interpretation v25；完整 Scene suite 375 项：372 通过、3 跳过；代码健康 441 Swift files、44 locked legacy files、400-line limit |
...
| Particle runtime | `f4173ea`/`7d53c10` 的 strict depth-one eventspawn/natural-eventdeath child 已由真实缓存、离屏 Metal 和 `2131872317` 延迟桌面截图验证；`928acca` 为 identity/no-CP/瞬时 Sprite `eventfollow` 建立 parent-ID owner、逐帧 follow 与 parent-death 回收；`c654571`/`f02f41d`/`a5a951f` 的 15-key built-in registry 已由真实样本及两层矩阵验证；`4a17ee6` 的非音频 turbulent velocity 有 seed/time/scale/audio 正反门与 5 个真实样本定向门；见 E-PARTICLE |
...
| 签名 App | 当前固定门与 Event Follow Flare 定向门均为 `2.0.8 (268)`，Team `H9QWU9XN8R`，CDHash `b7b3017bfca4ed726ac79bbcb40b6443e7103422`，executable SHA-256 `b2e78d3d579f69a6781fada6480abfbc8da00d8cffa7d85e6d558d84ecce81b7`；benchmark staged App 运行前后签名均验证。45 样本完整快照仍是 `a5a951f` 阶段证据，未冒充当前二进制全量门 |
| executable SHA-256 | `b2e78d3d579f69a6781fada6480abfbc8da00d8cffa7d85e6d558d84ecce81b7` |
...
- 运行门：`c654571`/`f02f41d`/`a5a951f` 依次把 registry 扩到 15-key，21 样本 asset census 的 `builtInTextureUnavailable` 从 15 降至 9；完整快照维持 particle `91/131`。`4a17ee6` 的五样本 turbulent 定向门继续作为对应阶段证据。`928acca` 的当前源码固定门 `.codex/scene-eventfollow-final-fixed13-20260725/report.json` 为 13/13、particle `18/27`，SHA-256 `97fb8b32c3ce54f8c71ab0f3d6b1ed05f73b8db87acce9b7b225b2b6c542b5ea`；Flare 定向门 `.codex/scene-eventfollow-final-flare-20260725/report.json` 为 1/1、particle `8/15`，SHA-256 `f9dfee453014d13381f5c8c89d8887d10dd6925a606bf4c91426461b7843f0e0`。两门 sample-root residue 0。`2998757800` Flare 仍缺 `halo_3`、`fog3`、`flare_1` 与非瞬时 child emitter；`.codex/scene-fog1-zero-299-20260725/report.json` 仍证明右下亮边与 `fog1` 无关。合成门锁定 Event Follow child count `0→1→1→0→0`、parent x `2→3`、parent death 回收，并证明持续 child 从 `unsupportedType:eventfollow` 推进到 `outsideStrictEventProfile`。默认隐藏的 `3742133044:189` 与 `3765760121:161` 仍不创建。真实缓存门中 `3768903841` layer 264 执行 eventspawn child；`2131872317` layer 529 在 fixed 60 Hz 第 233 帧生成 1,024 个 eventdeath trail instance，离屏 Metal 覆盖超过 1,000 像素。
...
- 边界：Turbulent Velocity 仅执行非音频 profile，使用项目自建确定性 gradient noise 消费 forward/right/up、phase、scale、time 和 speed range；它不是 WE 数值或像素等价实现，audio profile 继续 `unsupportedInitializer` + `audioResponseIgnored`。Child 仍仅接受 depth-one、identity transform、无 control-point mapping、probability 合法、child system limit 1...512、instantaneous emitter 和 Sprite/Sprite Trail renderer；Event Follow 以 parent ID 更新 origin 并随 parent death 回收，每 child system 最多 1,024 粒子。四类程序纹理均不是官方资产。`3769364482` 的 torch turbulence 已执行，但 ember child 仍未支持；`2131872317` layer 1375 的缺纹理/复杂 profile 继续 `childSystemsUnsupported`，非瞬时 emitter、collision/delete/static/nested child、event value/CP inheritance、跨空间 transform、总预算和 Windows WE pixel golden 均未完成。
