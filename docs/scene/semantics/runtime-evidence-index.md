...
| 最新合同门 | interpretation v29；完整 Scene suite 411 项：408 通过、3 跳过；代码健康 447 Swift files、44 locked legacy files、400-line limit |
...
- 语料判据：本机 452 个不同 text layer（29 个不同 scene 指纹，来自 `.codex/*/runtime-homes/*/Library/Caches/MyWallpaperX/SteamWorkshopScene/*/scene.json` 与只读官方安装）里 `limitwidth` False 393 / True 58 / 缺省 1，`limitrows` False 392 / True 59 / 缺省 1，`limituseellipsis` False 437 / True 14，`maxrows` 为 1 的 434 个、2 的 6 个、3 的 5 个、22 的 5 个、21 的 1 个。**开关语义由语料反证**：393 个 `limitwidth: false` 的 layer 全带编辑器默认 `maxwidth: 500`，其中 79 个去重后的作者宽度已经大于 500——若无条件套用就会把它们错误压窄，所以两个数值必须被开关门控。反向的 45/47 个 `limitwidth: true` 去重行满足 `size.x <= maxwidth`；剩下 2 个（653、624 对 `maxwidth: 620`）是 property 驱动 layer，作者 `size` 已过期。
...
- 类别判据：15 个 stock 字体的类别由本机对只读安装目录的 CoreText 实测得出，不按字体名猜——emoji 看 `traitColorGlyphs` 与 COLR/sbix/CBDT 表（仅 TwemojiMozilla 命中）；mono 看 fixed-pitch trait 或数字与字母 advance 是否一致（Monofur-PK7og、RobotoMono-Regular、Segment7Standard、opensticks 带 trait，CursedTimerUlil-Aznm 无 trait 但数字与字母 advance 全等）；NotoSans-Regular 数字等宽而字母不等宽，归 sans；其余 8 个归 display。类别现在只决定 app 包缺失时的兜底家族（Menlo / Noto Sans / Apple Color Emoji / Helvetica，家族缺失时统一退 Helvetica）和诊断后缀，不再决定正常命中时的字形。
- 字形来源判据：15 个名字全部有随 app 分发的真实字体文件（`MyWallpaperX/Resources/SceneStockFonts.bundle`，约 2.9 MB，13 个字体 + 9 份许可文本 + `NOTICE.md`）。官方客户端有权随自己分发全部 15 个，该权利不传递给本项目，因此逐个核验内嵌 name table 与上游许可后分成两类：**8 个原版**（8bitOperatorPlus8-Regular、Blackout 2 AM、Monofur-PK7og、NotoSans-Regular、RobotoMono-Regular、Segment7Standard、TwemojiMozilla、opensticks，许可为 OFL 1.1 / Apache 2.0 / CC-BY 4.0 / 可商用 freeware）直接随包，**7 个替代**（Atami-Regular、Alcubierre、spincycle_3d_ot、summer85、Lazer84、kust、CursedTimerUlil-Aznm）原文件禁止再分发，改用外形接近的自由字体。`Blackout 2 AM` 的内嵌 copyright 写 "All rights reserved"，但 [The League of Moveable Type](https://github.com/theleagueof/blackout) 以 OFL 发布的同名文件与客户端那份 SHA-256 逐字节相同（`48e96e2a…cea2d0`），按原版处理；`PermanentMarker-Regular` 同为未更新的 copyright 字段，以 Google Fonts `apache/` 目录随附的 Apache 2.0 文本为准。
...
- 自动门：[test_scene_text_rendering.py](../../../script/tests/test_scene_text_rendering.py)。门禁不加生产侧注入钩子：CLI 下 `Bundle.main.resourceURL` 就是可执行文件所在目录，把 `SceneStockFonts.bundle` 复制到 harness binary 旁边即命中与 app 完全相同的加载路径。断言分四组：**原版**逐个校 PostScript 名（Segment7Standard / RobotoMono-Regular / NotoSans-Regular / TwemojiMozilla / 8-bitOperatorPlus8-Regular / Blackout2AM），source 为 `stockBundled`、诊断为空；**替代**校得到许可明确的近似字体而非通用家族（Poppins-Medium / BungeeShade-Regular / Segment7Standard），source 为 `stockSubstituted`、诊断 `stockFontSubstituted:<category>`；**闭合**遍历语料出现过的全部 15 个 stock 引用，全部落在 `stockBundled`/`stockSubstituted` 且计数恰为 8/7，任一名字漏配随包文件即失败；**包缺失**把同一个 binary 放到没有 bundle 的目录再跑一次，5 个 key 从 `stockBundled` 翻到 `stockApproximation` + `stockFontFileUnavailable:<category>` 而其余 12 个 key 不变，证明断言非平凡且降级不抛错。形态断言仍用 CoreText symbolic traits 实测（等宽/彩色 glyph），`systemfont_consolas` 断言 `monospace == true` 且 postScript 不含 Helvetica。**三个负例**：包内存在同名 `fonts/summer85.ttf` 时必须回到 `embedded`（stock 不能抢包内文件）；`../Alcubierre.otf` 仍报 `unsafeFontPath`（stock 识别不给越界路径开后门）；表外 `systemfont_notarealalias` 报 `systemAliasUnknown`，而表内 8 个名字都不得报该诊断。另有许可门：13 个随包字体每一个都必须有对应许可文本、且文件名与许可文件名都出现在 `NOTICE.md` 里。
- 边界：7 个替代字形**不是官方字形**——轮廓、advance、kerning 与原版都不同，只做到"同类形态 + 许可干净 + 一定有字可渲染"，因此命中时始终带 `stockSubstituted` 诊断，不能在报告里读成等价。8 个原版是逐字节相同的官方文件，但仍无 Windows 逐像素对照，栅格化、hinting 与 LCD 滤波差异未验证。类别判据来自本机只读探针而非仓库内自动门（仓库里不放官方字体做 fixture，随包字体是产品资源不是测试 fixture）；`weight`/`italic`/CJK 缺字回退链仍缺，见 [E-TEXT](#e-text) 的同一条边界。`systemfont_segoe`/`systemfont_calibri` 在本机既无对应家族又无等宽/彩色特征，落到 Helvetica 后与通用 sans 结果相同，只能靠诊断区分。
