...
| 项目 | 当前证据 |
|---|---|
| 当前完整快照门 | `.codex/scene-attachment-full45-20260725/results-v1/report.json` 为 45/45；报告 SHA-256 `5aee296a65da3b94e5d0a76973691244ba1fe0b9024513d52360ab660fc694c3`，仓库矩阵 `script/scene_wallpaper_full_sample_matrix.json` SHA-256 `6a4896006a43bca98153b7ce12e5b52a7ec2be1d0f9011db4e6cd32b50f9a4fe` |
| 固定回归门 | 同一签名 App 的 `.codex/scene-attachment-fixed13-20260725/results-v1/report.json` 为 13/13；报告 SHA-256 `1a1a03b9007ea6448019da2f3e4605dff6ffe9693feb15cf8a8905811f536998`，矩阵 SHA-256 `4bc281897220b55ed0d0e6a5d57c9625155c4f142738a2a52ddb5d3cb98939b2` |
| 最新合同门 | interpretation v24；完整 Scene suite 422 项：419 通过、3 跳过；代码健康 435 Swift files、44 locked legacy files、400-line limit |
...
| Puppet/BC 合成正确性 | `3769688830` 定向门 7 个 puppet mesh 全部重组，三处静态 MDAT attachment frame 解析并应用，BC matte 与 REFRACT 白块消除；见 E-PUPPET-BC |
...
| 签名 App | 当前两层门均为 `2.0.8 (268)`，Team `H9QWU9XN8R`，CDHash `65cf71e5a1236091c9363e154ab42fc6bb657aa9`，executable SHA-256 `1afc01878dcfc6fcbb8ee1e8027e1a908123821653145c4432ede15e9b70e2c0`；benchmark staged App 运行前后签名均验证 |
| executable SHA-256 | `1afc01878dcfc6fcbb8ee1e8027e1a908123821653145c4432ede15e9b70e2c0` |
...
<a id="e-puppet-bc"></a>
### E-PUPPET-BC: Puppet bind-pose mesh/静态 attachment、BC premultiplied 解码与 REFRACT fail-closed

- 实现提交：mesh/BC/REFRACT `8bac86e`，静态 attachment `49ee89a`。代码：[SceneMdlPuppetMeshReader.swift](../../../MyWallpaperX/Core/SteamWorkshopScene/Format/SceneMdlPuppetMeshReader.swift)、[SceneMdlPuppetAttachmentReader.swift](../../../MyWallpaperX/Core/SteamWorkshopScene/Format/SceneMdlPuppetAttachmentReader.swift)、[ScenePuppetMeshRecomposer.swift](../../../MyWallpaperX/Core/SteamWorkshopScene/Rendering/ScenePuppetMeshRecomposer.swift)、[SceneLayerWorldFrameResolver.swift](../../../MyWallpaperX/Core/SteamWorkshopScene/Rendering/SceneLayerWorldFrameResolver.swift)、[SceneBCTextureDecoder.swift](../../../MyWallpaperX/Core/SteamWorkshopScene/Format/SceneBCTextureDecoder.swift)、[SceneParticleAssetGraph.swift](../../../MyWallpaperX/Core/SteamWorkshopScene/Particles/SceneParticleAssetGraph.swift)
- 自动门：[test_scene_puppet_mesh.py](../../../script/tests/test_scene_puppet_mesh.py)（12 项，含 MDLS hierarchy/MDAT local 组合、真实三挂点数值、missing bone/duplicate name fail closed）、[test_scene_layer_world_frame.py](../../../script/tests/test_scene_layer_world_frame.py)（3 项，锁定 `parent * attachment * child local`、parent scale 和非法 frame 回退）、[test_scene_bc_texture_decoder.py](../../../script/tests/test_scene_bc_texture_decoder.py)（7 项）、[test_scene_particle_assets.py](../../../script/tests/test_scene_particle_assets.py)（REFRACT 层置 nil + `refractionUnsupported` 诊断）。本批四组定向合同 61/61。
- 定向运行门：`.codex/scene-ahri-attachment-20260725/targeted-v1/report.json` PASS，报告 SHA-256 `a1638081c99bd2d23a0484f4676e921e774fdca8d892ab1d30edad54c3f94ee1`；interpretation v24 保存 `aaaaaaaaa/orb/Attachment` 三个 bind frame。日志中 ahriarm/ahriorb 为 `attachment=...:bind`；修复前漂在最右的 arm/shoulder layer 回到 Ahri 肩部，orb 回到右上挂点。`Water droplets` 当前粒子层仍 unavailable，只保存 attachment IR。
- 边界：bind-pose mesh 与静态 attachment 均为 `executed-degraded`；reader 仅接受已验证的 `MDLV0023 + MDLS0004 + MDAT0001`，MDLA/deformation/skinning 和动画 attachment follow 未执行。REFRACT 材质在真实 refraction pass 之前整层 fail closed；BC 解码性能门为未优化 Debug 构建 4K BC1 约 0.3 s（行级并发）。
