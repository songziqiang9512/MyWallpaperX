...
| 项目 | 当前证据 |
|---|---|
| 当前完整快照门 | `.codex/scene-snow-smoke-full45-20260725/report.json` 为 45/45、particle `101/131`、strict stage 91、chain 15、failed 0；报告 SHA-256 `6118c02a07439661b873d415b4eab9371944bc4b89389c8e2578ee2cbed3dd87`，仓库矩阵 `script/scene_wallpaper_full_sample_matrix.json` SHA-256 `b005da924cfefbcd08410d298f8795af67086f62c7df95ca802988fcfabea38e` |
| 固定回归门 | `8a27089` 签名 App 的 `.codex/scene-snow-smoke-fixed13-v2-20260725/report.json` 为 13/13、particle `19/27`、strict stage 24、chain 2、failed 0；报告 SHA-256 `0b73d8aec67702cd1108f6af8d123b13ce506ff11f8bcbcd14165b29a90ecdd5`，矩阵 SHA-256 `479b794d64b48369348b4b8e6583599e5ac70161a4f670102332f5cd1e2d653c` |
| Snow/Smoke 定向门 | `.codex/scene-snow-smoke-targeted-v3-20260725/report.json` 为 1/1、particle `17/19`；报告 SHA-256 `f10103a142a5420692b52675440638f8a19bb954865151fa96f10e2a3d100c51`，定向矩阵 SHA-256 `4f68c1655eec246c6a94093c82a098bbe03a161997b1aec46175d9612e169c9e`；`3088601835:513/534` 可见雪粒增加且未出现 additive 洗白 |
...
- 运行门：`c654571`/`f02f41d`/`a5a951f` 依次把 registry 扩到 15-key，`9748a8c` 扩到 18-key，`8a27089` 扩到 20-key。当前完整、固定、定向报告分别为 45/45 particle `101/131`、13/13 particle `19/27`、1/1 particle `17/19`，三门 sample-root residue 0。`3088601835` 的 Snow layers `513/534` 以 `64x64 additive` 执行，initial 分别为 272/291；preview 对照中兜帽、头发和右侧背景的雪量明确增加，主体、色调与背景细节保留。`smoke2` 不再产生 resource unavailable，但 `snowstormfog` 仍按 `unsupportedTransformOrControlPoint` 拒绝其 origin `0 650 0`，因此本批没有把 Smoke child 写成已执行。既有合成门继续锁定显式/legacy default static 单次创建、transform/probability fail-closed、continuous、instantaneous+rate、有限/无限 duration、完成回收、Event Follow parent-death 回收与 64-system/65,536-capacity aggregate budget。程序纹理不是官方资产，也不是 WE pixel parity；`.codex/scene-fog1-zero-299-20260725/report.json` 仍证明右下亮边与 `fog1` 无关。默认隐藏的 `3742133044:189` 与 `3765760121:161` 仍不创建。真实缓存门中 `3768903841` layer 264 执行 eventspawn child；`2131872317` layer 529 在 fixed 60 Hz 第 233 帧生成 1,024 个 eventdeath trail instance，离屏 Metal 覆盖超过 1,000 像素。隔离 45 样本 census 记录 static/default-static 119 条（显式 63、缺失 type 56），当前 strict identity 子集 14 条、覆盖 9 个样本；collision/delete、inherit-value-from-event、非空 child CP mapping 均为 0。
