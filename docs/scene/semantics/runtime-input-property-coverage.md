...
| typed target 族 | `L3 bounded` | scene/camera/layer/effect/text/particle/script instance 已定义；layer alpha/color、effect scalar及pass-owned Vec2/Vec3、exact stock Local Contrast strength/Opacity alpha、bounded Blend multiply 与 particle CP position/angles 进入 compiler | pass vector只准source/value/wrapper/object-effect-pass identity与provider provenance完整一致的finite pair/triple；其他 effect target不得直接开放live |
| 固定 source priority | `L3` | authored -> property -> Timeline -> SceneScript；property/Timeline与QuickJS scalar/String/Vec3及pass Vec2/Vec3同target时先形成current-frame preliminary value，VM成功结果再覆盖。pass Vec2仍发布为typed pair，pass Vec3发布typed triple，VM调用边界共用现有Vec3 host object | 重复、provider冲突或仍被bounded producer占用的candidate不形成owner并保留现有值；bool/Vec4/matrix、通用官方Vec2 class与typed conflict aggregation尚缺 |
