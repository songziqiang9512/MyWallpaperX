...
| 固定 source priority | `L3` | authored -> property -> Timeline -> SceneScript；property/Timeline与QuickJS scalar同target时先形成current-frame preliminary value，VM成功结果再覆盖。property、受限Timeline、bounded text Date、time-of-day Blend、media placeholder fade与launch-origin startup producer已执行，后四者复用`.sceneScript` source | 不消费preliminary value的bounded native producer彼此及其与property/Timeline冲突时launch失败关闭；重复或被bounded producer占用的QuickJS scalar candidate不形成owner并保留现有值，但typed conflict aggregation尚缺。generic SceneScript与live event mutation仍未接入 |
...
| direct user-property → exact fragment scalar/vector3 shader consumer | `L3 bounded; vector3 + producer-present scalar owner-migration` | binding compiler保留exact `{user,value}`、由authored fallback决定shape的typed definition与target；只有有效property default才发布launch-scoped live producer。Pulse vector3 tint与fragment-only `noisespeed/noiseamount/power` scalar均要求sole `.userProperty` contributor、无script attachment、active non-array fragment ABI、旧plan binding全集、key/target/type与author domain等值。范围内live值进入Program；越界值从snapshot authored lane恢复作者值；缺失producer只保留authored definition。exact producer不存在且旧plan仍可接纳时不撤旧owner。真实`1937925563:13#effect#321`证明`tinthigh` authored consumer；受控重包`9000000962/0963/0964`证明`noiseamount` live/range/missing三路Program/GPU/compositor/next-frame。最终App的scalar profile-local `disable-generic`为50 generic / 1 fallback / 0 dedicated，target previous-current与后缀compositor/next-frame继续，因此producer-present scalar达到bounded owner-migration。property kind错配反例不发布live producer，并只在finalizer拒绝Pulse effect，不拒绝整层。[E-V1-PULSE-FRAGMENT-SCALAR-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-fragment-scalar-user-property-owner) [E-V1-PULSE-TYPED-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-typed-user-property-owner) | 不做广播或类型转换；跨stage `speed/phase/amount`、关系域`bounds`、其他float2、alpha或audio+binding、额外wrapper key、多contributor、Timeline/SceneScript、数组、target/fallback/ABI/domain漂移与未消费binding继续失败关闭或保留incumbent；受控scalar fixture不是authored corpus正证 |
| direct user-property → current-stock X-Ray scalar ABI | `L3 bounded owner-migration` | 共享binding program保留exact `{user,value}` scalar与sole launch producer。旧 X-Ray owner只在唯一effect/material、常量全集`size,multiply`、无Timeline/SceneScript/material user value、spatial-weighted source/resource/profile完整重证时撤权；每个readiness variant必须证明`size`唯一映射active vertex非数组`float [0,1]`、`multiply`唯一映射active fragment非数组`float [0,10]`，且至少一项确有direct property producer。initially admitted exact bool visibility可由下行shared activation consumer持有，不再迫使这类scalar保留旧owner。真实`3757555836:69#effect#131`保留`size={user:"x",value:0.2}`；controlled `9000000131`又闭合live visibility与size-zero previous-current。[E-V1-XRAY-DIRECT-SCALAR-OWNER](runtime-evidence-index.md#e-v1-xray-direct-scalar-owner) [E-V1-XRAY-SHARED-STAGE-ACTIVATION-OWNER](runtime-evidence-index.md#e-v1-xray-shared-stage-activation-owner) | 缺producer、错误type/range/ABI、额外wrapper key、多contributor、Timeline/SceneScript scalar或frame-driven effect visibility仍不撤旧owner。startup-disabled graph admission、`multiply`真实corpus live门、其他X-Ray形态、独立ROI与官方parity仍缺 |
| initially admitted effect visibility / pointer / identity scalar activation | `L3 bounded / S4 mixed slice-visible + owner-migration` | binding compiler按exact layer/effect identity把direct bool映射为typed target，不按effect path准入。pair leaf继续支持visibility/pointer/identity scalar；pointer须全部variant实际消费spatial-weighted pointer host uniform，`size`须为exact static或sole scalar producer。exact sole bool visibility另可附加到already-admitted、单effect、无dependency/clear、且既有visual-failure predicate证明安全的nonpersistent FBO graph；pointer/scalar不随之扩张。inactive发布`activation-passthrough` previous-current并保留generation/publication/completion/compositor；越界user scalar恢复authored lane。controlled `9000000131`闭合pair-leaf owner-migration，真实`3211615441`两项Blur闭合FBO correctness。[E-V1-XRAY-SHARED-STAGE-ACTIVATION-OWNER](runtime-evidence-index.md#e-v1-xray-shared-stage-activation-owner) [E-V1-FBO-STAGE-ACTIVATION](runtime-evidence-index.md#e-v1-fbo-stage-activation) | startup false、Timeline/SceneScript、mixed owner、utility/child/dependency、多effect、persistent/unique/history、clear/condition/function/compose及任意FBO未开放。FBO批未改变Standard Blur owner/route；真实`3747492842`当前仍断在VHS/Chromatic script attachment |
| host-shared / surface-local scope | `L3` | property输入由host捕获，每个surface有独立transaction/snapshot/generation；QuickJS domain与binding owner为scene scope，当前scalar evaluation在surface循环前推进一次并向各surface广播同帧值；placeholder-fade与bounded launch-origin mutable state同为scene/host scope。launch-origin在scene activate时按编译计划重置、teardown/新scene清空，普通surface rebuild不重启动画；[E-LIVE-PROPERTY](runtime-evidence-index.md#e-live-property)、[E-MEDIA-PLAYBACK-PLACEHOLDER-FADE](runtime-evidence-index.md#e-media-playback-placeholder-fade)、[E-BOUNDED-LAUNCH-ORIGIN-TRANSITION](runtime-evidence-index.md#e-bounded-launch-origin-transition) | pointer/matrix/provider与真实多surface输入/snapshot/consumer/event一致性继续补双屏隔离门；launch-origin不是script instance或mutable shared object，不要求每surface另建VM |
| target invalidation domain | `L3` | alpha/color/effect scalar为value-only；已准入pair leaf或上述安全nonpersistent FBO graph的exact effect visibility只做frame-local activation；direct text为per-layer texture generation；mixed/hidden/no-consumer/SceneScript/unsupported key标记rebuild | startup-disabled visibility/topology、通用provider和simulation target继续登记 |
| per-surface evaluation transaction | `L3` | property、Timeline与bounded text/time-of-day/fade/launch-origin producer的validation、同帧合并及atomic commit已闭环；QuickJS scalar从authored/user/Timeline preliminary snapshot取值，成功后以同帧`.sceneScript`值进入各surface最终transaction，失败保留owner-entry current-frame值。fade与launch-origin runtime同样先在host推进一次再广播。bounded native producer冲突在launch admission整项拒绝；[E-LIVE-PROPERTY](runtime-evidence-index.md#e-live-property)、[E-TIMELINE](runtime-evidence-index.md#e-timeline)、[E-MEDIA-PLAYBACK-PLACEHOLDER-FADE](runtime-evidence-index.md#e-media-playback-placeholder-fade)、[E-BOUNDED-LAUNCH-ORIGIN-TRANSITION](runtime-evidence-index.md#e-bounded-launch-origin-transition) | generic VM event/mutation、同priority多脚本执行、typed conflict aggregation与live media ingress尚未接入 |
...
| live consumer | `L3` | layer alpha、solid-only color、visible direct text content/point-size/color、bounded Timeline text width、shared-Program Local Contrast/strict Opacity、initially admitted pair-leaf及上述安全FBO exact effect visibility、bounded time-of-day Blend multiply、bounded launch-origin layer origin、23处Timeline effect constant typed binding、5处ordinary relative layer transform、bounded 2D camera Combined projection，以及absolute Timeline particle CP position/root emitter读取per-surface snapshot；controlled X-Ray和真实Standard Blur visibility都在不换surface/window的live update后进入typed activation-passthrough。[E-LIVE-PROPERTY](runtime-evidence-index.md#e-live-property)、[E-TIMELINE](runtime-evidence-index.md#e-timeline)、[E-V1-XRAY-SHARED-STAGE-ACTIVATION-OWNER](runtime-evidence-index.md#e-v1-xray-shared-stage-activation-owner)、[E-V1-FBO-STAGE-ACTIVATION](runtime-evidence-index.md#e-v1-fbo-stage-activation) | typed binding不等于对应renderer/effect完整执行；startup-disabled visibility、utility/mixed/no-consumer、generic SceneScript与未准入particle/text/camera target仍重建或fail closed |
...
| effect visibility family | 129（最近一次authored snapshot） | target path与稳定layer/effect identity可识别；direct bool现在由通用binding compiler映射为typed target，不再依赖X-Ray或effect path。这个数字未因受控fixture增加，也不证明存在执行consumer | `L2 structural` | 条件binding、duplicate/mixed producer、startup topology与各effect consumer仍逐项准入 |
| effect visibility actionable subset | 22 authored rebuild口径 + 2 authored FBO live targets + 1 controlled pair-leaf live target | 20条旧白名单与2条Local Contrast继续按既有重建口径生效；controlled `9000000131:69#effect#131`在already-admitted pair leaf内消费exact bool。真实`3211615441:24#effect#613`与`26#effect#257`则在已准入安全FBO graph内随同一`blur` key由4/4 Program切到4/0/4 typed previous-current，surface/window不变且GPU/publication/后缀compositor继续。[E-PROPERTY](runtime-evidence-index.md#e-property) [E-V1-XRAY-SHARED-STAGE-ACTIVATION-OWNER](runtime-evidence-index.md#e-v1-xray-shared-stage-activation-owner) [E-V1-FBO-STAGE-ACTIVATION](runtime-evidence-index.md#e-v1-fbo-stage-activation) | `L3 bounded` | controlled target不计corpus；这里的2项authored live只是已验证consumer，不刷新129项全census。startup=false、Timeline/SceneScript、persistent/unique、dependency、condition/function/compose与完整authored consumer census仍缺 |
...
| shader constant exact direct user-property consumer subset | 1 authored active + 1 controlled scalar target | 真实`1937925563:13#effect#321`把`triangle_neon_color` vector3按exact target/fallback/`float3` ABI与`0...1`颜色域写入`tinthigh`；其资源树受控重包把同一stage的`noiseamount`绑定到scalar property，范围内live、越界snapshot-authored fallback与缺失producer三路都进入共享MaterialProgram并完成GPU/compositor/next-frame。scalar只在launch producer key/target/type精确一致时撤旧owner；exact producer不存在且旧plan仍可接纳时不撤权。profile-local rollback已证明50 generic / 1 fallback / 0 dedicated及后缀存活；property kind错配则只局部拒绝目标effect。[E-V1-PULSE-FRAGMENT-SCALAR-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-fragment-scalar-user-property-owner) [E-V1-PULSE-TYPED-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-typed-user-property-owner) | `L3 bounded / vector3 + producer-present scalar S4 owner-migration` | authored active计数仍为1；受控scalar target不计入corpus。跨stage/关系域/其他float2、alpha/audio+binding、其他property/type/effect、广播、Timeline/SceneScript、多contributor、整个Pulse family或视觉parity不外推 |
