...
| target invalidation domain | `L3` | alpha/color/effect scalar为value-only；已准入pair leaf、上述安全nonpersistent FBO graph或Program-backed startup-false安全pair leaf的exact effect visibility只做frame-local activation；direct text为per-layer texture generation；mixed/hidden/no-consumer/SceneScript/unsupported key标记rebuild | startup-false FBO/不安全topology与无Program consumer、通用provider和simulation target继续登记 |
...
| live consumer | `L3` | layer alpha、solid-only color、visible direct text content/point-size/color、bounded Timeline text width、shared-Program Local Contrast/strict Opacity、initially admitted pair-leaf、上述安全FBO与Program-backed startup-false安全pair-leaf exact effect visibility、bounded time-of-day Blend multiply、bounded launch-origin layer origin、23处Timeline effect constant typed binding、5处ordinary relative layer transform、bounded 2D camera Combined projection，以及absolute Timeline particle CP position/root emitter读取per-surface snapshot；controlled X-Ray、真实Standard Blur与真实`3767460992:17#effect#725` Film Grain都在不换surface/window的live update后进入typed activation/Program链。[E-LIVE-PROPERTY](runtime-evidence-index.md#e-live-property)、[E-TIMELINE](runtime-evidence-index.md#e-timeline)、[E-V1-XRAY-SHARED-STAGE-ACTIVATION-OWNER](runtime-evidence-index.md#e-v1-xray-shared-stage-activation-owner)、[E-V1-FBO-STAGE-ACTIVATION](runtime-evidence-index.md#e-v1-fbo-stage-activation)、[E-V1-STARTUP-FALSE-EFFECT-VISIBILITY](runtime-evidence-index.md#e-v1-startup-false-effect-visibility) | typed binding不等于对应renderer/effect完整执行；startup-false FBO/不安全topology、无Program/utility/mixed/no-consumer、generic SceneScript与未准入particle/text/camera target仍重建或fail closed |
...
| effect visibility family | 129（最近一次authored snapshot） | target path与稳定layer/effect identity可识别；direct bool现在由通用binding compiler映射为typed target，不再依赖X-Ray或effect path。这个数字未因受控fixture或本批运行增加，也不证明129项存在执行consumer | `L2 structural` | 条件binding、duplicate/mixed producer、startup FBO/复杂topology与各effect consumer仍逐项准入 |
| effect visibility actionable subset | 22 authored rebuild口径 + 2 authored FBO live targets + 1 controlled pair-leaf live target + 5 authored startup-false safe admissions（2 Program-backed / 3 inactive-only） | 20条旧白名单与2条Local Contrast继续按既有重建口径生效；controlled `9000000131:69#effect#131`在already-admitted pair leaf内消费exact bool。真实`3211615441:24#effect#613`与`26#effect#257`在已准入安全FBO graph内live切换；真实`3767460992`的5个startup-false安全leaf进入共享graph，其中Film Grain `17#effect#725`已由`sparklyonoff=true`在同一surface/window激活普通Program，另3项无Program只持有graph passthrough member且不形成CPU exact owner。[E-PROPERTY](runtime-evidence-index.md#e-property) [E-V1-XRAY-SHARED-STAGE-ACTIVATION-OWNER](runtime-evidence-index.md#e-v1-xray-shared-stage-activation-owner) [E-V1-FBO-STAGE-ACTIVATION](runtime-evidence-index.md#e-v1-fbo-stage-activation) [E-V1-STARTUP-FALSE-EFFECT-VISIBILITY](runtime-evidence-index.md#e-v1-startup-false-effect-visibility) | `L3 bounded` | controlled target不计corpus；5项startup admission不等于5项live consumer，且不刷新129项全census。startup-false FBO/target/dependency/command、Timeline/SceneScript、persistent/unique、condition/function/compose与完整authored consumer census仍缺 |
