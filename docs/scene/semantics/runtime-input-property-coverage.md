...
| 固定 source priority | `L3` | authored -> property -> Timeline -> SceneScript；property/Timeline与QuickJS scalar/String/Vec3及pass scalar/Vec2/Vec3同target时先形成current-frame preliminary value，VM成功结果再覆盖。pass scalar保留scalar，Vec2/Vec3分别发布typed pair/triple | 重复、outer provider冲突或仍被bounded producer占用的candidate不形成owner并保留现有值；bool/Vec4/matrix、通用官方Vec2 class与typed conflict aggregation尚缺 |
...
| per-surface evaluation transaction | `L3` | property、Timeline与bounded text/generic QuickJS scalar/String/Vec3/pass Vec2 producer的validation、同帧合并及atomic commit已闭环；QuickJS从preliminary snapshot取值，成功后以同帧`.sceneScript`值进入各surface最终transaction，失败保留owner-entry current-frame值。cursor/media/audio event与同帧update共用既有transaction；[E-V4-SCENESCRIPT-PASS-VEC2-AUDIO](runtime-evidence-index.md#e-v4-scenescript-pass-vec2-audio) | 同priority多脚本执行、typed conflict aggregation、multi-surface input一致性与live media ingress尚未接入 |
...
| live consumer | `L3` | 既有property/Timeline/text/activation/particle consumer继续不变；真实`2067939514`的25个pass Vec2 scale owner中7个active Transform从`script-attachment-unproven`局部fallback进入普通Program，2个direct property scalar→`float2` Precise Blur consumer在默认/隔离live值下进入Program；同一真实包的4个64-band Simple Audio Bars又以typed `barcount1→u_BarCount`、同代audio-stage uniform和统一Program完成GPU/publication/suffix compositor/next-frame。三路均不建立第二份property/audio state。[E-V4-SCENESCRIPT-PASS-VEC2-AUDIO](runtime-evidence-index.md#e-v4-scenescript-pass-vec2-audio) [E-V4-USER-PROPERTY-FLOAT2-BLUR](runtime-evidence-index.md#e-v4-user-property-float2-blur) [E-V4-SIMPLE-AUDIO-BARS-SCALAR-DIALECT](runtime-evidence-index.md#e-v4-simple-audio-bars-scalar-dialect) | typed binding不等于完整effect或样本视觉parity；其余pass vector shape、provider冲突、无Program/utility/mixed/no-consumer、bool/Vec4/matrix仍fail closed |
