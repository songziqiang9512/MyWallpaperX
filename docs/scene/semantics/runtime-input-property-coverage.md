...
| generic media thumbnail identity | `L3 bounded current + previous effect consumers` | `$mediaThumbnail`与`$mediaPreviousThumbnail`以exact `(name,purpose)`进入同一frame registry；同一encoded generation独立尝试premultiplied color与preserved-channels，新的不同current成功后才把真实上一成功source旋转为同代previous。首次current没有伪previous，replacement pending保留完整last-ready，ready后原子换代；同generation的presence+五色继续进入generic QuickJS event。见 [E-V4-MEDIA-PREVIOUS-PROVIDER-LIFECYCLE](runtime-evidence-index.md#e-v4-media-previous-provider-lifecycle) | live platform producer、base-image current/previous、完整texture transition/variant、status/timeline与跨事件完整lifecycle |
...
140样本 `authored-corpus-observation` 的compact occurrence index现为60个system usertexture consumer / 17样本，其中33个非隐藏 / 14样本：effect current 25 / 11（非隐藏13 / 8）、effect previous 20 / 6（8 / 3）、base-image current 14 / 8（11 / 8）、base-image previous 1 / 1（1 / 1）。这些数字只说明作者请求；当前运行分别闭合了effect current与effect previous各一个真实消费者，base-image/root失败半径不同，仍不合并宣称支持。

| 官方能力 | 等级 | 当前事实 | 最小实现门 |
|---|---|---|---|
| `$mediaThumbnail` generic reference | `L3 bounded current effect consumer` | Template→MaterialProgram→GraphExecutor按exact purpose消费同一typed current publication；`.premultipliedColor`与`.preservedChannels`不是别名。真实`3610154602:414#effect#415`首帧pending previous-current、下一帧普通Program、再下一帧同Program闭合GPU/publication/compositor；见 [E-V4-SYSTEM-PROVIDER-PRESERVED-CHANNELS](runtime-evidence-index.md#e-v4-system-provider-preserved-channels) | base-image current、variant、live platform producer与其他purpose consumer |
...
| `$mediaPreviousThumbnail` identity/provider | `L3 bounded effect consumer / S3 executed` | 共享store只在新的非空encoded image与上一成功source不同且当前代成功decode后旋转：新current与真实上一成功previous的color/preserved representations在同一ready generation原子发布；首次current没有伪previous，pending保留完整last-ready，invalid/clear对外unavailable但invalid replacement不破坏私有last-success，stale/cancel不能旋转。reserved request name与candidate lifecycle identity必须exact对应，current不能冒充previous。真实`3396722575:3588#effect#3590`由unavailable→pending previous-current恢复为普通Program及next-frame；见 [E-V4-MEDIA-PREVIOUS-PROVIDER-LIFECYCLE](runtime-evidence-index.md#e-v4-media-previous-provider-lifecycle) | base-image previous、live producer切歌/clear、multi-surface压力、完整transition视觉与官方timing/pixel golden |
| authored placeholder / previous-current fallback | `L3 bounded effect` | 首次exact representation pending/unavailable时，只有top system slot、lower asset、无graph binding且完整variant envelope的effect可复制进入effect前的current；其他effect和唯一compositor继续。已有ready的replacement pending继续发布完整last-ready current/previous；decode/clear完成后缺失purpose才变unavailable。没有恢复旧source-replacement renderer，也不伪造texture或Program成功 | base-image fallback、其他Blend topology与平台clear/switch语义 |
...
| system/media provider identity | `L3 bounded current + previous effect consumers` | Template、catalog demand、registry、publication、Program与runtime bridge统一使用exact `(name,purpose)`；producer snapshot携带pending generation及store-owned、正在尝试派生的exact identities，不能把其他system name/purpose误标pending。reserved current/previous request name与candidate lifecycle identity必须exact对应；wrong request identity/generation/content/incomplete publication/lifecycle继续硬拒绝。ready purpose/sampling mismatch只在严格结构证明下局部previous-current，否则拒绝最小不安全单元；current门见 [E-V4-SYSTEM-PROVIDER-PRESERVED-CHANNELS](runtime-evidence-index.md#e-v4-system-provider-preserved-channels)，previous门见 [E-V4-MEDIA-PREVIOUS-PROVIDER-LIFECYCLE](runtime-evidence-index.md#e-v4-media-previous-provider-lifecycle) | live producer、base-image/root ingress、pause/rebuild/stop与teardown |
