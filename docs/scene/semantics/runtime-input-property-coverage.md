...
> 最新 historical Pulse combined RGB+alpha 更正：同一个typed property producer与static author value可在shared RGB-blend/scalar-alpha source proof成立时跨historical CAST3与`saturate(rgba)`的mask/no-mask形态复用。static CAST3/literal-zero/saturate候选均撤权，producer-present CAST3/saturate候选也撤权并由产品Program执行；producer-present literal-zero产品虽然Program-first，compiler candidate仍因动态zero-vector rewrite边界保留。saturate只改变terminal color-transfer为`straightAlphaUNorm`，没有新增property state、clock或私有值副本；mixed contributor、SceneScript/Timeline、`bounds`与其他关系域保持原边界。见 [E-V1-PULSE-SATURATE-RGBA-OWNER](runtime-evidence-index.md#e-v1-pulse-saturate-rgba-owner)。
