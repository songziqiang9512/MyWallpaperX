...
>
> `311115e3` 新增的native click/launch-origin、hover、property→Vec3与audio-scaled value均已由同一QuickJS-NG domain中的generic cursor/vector/AudioBuffers owner取代并删除旧产品族；shared alpha、identity display与media colors仍只有`S2 wired / visible unknown`。真实`2974757317`闭合generic hover与single-surface ordered click，真实`3122339805`闭合inside-hit `cursorMove`拖动，真实`2067939514`让25个pass-owned Vec2 scale从同一audio snapshot进入QuickJS、dynamic snapshot、普通MaterialProgram与GraphExecutor；真实`2884628849`的隔离author-property门让4个pass-owned Vec3颜色值经同一VM与bounded `WEColor.hsv2rgb`发布；真实`2938612768`又让带四项static `scriptProperties`的pass scalar strength经AudioBuffers进入现有Water Waves Program。边界见[E-V4-SCENESCRIPT-PASS-VEC2-AUDIO](runtime-evidence-index.md#e-v4-scenescript-pass-vec2-audio)、[E-V4-SCENESCRIPT-PASS-VEC3-WECOLOR](runtime-evidence-index.md#e-v4-scenescript-pass-vec3-wecolor)与[E-V4-SCENESCRIPT-PASS-SCALAR-PROPERTIES](runtime-evidence-index.md#e-v4-scenescript-pass-scalar-properties)。
