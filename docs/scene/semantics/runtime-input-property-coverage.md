...
| generic SceneScript/media text | scalar/String VM与thumbnail presence/playback/properties event `L3 bounded` | QuickJS-NG已执行pass scalar、object-owned String、`WEMath`、immutable engine frame input与三个typed media event；旧placeholder fade已由generic playback event取代。thumbnail colors仍没有generic DTO，只由exact bounded partition暂时消费。没有live macOS producer、完整media handle/event queue或file module graph | 扩展typed producer时保持syntax/value/budget/lifecycle fail-closed；不得从三个bounded event或native color partition外推完整media API |
...
| media playback/colors/title/artist snapshot | playback/properties `L3 bounded`；colors `L3 bounded executed` | producer-agnostic inbox保存playback、artwork colors、title/artist及独立generation；generic playback、bounded color/text进入同一frame transaction。color candidates先于QuickJS Vec owner分区，material admission后仅consumer交集获得native owner，unclaimed无owner；真实`2974757317`的4个color consumer已完成blue/missing-palette正反执行 | generic color DTO与event ordering、live producer、missing-field/clear/stale、multi-surface、独立ROI；见 [E-V4-MEDIA-COLOR-OWNER-PARTITION](runtime-evidence-index.md#e-v4-media-color-owner-partition) |
...
| media events | thumbnail presence/playback/properties API `L3 bounded`；thumbnail colors/status/timeline API `L0`；live ingress `L0` | QuickJS按typed generation派发三个event：playback/properties在`update`前，thumbnail presence当前在成功`update`后；owner异常局部隔离并保留peer/previous-current。colors由同一inbox进入临时bounded native partition，不冒充generic event；当前producer仍是可注入inbox | live producer、跨类型ordered queue、color DTO与Vec3 coercion、status/timeline、切换仲裁与multi-surface生命周期 |
...
| 官方能力 | 等级 | 当前事实 | 最小实现门 |
|---|---|---|---|
| playback/colors/title/artist snapshot | playback/properties `L3 bounded`；colors `L3 bounded executed`；generic colors API `L0` | inbox与frame driver已有typed generation；真实`2974757317`的4个exact color consumer已完成blue与missing-palette正反执行，但仍由`disable-generic` native partition持权 | generic color DTO/event ordering、live producer、同host-frame原子更新、clear/stale与multi-surface门；见 [E-V4-MEDIA-COLOR-OWNER-PARTITION](runtime-evidence-index.md#e-v4-media-color-owner-partition) |
...
| thumbnail derived colors | `L3 bounded executed` | validated candidate集合`M`在generic Vec owner创建前排除；实际consumer集合`C`进入bounded native `disable-generic` route，`M-C`显式unclaimed且无第二state/renderer。真实`2974757317`的primary/secondary蓝色与missing-palette authored fallback均闭合GPU/compositor/next-frame；不等于generic `MediaThumbnailEvent` | tertiary/text/highContrast等公开色值、generic DTO/Vec3 coercion、live producer、clear/stale/multi-surface与独立ROI；见 [E-V4-MEDIA-COLOR-OWNER-PARTITION](runtime-evidence-index.md#e-v4-media-color-owner-partition) |
| authored color transition | `L3 bounded executed / disable-generic debt` | exact bounded native compiler/runtime按simulation delta更新，缺palette或stopped回到authored topColor；不是通用作者SceneScript或稳态owner migration | generic event DTO、Vec3 coercion、event/update ordering与output oracle、fresh独立ROI；闭合后切`generic-only`并删除native family |
