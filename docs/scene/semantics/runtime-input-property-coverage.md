...
| Scene Sound layer / self-playback | `L3 bounded / S3 real consumers` | Sound IR 保留路径数量、loop/startsilent、volume property及spatial/time字段；单一 FLAC/MP3/WAV、`loop`、`startsilent=false`、非spatial、finite volume由唯一launch-scoped registry播放，复用Scene pause/resume/stop与现有property snapshot。真实FLAC时钟前进，真实MP3 `music 0.5→0.2`不重建surface；双音源以typed reason只拒绝Sound layer。[E-V4-SCENE-SOUND-PLAYBACK](runtime-evidence-index.md#e-v4-scene-sound-playback) | 多音源选择、startsilent/非loop、spatial与min/max时序保持unsupported；decode失败尚只有日志而无结构化runtime evidence。当前system tap排除本进程，wallpaper-local声音仍不会进入共享Scene频谱 |
