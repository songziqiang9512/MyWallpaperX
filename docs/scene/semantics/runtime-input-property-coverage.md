...
| 固定 source priority | `L3` | authored -> property -> Timeline -> SceneScript；property/Timeline与QuickJS scalar同target时先形成current-frame preliminary value，VM成功结果再覆盖。property、受限Timeline、bounded text Date、generic VM time-of-day scalar、media placeholder fade与launch-origin startup producer已执行，后四者复用`.sceneScript` source | 不消费preliminary value的bounded native producer彼此及其与property/Timeline冲突时launch失败关闭；重复或被bounded producer占用的QuickJS scalar candidate不形成owner并保留现有值，但typed conflict aggregation尚缺。非scalar SceneScript与live event mutation仍未接入 |
...
| pointer buttons/down/up/click | `L2 bounded wiring；generic L0` | AppKit down/up更新per-surface primary-button state；Program host schema把primary状态编码到bounded `g_PointerState.z`，其余分量保持0；admitted launch-origin master仍在button edge使用实际camera/world-frame/quad UV hit-test并toggle cohort。fresh cursor门未按下 | `g_PointerState`不是已登记的`official-public-contract`；完整分量/多button、generic有序event queue、capture/drag、VM dispatch与fresh click可见门 |
| generic cursor enter/leave | `L3 bounded / S4 visible` | AppKit move/enter/exit经实际camera/world/model inverse与quad hit-test向QuickJS object owner派发immutable world/local Vec3 event；旧native hover owner已删除。真实`2974757317`exact 1 enter + 1 leave并产生可见卡片位移 | parent/rotated/puppet、跨owner顺序、边界抖动、move/down/up/click/capture与官方坐标/事件对照；见 [E-V2-SCENESCRIPT-CURSOR-ENTER-LEAVE](runtime-evidence-index.md#e-v2-scenescript-cursor-enter-leave) |
