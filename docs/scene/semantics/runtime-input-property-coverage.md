...
| generic SceneScript/media text | scalar VM `L3 bounded`; media/text API `L0` | QuickJS-NG 已执行 pass scalar、`WEMath` 与 immutable `engine` time/frame/runtime；bounded text update与placeholder fade仍是独立 Swift producer。没有通用 text/media handle、event snapshot/provider 或 file module graph | 扩展 typed producer 时保持 syntax/value/budget/lifecycle fail-closed；不得用 scalar VM 外推 media/text API |
