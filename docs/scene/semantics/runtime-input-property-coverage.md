...
| component/property/axis target | `L3` | lane 以 `c0/c1/c2` 对应 component 下标，必须从 `c0` 起连续否则 fail-closed；已 typed 编译为 `.layer(.alpha/.origin/.angles/.scale)`、`.effectConstant`、`.text(.maxWidth)` 与 particle `.controlPoint`/`.controlPointAngles`，lane 数或 authored vector 非法即整条拒绝。layer transform 仅在 `relative=true` 时准入 additive composition；6 条普通图层由共享 world-frame 消费，4 条 `lspot` 由既有 strict plan 消费；text width 仅准 `limitwidth=true`、absolute 单 lane、有限 `1...16384` 像素 | particle 仅 absolute position 有 root-emitter consumer；唯一 `zoom` 属 Combined camera `origin↔zoom`，无组 clock/consumer；absolute layer transform 与非 layer-transform `relative` 没有经合法语料定标，继续拒绝 |
...
5. Timeline 的 IR、绝对 scene-time evaluator 和 Workshop **47/48** typed target 子集已接入：effect constant 23、layer alpha 5、bounded relative layer transform 10、root particle scalar override 7、bounded text `maxwidth` 2。普通 transform 的 6 条走共享 hierarchy-aware world-frame，4 条 `lspot` 保持既有 strict consumer；不能外推为通用 lighting。另有 stock-wire/project-fixture 证明 particle CP position/angles typed target，其中仅 absolute position 驱动 root emitter。唯一剩余 Workshop `zoom` 与 relative camera `origin` 组成 Combined Animation，必须按组 clock/consumer 实现；particle relative/其他 field、Combined、tangent、其余 target 与 event crossing 继续 fail closed。SceneScript 文档级 inline binding 已对正式取证五类 owner/完整 target path/authored fallback/JSON value type 达到局部 `L1`，file/module、schema type、handle 与 VM 仍须在沙箱成立后接入同一 target 层。
