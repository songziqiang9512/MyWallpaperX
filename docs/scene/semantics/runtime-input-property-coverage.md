...
>
> `311115e3` 新增的native click/launch-origin、hover、property→Vec3与audio-scaled value均已由同一QuickJS-NG domain中的generic cursor/Vec3/AudioBuffers owner取代并删除旧产品族；shared alpha、identity display与media colors仍只有`S2 wired / visible unknown`。真实`2974757317`闭合generic hover与single-surface ordered click；真实`3122339805`又闭合inside-hit `cursorMove`驱动authored `thisLayer.origin`的generic-only drag切片。边界见[E-V4-SCENESCRIPT-ORDERED-POINTER-EDGES](runtime-evidence-index.md#e-v4-scenescript-ordered-pointer-edges)和[E-V4-SCENESCRIPT-CURSOR-MOVE-DRAG](runtime-evidence-index.md#e-v4-scenescript-cursor-move-drag)。
