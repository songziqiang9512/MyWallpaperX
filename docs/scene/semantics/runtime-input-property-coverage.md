...
| initially admitted effect visibility / pointer / identity scalar activation | `L3 bounded / S4 owner-migration` | binding compiler按exact layer/effect identity把direct bool映射为typed target，不再按effect path准入。只有单effect/单material pair leaf、`dependencyOwnership == .none`、无target/history/clear/condition/compose，且visibility为sole bool producer；pointer又须全部variant实际消费spatial-weighted pointer host uniform，`size`须为exact static或sole scalar producer时，GraphExecutor才附加frame-local policy。false/outside/按Program范围回退后仍`size<0.001`发布`activation-passthrough` previous-current并保留generation/publication/compositor/next-frame；越界user scalar恢复authored lane。controlled `9000000131`的pointer、visible→hidden、size-zero和shared rollback均1/1 PASS；size-zero的2个terminal transaction是explicit activation-only证据，普通`execution_succeeded=false`。[E-V1-XRAY-SHARED-STAGE-ACTIVATION-OWNER](runtime-evidence-index.md#e-v1-xray-shared-stage-activation-owner) | startup override为false时当前graph不会先准入，live update会按现役合同拒绝并触发重建；generic visibility topology、Timeline/SceneScript、mixed owner、utility/child/dependency、多effect/FBO/history和其他pointer consumer未开放。真实`3747492842:434#effect#537`先断在composition utility group，不是本门正证 |
...
| target invalidation domain | `L3` | alpha/color/effect scalar为value-only；已准入pair leaf的exact effect visibility也只做frame-local activation；direct text为per-layer texture generation；mixed/hidden/no-consumer/SceneScript/unsupported key标记rebuild | startup-disabled visibility/topology、通用provider和simulation target继续登记 |
...
| live consumer | `L3` | layer alpha、solid-only color、visible direct text content/point-size/color、bounded Timeline text width、shared-Program Local Contrast/strict Opacity、initially admitted pair-leaf exact effect visibility、bounded time-of-day Blend multiply、bounded launch-origin layer origin、23处Timeline effect constant typed binding、5处ordinary relative layer transform、bounded 2D camera Combined projection，以及absolute Timeline particle CP position/root emitter读取per-surface snapshot；controlled X-Ray visibility在不换surface/window的live update后进入typed activation-passthrough。[E-LIVE-PROPERTY](runtime-evidence-index.md#e-live-property)、[E-TIMELINE](runtime-evidence-index.md#e-timeline)、[E-V1-XRAY-SHARED-STAGE-ACTIVATION-OWNER](runtime-evidence-index.md#e-v1-xray-shared-stage-activation-owner) | typed binding不等于对应renderer/effect完整执行；startup-disabled visibility、utility/mixed/no-consumer、generic SceneScript与未准入particle/text/camera target仍重建或fail closed |
...
| effect visibility actionable subset | 22 authored rebuild口径 + 1 controlled live target | 20条旧白名单与2条Local Contrast继续按既有重建口径生效；controlled `9000000131:69#effect#131`在already-admitted pair leaf内由shared activation消费exact bool，visible→hidden不换surface/window并保留GPU/publication/compositor/next-frame。[E-PROPERTY](runtime-evidence-index.md#e-property) [E-V1-XRAY-SHARED-STAGE-ACTIVATION-OWNER](runtime-evidence-index.md#e-v1-xray-shared-stage-activation-owner) | `L3 bounded` | controlled target不计corpus；startup=false、真实`3747492842` utility group、generic topology/conditions、Timeline/SceneScript与完整authored consumer census仍缺 |
