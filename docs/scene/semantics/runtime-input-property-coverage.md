...
>
> `311115e3` 新增的native click/launch-origin、hover、property→Vec3、audio-scaled value与media colors均已由同一QuickJS-NG domain中的generic cursor/vector/AudioBuffers/`MediaThumbnailEvent` owner取代并删除旧产品族；shared alpha与identity display仍只有`S2 wired / visible unknown`。thumbnail event的presence与primary/secondary/tertiary/text/high-contrast五色现为generic `L3 bounded / S3 four real consumers executed`：metadata-only G阶段形成8个pass candidates，MaterialProgram先得到C=4，fresh candidate domain只构造C；失败candidate整域丢弃后重建，G−C不执行。默认`generic-only`；`disable-generic`丢弃发现域后fresh构造不含四个media owner的正式产品域，使active owner、event、update、mutation与owner output归零，同时把exact authored definitions留在统一resolver中作为lower-priority current，四个consumer继续完成exact/GPU/compositor/next-frame，fresh process可恢复。缺字段规范为零，callback/update失败同样不发布SceneScript高优先级值。真实`2974757317`闭合generic hover、single-surface ordered click与四个media-color owner，真实`3122339805`闭合inside-hit `cursorMove`拖动，真实`2067939514`让25个pass-owned Vec2 scale从同一audio snapshot进入QuickJS、dynamic snapshot、普通MaterialProgram与GraphExecutor，并让2个direct property scalar按exact isotropic projection进入Precise Blur `float2` Program consumer；真实`2884628849`的隔离author-property门让4个pass-owned Vec3颜色值经同一VM与bounded `WEColor.hsv2rgb`发布；真实`2938612768`又让带四项static `scriptProperties`的pass scalar strength经AudioBuffers进入现有Water Waves Program；真实`2902406982`的两个user-bound Water Waves则让2条bool activation和8条scalar uniform从同一property snapshot按generation进入普通Program。边界见[E-V4-SCENESCRIPT-PASS-VEC2-AUDIO](runtime-evidence-index.md#e-v4-scenescript-pass-vec2-audio)、[E-V4-USER-PROPERTY-FLOAT2-BLUR](runtime-evidence-index.md#e-v4-user-property-float2-blur)、[E-V4-SCENESCRIPT-PASS-VEC3-WECOLOR](runtime-evidence-index.md#e-v4-scenescript-pass-vec3-wecolor)、[E-V4-SCENESCRIPT-PASS-SCALAR-PROPERTIES](runtime-evidence-index.md#e-v4-scenescript-pass-scalar-properties)、[E-V4-USER-PROPERTY-WATER-WAVES](runtime-evidence-index.md#e-v4-user-property-water-waves)与[E-V4-MEDIA-COLOR-OWNER-PARTITION](runtime-evidence-index.md#e-v4-media-color-owner-partition)。
