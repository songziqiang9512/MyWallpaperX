...
| pointer ordered events / inside-hit drag | generic `L3 bounded single-surface / S4 click-visible + isolated drag-visible` | AppKit local/global primary edge和pointer位置按顺序写入surface有界batch；同一current camera/world/model inverse逐sample hit-test，位置真实变化只向current hit owner派发`cursorMove`，down捕获owner，release向capture派发up，同一object仍命中再click。超出512事件时整批拒绝并重同步，不执行partial callback。真实`2974757317`以click折叠卡片；真实`3122339805`的原脚本以move更新layer 207 origin，13-layer派生隔离fixture中窗口位移后稳定。旧native launch owner已删除，route=`generic-only`。[E-V4-SCENESCRIPT-ORDERED-POINTER-EDGES](runtime-evidence-index.md#e-v4-scenescript-ordered-pointer-edges) [E-V4-SCENESCRIPT-CURSOR-MOVE-DRAG](runtime-evidence-index.md#e-v4-scenescript-cursor-move-drag) | `g_PointerState`不是已登记的`official-public-contract`；multi-surface仍保留逐帧snapshot。move不跟随capture出界；多button、同帧多move写同target、候选传播、parent/puppet/perspective、失焦/monitor权限与官方顺序/坐标对照仍缺 |
