...
| live consumer | `L3` | 既有property/Timeline/text/activation/particle consumer继续不变；真实`2067939514`的25个pass Vec2 scale owner中7个active Transform从`script-attachment-unproven`局部fallback进入普通Program，另有2个direct property scalar→`float2` Precise Blur consumer在默认/隔离live值下进入Program；两路均保持GPU/publication/compositor/next-frame。[E-V4-SCENESCRIPT-PASS-VEC2-AUDIO](runtime-evidence-index.md#e-v4-scenescript-pass-vec2-audio) [E-V4-USER-PROPERTY-FLOAT2-BLUR](runtime-evidence-index.md#e-v4-user-property-float2-blur) | typed binding不等于完整effect或样本视觉parity；其余pass vector shape、provider冲突、无Program/utility/mixed/no-consumer、bool/Vec4/matrix仍fail closed |
