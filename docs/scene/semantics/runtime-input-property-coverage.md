...
| direct user-property → exact fragment scalar/vector3 shader consumer | `L3 bounded; vector3 owner-migration / scalar slice-visible` | binding compiler保留exact `{user,value}`、typed authored definition与target；只有有效property default才发布launch-scoped live producer。当前Pulse vector3 tint已完成owner门；fragment-only `noisespeed/noiseamount/power` scalar已进入同一Program执行切片，要求sole `.userProperty` contributor、无script attachment、active non-array fragment ABI、旧plan binding全集、key/target/type与author domain等值。范围内live值进入Program；越界值从snapshot独立authored lane恢复作者值。producer缺失或无效时不伪造live instruction，exact authored target可由Program安全输出，但旧compiler对可接纳plan仍保留incumbent。真实`1937925563:13#effect#321`证明`tinthigh` authored consumer；同资源树受控重包`9000000962/0963/0964`证明`noiseamount` live/range/missing三路Program/GPU/compositor/next-frame；scalar-specific rollback尚未运行。[E-V1-PULSE-FRAGMENT-SCALAR-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-fragment-scalar-user-property-owner) [E-V1-PULSE-TYPED-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-typed-user-property-owner) | scalar当前不是owner-migration完成。不做广播或类型转换；跨stage `speed/phase/amount`、关系域`bounds`、其他float2、alpha或audio+binding、额外wrapper key、多contributor、Timeline/SceneScript、数组、target/fallback/ABI/domain漂移与未消费binding继续失败关闭或保留incumbent；受控scalar fixture不是authored corpus正证 |
...
| shader constant exact direct user-property consumer subset | 1 authored active + 1 controlled scalar target | 真实`1937925563:13#effect#321`把`triangle_neon_color` vector3按exact target/fallback/`float3` ABI与`0...1`颜色域写入`tinthigh`；其资源树受控重包把同一stage的`noiseamount`绑定到scalar property，范围内live、越界snapshot-authored fallback与缺失producer三路都进入共享MaterialProgram并完成GPU/compositor/next-frame。scalar producer只有key/target/type精确一致时进入producer-present撤权代码路径，缺失producer不伪造state或丢layer但仍保留可达incumbent；[E-V1-PULSE-FRAGMENT-SCALAR-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-fragment-scalar-user-property-owner) [E-V1-PULSE-TYPED-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-typed-user-property-owner) | `L3 bounded / vector3 S4 owner-migration / scalar S4 slice-visible` | authored active计数仍为1；受控scalar target只证明共享执行与安全，尚缺scalar-specific rollback/owner门。跨stage/关系域/其他float2、alpha/audio+binding、其他property/type/effect、广播、Timeline/SceneScript、多contributor、整个Pulse family或视觉parity不外推 |
