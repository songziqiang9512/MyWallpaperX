...
| live consumer | `L3` | layer alpha、solid-only color、visible direct text content/point-size/color、strict Local Contrast/Opacity、bounded time-of-day Blend multiply、23 处 Timeline effect constant 与 absolute Timeline particle CP position/root emitter 读取 per-surface snapshot；[E-LIVE-PROPERTY](runtime-evidence-index.md#e-live-property)、[E-TIMELINE](runtime-evidence-index.md#e-timeline)、[E-EFFECT-BLEND-TRANSFORM](runtime-evidence-index.md#e-effect-blend-transform) | particle angles/relative/其他 field/consumer、hidden/no-consumer text、container/non-solid color 与 generic SceneScript 仍重建或 fail closed |
...
| component/property/axis target | `L3` | lane 以 `c0/c1/c2` 对应 component 下标，必须从 `c0` 起连续否则 fail-closed；已 typed 编译为 `.layer(.alpha)`、`.effectConstant` 与 particle `.controlPoint`/`.controlPointAngles`，lane 数或 authored vector 非法即整条拒绝 | particle 仅 absolute position 有 root-emitter consumer；layer `origin`/`angles`/`scale` 因 `relative` 未执行，`maxwidth`/`zoom` 无 target |
...
5. Timeline 的 IR、绝对 scene-time evaluator 和 Workshop 28/48 typed target 子集已接入；另有 stock-wire/project-fixture 证明 particle CP position/angles typed target，其中仅 absolute position 驱动 root emitter。Workshop 7 条 particle Timeline 属其他 scalar override，现役 28/48 统计不变；`relative`、Combined、tangent、其余 target 与 event crossing 继续 fail closed。SceneScript 文档级 inline binding 已对正式取证五类 owner/完整 target path/authored fallback/JSON value type 达到局部 `L1`，file/module、schema type、handle 与 VM 仍须在沙箱成立后接入同一 target 层。
