...
| per-surface evaluation transaction | `L3` | property、Timeline 与 bounded text/time-of-day SceneScript producer 的 validation、同帧合并及 atomic commit 已闭环；[E-LIVE-PROPERTY](runtime-evidence-index.md#e-live-property)、[E-TIMELINE](runtime-evidence-index.md#e-timeline)、[E-EFFECT-BLEND-TRANSFORM](runtime-evidence-index.md#e-effect-blend-transform) | generic VM event/mutation、同 target 脚本冲突尚未接入 |
...
| live consumer | `L3` | layer alpha、solid-only color、visible direct text content/point-size/color、strict Local Contrast/Opacity、bounded time-of-day Blend multiply、23 处 Timeline effect constant、6 处 ordinary relative layer `origin`/`angles`/`scale` 的共享 hierarchy-aware world-frame，以及 absolute Timeline particle CP position/root emitter 读取 per-surface snapshot；4 处 relative `lspot` angles 继续由 strict spotlight consumer 执行；[E-LIVE-PROPERTY](runtime-evidence-index.md#e-live-property)、[E-TIMELINE](runtime-evidence-index.md#e-timeline)、[E-EFFECT-BLEND-TRANSFORM](runtime-evidence-index.md#e-effect-blend-transform) | particle angles/relative/其他 field/consumer、hidden/no-consumer text、container/non-solid color 与 generic SceneScript 仍重建或 fail closed |
...
| component/property/axis target | `L3` | lane 以 `c0/c1/c2` 对应 component 下标，必须从 `c0` 起连续否则 fail-closed；已 typed 编译为 `.layer(.alpha/.origin/.angles/.scale)`、`.effectConstant` 与 particle `.controlPoint`/`.controlPointAngles`，lane 数或 authored vector 非法即整条拒绝。layer transform 仅在 `relative=true` 时准入 additive composition；6 条普通图层由共享 world-frame 消费，4 条 `lspot` 由既有 strict plan 消费 | particle 仅 absolute position 有 root-emitter consumer；`maxwidth`/`zoom` 无 target；absolute layer transform 与非 layer-transform `relative` 没有经合法语料定标，继续拒绝 |
| `relative` composition | `L3 bounded` | 10 处 layer transform 均以“作者基值 + 动画偏移”形成 typed additive binding；`2134765860` angles、`2938612768` Mirror origin、`3028090166` Single scale 有隔离真实门，另四束 `lspot` 保持既有 strict consumer。公开官方页只定义 property animation/modes 与 layer transform，不定义私有 serialized `relative` wire 或合成公式 | 合同来自合法语料、既有 black-box 校准与项目自有正反门；非 layer `origin`/`angles`/`scale`、非有限基值或形态不完整均 fail closed，仍需 Windows golden |
...
| 32/64 stereo buffers | `L3 bounded` | 与 16 档由同一次 FFT 生成并进入 host-shared snapshot；供 exact Simple Audio Bars `32+CLIP_LOW` / `64+CLIP_HIGH` profile，以及两个 exact native property-script 64-band profile；[E-AUDIO-INPUT](runtime-evidence-index.md#e-audio-input) | 其他 Workshop shader 与通用 SceneScript `registerAudioBuffers` 尚无 consumer/bridge，不外推 |
...
| generic media thumbnail identity | `L3 bounded cover` | `$mediaThumbnail` 与 `$mediaPreviousThumbnail` 已分成独立 system identity；replacement pending 保留 last-ready current/generation，ready 后原子发布 current/previous；current 有严格 Blend consumer，previous 有一个结构/内容严格准入的 gradient transition consumer | live platform producer、generic Blend Gradient/variant、通用 media event 与 SceneScript lifecycle |
...
| current cover identity/provider | `L3 bounded` | producer-agnostic inbox 原子发布 encoded current；共享 store 以 requested generation 拒绝过期 completion，replacement 异步 ImageIO decode 未完成时继续发布 last-ready current/content generation，最长边限制为 256，ready 后再换代广播所有 surface；当前只有隔离 debug producer，见 [E-MEDIA-THUMBNAIL](runtime-evidence-index.md#e-media-thumbnail) | 主动取消旧 decode、macOS live producer、权限/播放器 lifecycle、结构化 decode status telemetry 与真实切歌门 |
...
|---|---|---|---|
| layer/named/property identity/status/generation | `L3` | 受限 provider 有 ready/pending/unavailable；静态 resource generation 与 named frame epoch 分离；direct text、embedded MP4 和 bounded current/previous cover 以显式 content generation publication 进入 frame registry，stale generation、同代换纹理和 publication/texture 不匹配均拒绝；media replacement pending 继续发布 last-ready generation，clear/failure 最终回到作者 fallback；[E-PROVIDER](runtime-evidence-index.md#e-provider) / [E-MEDIA-THUMBNAIL](runtime-evidence-index.md#e-media-thumbnail) | 其他 media transition/variant、通用 metadata、主动取消与 platform producer/consumer 生命周期 |
...
5. Timeline 的 IR、绝对 scene-time evaluator 和 Workshop 38/48 typed target 子集已接入：effect constant 23、layer alpha 5、bounded relative layer transform 10。后者只有 ordinary `origin`/`angles`/`scale` 的 6 条走共享 hierarchy-aware world-frame，4 条 `lspot` 保持既有 strict consumer；不能外推为通用 lighting。另有 stock-wire/project-fixture 证明 particle CP position/angles typed target，其中仅 absolute position 驱动 root emitter。Workshop 7 条 particle Timeline 属其他 scalar override；`maxwidth`/`zoom`、particle relative/其他 field、Combined、tangent、其余 target 与 event crossing 继续 fail closed。SceneScript 文档级 inline binding 已对正式取证五类 owner/完整 target path/authored fallback/JSON value type 达到局部 `L1`，file/module、schema type、handle 与 VM 仍须在沙箱成立后接入同一 target 层。
