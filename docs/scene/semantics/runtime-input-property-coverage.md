...
| typed target 族 | `L2` | scene/camera/layer/effect/text/particle/script instance 已定义；layer alpha/color、exact stock Local Contrast strength/Opacity alpha 与 particle CP position/angles 进入 compiler | particle 仅 absolute CP position 有 consumer；其他 effect target 不得直接开放 live，SceneScript 值必须与 direct binding 分开 |
...
| live consumer | `L3` | layer alpha、solid-only color、visible direct text content/point-size/color、strict Local Contrast/Opacity、23 处 Timeline effect constant 与 absolute Timeline particle CP position/root emitter 读取 per-surface snapshot；[E-LIVE-PROPERTY](runtime-evidence-index.md#e-live-property)、[E-TIMELINE](runtime-evidence-index.md#e-timeline) | particle angles/relative/其他 field/consumer、hidden/no-consumer text、container/non-solid color 与 SceneScript 仍重建或 fail closed |
