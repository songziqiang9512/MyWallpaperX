...
> 最新 historical Pulse combined RGB+alpha 更正：同一个typed property producer与static author value现在可在shared RGB-blend/scalar-alpha source proof成立时跨historical CAST3的mask/no-mask形态复用。static CAST3/literal-zero候选均撤权；producer-present CAST3候选撤权并由产品Program执行；producer-present literal-zero产品虽然Program-first，compiler candidate仍因动态zero-vector rewrite边界保留，不能把其运行结果表述为owner migration。没有新增property state、clock或私有值副本；saturate、mixed contributor、SceneScript/Timeline、`bounds`与其他关系域保持原边界。见 [E-V1-PULSE-HISTORICAL-RGB-ALPHA-OWNER](runtime-evidence-index.md#e-v1-pulse-historical-rgb-alpha-owner)。
