...
| direct user-property → exact scalar/vector3 shader consumer | `L3 bounded; vector3 + fragment scalar + same-domain cross-stage scalar owner-migration` | binding compiler保留exact `{user,value}`、由authored fallback决定shape的typed definition与target；只有有效property default才发布launch-scoped live producer。Pulse vector3 tint、fragment-only `noisespeed/noiseamount/power`与same-domain双stage `speed/amount`均要求sole `.userProperty` contributor、无script attachment、旧plan binding全集及key/target/type/domain等值。fragment-only项要求唯一active fragment non-array ABI；`speed/amount`要求同一material key的exact active set为`{vertex,fragment}`、每stage唯一non-array `float`且author range逐stage相等。范围内live值进入Program；越界值从snapshot authored lane恢复作者值；缺失producer只保留authored definition。exact producer不存在且旧plan仍可接纳时不撤旧owner。受控`9000000967`同时绑定`speed + amount + noiseamount`，default/restore均51 generic / 0 dedicated，profile-local rollback为50 generic / 1 fallback / 0 dedicated，target previous-current与后缀compositor/next-frame继续。[E-V1-PULSE-CROSS-STAGE-SCALAR-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-cross-stage-scalar-user-property-owner) [E-V1-PULSE-FRAGMENT-SCALAR-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-fragment-scalar-user-property-owner) [E-V1-PULSE-TYPED-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-typed-user-property-owner) | 不做广播或类型转换；跨stage异域`phase`、关系域`bounds`、其他float2、alpha或audio+binding、额外wrapper key、多contributor、Timeline/SceneScript、数组、target/fallback/ABI/domain漂移与未消费binding继续失败关闭或保留incumbent；受控scalar fixture只作`MyWallpaperX-current-evidence`，不是authored corpus正证 |
...
| shader constant exact direct user-property consumer subset | 1 authored active + 2 controlled scalar targets | 真实`1937925563:13#effect#321`把`triangle_neon_color` vector3按exact target/fallback/`float3` ABI与`0...1`颜色域写入`tinthigh`；其资源树受控重包先证明fragment-only `noiseamount`的live/range/missing，再以`9000000967`把`speed + amount + noiseamount`同时绑定到一个scalar property，证明same-domain `{vertex,fragment}` consumer set进入共享MaterialProgram并完成GPU/publication/compositor/next-frame。scalar只在launch producer key/target/type与每个expected stage的ABI/domain精确一致时撤旧owner；exact producer不存在且旧plan仍可接纳时不撤权。profile-local rollback证明50 generic / 1 fallback / 0 dedicated及后缀存活。[E-V1-PULSE-CROSS-STAGE-SCALAR-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-cross-stage-scalar-user-property-owner) [E-V1-PULSE-FRAGMENT-SCALAR-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-fragment-scalar-user-property-owner) [E-V1-PULSE-TYPED-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-typed-user-property-owner) | `L3 bounded / vector3 + producer-present scalar S4 owner-migration` | authored active计数仍为1；两个受控scalar target均不计入corpus。异域`phase`、关系域`bounds`、其他float2、alpha/audio+binding、其他property/type/effect、广播、Timeline/SceneScript、多contributor、整个Pulse family或视觉parity不外推 |
