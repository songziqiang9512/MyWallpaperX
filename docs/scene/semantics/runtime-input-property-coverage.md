...
>
> `311115e3` 新增的native click/launch-origin、hover、property→Vec3与audio-scaled value均已由同一QuickJS-NG domain中的generic cursor/Vec3/AudioBuffers owner取代并删除旧产品族；shared alpha、identity display与media colors仍只有`S2 wired / visible unknown`。真实`2974757317`现分别闭合generic hover，以及single-surface同一frame内press→release仍产生exact一次`cursorClick`并折叠卡片；边界见[E-V4-SCENESCRIPT-ORDERED-POINTER-EDGES](runtime-evidence-index.md#e-v4-scenescript-ordered-pointer-edges)。
...
| pointer buttons/down/up/click | generic `L3 bounded single-surface / S4 click-visible` | AppKit local/global primary event按顺序写入surface有界batch；同一current camera/world/model inverse逐事件hit-test，down捕获owner，release向capture派发up，同一object仍命中再派发click。超出512事件时整批拒绝并重同步，不执行partial edge。Program host schema仍把当前primary编码到bounded `g_PointerState.z`。真实`2974757317`在同一main callback内press→release，下一帧exact一次click使卡片折叠；旧native launch owner已删除。[E-V4-SCENESCRIPT-ORDERED-POINTER-EDGES](runtime-evidence-index.md#e-v4-scenescript-ordered-pointer-edges) | `g_PointerState`不是已登记的`official-public-contract`；multi-surface仍保留逐帧snapshot。`cursorMove`、多button、drag-out/capture真实门、候选传播、失焦/monitor权限与官方顺序/坐标对照仍缺 |
