...
| SceneScript polling pointer/canvas snapshot | `L3 bounded single-surface / S4 representative visible` | 唯一surface的同帧AppKit位置/左键、drawable像素尺寸与现有camera inverse形成callback-scoped immutable `input.cursorWorldPosition`、`cursorScreenPosition`、`cursorLeftDown`及`engine.canvasSize`；global phase、无精确surface、非法尺寸/非有限值局部拒绝。代表text consumer完成callback并显示canvas/world数值；stock 3D Clock首断点已前移到静态`thisLayer.angles` setter；[E-V4-SCENESCRIPT-SURFACE-INPUT](runtime-evidence-index.md#e-v4-scenescript-surface-input) | multi-surface不猜screen identity；`screenResolution`、resize/Retina/屏外与event同帧golden、静态layer mutation及官方坐标对照未闭合 |
