...
| generic media thumbnail identity | `L3 bounded current multi-representation` | `$mediaThumbnail`现以exact `(name,purpose)`进入同一frame registry；同一encoded generation独立尝试premultiplied color与保留alpha-zero RGB的preserved-channels data texture，两者成功时同代原子发布，partial ready时缺purpose转unavailable。首次请求把store-owned的exact purpose尝试标pending，replacement保留last-ready，ready后原子换代；同generation的presence+五色继续进入generic QuickJS event。B20删除的`$mediaPreviousThumbnail`尚未恢复 | live platform producer、previous/texture transition/variant、status/timeline与跨事件完整lifecycle |
...
| authored placeholder / previous-current fallback | `L3 bounded effect` | 首次exact representation pending/unavailable时，只有top system slot、lower asset、无graph binding且完整variant envelope的effect可复制进入effect前的current；其他effect和唯一compositor继续。已有ready的replacement pending继续发布last-ready；decode/clear完成后缺失purpose才变unavailable。没有恢复旧source-replacement renderer，也不伪造texture或Program成功 | base-image fallback、其他Blend topology、previous cover/transition与平台clear/switch语义 |
...
| system/media provider identity | `L3 bounded current effect consumer` | Template、catalog demand、registry、publication、Program与runtime bridge统一使用exact `(name,purpose)`；producer snapshot携带pending generation及store-owned、正在尝试派生的exact identities，不能把其他system name/purpose误标pending。wrong request identity/generation/content/incomplete publication/lifecycle继续硬拒绝；ready purpose/sampling mismatch只在严格结构证明下局部previous-current，否则拒绝最小不安全单元；current ready/pending/unavailable正反门见 [E-V4-SYSTEM-PROVIDER-PRESERVED-CHANNELS](runtime-evidence-index.md#e-v4-system-provider-preserved-channels) | live producer、previous、base-image/root ingress、pause/rebuild/stop与teardown |
