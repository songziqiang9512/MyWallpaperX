...
| direct user-property → exact vector3 color shader consumer | `L3 bounded` | binding compiler保留exact `{user,value}`、typed fallback与target；当前Pulse owner只迁移非audio、color-only的`tintlow/tinthigh`，要求sole `.userProperty` contributor、无script attachment、active non-array fragment `float3` ABI、旧plan binding全集等值，并由显式`range`或exact `type=color`证明`0...1` author domain。范围内live值进入Program；越界值保留同shape作者fallback。真实`1937925563:13#effect#321`将`triangle_neon_color`写入`tinthigh`，有效与越界两路均经Program/GPU/publication/后缀compositor/next-frame执行；[E-V1-PULSE-TYPED-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-typed-user-property-owner) | 不做广播或类型转换；scalar/float2、alpha或audio+binding、额外wrapper key、多contributor、Timeline/SceneScript、数组、target/fallback/ABI/domain漂移与未消费binding继续失败关闭或保留incumbent |
...
| shader constant actionable subset | 20 + 5 bounded | census的19条旧白名单经重建进入受限executor；Local Contrast strength与`1937925563:13#effect#321`的direct vector3 `tinthigh`现由共享Program live消费snapshot，`2902406982:[365,372,647,664]` stock Opacity alpha继续由其现役bounded consumer读取；[E-LIVE-PROPERTY](runtime-evidence-index.md#e-live-property)、[E-V1-PULSE-TYPED-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-typed-user-property-owner) | `L3` | 其他generic executor、SceneScript与通用live uniform |
...
| shader constant exact direct user-property consumer subset | 1 active | 真实`1937925563:13#effect#321`把`triangle_neon_color` vector3按exact target/fallback/`float3` ABI与`0...1`颜色域写入`tinthigh`；范围内live update进入共享MaterialProgram，越界值保留作者fallback，route disable仅局部previous-current且不复活Pulse owner；[E-V1-PULSE-TYPED-USER-PROPERTY-OWNER](runtime-evidence-index.md#e-v1-pulse-typed-user-property-owner) | `L3 bounded / S4 owner-migration` | 只统计当前复核的非audio color-only exact direct vector3 consumer；不外推scalar/float2、alpha/audio+binding、其他property/type/effect、广播、Timeline/SceneScript、多contributor、整个Pulse family或视觉parity |
