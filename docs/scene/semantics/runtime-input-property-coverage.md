...
>
> `311115e3` 新增的native click/launch-origin、hover、property→Vec3与audio-scaled value均已由同一QuickJS-NG domain中的generic cursor/vector/AudioBuffers owner取代并删除旧产品族；shared alpha、identity display与media colors仍只有`S2 wired / visible unknown`。真实`2974757317`闭合generic hover与single-surface ordered click，真实`3122339805`闭合inside-hit `cursorMove`拖动，真实`2067939514`让25个pass-owned Vec2 scale从同一audio snapshot进入QuickJS、dynamic snapshot、普通MaterialProgram与GraphExecutor，并让2个direct property scalar按exact isotropic projection进入Precise Blur `float2` Program consumer；真实`2884628849`的隔离author-property门让4个pass-owned Vec3颜色值经同一VM与bounded `WEColor.hsv2rgb`发布；真实`2938612768`又让带四项static `scriptProperties`的pass scalar strength经AudioBuffers进入现有Water Waves Program；真实`2902406982`的两个user-bound Water Waves则让2条bool activation和8条scalar uniform从同一property snapshot按generation进入普通Program。边界见[E-V4-SCENESCRIPT-PASS-VEC2-AUDIO](runtime-evidence-index.md#e-v4-scenescript-pass-vec2-audio)、[E-V4-USER-PROPERTY-FLOAT2-BLUR](runtime-evidence-index.md#e-v4-user-property-float2-blur)、[E-V4-SCENESCRIPT-PASS-VEC3-WECOLOR](runtime-evidence-index.md#e-v4-scenescript-pass-vec3-wecolor)、[E-V4-SCENESCRIPT-PASS-SCALAR-PROPERTIES](runtime-evidence-index.md#e-v4-scenescript-pass-scalar-properties)与[E-V4-USER-PROPERTY-WATER-WAVES](runtime-evidence-index.md#e-v4-user-property-water-waves)。
...
| media events | API `L3 bounded`；live ingress `L0` | QuickJS已按typed generation在update前派发thumbnail/playback/properties三个event，owner异常局部隔离并保留peer/previous-current；当前producer仍是可注入inbox而非公开授权的live macOS provider | live producer、跨类型ordered queue、status/timeline、切换仲裁与多屏生命周期 |
