...
| 32/64 stereo buffers | `L3 bounded` | 与 16 档由同一次 FFT 生成并进入 host-shared snapshot；供统一Program中的Simple Audio Bars 32/64-band作者shader，以及两个exact native property-script 64-band profile；旧Simple专用consumer已删除；[E-AUDIO-INPUT](runtime-evidence-index.md#e-audio-input) | 其他 Workshop shader 与通用 SceneScript `registerAudioBuffers` 尚无 consumer/bridge，不外推 |
