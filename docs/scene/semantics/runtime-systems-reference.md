...
| 系统 | 当前实现状态 | 不能据此宣称 |
|---|---|---|
| Particle | 作者 2D sprite、部分 emitter/initializer/operator、bounded list-only Color List、bounded initial emitter delay、bounded static CP-angle emitter frame、bounded classic Vortex、bounded Cap Velocity、root/strict-child 共用的 bounded Sprite Trail、static/spawn/death/follow Sprite profile 共用的 bounded uniform screen-plane child scale、bounded root/strict-child Rope 与 strict child 子集；initial delay 只准有限 `0...3600 s`，Sphere/Box/Layer Image 共用 fixed-step gate，duration 从 delay 后计时并可与已准入 periodic profile 组合；static CP angles 只准 root Sphere/Box 的有限 local vector3，并以项目自有 X→Y→Z frame 旋转 origin/shape/velocity；classic Vortex 只准 finite non-audio v1 axis/distance/speed 与 standard/infinite-axis flags，使用项目自有 fixed-step 切向加速度；Cap Velocity 只准显式 finite max speed、strict normalized-age blend 与无扩展 profile，并按项目自有 weight 保持方向地移除超速比例；Sprite Trail 对 omitted/`null` length 使用项目默认 `1` 并保留 malformed fail-closed，child scale 同步传播 local position/size/velocity/trail stretch，Rope 按 birth ID 建隔离 topology，支持 bounded subdivision、UV scale/equal-lifetime smoothing/scrolling 与 child birth-time world origin；exact pointer-lock CP 1...7 可驱动 bounded local Control Point Force；Particle slot 0 可按 exact identity 消费 bundle 内 164 项 TEX，22-key 程序纹理只作缺失回退 | 路径存在或程序 fallback 等于官方视觉资产，或 child/Rope/world-space/control point/collision/audio/全部 preset 完整；child 非零 angles、event offset、镜像/XY 非均匀、Rope/nested/world-space-movement scale 与 Windows transform golden 仍失败关闭；CP-angle 没有 Windows Euler/trajectory/pixel golden，dynamic angle 与其他 consumer 仍失败关闭；initial delay 没有 Windows fixed-step/组合 timing golden；classic Vortex 没有 Windows 数值/轨迹 golden，audio/CP/center force 与 `vortex_v2` ring 仍失败关闭；Cap Velocity 没有 Windows blend/clamp/轨迹 golden；Sprite Trail 默认值没有 Windows golden，Rope 仍不含 animated/root-world/multi-renderer、official curve/scroll/ribbon join 等高级语义且急弯有接缝；pointer force 没有 previous pointer、其他 consumer、world/perspective、官方 falloff/轨迹 golden，Color List 的 HSV/noise 与 Windows RNG 仍未闭合；Particle audio 只开放现役 E-PARTICLE 所列 bounded consumers，其余声明仍不消费 |
