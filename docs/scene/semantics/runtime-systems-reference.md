...
`36bfef0` 建立了六类 `SceneDynamicValue`、主要 target、固定覆盖顺序和不可变 snapshot。当前已有 property binding program、per-surface transaction/generation，以及 layer alpha、solid color、direct text content/point-size/color、bounded Timeline text width、Local Contrast/Opacity producer/consumer。dynamic text 按 layer signature 异步生成：普通并发拒绝 stale completion，连续 Timeline 则限制每层一个任务在途、合并为最新 generation，并以 last-ready 保持可见输出。SceneScript 已有单一 QuickJS-NG scene domain 的 bounded scalar/Vec3 VM，执行 inline `init/update`、allowlisted `WEMath.smoothStep`、immutable `engine.timeOfDay/frametime/runtime/userProperties`，并提供descriptor-bound callback-scoped `thisLayer -> IEffect` name/index/count lookup与typed material-function request；旧 time-of-day/property-vector Swift owner已撤销。property-bound Text Date/string、media placeholder fade 与 launch-origin startup 仍是独立 bounded typed producer；bool/string/Vec2/Vec4/matrix、mutable shared、scene/layer mutation、event/timer runtime 尚未建立。
