...
`36bfef0` 建立了六类 `SceneDynamicValue`、主要 target、固定覆盖顺序和不可变 snapshot。当前已有 property binding program、per-surface transaction/generation，以及 layer alpha、solid color、direct text content/point-size/color、bounded Timeline text width、Local Contrast/Opacity producer/consumer。dynamic text 按 layer signature 异步生成：普通并发拒绝 stale completion，连续 Timeline 则限制每层一个任务在途、合并为最新 generation，并以 last-ready 保持可见输出。SceneScript 现役执行仍只由彼此独立的 bounded typed producer 组成：property-bound Text Date/string、time-of-day Blend、media placeholder fade 以及 launch-origin startup cohort；它们都不建立通用 VM、module、mutable shared object 或 event runtime。

`311115e3` 又把 bounded hover/click、data-only shared alpha、audio-scaled particle rate/layer scale、property slider -> Vec3 origin/scale、identity display projection，以及 media playback/artwork colors/title/artist 接入同一 definition/transaction/snapshot 链。这组能力只有产品接线和自动测试源码，尚无提交后的 fresh App/runtime/ROI，因此统一记为 `S2 wired / visible unknown`；identity display 只是 exact no-op ownership projection，hover/click 也只是 surface-local primary-button edge 与 camera/world/UV hit-test，不是 ECMAScript、通用事件队列或完整 SceneScript API。七个 fixed native Text profile和两个 fixed 64-band audio profile分别在历史 B18/B21 中退役，旧报告只作 sealed historical evidence。
...
当前实现把文档级 inline property binding 的 scene/object/effect/pass owner、完整 JSON target path、source、properties、authored fallback 与 JSON value type 保真进 IR（`L1`），正式取证五类位置有 13-shape 自建门；nested/未知 owner 不提升，`script + user` 冲突 fail-closed。Text 只可按公共语法准入唯一 `update(value)` 的无循环 AST，以 primitive property、Date getter 与 string operations 产生 typed String；unsupported 语法保留 authored fallback。`311115e3` 的 hover/click、shared alpha、audio-scaled value、property -> Vec3、identity display 与 media projections 都是 bounded native typed wiring，不创建 JS instance。七个 exact Text profile和两个64-band audio profile已在历史 B18/B21 中退役。file/module、schema-resolved 通用 value、ECMAScript VM、`registerAudioBuffers`、`AudioBuffers` object、`createLayer`、通用事件队列和实例生命周期仍为 `L0`。
...
| Dynamic target snapshot | 六类 typed value、主要 target 族、固定优先级、binding program、per-surface evaluation transaction/snapshot/generation；既有 layer alpha、solid color、direct text、bounded text width、Local Contrast/Opacity、launch-origin 与 Timeline 子集有运行证据。`311115e3` 另接入 shared alpha、audio-scaled particle rate/layer scale、property slider -> Vec3 origin/scale、identity display、bounded hover/click 与 media playback/colors/title/artist | 新接线只有 `S2 wired / visible unknown`，不能与既有运行子集合并升级；generic Combined/SceneScript/event/live media 与多数 particle dynamic target仍缺，unsupported host 保留诊断 |
...
| SceneScript | binding IR 对正式五类 owner/target 为 `L1` 并保留可选 wrapper keys；property-bound Text Date/string、time-of-day Blend、media placeholder fade 与 initial-false launch-origin startup 是各自 bounded typed producer。`311115e3` 的 hover/click、shared/audio/property/identity/media projections 是 `S2` native wiring，不创建 JS instance/shared object。fixed Text 与 64-band Audio Bars profiles 已在历史 B18/B21 退役，旧报告只作 sealed evidence | 通用 ECMAScript VM/API、mutable shared、generic cursor/media events、inverse4、`registerAudioBuffers`/`AudioBuffers`、`createLayer`/`ILayer` handles仍不可用；bounded hover/click 不等于 callback/event runtime |
| User Properties | 独立窗口、条件、默认/override、部分 target 与持久化；`texture`/`scenetexture` 内部归一；受限静态 consumer 可选择 PNG/JPEG；基础 layer alpha/direct text/X-Ray target 可无重建更新，property slider -> Vec3 origin/scale已有 `S2` 接线 | 新 Vec3 接线没有 fresh visible evidence；全部样本属性可调、所有 texture target/variant/live value仍未闭环 |
