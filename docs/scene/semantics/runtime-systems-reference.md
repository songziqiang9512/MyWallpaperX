...
`36bfef0` 建立了六类 `SceneDynamicValue`、主要 target、固定覆盖顺序和不可变 snapshot。当前 v22 已完成 property binding program、per-surface transaction/generation，以及 layer alpha、solid color、direct text content/point-size/color、bounded Timeline text width、Local Contrast/Opacity producer/consumer。dynamic text 按 layer signature 异步生成：普通并发拒绝 stale completion，连续 Timeline 则限制每层一个任务在途、合并为最新 generation，并以 last-ready 保持可见输出。SceneScript现役执行只由彼此独立的bounded typed producer组成：property-bound Text Date/string、time-of-day Blend、media placeholder fade以及launch-origin startup cohort；它们都不建立通用VM、module、mutable shared object或event runtime。七个fixed native Text profile已于R4-B18退役，exact native audio profiles仍是独立bounded renderer consumer；这些都不能外推为通用脚本、particle、system/media text或其他effect constant。
...
当前实现把文档级 inline property binding 的 scene/object/effect/pass owner、完整 JSON target path、source、properties、authored fallback 与 JSON value type 保真进 IR（`L1`），正式取证五类位置有 13-shape 自建门；nested/未知 owner 不提升，`script + user` 冲突 fail-closed。Text只可按公共语法准入唯一`update(value)`的无循环AST，以primitive property、Date getter与string operations产生typed String；七个exact Text source profile已于R4-B18退役，unsupported语法保留authored fallback。两个64-band audio source profile仍编译为严格失败关闭的native `L3 bounded` consumer；其64次Metal draw是renderer instance，不产生动态Scene layer、`ILayer` handle或topology。file/module、schema-resolved non-String value type、通用ECMAScript VM、`registerAudioBuffers`、`AudioBuffers` object、`createLayer`、事件和实例生命周期仍全部为`L0`。
...
当前effect route精确为`.unclaimed`时，产品另有一条极窄的source-stage降级门：只在不存在已授权/可执行typed dependency、source-copy、frame target或final-alpha，style中性，exact static-file或current-media publication atom完整，且有限authored quad与viewport有正面积交集时，才把原始source以作者几何交给普通final layer compositor。coverage/opacity资源默认阻断；唯一收窄是exact stock Pulse单pass且`PULSEALPHA`缺省/显式为0时，其可选mask只属于未执行effect的局部输入，不改变source coverage。`PULSEALPHA=1`即使mask未加载、非法pass/path/combo及任一typed Opacity entry都继续阻断。可见consumer的显式dependency或named reference还会同时阻断static-file provider/consumer；hidden consumer、self reference与unreferenced layer不阻断，current-media保留既有独立降级authority。该路径记录`degraded-layer-source-passthrough`，不执行effect、不发布`effectOutput`、不消费authored dependency，benchmark必须保持NON-PASS。`2241938645:68`只恢复基础摩托车source，`1636394814:24/195`仍因authored X-Ray跨层关系失败关闭；详见[E-LAYER-SOURCE-PASSTHROUGH](runtime-evidence-index.md#e-layer-source-passthrough)。
...
| Dynamic target snapshot | 六类typed value、主要target族、固定优先级、binding program、per-surface evaluation transaction/snapshot/generation；layer alpha、纯solid color、direct text、bounded text width、exact Local Contrast/Opacity、受限X-Ray target、bounded launch-origin layer origin，以及Timeline的effect constant/layer alpha/bounded relative layer transform/root particle scalar/camera子集有真实producer/consumer。launch-origin scene/host state每host frame在surface loop外推进一次，再合并到各surface snapshot；target collision整项失败关闭 | 非layer-transform `relative`、generic Combined/其他target、generic SceneScript与多数particle dynamic target仍未live；297 inverse4/click/封面/effects不属于launch-origin producer；unsupported host留诊断 |
...
| SceneScript | binding IR对正式五类owner/target为`L1`并保留可选wrapper keys；独立source evidence只提供provenance/conflict rejection。property-bound Text Date/string、time-of-day Blend、media placeholder fade与initial-false launch-origin startup均为各自`L3 bounded` typed producer；launch-origin不创建JS instance/shared object。七个fixed Text profile与两个fixed 64-band Audio Bars profile已分别于B18/B21退役，旧报告只作历史 | 通用ECMAScript VM/API、mutable shared、cursorClick/hover、inverse4、`registerAudioBuffers`/`AudioBuffers`、`createLayer`/`ILayer` handles仍不可用 |
...
| Audio | Scene 16/32/64 host left/right、普通/Workshop Effect Audio Bars及已登记typed effect/particle consumers使用共享频谱输入；fixed SceneScript Audio Bars运行时已退役 | JS `AudioBuffers`、Sound/self-play、未登记consumer或任何数值/视觉parity |
| Media | Scene已有bounded current `$mediaThumbnail` provider、严格current Blend source replacement与visibility consumer；current-only store保留pending last-ready、cancel/stale及clear/failure fallback。exact current publication可在`.unclaimed`、不存在可执行typed dependency且无mask/非中性合成时仅显示source。previous/gradient transition已于B20退役；只有隔离debug producer | source-only降级不执行任何media effect或named/media chain。live macOS producer、metadata/status/timeline snapshot、通用event/SceneScript、previous/transition/variant与真实切歌lifecycle仍未闭环 |
...
5. exact stock Opacity、Tint mask与stock Radial God Rays子集已完成；新能力从专项覆盖表和能力依赖图按公共依赖、真实收益与fail-closed边界重新选择，不从已完成的历史能力计划续接，也不新增effect-name或样本ID近似。
