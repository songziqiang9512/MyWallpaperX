...
| 系统 | 当前实现状态 | 不能据此宣称 |
|---|---|---|
| Particle | 作者 2D sprite、部分 emitter/initializer/operator、bounded initial emitter delay、bounded classic Vortex、bounded Cap Velocity、root/strict-child 共用的 bounded Sprite Trail、bounded root/strict-child Rope 与 strict child 子集；initial delay 只准有限 `0...3600 s`，Sphere/Box/Layer Image 共用 fixed-step gate，duration 从 delay 后计时并可与已准入 periodic profile 组合；classic Vortex 只准 finite non-audio v1 axis/distance/speed 与 standard/infinite-axis flags，使用项目自有 fixed-step 切向加速度；Cap Velocity 只准显式 finite max speed、strict normalized-age blend 与无扩展 profile，并按项目自有 weight 保持方向地移除超速比例；Sprite Trail 对 omitted/`null` length 使用项目默认 `1` 并保留 malformed fail-closed，Rope 按 birth ID 建隔离 topology，支持 bounded subdivision、UV scale/equal-lifetime smoothing/scrolling 与 child birth-time world origin；exact pointer-lock CP 1...7 可驱动 bounded local Control Point Force；Particle slot 0 可按 exact identity 消费 bundle 内 164 项 TEX，22-key 程序纹理只作缺失回退 | 路径存在或程序 fallback 等于官方视觉资产，或 child/Rope/world-space/control point/collision/audio/全部 preset 完整；initial delay 没有 Windows fixed-step/组合 timing golden；classic Vortex 没有 Windows 数值/轨迹 golden，audio/CP/center force 与 `vortex_v2` ring 仍失败关闭；Cap Velocity 没有 Windows blend/clamp/轨迹 golden；Sprite Trail 默认值没有 Windows golden，Rope 仍不含 animated/root-world/multi-renderer、official curve/scroll/ribbon join 等高级语义且急弯有接缝；pointer force 没有 previous pointer、其他 consumer、world/perspective、官方 falloff/轨迹 golden，粒子 audio 声明虽已保真解析但 simulation 仍不消费 |
