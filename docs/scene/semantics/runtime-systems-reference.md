...
| SceneScript | 顶层layer binding IR为`L1`；property-bound Text Date/string update bounded AST子集可执行。七个fixed Text profile与两个fixed 64-band Audio Bars profile已分别于B18/B21退役，旧报告只作历史 | 通用ECMAScript VM/API、`registerAudioBuffers`/`AudioBuffers`、`createLayer`/`ILayer` handles可用 |
