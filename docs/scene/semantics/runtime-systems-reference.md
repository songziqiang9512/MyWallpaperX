...
>
> 当前实现等级、样本运行结果、签名 App 身份及聚合缺口统一见 [运行证据索引](./runtime-evidence-current.md)；本表不复制基线 commit，避免随代码演进失真。文内出现的 commit 号是该项能力的**历史落地提交**，不是当前基线。旧 `R*`、`B*`、`D*` 编号只标识 sealed historical evidence，不是现役顺序；唯一实施顺序见 [Scene 兼容路线](../scene-compatibility-roadmap.md)。
...
`36bfef0` 建立了六类 `SceneDynamicValue`、主要 target、固定覆盖顺序和不可变 snapshot。当前已有 property binding program、per-surface transaction/generation，以及 layer alpha、solid color、direct text content/point-size/color、bounded Timeline text width、Local Contrast/Opacity producer/consumer。dynamic text 按 layer signature 异步生成：普通并发拒绝 stale completion，连续 Timeline 则限制每层一个任务在途、合并为最新 generation，并以 last-ready 保持可见输出。SceneScript 当前复用单一 QuickJS-NG scene domain 执行 bounded scalar/String/Vec2/Vec3 owner、`init/update`、typed handles、media/cursor/audio callback、job/timer 与 destroy 生命周期；旧 time-of-day/property-vector、media fade/color、fixed Text/audio 与 cursor native owner 已按各自证据包撤权。bool/Vec4/matrix、完整 mutable shared/host API、完整事件传播与官方对照仍未闭合，精确现役范围以专项覆盖表和运行证据索引为准。

`311115e3` 最初把bounded hover/click、data-only shared alpha、audio-scaled particle rate/layer scale、property slider -> Vec3 origin/scale、identity display projection，以及media playback/artwork colors/title/artist接入同一definition/transaction/snapshot链。后续 origin/scale、cursor、audio、media playback/properties/thumbnail 已分别迁入共享 QuickJS owner，并按运行证据达到各自 bounded `S3/S4 + owner-migration`；shared alpha、identity display与未登记consumer仍保持较低证据等级。旧报告只作sealed historical evidence。
...
当前 `9058a8b3` 另开放 bounded HSV Color Random：只接受七个显式 direct-number 字段，Hue/Saturation/Value min/max 必须 finite、ordered 且位于 `0...1`，Hue steps 必须为 `1...1024` 的整数。创建时按项目自有 `[hueMin,hueMax)` 离散 hue、连续 saturation/value 与标准 HSV→RGB 近似写入颜色，并继续服从 authored initializer order；合法静态 General instance `color`/`colorn` 按官方 factor 合同与 initializer 结果逐通道相乘，General 禁用颜色 override 时保持 initializer 结果，非法动态 override 继续失败关闭。官方公开页只说明 Hue range/steps 和 Saturation/Value range，2.8.42 field census 与完整 stock 正例足以确认七个 wire，因此本批没有启动 Ghidra。省略字段的官方 default、随机端点/序列、linear-vs-sRGB 与 Windows 数值/像素等价仍保持 unknown；当前执行器不是官方 RNG 或色彩算法复刻。
...
当前 `319214c5` 开放 bounded Rope 公共子集：单一 Rope renderer、Screen/nil orientation、静态纹理、`maxcount 2...512`，按单调 birth ID 连接相邻存活粒子；subdivision `0...100` 用 Catmull-Rom/Hermite position 曲线生成独立 quad，size/color/alpha 在细分端点线性插值，root 每帧最多 32,768 段（2026-09-24 按语料作者形态上调：subdivision 100 × maxcount 256 = 25,755 段、subdivision 16 × maxcount 500 = 8,483 段）。UV 支持 finite scale `1/1024...1024`、equal-lifetime smoothing 与 simulation-time scrolling；scrolling 按官方优先级覆盖 smoothing，具体每秒一轮是项目 bounded 解释。strict child Rope 为每个 child system 独立建链以避免跨系统连接，保留 General world-space 的 birth-time origin，并受 32,768 生成段聚合预算约束（与 root 上限同一常量）。Emitter one-per-frame 只准 stock corpus 对应的 raw bit 2；无已知 consumer、零 transform/flags 且没有未知组件的 parent CP mapping 可透明忽略，其他 CP 形态仍失败关闭。animated texture、root world orientation、多个 renderer、length/segments/fades 与 malformed/未知字段也继续拒绝。官方 stock subdivision 样本可见曲线和运动，但急弯处独立 quad 仍有三角/明暗接缝；这证明 bounded clean-room 执行，不是官方 ribbon join 或像素等价。
...
MyWallpaperX 当前按 alias → 包内文件 → 客户端自带（stock）→ 缺失四层解析，路径安全判定优先于后三层。官方 `systemfont_*` 别名共 8 个，缺失家族退到形态最接近的系统家族（`consolas` 必须落回等宽，否则语料引用最多的别名会从等宽退化成比例字体）；客户端 `assets/fonts` 的 15 个 stock 名字全部物理位于 `SceneStockAssets.bundle/assets/fonts`：8 个原版报 `stockBundled`；7 个替代字体按官方文件名读取并报 `stockSubstituted`。只有 app 包缺失或损坏时才落回系统家族近似并报 `stockApproximation`。替代字形不是官方字形等价，见 [E-TEXT-FONTREF](../../history/scene/runtime-evidence-index.md#e-text-fontref)。

MyWallpaperX 当前的 `pointsize * 300 / 72` 直接来自官方 typings 对 `ITextLayer.pointsize` 的 300 DPI 说明，并由随包 `dino_run` 两个记分标签的作者 size 逐位复现，见 [E-TEXT-POINTSIZE](../../history/scene/runtime-evidence-index.md#e-text-pointsize)。但 scene unit、raster backing scale 和 Retina 输出之间的精确关系仍需官方 Windows 对照，本地还对像素字号做了 1024 的夹取；实现和文档都不能把这一个换算提升为完整文字语义。

第 6 条里的 width/line/ellipsis 约束现在按作者开关执行：`limitwidth`/`maxwidth` 压窄换行宽度、`limitrows`/`maxrows` 丢弃多余行、`limituseellipsis` 在末行补省略号并回退到不越界，两个数值在开关关闭时必须完全不生效（本机语料 393 个关闭态 layer 都带着编辑器默认 `maxwidth: 500`，其中 79 个作者宽度已超过 500）。absolute 单 lane Timeline `maxwidth` 仅在 `limitwidth=true` 且作者、关键帧和实际消费的 Bézier control values 都位于有限 `1...16384` 像素预算时驱动同一 CoreText consumer；relative、Combined、越界与非文字形态失败关闭。省略号的回退粒度和被裁断点仍未与 Windows 逐像素对照，见 [E-TEXT-LIMITS](../../history/scene/runtime-evidence-index.md#e-text-limits)。
...
producer 到 consumer 的公共路径应为 `VM/event/property/provider -> typed mutation -> Surface EvaluationTransaction -> immutable snapshot/resource publication -> consumer`。每次 mutation 只触发[兼容运行时架构](../design/runtime-architecture.md)定义的最小失效域：time/pointer/audio/alpha/color/transform 等 value-only 更新只换 snapshot/uniform；媒体封面或视频帧只推进 resource generation；动态文字尺寸、mesh/atlas geometry 或 target extent 只失效受影响 geometry/target；combo/optional slot 改变才失效 Program variant；layer/effect 增删或 target 结构改变才重建依赖子图。不得把 value-only 更新升级成 Scene relaunch，也不得在 topology 已变化时继续复用 stale graph。

当前实现把文档级inline property binding的scene/object/effect/pass owner、完整JSON target path、source、properties、authored fallback与JSON value type保真进IR（`L1`），正式五类位置有13-shape自建门；nested/未知owner不提升，非null `script + user`冲突fail-closed。QuickJS-NG per-scene domain现执行bounded scalar/String/Vec2/Vec3 `init/update`，并提供immutable frame/user-property snapshot、typed layer/effect/input handles、16/32/64 `AudioBuffers`、media/cursor callback、job/timer与destroy生命周期；共享完整性门另以单owner 256 KiB/candidate 2 MiB source硬上限、QuickJS interrupt内construction cancellation，以及不可配置、无setter的`engine`与`thisLayer/thisScene/thisObject`私有slot accessor限制失败半径。旧property-vector、media和cursor native owner已删除。`shared`仍按作者合同可变；shared alpha与identity display仍是bounded typed wiring，file module、bool/Vec4/matrix、完整domain state rollback、通用传播事件队列和官方行为对照仍未闭合。
...
MyWallpaperX 当前状态（见 [E-AUDIO-INPUT](../../history/scene/runtime-evidence-index.md#e-audio-input)、[E-AUDIO-EFFECT](../../history/scene/runtime-evidence-index.md#e-audio-effect)）：host-shared snapshot 同一次 FFT 生成 16/32/64 档 left/right 并每帧广播给所有 surface；采集按 consumer 存在性驱动，无消费者/暂停/锁屏/休眠停采并归零，失败输出稳定全零。consumer 包括 stock Shake/Pulse、普通/Workshop Effect Audio Bars 的统一 Program/typed effect 路径，以及已登记的 bounded particle emitter/initializer/operator 子集。`311115e3` 另把 audio-scaled particle rate/layer scale 接入 typed snapshot，但只有 `S2 wired / visible unknown`，不能与既有粒子运行证据合并升级。两个 fixed native property-script 64-band Audio Bars consumer已在历史 B21 中删除；项目仍没有 JS `AudioBuffers` object、Float32Array identity、脚本订阅或通用 `average` bridge。effect 与粒子两套 schema 不能互推，未登记组合继续失败关闭。
...
producer-agnostic inbox 现分别保存 playback、artwork five-color、title/artist 及独立 generation；bounded generic QuickJS scalar/String/Vec owner 已消费 playback/properties/thumbnail event，旧 fade/color native owner 已撤权。现役证据包含真实 callback/GPU/compositor/next-frame与部分可见切片，但 five-color cohort 没有预登记 ROI，且仍无 live macOS producer、status/album/timeline、previous/transition或完整官方对照，因此不能表述为系统媒体链已经闭合。
...
当前事实以 [总覆盖台账](coverage-ledger.md) 与 [运行证据索引](./runtime-evidence-current.md) 为准。这里仅列语义边界：
...
| Particle HSV initializer | `9058a8b3` 只为七个显式 direct-number wire 开放 normalized HSV ranges 与 `1...1024` hue steps，创建时执行项目自有离散 hue、连续 saturation/value 和 HSV→RGB 近似；合法静态 instance color 按 factor 相乘，unknown/malformed/省略字段及非法动态 override 继续失败关闭 | 官方 default、RNG/端点、linear-vs-sRGB、Windows 数值/像素等价或通用 color/noise initializer |
...
| Dynamic target snapshot | 六类typed value、主要target族、固定优先级、binding program、per-surface evaluation transaction/snapshot/generation；layer alpha、solid/image direct color、direct text、bounded text width、Local Contrast/Opacity、Timeline及bounded QuickJS scalar/String/Vec2/Vec3已有运行证据。cursor、audio与media event也复用同一typed carrier | 各证据只覆盖登记的bounded target/consumer，不能外推generic Combined、live media、完整host API与多数particle dynamic target；unsupported host保留诊断 |
...
| SceneScript | binding IR对正式五类owner/target为`L1`并保留可选wrapper keys；同一QuickJS-NG scene domain已以`L3 bounded`执行scalar/String/Vec2/Vec3 `init/update`、typed layer/effect/input handles、cursor/media/audio callback、jobs/timers、dynamic layer mutation与destroy生命周期，旧fixed/native重叠owner已按证据包撤权 | bool/Vec4/matrix、完整mutable shared/module/reload/host API、通用事件传播与官方行为/视觉parity仍未闭合；bounded正证不能外推完整SceneScript平台 |
...
| Texture Provider | frame identity/status/generation、named variant隔离、property absent -> authored fallback、受限file-backed property source；direct text/embedded MP4使用显式content generation，视频有launch-scoped pause/rebuild/stop与command-generation/EOF-current-anchor合同；bounded current media cover保留旧request协作取消、last-ready/fallback与strict current Blend/visibility consumer。exact static-file/current-media publication atom还可在上述source-stage极窄门中按authored quad降级进入普通final layer compose | 降级门不执行/发布effect，不消费raw dependency，也不证明generic effectful/nested provider。live system-media adapter、Texture Variants、不同duration/rate/系统休眠长稳、generic video/material、previous/transition、其余provider cancellation与单次decode抢占尚未闭环 |
...
| Media | Scene已有bounded current `$mediaThumbnail` provider、strict current Blend consumer及pending last-ready/cancel/stale/clear fallback；playback/properties/five-color thumbnail generation现进入共享QuickJS event owner，旧fade/color native owner与previous/gradient transition已退役 | source-only降级不执行media effect或named/media chain；live macOS producer、status/album/timeline、previous/transition/variant、完整跨类型事件顺序与真实切歌生命周期仍未闭环 |

## 11. 现役推进与封存边界

本文定义运行时语义，不另建能力顺序，也不复述 V1–V5 的串行、并行或完成状态。现役段位、V4 横切方式和 V5 itemization 只来自 [Scene 兼容路线](../scene-compatibility-roadmap.md)；旧 D/R/B 计划及其“下一批”均已封存，历史提交和数字只能从[运行证据索引](./runtime-evidence-current.md)追溯。
