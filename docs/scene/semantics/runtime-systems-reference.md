...
当前effect route精确为`.unclaimed`时，产品另有一条极窄的source-stage降级门：只在不存在已授权/可执行typed dependency、source-copy、frame target或final-alpha，没有已加载coverage/opacity mask，style中性，exact static-file或current-media publication atom完整，且有限authored quad与viewport有正面积交集时，才把原始source以作者几何交给普通final layer compositor。它记录`degraded-layer-source-passthrough`，不执行effect、不发布`effectOutput`、不消费样本里仍可存在但无可执行owner的raw dependency，benchmark必须保持NON-PASS。任一原子或几何条件不合同时继续失败关闭。
