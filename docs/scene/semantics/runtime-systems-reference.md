...
| SceneScript | binding IR 对正式五类 owner/target 为 `L1` 并保留可选 wrapper keys；同一 QuickJS-NG scene domain 已以 `L3 bounded` 执行 numeric scalar `init/update`、`WEMath.smoothStep` 与 immutable `engine.timeOfDay/frametime/runtime`，真实 time-of-day pass consumer 达到 `S4`。property-bound Text Date/string、media placeholder fade 与 initial-false launch-origin startup 仍是独立 bounded typed producer。`311115e3` 的 hover/click、shared/audio/property/identity/media projections 是 `S2` native wiring，不创建通用 JS handle/shared object | 非scalar typed conversion、mutable shared、generic cursor/media events、inverse4、`engine.userProperties`、`registerAudioBuffers`/`AudioBuffers`、`createLayer`/`ILayer`、timer/job/destroy/reload仍不可用；bounded scalar与hover/click不能外推完整callback/event runtime |
