...
| Water Ripple / `waterripple` | `L3` | `strict-graph-profile`：exact stock 单 pass profile，消费 authored mask + normal 并保持 animation/ratio/strength/scale/scroll 常量，可进入 ordered chain；旧 normal-map/legacy inline 子集仍只服务非 exact 声明 | [D2](capability-dependency-map.md#d2) [D5](capability-dependency-map.md#d5) [D7](capability-dependency-map.md#d7) [D8](capability-dependency-map.md#d8) | [E-EFFECT-WATER-MOTION](runtime-evidence-index.md#e-effect-water-motion) [E-EFFECT-INLINE](runtime-evidence-index.md#e-effect-inline) [E-EFFECT-CHAIN](runtime-evidence-index.md#e-effect-chain) | variant、双 normal/specular、Perspective、动态 SceneScript 值、Windows golden |
...
| Tint / `tint` | `L3` | `strict-graph-profile`：exact stock 单 pass，fingerprint 准入后由项目自研的 32 模式 `ApplyBlending` Metal 表执行；`BLENDMODE` 缺省按官方 `[COMBO]` 默认 30（去重语料 203/224 未声明），`alpha` 缺省按 uniform 默认 1（12 个 pass 省略）；`color`/`alpha` 支持 vec3/scalar direct binding live。base layer tint 不是此 Effect | [D5](capability-dependency-map.md#d5) [D7](capability-dependency-map.md#d7) [D8](capability-dependency-map.md#d8) | [E-EFFECT-TINT](runtime-evidence-index.md#e-effect-tint) | 绑定 `g_Texture1` 遮罩的实例（去重语料 30/224）当前整条拒绝，不按无遮罩降级——原始理由已被 [E-MASK-SLOT-COMBO](runtime-evidence-index.md#e-mask-slot-combo) 推翻，属待办欠账；SceneScript/animation binding、premultiplied source alpha 与 Windows 像素/色彩空间 golden |
...
`b541867` 只增加 strict profile 之间的有序、全有或全无调度，没有改变 45 项数量或等级；其阶段报告 `.codex/scene-effect-chain-gated-final13-20260723/report.json` 的 8 stage、0 real chain 是 Shadow 前的历史负门。`809b75e` 的 exact Workshop Shadow、`b8842d8` 的 stock Opacity 与 `4f13daf` 的 exact legacy Blur Precise compose 都只扩充既有 `L3` 行的受限 profile。`e505a9e`、`31ae557` 与 `94aebc5` 分别让 exact stock Shake、Water Waves 与 Water Flow 进入 ordered scheduler；`3baf1fc` 又加入 exact Foliage Sway、Water Ripple 与 pointer-driven X-Ray，并修正 X-Ray 组合区域捕获。当前完整门为 91 stage、15 条 chain、Water Flow 10、Water Waves 11、Shake 24、0 failed；固定门保护 24 stage、2 条真实 chain、Water Flow 1、Water Waves 6、Shake 1、Opacity 4、Workshop Shadow 1、0 failed。最新路径与边界统一见 [运行证据索引](runtime-evidence-index.md)。Tint 是第一项把 `L1` 抬到 `L3` 的 Effect：它带来 32 模式 `ApplyBlending` 共享 shader 源；chain renderer 的 `.tint` 分支有 GPU 运行门（换 plan 的 `blendMode` 得到两组不同像素），但还没有进入 ordered scheduler 的整包 chain 统计。Pulse 是第二项 `L1` 到 `L3`，也是 `ApplyBlending` 表的第二个 backend 消费者与第一个多指纹 shader profile（stock + 3 legacy 变体各绑独立语义参数）；其结构化 census 入口 [scene_effect_census.py](../../../script/scene_effect_census.py) 已按第 9 节要求落地，`45` 样本语料中 pulse 层全部混排 unsupported sibling（老包 legacy 指纹 tint/waterwaves/waterripple、新包遮罩槽 shake），当前 PulseCount=0，正门为 GPU 像素门、planner 门与逐样本 fail-closed 运行证据（[E-EFFECT-PULSE](runtime-evidence-index.md#e-effect-pulse)）。
