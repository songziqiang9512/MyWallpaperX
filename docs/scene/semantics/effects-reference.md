...
当前 MyWallpaperX v15 已把 EffectDefinition 计为 `recognized`，v16 已把 13 样本中可见实例结构化编译为 `graph-built` 并用 canonical SHA 验证身份；仍有 5 个 fluid condition/function blocker。严格匹配 2-pass / 1 full-size RT 的 `blur_precise_gaussian` 子集已有 5 个 visible layer 的固定近似 GPU 执行；stock standard Blur 默认 profile 另有 1 个 visible layer 走真实 4-pass / 2 quarter-RT、alpha-aware downsample、13-tap 横纵 Gaussian 与默认 combine，stock straight-alpha 输出也已显式转回宿主 premultiplied 约定并通过半透明端到端像素门。两者都只能记为 `executed-degraded`：precise 未执行 authored shader，standard 的 full capture 可受 2048 clamp，奇数尺寸取整、颜色空间、sampler、空间变化 alpha，以及 KERNEL1/2、COMPOSITE1-3、其他 blend/mono/alpha/mask 组合均未闭合；当前也未验证同路径 shader asset 的内容哈希。其他 graph-built 节点不能整体升级。
