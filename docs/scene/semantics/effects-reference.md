...
| [Light Shafts](https://docs.wallpaperengine.io/en/scene/effects/effect/lightshafts.html) / `lightshafts` | noise、gradient、opacity mask 的槽位依 rendering/direct-draw combo 变化；`point0...3` 是 effect-local perspective UV，不是独立世界几何 | 1P | exact stock linear + direct-draw + Gradient standalone quad 已由项目自有 noise/gradient 资源与 clean-room Metal 近似执行；carrier 保持 stock preview/真实实例共同支持的 half-canvas extent。只有 prepared combo、静态值和 vertex source 同时证明 linear perspective active-content 边界时，通用 output-geometry 合同才携带顶部 inset，四点 uniform 本身不被改写；radial/corner、动态点、Color、mask、链式输入和其他 slot layout 继续关闭 | A+C |
...
| [Opacity](https://docs.wallpaperengine.io/en/scene/effects/effect/opacity.html) / `opacity` | 只调整目标区域 alpha；可选 opacity mask | 1P | 普通单 pass 已由统一 Program/executor 接管；静态/direct-binding alpha 经 per-surface snapshot，optional mask 以 reachable typed sampler schema、资源 readiness 与 per-slot resolution 选择 `MASK=0/1`，未证 purpose 或数据流失败关闭；无法完整接管的 SceneScript/legacy composition/unsupported mixed chain不冒充迁移 | A+C |
...
| [Water Caustics](https://docs.wallpaperengine.io/en/scene/effects/effect/watercaustics.html) / `watercaustics` | mask、Voronoi、uniform/noise、offset noise、glow 共五类额外输入 | 1P | 五槽继续按作者material/schema分别解析；当前exact realistic/no-Perspective静态source只有在公共analyzer证明唯一graph source、唯一RGB写回、terminal whole output及所有辅助sampler的typed data projection/sample count后，才由`source-proven-graph-input-typed-data-rgb-filter`、普通MaterialProgram与GraphExecutor执行。stock pattern/uniform只按registry发布typed noise asset，旧项目固定MSL近似已撤权删除；illustrative、Perspective、动态值/provider、其他source/resource shape与官方像素语义继续失败关闭 | A+C |
