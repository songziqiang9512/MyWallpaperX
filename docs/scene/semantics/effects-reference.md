...
| [Light Shafts](https://docs.wallpaperengine.io/en/scene/effects/effect/lightshafts.html) / `lightshafts` | noise、gradient、opacity mask 的槽位依 rendering/direct-draw combo 变化 | 1P | exact stock linear + direct-draw + Gradient standalone quad 已由项目自有 noise/gradient 资源与 clean-room Metal 近似执行；radial/corner、Color、mask、链式输入和其他 slot layout 继续关闭 | A+C |
