...
当前已有资料、固定客户端身份和既往结论只从[资料来源与证据索引](source-index.md)进入；当前能力与运行结果仍只由[能力台账](coverage-ledger.md)和[运行证据索引](./runtime-evidence-current.md)决定。本文规定研究流程，不证明任何功能已经支持。
...
上下文隔离针对的是能还原专有实现、并与待实现职责重叠的**原始或可还原实现表达**，不是对所有旧资料的关键词式污染判定。它包括地址/offset、机器指令、反编译伪代码或函数体、控制流、连续私有字段布局或调用链，以及私有 shader、payload、算法表达、公式或常量表。

只要研究任务实际查看了上述内容，就必须保持研究者与对应产品实现者分离，在交接审核完成后结束该研究职责。产品实现由**新任务或新上下文**开始，并满足：
...
历史摘要、旧计划、高层静态结论、经审查的中性行为合同和历史证据不属于原始静态输出，也不会因为被 implementation task 读到就自动触发停手。它们只提供低权重线索：其中的版本、owner、route、“当前”“下一批”和实现判断必须回到当前代码、现役权威文档与可复现运行证据重新验证，不得直接续接。

implementation task 仍不得宽泛检索或预读标为 `research-context-only` 的取证页。若意外读到原始或可还原实现表达，按下列最小隔离半径处理：

1. 与当前实现职责、算法选择、字段/状态归属或执行顺序重叠时，立即停止该职责的产品写入，隔离原始内容，形成经审查的中性交接，再由 fresh implementation context 接手；
2. 能够明确证明与当前纵向切片无关时，记录不重叠边界，隔离精确内容，不在实现、fixture、测试或文档中使用、复述或传播，可继续其他不重叠职责；
3. 无法可靠判断是否重叠时按重叠处理。一次偶遇不把整个任务、全部产品改动或所有无关职责自动降级为研究上下文。
