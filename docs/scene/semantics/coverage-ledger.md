...
| Child execution/events | `L3` | deterministic birth/natural-death queue；`eventspawn`、natural-`eventdeath` 与 identity/no-CP `eventfollow` strict child 独立模拟、绘制、跟随和回收；Sphere/Box child 可执行 instantaneous、continuous rate、混合发射与有限/无限 duration，每 system 1,024 粒子、每 root runtime 64 systems/65,536 capacity | static、collision/delete、transform/CP/value inheritance、nested child、跨层/递归总预算 |
