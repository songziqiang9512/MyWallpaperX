...
| Particle runtime | `L3` | 作者 Sprite、常见组件、Sprite Trail，以及严格单 renderer、Screen orientation、renderer world-space 关闭的 RopeTrail 子集；普通 Sprite 现消费可信单 sequence sidecar nominal frame aspect，缺失时按 raw TEX axes × physical pixels 回退，current/next aspect 随 frame blend 进入 quad 与 REFRACT tangent，root/child 共用。RopeTrail 要求 finite `length`，`segments` 缺省 4 且只准 1...8，可选 `fadealpha`，以 fixed-step/prewarm snapshot 和 live particle ID 保存有界轨迹。custom/missing shader 与显式空 renderer 继续 fail closed；项目 RopeTrail 预算不是官方数值边界 | Rope 仍未执行；RopeTrail 的 subdivision、UV scale/scroll、animated TEX、非 Screen/world-space、multi renderer、`fadesize`、死亡尾迹、pause/seek/reset 与 whole-scene 聚合预算未实现。普通 Sprite 仍缺 trim pivot、多 sequence/multi-image 异尺寸与 Windows atlas golden；粒子 live override、动态 world-space transform、Lighting 和 custom shader executor 仍缺失 | **B4** |
