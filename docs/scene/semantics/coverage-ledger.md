...
> 最近核对：2026-07-30
>
> Scene 实现基线：`34212326`（在 `44b56cfc` 的 typed render-state 基线上，把 direct text 与内嵌 MP4 统一为显式内容 generation publication；视频按共享 `SceneFrameTiming` 求 item time，由 launch-scoped、per-device/source registry 跨 surface rebuild 复用，并随 Scene pause/resume/stop 管理。frame registry 拒绝 generation 回退、同代换纹理和 publication/texture 不匹配；这不是通用 video/material replacement、seek、首帧黑场策略或 Windows 播放 parity）
>
...
>
> 最新聚焦门：`34212326` 的签名 Debug App 对含两路内嵌 MP4、动态文字、粒子和 effect 的 `3743305891` 运行 `.codex/scene-video-provider-lifecycle-targeted-20260730-v7/report.json`，**1/1 PASS**，报告 SHA-256 `2167dbd6af183414458bd393efa8137b1e4aaec356e8f516fa0c5e596b7fe57c`。preview 日志记录 video layer `147/23` source ready；ready/6 秒 after 均非黑，`changed_ratio=0.821885`，completed frame **209**、failed frame **0**、driver **59.82 FPS**，stop 后 surface `1 -> 0`；人工 montage 未见黑块、异常裁切或 alpha 破坏。最终源码 Scene **147 modules ALL OK**，代码健康 669 Swift files / 44 locked legacy / 400-line limit，`script/build_and_run.sh verify` 通过。本批未重跑 fixed13/full45；真实门证明启动、连续画面与 stop，不把纯状态机门外推为真实系统 pause、screen hot-plug、seek/loop 首帧或 Windows parity。
