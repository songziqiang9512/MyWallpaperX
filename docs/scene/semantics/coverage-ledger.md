...
> 最近核对：2026-07-26
>
> Scene 实现基线：`678a052` 加当前 stock 资产批次（Puppet bind-pose mesh `8bac86e`，静态 MDAT attachment `49ee89a`，严格单 clip MDLA/LBS 播放 `f1ee79b`；strict particle eventspawn/natural-eventdeath `f4173ea`/`7d53c10`、eventfollow `928acca`、持续 child `2f897bc`、static child `899704b` 与有限 static origin `678a052`；22-key 程序纹理 registry 作为 bundle 缺失回退；919 个播放候选物理路径中有 223 TEX，particle 164 项已直接按官方路径查找；15 个 stock 字体也已合并到同一 bundle；非音频 turbulent velocity `4a17ee6`；超预算/多 image BC 颜色归一 `18d0056`）
>
...
| 字体解析/fallback | `L3` | 解析顺序 alias → 包内文件 → 客户端自带（stock）→ 缺失，路径安全判定优先于后三层。官方 `systemfont_*` 别名共 8 个（`arial`/`verdana`/`segoe`/`sansserif`/`consolas`/`comicsans`/`cambria`/`calibri`），`consolas` 缺失时退到 Menlo 保住 fixed-pitch。客户端 `assets/fonts` 的 15 个官方名字现在全部物理位于 `SceneStockAssets.bundle/assets/fonts`：8 个许可允许再分发的原版报 `stockBundled`；7 个禁止再分发的原文件换成许可替代 payload，并复制成官方文件名，直接按作者路径读取，报 `stockSubstituted` + `stockFontSubstituted:<category>`。9 份许可与 `NOTICE.md` 同包保留；包缺失或损坏才报 `stockApproximation`。**等级仍为 `L3`**：7 个替代字形的轮廓、advance 与 kerning 不是官方字形，8 个原版也无 Windows 栅格化逐像素对照，见 [E-TEXT-FONTREF](runtime-evidence-index.md#e-text-fontref) |
