...
> 最近核对：2026-07-29
>
> Scene 实现基线：`27040be`（在原始项目目录直达宿主与内存 `SceneRuntimeInput` 基础上，包含 image/solid 作者 `alignment` quad pivot、exact single-texture Blend、identity-only Transform、exact Workshop Simple Audio Bars、strict linear/direct-draw/gradient Light Shafts quad、Workshop Clipping Mask 作者顺序合成、strict Cursor Ripple 跨帧波场、effect-instance Foliage Sway / Water Ripple 资源身份与 Foliage `util/noise` consumer、exact modern static Shine 五 pass、strict particle REFRACT 双纹理/前序 framebuffer snapshot、static particle General/Movement/Renderer world-space、16/32/64 host spectrum、完整 authored utility-chain capture、launch-owned lazy pipeline repository、结构化帧性能遥测、透明粒子剔除与内容寻址 PKG cache；逐能力落地提交见专项表和运行证据索引）
>
> 当前完整快照门：`.codex/scene-foliage-full45-v2-20260729/report.json`（`27040be`），**45/45 PASS**；当前 fixed13 仍为 `.codex/scene-particle-worldspace-fixed13-20260729-v1/report.json`（`54a6ebc`），**13/13 PASS**
>
> 最新运行门：`27040be` 复用从真实只读目录重建的 45 样本隔离副本；final-matrix full45 **45/45 PASS**，particle `127/130`、strict graph 220 stage/34 chain、Foliage Sway 5、Water Ripple 7、Shine 6（isolated 3）、Cursor Ripple 3（isolated 3）、Clipping Mask 8、graph failed 0。报告/full matrix SHA-256 为 `78208141c995671445cf8ca7d18d12c3370ec7adb2f3034f4e409bfa07b5b80e` / `906ca2e416520f0c83c2e85e179c36f20a6d9cbd865e54fe1182b99813273e91`。最近 fixed13 仍为 `54a6ebc` 的 **13/13 PASS**，particle `76/76`、REFRACT root batch 7，报告/fixed matrix SHA-256 为 `6f49ee0024a5bfcda2bbea0f1d90aa8c734a9f6492d306bd377c0c9f6657aa9e` / `2a405c543e586d9c4d004538c5f719ecaad75d0dc57ce560011ba16fd9f92241`。本批代码健康 620 Swift files、44 个锁定历史文件、400 行上限，完整 Scene suite 642 项通过、2 项按既有条件跳过，Developer ID 签名 Debug build 通过。聚合缺口、性能/视觉边界和签名身份见 [运行证据索引](runtime-evidence-index.md)。
...
| **B2 Graph Resource Runtime** | `S2 第 1-5 项` | strict Blur、stock Local Contrast、exact Workshop `shadow_____________`、exact stock Opacity、exact stock Shake、exact Workshop Clipping Mask、exact modern static Shine、exact Cursor Ripple 与 ordered strict effect-chain 已消费 target table；cache/resize/reset、整链原子 allocation、D7 ShaderContract IR v1、BGRA/RGBA/RG/R8 format、exact shader fingerprint、同帧 copy/swap foundation、受限 history seed/clear、Cursor Ripple 跨帧 history、Precise Blur 两种 material-command interleave 与 exact legacy compose 归一化已完成；generic compose/scene-background、composition effect consumer、通用 history consumer 与 typed shader defaults/built-ins/state 未完成 | read/write、RT lifecycle、slot/combo/state 和 resize/switch/stop 门；下一批用 preview 并排图和同样本分项变化验证画面收益 |
...
研究可以并行，产品执行不能倒置：B0 live-property、direct dynamic text generation、Timeline 的受限 typed target、16/32/64 档 Scene audio、stock Shake/Pulse 与 exact Simple Audio Bars consumer、B2 ordered strict chain、Workshop Shadow、Workshop Clipping Mask、stock Opacity、exact stock Shake/Foliage Sway/Water Ripple/Water Waves/Water Flow/X-Ray/God Rays、modern static Shine、Cursor Ripple、strict linear/direct-draw/gradient Light Shafts quad、single-texture Blend、identity-only Transform、同帧 copy/swap foundation、受限 history seed/clear、Precise Blur material-command interleave 与 legacy compose 归一化已合龙；stock 资源子集、透明粒子静态剔除、非音频 turbulent velocity、strict child 子集、超预算/多 image BC1/2/3 GPU premultiply 与静态首帧 fallback 已进入公共 runtime。unsupported Effect/Timeline/粒子/SceneScript 形态继续 fail closed。当前完整门为 220 stage/34 chain/Foliage Sway 5/Water Ripple 7/Shine 6（isolated 3）/Cursor Ripple 3（isolated 3）/Clipping Mask 8/0 failed；`2998757800:13` 新增完整 `[Foliage, Foliage, Water Ripple, Water Ripple]` 链并以四份 effect-instance 资源执行。三个 Shine 在 `[Blur Precise, Shine]` 完整 text chain 中执行，另三个 Shine 与三个 Ripple 都是明确报告省略 sibling 的 isolated degraded stage，不代表所在 Shake/Tint/Blend/Reflection/Sharpen/Water Ripple/Pulse 整链已经执行。`3765760121:187` 是 project layer，仍不进入 image effect executor；该样本已有 image layer 74 的单 Foliage 执行，不能据此外推组合层头发/叶片已完整修复。legacy Shine、Timeline/user 动态值与 `COPYBG` 继续 fail closed。混合链纹理预算按实际 target extent 折算，超过默认 128 MiB（八个 2048×2048 BGRA units）时在规划阶段整链拒绝。Light Shafts 是独立 clean-room 近似：`point0...3` 已与世界几何分离为 effect-local perspective UV，standalone quad extent 仍是由 stock preview/真实实例支持的 half-canvas 有界推断；radial/corner/color/mask/chain 形态保持关闭。Simple Audio Bars 真实门为系统静音，只证明 stage/capture/稳定零输入；注入式非零 GPU 门不等于 Windows visual golden。206、两个 Light Shafts 正例、两个 Clipping Mask 正例与三个 Shine 样本的人工截图只证明方向性变化；Foliage/Water Ripple 没有固定时钟/逐像素 Windows 对照，Shine 没有固定时钟/逐像素对照，Cursor Ripple benchmark 没有脚本化鼠标轨迹截图，均不能冒充 Windows visual golden。lazy pipeline A/B 没有证明单屏启动加速，Performance budgets 仍为 `L0`。下一代码批仍须从开发计划和专项缺口重新选择，不从 bounded 正门外推通用 shader、project/composition effect source、Transform、Audio Bars、Light Shafts、Clipping Mask、Shine、Cursor Ripple 或 Effect 兼容。
