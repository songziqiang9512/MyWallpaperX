...
> 最近核对：2026-08-01
>
> Scene 实现基线：`fe780ef5`（普通 image/solid 已消费作者 `color × brightness`；精确 delayed-loop texture script 已建立实例级 stop/play 时钟；Tint 现可从 definition/material 推导资产身份，并对精确 material/shader 内容命中的 authored relocation 执行槽位 1 遮罩；content-verified fullscreen Color Grading `TOOLS=2` 已进入 utility capture 与 ordered effect chain。`3768903841` 已恢复暖桃色、烟花错峰、masked Tint 天空压暗与作者 luminance/saturation/vibrance；Worley 云形、底部射灯视觉和四个日期时间 text script 仍未闭合）
>
...
>
> 最新公共合同门：`fe780ef5` 的 Color Grading planner **3/3 PASS**、GPU **3/3**、framebuffer **35/35**、chain planner **5/5**、authored execution **11/11**、benchmark contract **59/59**、语义目录 **2/2**；代码健康 691 Swift files / 44 locked legacy / 400-line limit，`script/build_and_run.sh --verify` 通过。上一 `4622b8bb` Light Shafts 聚焦门 **38/38 PASS** 与 Scene **150 modules ALL OK** 仍是对应能力的最近证据，不能外推到本批。
...
| Exact native texture-animation delayed-loop profile | `L3 bounded` | 精确 source SHA、`visible` host、完整 `script/scriptproperties/user/value` wrapper 与三个 bounded delay 数值共同准入；命中的多 image playback 以实例级 scene-time 状态机执行首轮 stop、逐层 initial delay、末帧后 reset 与确定性独立随机等待，不创建系统 timer；未知/变异脚本维持原生 autoplay | 不执行 JavaScript，不开放通用 `ITextureAnimation` handle、detach/join、rate/pause/seek/frame mutation 或 owner timer；只覆盖一个已验证 delayed-loop profile，见 [E-PUPPET-BC](runtime-evidence-index.md#e-puppet-bc) | **B4** |
