...
| Particle runtime | `L3` | 作者 sprite、常见组件、Sprite Trail、13 个精确 built-in key；固定门 `18/27`、完整门 `83/131` | 逐项状态见粒子专项表 | **B4** |
...
| Built-in textures | `L3` | 13 个精确 key；程序图形只保证确定性，不等于官方资产 | `rain_drops_sheet` 等剩余高频 key、atlas metadata、多纹理 material |
...
当前固定矩阵可见粒子为 `18/27`，完整矩阵为 `83/131`（REFRACT 材质层 fail closed 不计入）；固定门 `3724289844` 的 Sakura 为 `5/5`，`3750813609` 是 `7/9`，后者另外两层因 world-space 不支持而保持 fail closed。这些数字只度量对应矩阵实际加载的 layer，不代表粒子组件覆盖率。
