...
> 最近核对：2026-07-29
>
> Scene 实现基线：`e698c18`（在原始项目目录直达宿主与内存 `SceneRuntimeInput` 基础上，包含 image/solid 作者 `alignment` quad pivot、exact single-texture Blend、identity-only Transform、exact Workshop Simple Audio Bars、strict linear/direct-draw/gradient Light Shafts quad、Workshop Clipping Mask 作者顺序合成、strict particle REFRACT 双纹理/前序 framebuffer snapshot、16/32/64 host spectrum、完整 authored utility-chain capture、launch-owned lazy pipeline repository、结构化帧性能遥测、透明粒子剔除与内容寻址 PKG cache；逐能力落地提交见专项表和运行证据索引）
>
> 当前完整快照门：`.codex/scene-particle-refraction-full45-20260729-v2/report.json`（`e698c18`），**45/45 PASS**；当前 fixed13 为 `.codex/scene-particle-refraction-fixed13-20260729-v2/report.json`（`e698c18`），**13/13 PASS**
>
> 最新运行门：`e698c18` 使用从真实只读目录重建并复核未变的 45 样本隔离副本；fixed13 **13/13 PASS**，particle `71/76`、REFRACT root batch 5；full45 **45/45 PASS**，particle `120/130`、REFRACT root batch 11。报告/full matrix SHA-256 为 `989d169aa6702ac4004d03b43a64cf29bf335a086ef507e1cb195b07f7f9fd21` / `0faa11fb0f4d7829b2405e76e5e1b7cb8a833f07085eb27f777a8f5345629469`；报告/fixed matrix SHA-256 为 `88e66b867247c67c60e4d5ff55c1e5c5e404897b332fc1326d33d944560f2c8d` / `0d8dfdf78bbc472867cd84d7fd965625d1c6991486a6e99f57ddef2e174698f1`。Scene suite 共运行 630 项并通过（2 项按既有环境条件跳过），代码健康 593 Swift files、44 个锁定历史文件、400 行上限，Developer ID 签名 Debug build 通过。聚合缺口、性能/视觉边界和签名身份见 [运行证据索引](runtime-evidence-index.md)。
...
| Bounded effect executors | `L3` | 两个严格 Blur 图、strict stock Local Contrast、exact Workshop Shadow、exact stock Opacity、Shake、Water Waves、Water Flow、Foliage Sway、Water Ripple、X-Ray、Tint、Pulse、God Rays、exact single-texture Blend、identity-only Transform、exact Simple Audio Bars 与 exact Workshop Clipping Mask 共十八类 bounded backend 的 ordered strict chain；一次 launch 的 immutable effect pipelines 按需构造并跨 surface/rebuild 复用，失败缓存；另有受限 authored-source 单 material/framebuffer-only frontend | 其他 Effect/variant、composition Clipping Mask consumer、非 identity Transform、其他 Blend/Audio Bars mode、任意 include/combo/material texture/render state、动态/递归控制流、SceneScript/mixed graph 与 Windows visual golden；pipeline repository 未证明单屏启动加速，真实双屏运行门仍缺 | B3/B4 |
...
| Generic FBO command graph | `L2` | target/bind/compose/copy/swap/condition/function 可保留或 blocker；effect-scoped target/lifetime table 已由 bounded strict backend 与 ordered chain 消费；同帧 copy/swap、unique history seed、Precise Blur material-command interleave 与 legacy compose 归一化已有严格门；exact Shake/Foliage/Water/X-Ray/Blend/identity Transform 等只在完整 profile 和连续输入成立时执行 | 上述 exact profile 为受限 `L3`；particle REFRACT 使用独立的前序 framebuffer snapshot，不代表 generic FBO 已接 scene-background；generic compose、Effect Refraction、真实 history consumer、condition/function、typed state、跨帧 logical swap 和完整生命周期仍缺失 | B2 |
...
当前 `e698c18` 完整矩阵可见粒子为 `120/130`，fixed13 为 `71/76`；两门的初始 REFRACT root batch 分别为 11 与 5。`2131872317` 的 event-death child REFRACT 在初始报告中仍为 0，另由 7 秒延迟 runtime 门证明生成非空折射 batch。`3088601835` 为 `19/19`，其中 Snow root layers `513/534` 已执行，static `snowstormfog` child 也已由真实缓存门确认在两层生成实例，matrix-code 两层随 `b86db59` 解除 nested 阻断进入执行。child 不增加 root loaded-layer 计数。`3769364482` 为 `3/3`、`3757555836` 为 `7/7`、`2974757317` 为 `3/3`（含 matrix 头层 85705），`3750813609` 是 `7/9`；`2998757800` 与 `3769688830` 仍分别有 2/1 个 World Space 粒子层失败关闭，`3770444459` 另有 Rope/Boids 层失败关闭。这些数字只度量对应矩阵实际加载的 root layer，不代表粒子组件覆盖率或 Windows 视觉等价。
...
| **B2 Graph Resource Runtime** | `S2 第 1-5 项` | strict Blur、stock Local Contrast、exact Workshop `shadow_____________`、exact stock Opacity、exact stock Shake、exact Workshop Clipping Mask 与 ordered strict effect-chain 已消费 target table；cache/resize/reset、整链原子 allocation、D7 ShaderContract IR v1、BGRA/RGBA/RG/R8 format、exact shader fingerprint、同帧 copy/swap foundation、受限 history seed/clear、Precise Blur 两种 material-command interleave 与 exact legacy compose 归一化已完成；generic compose/scene-background、composition effect consumer、真实 history consumer 与 typed shader defaults/built-ins/state 未完成 | read/write、RT lifecycle、slot/combo/state 和 resize/switch/stop 门；下一批用 preview 并排图和同样本分项变化验证画面收益 |
...
研究可以并行，产品执行不能倒置：B0 live-property、direct dynamic text generation、Timeline 的受限 typed target、16/32/64 档 Scene audio、stock Shake/Pulse 与 exact Simple Audio Bars consumer、B2 ordered strict chain、Workshop Shadow、Workshop Clipping Mask、stock Opacity、exact stock Shake/Foliage Sway/Water Ripple/Water Waves/Water Flow/X-Ray/God Rays、strict linear/direct-draw/gradient Light Shafts quad、single-texture Blend、identity-only Transform、同帧 copy/swap foundation、受限 history seed/clear、Precise Blur material-command interleave 与 legacy compose 归一化已合龙；stock 资源子集、透明粒子静态剔除、非音频 turbulent velocity、strict child 子集、超预算/多 image BC1/2/3 GPU premultiply 与静态首帧 fallback 已进入公共 runtime。unsupported Effect/Timeline/粒子/SceneScript 形态继续 fail closed。当前完整门为 204 stage/30 chain/Clipping Mask 8/0 failed；其中 8 个 stage 只覆盖两个样本的 classic image-layer profile，12 个 composition 声明、1 个 unsupported sibling 和 2 个隐藏声明未执行。混合链纹理预算按实际 target extent 折算，超过默认 128 MiB（八个 2048×2048 BGRA units）时在规划阶段整链拒绝。Light Shafts 是独立 clean-room 近似：`point0...3` 已与世界几何分离为 effect-local perspective UV，standalone quad extent 仍是由 stock preview/真实实例支持的 half-canvas 有界推断；radial/corner/color/mask/chain 形态保持关闭。Simple Audio Bars 真实门为系统静音，只证明 stage/capture/稳定零输入；注入式非零 GPU 门不等于 Windows visual golden。206、两个 Light Shafts 正例与两个 Clipping Mask 正例的人工截图只证明方向性改善；lazy pipeline A/B 没有证明单屏启动加速，Performance budgets 仍为 `L0`。下一代码批仍须从开发计划和专项缺口重新选择，不从 bounded 正门外推通用 shader、Transform、Audio Bars、Light Shafts、Clipping Mask 或 Effect 兼容。
