...
> 最近核对：2026-07-30
>
> Scene 实现基线：`b9bdbd5e`（在 `9a6a028` 的 native 64-band Audio Bars 与既有 bounded effect/resource/runtime 基线上，新增所有 Effect 辅助纹理调用的显式用途合同：exact Blend 保持 premultiplied color，其余 mask/flow/phase/normal/noise/gradient/X-Ray 通道保持原始通道；X-Ray 的同一用户属性可同时建立普通颜色与 preserved-channel 两种纹理身份，逐能力落地提交见专项表和运行证据索引）
>
...
>
> 最新聚焦门：`b9bdbd5e` 的签名 Debug App 对 `3028090166`、`3750813609`、`3770444459` 运行 `.codex/scene-effect-aux-purpose-20260730-v1/report.json`，**3/3 PASS**，报告 SHA-256 `de12beeb9f62416a60a601eac55c107d1a06fde5fd7dad050e3ee3a9202228b4`，image texture 均 loaded 1.0、failed frame/drawable miss 均为 0；自动门共 **101 项通过 / 1 项按既有环境条件跳过**，代码健康为 649 Swift files / 44 locked legacy / 400-line limit，签名 Debug build 与严格 codesign 通过。`3028090166`/`3770444459` 的构图未出现新增黑块、接缝或 alpha 损伤；`3750813609` 只证明 BC3 Foliage mask 装载和 mapped UV `(0.9375, 0.52734375)` 进入有界 legacy route，其时钟、雨和叶片粒子仍明显不同于官方截图。该聚焦门没有重跑 fixed13/full45，也不构成 Wallpaper Engine 动态或像素等价。
