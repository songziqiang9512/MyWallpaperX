...
> 最近核对：2026-07-26
>
> Scene 实现基线：`678a052` 加当前 stock TEX resolver 批次（Puppet bind-pose mesh `8bac86e`，静态 MDAT attachment `49ee89a`，严格单 clip MDLA/LBS 播放 `f1ee79b`；strict particle eventspawn/natural-eventdeath `f4173ea`/`7d53c10`、eventfollow `928acca`、持续 child `2f897bc`、static child `899704b` 与有限 static origin `678a052`；22-key 程序纹理 registry 作为 bundle 缺失回退；311 项 stock identity 中 particle 164 项已按 catalog 路由到同路径 TEX；非音频 turbulent velocity `4a17ee6`；超预算/多 image BC 颜色归一 `18d0056`）
>
...
>
> 最新运行门：`678a052` 的真实目录 45 个可运行样本完整快照 45/45、particle 101/131；另有 `3770500543` 缺 package 未进入矩阵。固定 13 样本门 13/13、particle 19/27，static-origin 定向门 1/1、particle 17/19；这些历史门仍单独保留且不能互相替代。当前 stock TEX 批次的固定门本机仅 6/13 样本可用，六个均完成运行、image 82/82、particle 11/13、sample-root residue 0，且无 `builtInTextureUnavailable`/`textureLoadFailed`；但矩阵 interpretation v25 与当前 v29 及 `2902406982` strict graph 断言过期，不能宣称固定门 PASS。此前 20/22-key 的 unavailable census 保留为接 bundle 前历史，不代表当前 catalog 可达性。完整 Scene suite 435 项：433 通过、2 项跳过；代码健康 453 Swift files、44 个锁定历史文件、400 行上限；语义覆盖 11/11；签名 Debug 构建和 `codesign --deep --strict` 通过。preview 方向性证据基线为 `3194ac5`；聚合缺口、视觉边界和签名身份见 [运行证据索引](runtime-evidence-index.md)。
...
| Particle runtime | `L3` | 作者 sprite、常见组件、Sprite Trail；2.8.42 的 311 项 stock TEX 与 298 项 sidecar 已按官方原路径全部建立占位，资产身份库存完整；其中 particle 164 项已按 exact identity 路由到同路径 TEX，样本本地资源优先，22-key 程序纹理只作 bundle 缺失回退；素材替换保持 catalog 路径，后续 consumer 必须复用这些 identity | 库存完整不等于执行完整：当前只有 Particle slot 0 是真实 consumer，resolver 只使其余 identity 达 `L2 wired`；311 个 TEX 均封装同一 16x16 占位，无对应官方尺寸/通道/mip/atlas/像素语义；298 个 sidecar 也仅是占位且不进入 runtime | **B4** |
...
| 字体解析/fallback | `L3` | 解析顺序 alias → 包内文件 → 客户端自带（stock）→ 缺失，路径安全判定优先于后三层。官方 `systemfont_*` 别名共 8 个（`arial`/`verdana`/`segoe`/`sansserif`/`consolas`/`comicsans`/`cambria`/`calibri`，只读安装目录内各出现 6~8 次），此前表内只有 4 个：本机语料引用最多的 `systemfont_consolas`（480 次原始引用）走"未知别名"分支落到比例字体 Helvetica，等宽版式错位，现在 Consolas 缺失时退到 Menlo 保住 fixed-pitch。客户端 `assets/fonts` 下 15 个 stock 字体（另有 4 个 license `.txt`）；`fonts/X` 的官方语义是先包内、后客户端自带，本机语料 244 个去重 text layer 归属为包内 184、stock 41、alias 19、**未知 0**，其中 41 个 stock 引用此前全部被误报 `missingBundledFont` 并静默变成 Helvetica。现在 15 个名字**全部有随 app 分发的真实字体文件**（`SceneStockFonts.bundle`，2.9 MB，13 字体 + 9 份许可 + `NOTICE.md`）：逐个核验上游许可后，8 个许可允许再分发的搬官方原版（逐字节相同，含 SHA-256 反证为 OFL 的 `Blackout 2 AM`）报 `stockBundled`、无诊断；7 个原文件禁止再分发的（Atami 的 EULA 明确写不得重新打包分发）换成逐个并排比对 "Hamburg 0123" 字形选出的自由字体，报 `stockSubstituted` + `stockFontSubstituted:<category>`。类别近似（Menlo/Noto Sans/Apple Color Emoji/Helvetica）降级为只在 app 包缺失或损坏时兜底，报 `stockApproximation` + `stockFontFileUnavailable:<category>`。表外别名与"表内但本机缺字体"分别报 `systemAliasUnknown` / `systemAliasUnavailable`，别名表是否补全在诊断里可观察。**等级仍为 `L3`**：7 个替代字形的轮廓、advance 与 kerning 都不是官方字形，8 个原版虽逐字节相同但栅格化、hinting 与 LCD 滤波无 Windows 逐像素对照，family/weight/CJK golden 也仍缺，见 [E-TEXT-FONTREF](runtime-evidence-index.md#e-text-fontref) |
