...
> 最近核对：2026-07-30
>
> Scene 实现基线：`df9e3ef3`（受限 framebuffer-only authored shader 的静态常量除 uniform 名外，可按 uniform 声明同行 annotation 中精确、非空、无首尾空白的字符串 `material` 键读取；direct key 与 annotation key 同时存在、同一 material key 跨 uniform 复用、动态 user/timeline、缺值和非字符串 annotation 都失败关闭。这不消费 annotation `default`、sampler/material texture、combo/condition/function/include、外部 slot 或新的 render state）
>
...
>
> 最新聚焦门：同一签名 Debug App 对隔离 `3768020435` 运行 `.codex/scene-authored-material-annotation-targeted-20260730-v2/report.json`，**1/1 PASS**；layer `389` authored shader/stage **1/1**、graph failed layer、failed frame、drawable miss 与 sample residue 均为 0。报告/matrix SHA-256 为 `d8000621672c5508ea47d3ac2a8fe024fff1d67a4d05e8dfd8a41ba88e23853a` / `ff3707d667c14bb5ad552b23a5d47a01857701ba1e8b35af646bc4aaccbd7618`；App executable SHA-256 `4185846e1def81bb7e8749980afeec970091d4fae5134d39ea91271679b8c8ef`、CDHash `ace668504890b95d640e7857546302bc647143d0`，签名前后 verified。同行 annotation 的作者常量实际绑定 `0.75`，同样本前后图显示 Spotify 封面从硬直角变为作者参数控制的透明圆角；这是方向性同样本证据，不是 Windows pixel golden。planner GPU 门同时锁定 direct+alias、动态 binding、缺值与 alias collision 失败关闭。最终源码 Scene **780 tests 通过、2 项既有 skip**，相关回归 **26/26 PASS**，代码健康 669 Swift files / 44 locked legacy / 400-line limit，`script/build_and_run.sh verify` 与当前 full45 均通过。
