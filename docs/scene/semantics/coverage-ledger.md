...
> 最近核对：2026-07-31
>
> Scene 实现基线：`7866dbc0`（`69cc7f01` 保留 FBO `width`/`height`/`fit`/`scale` 的独立与组合声明，并让 allocation 与 2048² preflight budget 共用 override -> fit -> scale -> floor/min-1 算法；`7866dbc0` 以 canonical path、size、mtime、device、inode 与 ctime 建立普通文件 generation，并在 image/TEX/video 读取或发布前后复核同一 revision。两项都保持 generic executor、未知 metadata 与未证形态失败关闭）
>
...
>
> 最新公共合同门：`69cc7f01` 的 effect-chain / effect graph / target plan / offscreen pool 定向门分别为 **5/5、8/8、11/11、13/13**；`7866dbc0` 的 texture candidate 与 video source 门分别为 **1/1、7/7**。最终 checkout 的 Scene **150 modules / 794 tests ALL OK**、11 skipped，代码健康 677 Swift files / 44 locked legacy / 400-line limit，`script/build_and_run.sh verify` 通过。最终签名 Debug App 对隔离 `3299228616`、`3750342273`、`3765760121` 运行 `.codex/scene-texture-fbo-targeted-20260731-v3/report.json`，**3/3 PASS**、loaded ratio 均为 1，合计 completed frame 713，failed frame、drawable miss、graph blocker 与 sample residue 均为 0；报告/full matrix SHA-256 为 `e75bf6d61f67b1f3d4d77cdec56d56c0d65af5ad6825256cb2c8d8fc10d5e916` / `bc92b3886c1e4f82e53f5f1a211fe0a21e1bbb17580d61d7704b1adc6b933e7e`，App executable SHA-256 `b8fabde3b29ead5872e38ed9d5dc297504e04013656e2098764c7d4af3b5e1b8`、CDHash `414302be3f113596c8e0c5b148e8f21c2c90312d`，签名前后 verified。三份 graph SHA 与现役 full45 一致，montage 人工未见新增黑块、硬矩形、裁切接缝或 alpha 异常。本批没有重跑 fixed13/full45；该真实门只证明 strict fit/scale、静态纹理读取与既有图结构在共享改动后无回归，不是 arbitrary composed FBO 正向执行、文件热替换 runtime 门或 Windows parity。D6 仍是 generic `L2`，本机普通文件 revision 门也不等于跨文件系统 content digest。
...
| Generic FBO command graph | `L2` | target/bind/compose/copy/swap/condition/function 可保留或 blocker；FBO `width`/`height`/`fit`/`scale` 现按独立字段保真并支持组合形态，公共 extent resolver 依次执行单轴/双轴 override、fit 不放大、scale divisor、floor/min-1，allocation 与 2048² preflight budget 共用该算法；effect-scoped target/lifetime table 已由 bounded strict backend 与 ordered chain 消费；同帧 copy/swap、unique history seed、Cursor Ripple exact 非 unique history 白名单、Precise Blur material-command interleave 与 legacy compose 归一化已有严格门；exact Shake/Foliage/Water/X-Ray/Blend/identity Transform 等只在完整 profile 和连续输入成立时执行 | 上述 exact profile 为受限 `L3`；composed extent 仍没有 generic material/FBO executor 或真实正向 consumer；particle REFRACT 使用独立的前序 framebuffer snapshot，不代表 generic FBO 已接 scene-background；generic compose、Effect Refraction、condition/function、通用 render-state translator/cache、未映射 state、通用跨帧 logical swap 和完整生命周期仍缺失 | B2 |
