...
|---|---|---|---|---|
| PKG/TEX/资源索引 | `L3` | loose/PKG/stock 查找、常见 TEX、内嵌 MP4、诊断式失败；PKG extraction 以 package/content SHA 和 exact file set 校验损坏/陈旧 cache，单次读取包体；同一 surface 复用 TEX parse 与 device texture，文件 size/mtime 变化失效；BC1/2/3 的 color/data purpose 进入独立 cache/upload identity：单 image 预算内 color CPU premultiply、preserved-channel 保留通道与物理 extent，超预算/多 image color GPU premultiply、data native BC；跨 image color sprite 当前只裁 authored 首帧作静态 fallback | 跨进程 extraction lock、同 size 且恢复同 mtime 的纹理替换、完整跨-image sprite animation、旋转 frame、Effect 辅助纹理的 typed purpose、BC5 consumer、case/symlink/duplicate/多格式边界与完整 VFS golden | B0 |
...
| User Properties | `L3` | 独立窗口、条件、持久化、PNG/JPEG `sceneTexture`；layer alpha、纯 solid color、direct text、strict Local Contrast/Opacity 与受限 X-Ray target 已无重建 live 更新；exact Simple Audio Bars `Bar Color` 与 relocated 16-band profile 的 `Opacity` 已有 typed compiler/snapshot/GPU consumer | Audio Bars 尚无真实 live override 门；unsupported/mixed/SceneScript bindings、Texture Variants、shortcut、跨重启 UI 门；精确 census 见 runtime-input 专项表 | **B0/B1** |
