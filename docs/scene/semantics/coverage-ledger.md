...
> 最近核对：2026-07-28
>
> Scene 实现基线：`88510a6`（在原始项目目录直达宿主与内存 `SceneRuntimeInput` 基础上，包含 image/solid 作者 `alignment` quad pivot、exact single-texture Blend、identity-only Transform、exact Workshop Simple Audio Bars、strict linear/direct-draw/gradient Light Shafts quad 及独立 effect-UV/几何修正、16/32/64 host spectrum、完整 authored utility-chain capture、launch-owned lazy pipeline repository、结构化帧性能遥测、透明粒子剔除与内容寻址 PKG cache；逐能力落地提交见专项表和运行证据索引）
>
> 当前完整快照门：`.codex/scene-light-shafts-full45-20260728/report.json`（`49a5878`），它早于 image/solid alignment 修复，不能证明当前几何合同；当前 fixed13 为 `.codex/scene-image-alignment-fixed13-20260728-v1/report.json`（`88510a6`），**13/13 PASS**
>
> 最新运行门：`88510a6` 的 image/solid alignment 隔离定向门 **2/2 PASS**，`3122339805` 与 `3767460992` 均 loaded 1.0、sample-root residue 0；报告/full matrix SHA-256 为 `f66cb394e9172fd7ce00126ada36af6bcd5a54b8603ca4d7e4f7cd85912722a8` / `f198ab106341a46530806e40219181176ac61d98edbdd7bff2d1f3032071c998`。同一签名 App 的 fixed13 **13/13 PASS**，报告/fixed matrix SHA-256 为 `7a81204b6e7c4b2a60867adc951487fcbbd6a364ef64d22c8f98fbeffc39ba87` / `a41d0da9ec80b23290f3adb412efa36c89a6a92f95a79e0f49f50261ae59f00d`。最近完整门仍是 `49a5878` 的 **45/45 PASS**，不证明当前 alignment 修复。Scene suite 共运行 622 项并通过（2 项按既有环境条件跳过），代码健康 583 Swift files、44 个锁定历史文件、400 行上限，签名 Debug build/codesign 通过。聚合缺口、性能/视觉边界和签名身份见 [运行证据索引](runtime-evidence-index.md)。
...
| Generic FBO command graph | `L2` | target/bind/compose/copy/swap/condition/function 可保留或 blocker；effect-scoped target/lifetime table 已由 bounded strict backend 与 ordered chain 消费；同帧 copy/swap、unique history seed、Precise Blur material-command interleave 与 legacy compose 归一化已有严格门；exact Shake/Foliage/Water/X-Ray/Blend/identity Transform 等只在完整 profile 和连续输入成立时执行 | 上述 exact profile 为受限 `L3`；generic compose、Refraction scene-background capture、真实 history consumer、condition/function、typed state、跨帧 logical swap 和完整生命周期仍缺失 | B2 |
