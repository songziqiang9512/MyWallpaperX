...
> 最近核对：2026-08-01
>
> Scene 实现基线：`4622b8bb`（在既有 FBO、资源 generation、粒子 shader/renderer fail-closed、跨 image sprite 与 legacy Procedural Noise 基线上，新增 exact stock standalone `DIRECTDRAW=1 + RAYMODE=1` Radial/Color Light Shafts；Corner、Gradient 混合、动态 binding、实例纹理和未知组合继续失败关闭）
>
