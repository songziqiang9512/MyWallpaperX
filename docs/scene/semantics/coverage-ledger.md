...
> 最近核对：2026-07-30
>
> Scene 实现基线：`d432d4d5`（把 Effect 的 mask/noise/flow/phase/normal 从通用 preserved identity 拆成 typed purpose，连同 straight REFRACT albedo、generic preserved 与 premultiplied color 共八类 cache/upload identity；除 composite color 外的七类角色仍按严格 source-channel policy 上传，strict REFRACT 同文件双角色只在两者均保留通道时复用物理纹理）
>
> 最近完整快照门：对应 `7fa4e508` 源码的最终签名 Debug App 在 2026-07-30 从真实只读根重新构建的 45 样本隔离副本上运行 `.codex/scene-full45-masked-blur-foliage-20260730-v1/report.json`，**45/45 PASS**、image texture **451/451**、particle **130/130**、strict stage/chain **305/55**、Foliage Sway **17**、Water Flow **14**、Water Waves **25**、Water Ripple **10**、Godrays **16**，graph succeeded/failed layer **157/0**、failed frame、drawable miss 均为 0。报告/full matrix SHA-256 为 `3996d23f9b5a974c7a44eb0ed42de232d63054365356b8f63485b60a2f1c9c34` / `1588bf8fdfca04c4f5bca7c28f54b42280027bbf1c34ba23d4cb065db491e532`，隔离前后 source/isolated changes 均为空。历史 fixed13 仍为 `.codex/scene-fixed13-ropetrail-20260729-v1/report.json` 的 **13/13 PASS**；它与 full45 都没有在 `d432d4d5` 构建上重跑，三类门不能互相替代。
>
> 最新聚焦门：`d432d4d5` 的签名 Debug App 对隔离 `3028090166`、`3750813609`、`3770444459` 运行 `.codex/scene-effect-semantic-purpose-targeted-20260730-v1/report.json`，**3/3 PASS**，报告 SHA-256 `a9eb9cd1f6b614bcb994d47d0f8d82d95eff7747c4ff00b331d8799db5a8cffa`。302 为 texture **8/8**、chain/stage **3/15**，377 为 texture **1/1**、particle **5/5**、REFRACT **4**、chain/stage **1/4**；三样本 loaded ratio 均为 1、failed layer/frame/drawable 均为空，ready/after 均非黑。最终聚焦自动门 **32/32 PASS**，另有受影响 loader/rendering 回归通过；代码健康 654 Swift files / 44 locked legacy / 400-line limit，签名 Debug build 通过且 benchmark 签名前后 verified。人工核对三组 ready/after 未见本批新增的黑块、透明挖空、硬矩形 mask、整图变暗或 UV 接缝；GPU 门而非整场截图证明项目内部 packed-normal storage 等价与 coverage 单次作用，仍不宣称官方恢复公式或 Windows pixel parity。
