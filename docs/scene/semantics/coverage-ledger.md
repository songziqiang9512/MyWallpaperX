...
> 最近核对：2026-07-30
>
> Scene 实现基线：`1cbc1451`（在 `7fa4e508` 的 masked Blur/Water Flow/Foliage 链路上，为三个 exact Shake profile 补齐可选 instance slot 3 opacity mask：保持 preserved channels 与 mapped UV scale，在 displaced UV 取 mask.r 混合原始/位移 source；未知第五槽、路径/material 错配和资源缺失继续整链失败关闭）
>
> 最近完整快照门：对应 `7fa4e508` 源码的最终签名 Debug App 在 2026-07-30 从真实只读根重新构建的 45 样本隔离副本上运行 `.codex/scene-full45-masked-blur-foliage-20260730-v1/report.json`，**45/45 PASS**、image texture **451/451**、particle **130/130**、strict stage/chain **305/55**、Foliage Sway **17**、Water Flow **14**、Water Waves **25**、Water Ripple **10**、Godrays **16**，graph succeeded/failed layer **157/0**、failed frame、drawable miss 均为 0。报告/full matrix SHA-256 为 `3996d23f9b5a974c7a44eb0ed42de232d63054365356b8f63485b60a2f1c9c34` / `1588bf8fdfca04c4f5bca7c28f54b42280027bbf1c34ba23d4cb065db491e532`，隔离前后 source/isolated changes 均为空。历史 fixed13 仍为 `.codex/scene-fixed13-ropetrail-20260729-v1/report.json` 的 **13/13 PASS**；它与 full45 都没有在 `1cbc1451` 构建上重跑，三类门不能互相替代。
>
> 最新聚焦门：`1cbc1451` 的签名 Debug App 对 `3738202317` 与正向回归 `3757555836` 运行 `.codex/scene-breathing-masked-shake-20260730-v2/report.json`，**2/2 PASS**，报告 SHA-256 `a3db59adf6b76b5316eee90c639c9b99cebaea4c13e328102ac5deecf9ba74cf`。373 的 layer 16 恢复为 succeeded `[16]`、Shake/Pulse/Water Ripple **2/1/1**、chain/stage **1/4**；375 保持 **1/5**，两者 failed layer/frame/drawable 均为 0。静态扫描命中的另外三个无 DIRECTION slot 3 Shake 候选以 `.codex/scene-breathing-masked-shake-candidates-20260730-v1/report.json` 跑 **3/3 PASS**（SHA-256 `e61eb45ad70a5c52410ba5d82c35c26a36936e6afed6a8fca05b912169bed0b8`），既有 chain/stage 与 succeeded layer 集合未变。当前 full matrix SHA-256 `f47997b7b4846dac82a947b31ac502c8a4a867259d62bbe43207b628900b4df7`；自动门 **85/85 PASS**、独立 Metal rendering **6 PASS / 1 环境 SKIP**、代码健康 654 Swift files / 44 locked legacy / 400-line limit，签名 Debug build 通过且 benchmark 签名前后 verified。人工只确认 373 主体动态方向恢复且未见明显黑帧/接缝/粗大 mask 伪影；没有 Windows 动态或像素 golden，不宣称相位、幅度或 pixel parity。
...
|---|---|---|---|---|
| PKG/TEX/资源索引 | `L3` | loose/PKG/stock 查找、常见 TEX、内嵌 MP4、诊断式失败；PKG extraction 以 package/content SHA 和 exact file set 校验损坏/陈旧 cache，单次读取包体；同一 surface 复用 TEX parse 与 device texture，文件 size/mtime 变化失效；PNG/JPEG、raw/embedded format 0 与 BC1/2/3 的 premultiplied-color / preserved-channel purpose 进入独立 cache/upload identity，全部 31 个 Effect 辅助纹理调用已显式选用途；data 图片只接受原尺寸、无 decode 映射、integer packed、明确非预乘 alpha 或无 alpha（skip）且使用受控 32-bit byte order 的 RGB(A/X) provider，并归一为 RGBA；其他 layout/预乘/缩放 fallback 失败关闭，避免把隐藏 RGB 静默改写；跨 image color sprite 当前只裁 authored 首帧作静态 fallback | 跨进程 extraction lock、同 size 且恢复同 mtime 的纹理替换、完整跨-image sprite animation、旋转 frame、受支持 decoder 的 straight-RGBA 来源与 Windows 通道 golden、BC5 consumer/encoding、全部辅助纹理的 physical/mapped UV 与 sampler、case/symlink/duplicate/多格式边界及完整 VFS golden | B0 |
...
| Material/blend | `L3` | `genericparticle` slot 0、additive/translucent 子集；color TEX sampler 显式传到 renderer，strict REFRACT 的 slot 0 straight albedo / slot 1 packed normal 均走 preserved-channel texture identity、各自使用独立 sampler，background 固定 linear-clamp；普通 color/additive 的既有 premultiply 合同不变 | compressed normal 的 Windows 数值/像素 golden、coverage 单次作用门仍缺；unsupported shader/blend 尚可能回退；除 REFRACT 外的多纹理/combo/render state、HDR/Lighting/Cutout、clamp-border 与 Windows 像素标定仍缺 |
...
`8b06538d` 修复 strict REFRACT 的 BC3 purpose 丢失：同一 numeric format 4 不再一律按 color premultiply。自建 Metal 门锁定 color/data 的独立 G/A 和 cache identity；`3770444459` 的 8 秒固定步长门确认 layers 239/245/248 分别有 72/77/56 个非空 REFRACT 活动帧，隔离签名 App 门仍为 5/5 particle、REFRACT 4、failed frame/drawable miss 0。等级保持 `L3`：当前没有 Windows 法线位移 golden，也没有证明水花密度、生命周期或像素等价。
