...
> 最近核对：2026-07-30
>
> Scene 实现基线：`378e68e7`（在八类 typed texture purpose 上增加原子 `SceneTextureCandidate`：同一候选携带 identity/generation、purpose、实际 physical size、已验证 mapped size、UV transform、sampler 与 pixel format；当前只接入 Water Flow flow/phase 和 Standard Blur mask 三个已证槽位，generic material slot 保持关闭）
>
> 最近完整快照门：对应 `7fa4e508` 源码的最终签名 Debug App 在 2026-07-30 从真实只读根重新构建的 45 样本隔离副本上运行 `.codex/scene-full45-masked-blur-foliage-20260730-v1/report.json`，**45/45 PASS**、image texture **451/451**、particle **130/130**、strict stage/chain **305/55**、Foliage Sway **17**、Water Flow **14**、Water Waves **25**、Water Ripple **10**、Godrays **16**，graph succeeded/failed layer **157/0**、failed frame、drawable miss 均为 0。报告/full matrix SHA-256 为 `3996d23f9b5a974c7a44eb0ed42de232d63054365356b8f63485b60a2f1c9c34` / `1588bf8fdfca04c4f5bca7c28f54b42280027bbf1c34ba23d4cb065db491e532`，隔离前后 source/isolated changes 均为空。历史 fixed13 仍为 `.codex/scene-fixed13-ropetrail-20260729-v1/report.json` 的 **13/13 PASS**；它与 full45 都没有在 `378e68e7` 构建上重跑，三类门不能互相替代。
>
> 最新聚焦门：`378e68e7` 的签名 Debug App 对隔离 `3028090166` 运行 `.codex/scene-typed-texture-candidate-3028090166-20260730-v1/report.json`，**1/1 PASS**，报告 SHA-256 `7aaba4a44f2995fb6f25e8c581ec6540ec137f59edae06eb3a1b31b55f6cf233`。样本 texture **8/8**、particle **2/2**、succeeded `[13,82,86,111,129]`、Water Flow **3**、Water Waves **6**、Foliage Sway/Water Ripple/Godrays 各 **1**、Opacity **2**、chain/stage **3/15**，failed layer/frame/drawable 均为空；preview 日志逐槽记录 typed purpose、physical/mapped、UV scale、sampler 与 pixel format。candidate 聚焦自动门 **70/70 PASS**，代码健康 658 Swift files / 44 locked legacy / 400-line limit，签名 Debug build 通过且 benchmark 前后签名 verified。人工核对 ready/after 的完整云层与龙链持续运动，未见新增黑块、透明挖空、硬矩形 mask、整图变暗或 UV 接缝；真实样本三个新槽均为 physical=mapped，padding 防泄漏与非 1 UV scale 只由 production GPU 门证明，不宣称通用 slot binder 或 Windows pixel parity。
...
|---|---|---|---|---|
| PKG/TEX/资源索引 | `L3` | loose/PKG/stock 查找、常见 TEX、内嵌 MP4、诊断式失败；PKG extraction 以 package/content SHA 和 exact file set 校验损坏/陈旧 cache，单次读取包体；同一 surface 复用 TEX parse 与 device texture，文件 size/mtime 变化失效；PNG/JPEG、raw/embedded format 0 与 BC1/2/3 的八类 typed purpose（premultiplied color、straight albedo、generic preserved、mask、noise、flow、phase、normal）进入独立 cache/upload identity，全部 32 个 Effect 辅助纹理调用已显式选用途；唯一 Blend color 预乘，其余 31 个按当前 purpose 保持源通道，其中 3 个仍是 generic preserved；data 图片只接受原尺寸、无 decode 映射、integer packed、明确非预乘 alpha 或无 alpha（skip）且使用受控 32-bit byte order 的 RGB(A/X) provider，并归一为 RGBA；其他 layout/预乘/缩放 fallback 失败关闭，避免把隐藏 RGB 静默改写；Water Flow flow/phase 与 Standard Blur mask 已从同一 source generation 原子取得 identity、purpose、实际 physical/已验证 mapped size、UV transform、sampler 和 pixel format，非法尺寸、sprite/animation、错误 purpose 与 phase 非 identity mapping 均失败关闭；跨 image color sprite 当前只裁 authored 首帧作静态 fallback | 跨进程 extraction lock、同 size 且恢复同 mtime 的纹理替换、完整跨-image sprite animation、旋转 frame、受支持 decoder 的 straight-RGBA 来源与 Windows 通道 golden、BC5 consumer/encoding、其余辅助纹理和通用 material slot 的候选元数据/动态 generation、case/symlink/duplicate/多格式边界及完整 VFS golden | B0 |
...
| Typed texture provider | `L3` | layer/named/property identity、status/fallback；静态 resource generation 与 named frame epoch 已分离；Water Flow flow/phase 和 Standard Blur mask 已用共享 typed candidate 原子携带静态 file/built-in generation、purpose、physical/mapped、UV、sampler 与 pixel format | 显式 dynamic generation、stale cancellation、system/media/video/variant、其余 Effect/通用 material、nested/effectful/child | **B1** |
