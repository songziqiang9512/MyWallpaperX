...
> 最近核对：2026-07-30
>
> Scene 实现基线：`7fa4e508`（在 `b9bdbd5e` 的 Effect 辅助纹理用途合同上，补齐 exact stock Blur 的可选 instance R mask、Water Flow 的 TEX sampler 传递，以及 stock Foliage 的可选 mask 与 noise G 通道/原始 scale 数学；`3028090166` 的五段上层云链和六段龙主体链由公共 profile 恢复，未知形态继续失败关闭，逐能力证据见专项表和运行证据索引）
>
> 当前完整快照门：对应 `7fa4e508` 源码的最终签名 Debug App 在 2026-07-30 从真实只读根重新构建的 45 样本隔离副本上运行 `.codex/scene-full45-masked-blur-foliage-20260730-v1/report.json`，**45/45 PASS**、image texture **451/451**、particle **130/130**、strict stage/chain **305/55**、Foliage Sway **17**、Water Flow **14**、Water Waves **25**、Water Ripple **10**、Godrays **16**，graph succeeded/failed layer **157/0**、failed frame、drawable miss 均为 0。报告/full matrix SHA-256 为 `3996d23f9b5a974c7a44eb0ed42de232d63054365356b8f63485b60a2f1c9c34` / `1588bf8fdfca04c4f5bca7c28f54b42280027bbf1c34ba23d4cb065db491e532`，隔离前后 source/isolated changes 均为空。历史 fixed13 仍为 `.codex/scene-fixed13-ropetrail-20260729-v1/report.json` 的 **13/13 PASS**；它没有在当前构建上重跑，也不能替代当前 full45。
>
> 最新聚焦门：`7fa4e508` 的签名 Debug App 对 `3028090166` 运行 `.codex/scene-cloud-dragon-3028090166-20260730-v3/report.json`，**1/1 PASS**，报告 SHA-256 `dbe0b6e6cee4462ed5a92b3732189b6b3715c60e37569e51d26604f77e3f3b52`；image **8/8**、succeeded `[13,82,86,111,129]`、chain/stage **3/15**、failed frame/drawable miss **0/0**。另以 `.codex/scene-foliage-math-targeted-20260730-v1/report.json` 对 legacy 重复 Foliage `2470144420` 与 stock masked Foliage `3769688830` 跑 **2/2 PASS**。本批四组自动门分别 **11/11、3/3、54/54、31/31 PASS**，矩阵合同 **62/62 PASS**，代码健康为 653 Swift files / 44 locked legacy / 400-line limit，签名 Debug build 通过且 benchmark 签名前后 verified。用户实际播放复核认为 302 与官方肉眼已无明显差距；未支持的 `MDLV0016` Puppet、精确相位和 Windows pixel golden 仍不在本结论内。
