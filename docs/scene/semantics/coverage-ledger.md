...
> 最近核对：2026-07-30
>
> Scene 实现基线：`6665cb98`（新增公开 authored `0...7` 的共享 `SceneTextureSlotBinding` 原子；Shake 1/2/3、Foliage Sway 1/2、Water Ripple 1/2 七个已准入槽分别把 texture 与各自 candidate 的 generation、identity、purpose、physical/mapped、完整 UV、TEX sampler、pixel format、mip/resolution/texel metadata 原子绑定。binding 构造时验证载体，最终 GPU split 复核 plan path、slot、purpose、format、UV 与 clamp-border 并消费 sampler；未新增 sample/layer-ID 或 ad hoc path/name 分支，仍沿用 strict profile 的 exact effect/asset path 准入，不是 generic material executor）
>
> 最近完整快照门：对应 `7fa4e508` 源码的最终签名 Debug App 在 2026-07-30 从真实只读根重新构建的 45 样本隔离副本上运行 `.codex/scene-full45-masked-blur-foliage-20260730-v1/report.json`，**45/45 PASS**、image texture **451/451**、particle **130/130**、strict stage/chain **305/55**、Foliage Sway **17**、Water Flow **14**、Water Waves **25**、Water Ripple **10**、Godrays **16**，graph succeeded/failed layer **157/0**、failed frame、drawable miss 均为 0。报告/full matrix SHA-256 为 `3996d23f9b5a974c7a44eb0ed42de232d63054365356b8f63485b60a2f1c9c34` / `1588bf8fdfca04c4f5bca7c28f54b42280027bbf1c34ba23d4cb065db491e532`，隔离前后 source/isolated changes 均为空。历史 fixed13 仍为 `.codex/scene-fixed13-ropetrail-20260729-v1/report.json` 的 **13/13 PASS**；它与 full45 都没有在 `6665cb98` 构建上重跑，三类门不能互相替代。
>
> 最新聚焦门：`6665cb98` 的签名 Debug App 对隔离 `1937925563`、`2470144420`、`2802243144`、`2998757800`、`3028090166`、`3738202317` 运行 `.codex/scene-texture-slot-binding-20260730-v4/report.json`，**6/6 PASS**，报告 SHA-256 `1eb73e518a5b1ac1a3042bf53958ac3eda1e1c4d06df75b7cca2e1900118b858`。六者 loaded ratio 均为 1，Shake/Foliage Sway/Water Ripple stage 合计 **33/7/6**、strict stage/chain **103/26**，graph failed layer、failed frame、drawable miss 均为 0；ready/after 均非黑且 changed ratio `0.2265...0.6757`，人工未见黑块、单侧遮罩丢失、异常裁切或 alpha 破坏。最终源码的 Scene **145 modules ALL OK**，代码健康 662 Swift files / 44 locked legacy / 400-line limit，签名 build 通过。该六样本集合覆盖现存三类 consumer tuple；本批未重跑 fixed13/full45，三类门不可互相替代，也不宣称 generic material、Windows pixel parity 或完整兼容。
