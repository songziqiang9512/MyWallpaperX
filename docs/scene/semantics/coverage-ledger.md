...
> 最近核对：2026-08-01
>
> Scene 实现基线：`ce2276c8`（普通 image 与 solid 现在都在 color blend/effect 前消费作者 `color × brightness`；text 继续由 CoreText 独占同名颜色/亮度，现有 mode 31 数学不变。`3768903841` 只证明烟花从白/蓝白输入恢复粉桃方向，整体曝光、错峰、天空后处理、云形与底部光束视觉仍未闭合）
>
...
>
> 最新公共合同门：`4622b8bb` 的 Light Shafts 聚焦门 **38/38 PASS**，benchmark/layout 合集 **61/61 PASS**；Scene **150 modules ALL OK**，代码健康 683 Swift files / 44 locked legacy / 400-line limit，`script/build_and_run.sh verify` 通过。上一批 `c73aa0b0` 的 effect graph / target plan / target table / offscreen pool / Cursor Ripple / chain planner / execution / framebuffer GPU / Local Contrast 聚焦门合计 **101 tests OK**；其隔离 `3299228616`、`3767343314` 门为 **2/2 PASS**，证明已有 strict FBO/history consumer 在共享初始化改动后无回归，不是非零 clear、arbitrary FBO、Fluid/Motion Blur 或 Windows parity。Generic FBO command graph 仍为 `L2`。
