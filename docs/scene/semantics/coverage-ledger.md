...
> 最近核对：2026-07-29
>
> Scene 实现基线：`54a6ebc`（在原始项目目录直达宿主与内存 `SceneRuntimeInput` 基础上，包含 image/solid 作者 `alignment` quad pivot、exact single-texture Blend、identity-only Transform、exact Workshop Simple Audio Bars、strict linear/direct-draw/gradient Light Shafts quad、Workshop Clipping Mask 作者顺序合成、strict particle REFRACT 双纹理/前序 framebuffer snapshot、static particle General/Movement/Renderer world-space、16/32/64 host spectrum、完整 authored utility-chain capture、launch-owned lazy pipeline repository、结构化帧性能遥测、透明粒子剔除与内容寻址 PKG cache；逐能力落地提交见专项表和运行证据索引）
>
> 当前完整快照门：`.codex/scene-particle-worldspace-full45-20260729-v1/report.json`（`54a6ebc`），**45/45 PASS**；当前 fixed13 为 `.codex/scene-particle-worldspace-fixed13-20260729-v1/report.json`（`54a6ebc`），**13/13 PASS**
>
> 最新运行门：`54a6ebc` 使用从真实只读目录重建并复核未变的 45 样本隔离副本；fixed13 **13/13 PASS**，particle `76/76`、REFRACT root batch 7；full45 **45/45 PASS**，particle `127/130`、REFRACT root batch 13。报告/full matrix SHA-256 为 `0497ca4c2f3b84abfb1c9fc29ed365879a389c16b06f36de6003815bed19fadf` / `410df4bfd8af5dbd529b544bd03d207f4186697a021ca64acce15a5bae0eb5a6`；报告/fixed matrix SHA-256 为 `6f49ee0024a5bfcda2bbea0f1d90aa8c734a9f6492d306bd377c0c9f6657aa9e` / `2a405c543e586d9c4d004538c5f719ecaad75d0dc57ce560011ba16fd9f92241`。Scene suite 共运行 631 项并通过（2 项按既有环境条件跳过），代码健康 597 Swift files、44 个锁定历史文件、400 行上限，Developer ID 签名 Debug build 通过。聚合缺口、性能/视觉边界和签名身份见 [运行证据索引](runtime-evidence-index.md)。
