...
> 最近核对：2026-07-30
>
> Scene 实现基线：`b623f421`（strict stock 2.8.42 Depth Parallax 只接受单 image layer、单 effect、单 pass、exact definition/material/shader 指纹与 slot 1 R8 depth；quality 0/1/2 分别走 1/24/64 次采样，mask、动态 binding、未知 state/combo、非轴对齐 UV、错误 purpose/format 与 clamp-border 继续失败关闭）
>
...
>
> 最新聚焦门：`b623f421` 的最终签名 Debug App 对隔离 `3768229922` 运行 `/private/tmp/mwx-depth-parallax-targeted-20260730.LrQTEN/run-execution-pass/report.json`，**1/1 PASS**；loaded ratio 1，graph succeeded `[85,107,171,185,231,234]`、failed `[]`，Depth Parallax 3、Water Flow 2、Water Waves 6、chain/stage **1/11**、route-only 2，failed frame、drawable miss 与 sample residue 均为 0。三个 Depth layer 均加载 R8 `.depth` candidate；85/185 的 `sens=-1.59`，107 的 `sens=0`，后者证明执行但不产生可见位移。报告/matrix SHA-256 为 `59f9c3f73cca2ec3acf795b8eec404521e6c78e821bcea5accdb864125ff6bca` / `4bc44777e07bd0d76543f803a75c4aa4bb3ff448ac29b7d85518b536fea2f3d4`；App executable SHA-256 `5558bbbb0ed81c5e1fffa4115c436eb5d6cb157c6a9f5dd680835823b5138211`、CDHash `392735dbc69c2b170ab14fb541747ef32da93af7`，签名前后 verified。`logo=false` 已进入 effective property values，但通用 SceneScript `applyUserProperties` 尚未执行，layer 354 的全屏提示仍遮挡画面；因此该门只证明真实执行与资源闭环，不是整场方向视觉 PASS。最终源码 Scene **150 modules ALL OK**，benchmark/snapshot/test-runner **69 tests OK**，代码健康 676 Swift files / 44 locked legacy / 400-line limit，`script/build_and_run.sh verify` 通过。本批没有重跑 fixed13/full45。
