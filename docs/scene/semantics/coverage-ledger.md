...
> 最近核对：2026-07-30
>
> Scene 实现基线：`eedb3c62`（在既有 typed texture candidate/slot-binding 上，把 exact legacy Blend 的 authored TEX 与普通 PNG/JPEG property overlay 统一为 slot 1 typed candidate；property ready 时优先、缺失时回退 authored asset，最终 binding 原子携带 generation、purpose、physical/mapped UV、sampler 与 format。GPU 拆分前复核 slot/purpose、RGBA/BGRA、axis-aligned UV 与 clamp-border，并实际使用作者 sampler；这不开放 modern Blend、多纹理、effect-combo UV transform/repeat、opacity mask、write alpha 或动态 shader 值）
>
> 最近完整快照门：对应 `7fa4e508` 源码的最终签名 Debug App 在 2026-07-30 从真实只读根重新构建的 45 样本隔离副本上运行 `.codex/scene-full45-masked-blur-foliage-20260730-v1/report.json`，**45/45 PASS**、image texture **451/451**、particle **130/130**、strict stage/chain **305/55**、Foliage Sway **17**、Water Flow **14**、Water Waves **25**、Water Ripple **10**、Godrays **16**，graph succeeded/failed layer **157/0**、failed frame、drawable miss 均为 0。报告/full matrix SHA-256 为 `3996d23f9b5a974c7a44eb0ed42de232d63054365356b8f63485b60a2f1c9c34` / `1588bf8fdfca04c4f5bca7c28f54b42280027bbf1c34ba23d4cb065db491e532`，隔离前后 source/isolated changes 均为空。历史 fixed13 仍为 `.codex/scene-fixed13-ropetrail-20260729-v1/report.json` 的 **13/13 PASS**；它与 full45 都没有在 `eedb3c62` 构建上重跑，三类门不能互相替代。
>
> 最新聚焦门：对应 `eedb3c62` 源码的最终签名 Debug App 从 full matrix 只选择隔离 `2067939514` 与 `2134765860` 运行 `.codex/scene-blend-texture-binding-targeted-20260730-v2/report.json`，**2/2 PASS**，报告/full matrix SHA-256 为 `c58319d1bbdeef3603853083323dcb67db231e70fbe21f82931bd8ab041a2d1c` / `f47997b7b4846dac82a947b31ac502c8a4a867259d62bbe43207b628900b4df7`。206 为 Blend 2、chain/stage **3/11**、succeeded `[65,161,342,1063]`、failed `[]`、route-only 0；213 保持 Blend/chain/stage **0/0/0**、failed `[]`、route-only 7，锁定 modern profile 未开放。另在 206 一次性隔离副本注入样本内 PNG 为 `custombackground`，layer 342 报 `blend property texture OK`，四个 graph layer 均 succeeded，layer 161 保留 `linear/repeatWrap`；覆盖前后截图均为 3024×1964、changed ratio `0.911609`，人工确认覆盖生效且无黑帧。App executable SHA-256 `22c20f53ca13b00b4205da16cc296003c2586fe65a4004ba108d6fd64a1a4145`，签名前后 verified。最终源码 Scene **147 modules ALL OK / 780 tests discovered**，提交前受改模块 **60/60 PASS**，代码健康 669 Swift files / 44 locked legacy / 400-line limit，`script/build_and_run.sh verify` 通过。本批未重跑 fixed13/full45；2/2 是定向门，不替代完整门，也不证明其他 Blend profile 或 Windows parity。
...
| Typed texture provider | `L3` | layer/named/property identity、status/fallback；静态 resource generation 与 named frame epoch 已分离；direct text 与 embedded MP4 使用显式 content generation publication，frame registry 拒绝 stale generation、同代换纹理和 publication/texture 不匹配；Water Flow flow/phase、Standard Blur mask、static plain base image 与 bounded REFRACT normal 已用共享 typed candidate 原子携带 generation、purpose、actual physical/mapped、UV、sampler 与 pixel format；其上新增公开 authored `0...7` 的共享 slot-binding 原子，Shake 1/2/3、Foliage Sway 1/2、Water Ripple 1/2 与 exact legacy Blend 1 共八个 bounded 槽进一步原子携带 slot、identity/generation、purpose、完整 UV、sampler、format、mip/resolution/texel；Blend 的 PNG/JPEG property 与 authored TEX 共用最终 ready candidate 选择，已选 candidate 不合合同时失败关闭；TEXB0003 embedded image 按 payload-class 尺寸合同准入 | system/media/variant、通用异步取消、其余 Effect 与 generic material 的 slot population/readiness、effectful base、nested/child、sprite rotation、BC5 与 clamp-border parity | **B1** |
