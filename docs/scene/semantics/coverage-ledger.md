...
>
> 最新运行门：alpha/solid color live 正反例保持通过；provider 双代门中 `2938612768` static image blend 5/5、`2902406982` named capture 6/6 与 binding 7/7。B2 target plan/table 只有自动门和 unsigned build 证据；当前 Scene 全量测试共运行 211 项、210 项通过、1 项跳过，最新签名身份仍见 [运行证据索引](runtime-evidence-index.md)。
...
| Generic FBO command graph | `L2` | target/bind/compose/copy/swap/condition/function 可保留或 blocker；effect-scoped target/lifetime plan 与原子 Metal table 已建立但尚无 compositor consumer | strict Blur 迁移、scheduler、copy/swap/compose/history 与完整生命周期 | B2 |
...
| **B2 Graph Resource Runtime** | `S2 第 1-5 项` | target/lifetime plan 与不可变原子 allocation table 已完成；继续 strict Blur consumer、cache/resize/reset，再补 scheduler、copy/swap/compose/history、stock ShaderContract/built-ins/state | read/write、RT lifecycle、slot/combo/state 和 resize/switch/stop 门 |
