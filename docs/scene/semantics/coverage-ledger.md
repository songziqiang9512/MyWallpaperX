...
45 项的作者条件、执行通道、公共依赖、当前证据和下一验收门统一维护在 [官方 Effect 执行覆盖表](effect-execution-coverage.md)。汇总为 `L1=20`、`L2=4`、`L3=21`、`L4=0`；Transform 的 `L3` 仅指 identity-only exact profile，所有 `L3` 都是表内明确限定的子集，不是 WE parity。内部 `_empty`、Workshop `gradient_color` 和 layer Bloom 另列，不能替代任何官方 Effect。
...
| Audio effect consumer | `L3` | stock Shake（`whitePhaseFallback`/`timeOffsetCombo`）、exact old-editor Shake（`legacyUnconditionalPhase`）、stock Pulse（`stock2842`，并接受同一精确旧版 profile 缺省 `replacementkey`）与 exact Workshop Simple Audio Bars 两 profile；其余 legacy 指纹、其他 Workshop audio shader、未支持 combo 与 SceneScript binding fail closed |
