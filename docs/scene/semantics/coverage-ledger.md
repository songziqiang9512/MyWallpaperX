...
> 最近核对：2026-07-30
>
> Scene 实现基线：`de5f01b6`（原子 `SceneTextureCandidate` 已从 Water Flow flow/phase、Standard Blur mask 扩到 static plain base image 与 bounded REFRACT normal；全部按公开资源形态、用途和最终 GPU 合同准入，没有样本、图层、路径或名称特判，generic material slot 保持关闭）
>
...
>
> 最新聚焦门：`de5f01b6` 的签名 Debug App 对隔离 `3028090166`、`3770444459`、`3766415113` 运行 `.codex/scene-texture-conformance-base-refraction-20260730-v2/report.json`，**3/3 PASS**，报告 SHA-256 `5b22b783549ce90163f0687b492143a98ce33bd3d2c8cb59da60a27dc37ec313`。三者 loaded ratio 1、ready/after 非黑且有运动、failed frame/drawable miss 为 0；302 的六个 static TEXB0003 embedded base layer 恢复，377 保持 particle **5/5**、REFRACT **4**，376 的动画 base 明确保留 legacy。聚焦自动门 **72/72 PASS**，完整 Scene discover **763 PASS、2 SKIP**，代码健康 659 Swift files / 44 locked legacy / 400-line limit，签名 build 通过。真实门证明回归与可见连续性；最终 candidate 消费由 production GPU 门和 real-cache runtime harness 证明，不宣称通用 slot binder 或 Windows pixel parity。
