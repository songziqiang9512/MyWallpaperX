...
> 最近核对：2026-07-31
>
> Scene 实现基线：`0fc38b59`（exact Scroll authored-source profile 只接受 stock 2.8.42、Workshop `3302578859` 与 `3387825383` 三组完整 definition/material/raw+canonical shader 指纹、单 image/solid/text effect/pass/slot、静态有限 repeat/speed 与既有 typed fullscreen-overwrite state；仅在整组合同匹配时把未注解的 `g_Texture0` 推断为当前 effect input，composition、动态值、额外纹理与未知 revision 继续失败关闭）
>
