...
> 最近核对：2026-08-01
>
> Scene 实现基线：`d12a4918`（在既有 image color、delayed-loop texture animation、Tint 与 fullscreen Color Grading 基础上，严格 standalone volumetric `lspot` 子集已按 authored relative Mirror angle Timeline 绘制；`3768903841` 的 compact day、long-month date、12/24-hour clock、time-of-day greeting 与三帧 day/night toggle 又共用同一 wall-clock schedule。粒子侧新增静态同空间 plain-image Layer Image emitter 的 bounded 子集；动态/text/puppet source、颜色/运动继承和 bitmap 更新仍失败关闭）
>
...
| Particle runtime | `L3` | 作者 Sprite、常见组件、Sprite Trail、严格单 renderer/Screen RopeTrail，以及从唯一 typed `emitterimage` dependency 的静态同空间 plain image 非零 alpha 像素中心发射的 Layer Image 子集；普通 Sprite 现消费可信单 sequence sidecar nominal frame aspect，缺失时按 raw TEX axes × physical pixels 回退，root/child 共用。Layer Image 只准 shared/managed RGBA8/BGRA8 单层纹理和静态同 transform source；custom/missing shader 与显式空 renderer 继续 fail closed | Layer Image 的动态/text/puppet source、颜色/运动继承与 bitmap generation 更新仍未实现；Rope、通用 RopeTrail、trim pivot、多 sequence/multi-image 异尺寸、live override、动态 world-space transform、Lighting、custom shader executor 和 Windows golden 仍缺失 | **B4** |
...
当前完整矩阵可见粒子为 `126/130`；初始 REFRACT root batch 为 14。四个未加载 root 中，`2974757317:85705` 的显式空 `renderer: []` 沿 `missingSpriteRenderer` 失败关闭；`3088601835:550/558` 与 `3750813609:200` 的 material `cullmode=normal` 不满足只准 `nocull` 的 typed-state 合同，矩阵分别锁定 `17/19` 与 `8/9`。RopeTrail 三个目标样本仍为 `3299228616=7/7`、`3743305891=1/1`、`3770444459=5/5`，后者 REFRACT 为 4。这不等于粒子组件完整兼容，`3299228616` 的音频响应、`3743305891` 的 child transform、`3770444459` 的 Boids/滴水与 splash children，以及 Rope、动态 world-space、Lighting 等 unsupported component 仍在。`2131872317` 的 event-death child REFRACT 继续由独立 7 秒延迟 runtime 门证明，child 不增加 root loaded-layer 计数。上述数字只度量对应矩阵实际加载的 root layer，不代表 Windows 视觉等价。
