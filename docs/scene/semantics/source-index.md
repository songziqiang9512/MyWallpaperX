...
2026-08-12 复核官方 [Shader Variables](https://docs.wallpaperengine.io/en/scene/shader/variables.html) 的 Hidden/Visible/Optional Texture 合同：sampler `default` 是作者未另选纹理时的默认资源，而 texture `combo` 只在作者实际定义、选择或绘制对应纹理时启用；未绑定 optional texture 时不添加该 preprocessor toggle，采样也必须位于对应条件块内。因此 `default + combo` 不能由 default 反向制造 authored presence/readiness，`[COMBO]` marker 的数值 default 也不能冒充 sampler readiness metadata。Mirage 固定 revision `da4fa7b3ee33e9e94c59307f47098aa521f21aa6` 仅作 D 级交叉验证：`ShaderAnnotations.cpp` 分开保存 default 与 `texinfos.enabled`，`SceneCompiler.cpp` 先从 material texture slots 形成 enabled/combo，再回填 default，顺序与公开合同一致；它不是官方真值，项目未复制其 payload、shader或算法表达。现役 typed schema据此把 unmarked sampler presence combo 与 default resource 分开，并在 launch/reachability/demand/frame selection 四处保持同一 fixed point；作者 candidate 已存在但 missing/pending/unavailable/purpose 不符时仍失败关闭，不降成 combo-off，也不回退 default。本次证据已足够，无需新增 Ghidra 复核；它不证明官方私有 candidate 优先级、Windows backend 定义形式或像素等价。
