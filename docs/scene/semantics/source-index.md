...
- **Shader 与兼容研究上下文**：[Shader source/prelude 固定客户端与语料研究](shader-prelude-and-backend-abstraction.md)记录 source token、format branch 与 uniform census；[zcompat 取证](zcompat-backward-compatibility-forensics.md)记录 patch record schema。它们不直接授权产品实现；未闭合的注入者、矩阵/NDC/Metal 映射、匹配方向和运行时机继续保持 unknown。
