...
- 当前完整快照门：仓库矩阵 `script/scene_wallpaper_full_sample_matrix.json` 固定真实目录中 45 个具备 package 的可运行样本，`.codex/scene-puppet-animation-20260725/full45-v2/report.json` 为 45/45；`3770500543` 因缺 package 只记录在 source manifest。同一签名 App 的固定回归门 `.codex/scene-puppet-animation-20260725/fixed13-v3/report.json` 为 13/13；两门合计 52 个唯一样本快照。Puppet 定向正向门与 v24 对照分别为 `.codex/scene-puppet-animation-20260725/targeted-v1/report.json`、`.codex/scene-puppet-animation-20260725/control-v24-v1/report.json`。当前实现基线、报告/矩阵哈希、App 身份、聚合缺口与测试数统一见 [运行证据索引](runtime-evidence-index.md)。
