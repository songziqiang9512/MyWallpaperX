...
- 当前完整快照门：仓库矩阵 `script/scene_wallpaper_full_sample_matrix.json` 固定真实目录 26 个样本，`.codex/scene-xray-final-20260724/full26-duration7/report.json` 为 26/26。同一签名 App 的固定回归门 `.codex/scene-xray-final-20260724/fixed13-duration7-final/report.json` 为 13/13；两门合计 33 个唯一样本快照。当前实现基线、报告/矩阵哈希、App 身份、聚合缺口与测试数统一见 [运行证据索引](runtime-evidence-index.md)。
