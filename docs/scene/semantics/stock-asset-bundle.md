...
`SceneStockAssets.bundle` 一次性建立官方 `assets/**` 中所有播放候选资产的原相对路径、文件名和扩展名。素材制作完成后直接原位替换，不新增平行命名、目录或 runtime 别名。官方纹理、shader、脚本、配置、模型和禁止再分发字体 payload 均未复制；编辑器 preset、preview、example、thumbnail 与 source-only 文件不进入 App。
...
| `.ttf` / `.otf` | 15 | 8 个许可允许再分发的原版；7 个许可替代 payload 复制为官方物理文件名 |
| **合计** | **919** | 包内另有 9 份字体许可与 `NOTICE.md`，不属于官方 `assets` 路径 |
...
15 个字体全部位于 `assets/fonts/<官方文件名>`。例如 Poppins Medium payload 物理保存为 `Atami-Regular.otf`，Permanent Marker 分别复制为 `summer85.ttf` 与 `Lazer84.ttf`，Segment7 Standard 另复制为 `CursedTimerUlil-Aznm.ttf`。字体解析器直接读取作者请求的官方路径；替代字体仍报 `stockSubstituted`，许可与逐项来源见 bundle 内 `NOTICE.md`。
...
| 官方字体许可文本 | 4 | 不复制官方文本；本项目许可另放 bundle `Licenses/` |
...
- A 级：官方 2.8.42 安装的 3,113 个相对路径、扩展名及目录归属来自只读静态枚举。
