...
| Generic Vec3 property owner | `L3 bounded / S4 bounded visible` | object `origin/scale/angles` 的loss-preserving binding由同一QuickJS domain执行；真实`2067939514`的3个angles owner发布非零z角度，真实`3509243656`的8个angles及layer `222` scale owner进入统一frame snapshot。scale的嵌套property从最外层`size x`取live值，默认输出`(0.6,0.4,1)`，覆盖后输出`(1,0.4,1)`并继续由共享world-frame/唯一compositor消费。route=`generic-only`、failure=`previous-current`；optional scale producer未准入或失败只保留作者current，不再隐藏整层。不外推独立geometry ROI、所有Vec3 target、完整Vec3 API或官方parity；见[E-V4-LAYER-SCALE-TYPED-PROPERTY](runtime-evidence-index.md#e-v4-layer-scale-typed-property) |
| Object-bound text/direct-image color | `L3 bounded / S4 one real slice-visible` | exact `objects[index].color` wrapper、finite Vec3 fallback与static/typed `scriptProperties`进入同一QuickJS vector owner。text target经CoreText重栅格、same-layer dynamic-text publication与普通Precise Blur Program；effectless direct image走既有tint。真实`3655958892`的3 text + 1 image颜色恢复，route=`generic-only`；`disable-generic`归零7个media/color active owner但保留作者current、文字、Blur与compositor。layer176无live media仍为black，不开放完整`ITextLayer` setter、effectful image或官方parity；见[E-V4-SCENESCRIPT-TEXT-IMAGE-COLOR-CONSUMERS](runtime-evidence-index.md#e-v4-scenescript-text-image-color-consumers) |
