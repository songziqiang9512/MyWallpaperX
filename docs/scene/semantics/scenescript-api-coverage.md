...
当前结论只有一条：项目已用同一QuickJS-NG scene domain执行scalar、String、object Vec3与pass-owned typed Vec2/Vec3 owner，并开放bounded `WEMath`、`WEColor.hsv2rgb`、callback-scoped immutable engine/user-property/single-surface pointer与canvas snapshot、typed handles、16/32/64 AudioBuffers、events、timer/job及exact-once teardown；它们进入现有property transaction、resource publication和唯一Metal compositor，但**没有完整SceneScript API/host runtime**。pass Vec2当前复用Vec3 host object完成作者现有`multiply`脚本，published value仍是Vec2；这不等于官方Vec2 class/API已实现。

| 能力面 | 当前结果 | 精确边界 |
|---|---|---|
| Generic ECMAScript VM/API | `L3 bounded scalar+String+object Vec3+pass Vec2/Vec3+typed snapshots/handles/events/lifecycle` | QuickJS-NG开放inline scalar、object String/Vec3及exact pass Vec2/Vec3 `init/update`，共用WEMath、bounded WEColor、immutable input、typed handles、AudioBuffers、events、timer/job和exact-once teardown。pass Vec2只复用现有Vec3 host object的x/y/basic multiply，不宣称官方Vec2 class。仍无file module graph、Vec4/matrix、完整Vec2 API、多按钮、drag-out/capture、async module或完整multi-surface lifecycle |
...
| `311115e3` 其余 bounded native projections | shared/display `L2 / S2 wired`；media colors exact bounded `S3 executed`、generic API `L0` | native hover、primary-button click、launch-origin与audio-scaled value已分别被generic cursor/Vec3/AudioBuffers owner取代并删除。shared alpha、identity display仍只有有界产品接线；media colors已有4个真实consumer正反执行，但route仍为`disable-generic`，不是通用`MediaThumbnailEvent` colors或live media API。见 [E-V4-MEDIA-COLOR-OWNER-PARTITION](runtime-evidence-index.md#e-v4-media-color-owner-partition) |
...
| `init(value)` / `update(value)` 的 typed value | 入参是绑定property当前值；返回兼容值写回；无返回则保持原值 | QuickJS pass scalar、pass Vec2、object String/Vec3为`L3 bounded` | pass scalar接受完整identity/provenance守恒的可序列化static `scriptProperties`；真实293的Water Waves strength随audio generation发布并被普通Program消费。pass Vec2从同一preliminary snapshot取得pair，以Vec3 host object执行后仅发布x/y；真实206的25项均随audio generation改变。callback异常/错型保留owner-entry current | generic bool/Vec4/matrix、官方Vec2 class与完整coercion/API、dynamic nested property provider、更多target及官方行为对照 |
...
| `update(value)` | 每个渲染帧调用；动画应乘 `engine.frametime` | `L3 bounded scalar+String+object Vec3+pass Vec2` | generic callback从preliminary current取值；失败只保留该owner previous-current。真实206的25个pass Vec2随audio generation产生非unit输出 | bool/Vec4/matrix、官方Vec2 API、raw/effective delta parity与更广mutation |
...
| 五个 media events | status/playback/properties/thumbnail/timeline 变化事件 | thumbnail presence/playback/properties `L3 bounded`；properties为`S4 bounded visible`、前两项为`S3 executed`；thumbnail colors与其余generic事件`L0` | artwork、playback与properties各自generation只向每个active property owner派发一次对应callback；playback/properties在同帧`update`前执行，thumbnail presence当前在成功`update`后派发，均复用typed handles与原子mutation buffer。真实`2067939514`证明thumbnail animation命令；真实`2938612768`证明playback接管10个scalar target，并证明3个String owner接管旧Text AST且title可见更新。没有通用event queue或live macOS producer | status/timeline JS事件、properties除title/artist外字段、thumbnail color DTO/previous、跨类型完整event ordering、重复/乱序provider仲裁、reload/teardown与官方行为对照 |
...
| `MediaThumbnailEvent` | thumbnail presence 和 primary/secondary/tertiary/text/high-contrast colors | presence API/current texture `L3 bounded`；colors generic API `L0`；exact bounded colors `S3 executed` | current `$mediaThumbnail`有typed provider/visibility/last-ready。exact validated color candidates在QuickJS Vec owner创建前排除，material admission后仅真实consumer交集由bounded native runtime持权，unclaimed无owner；真实`2974757317`为6/4/2，四个target完成蓝色或missing-palette authored fallback并到GPU/compositor/next-frame。route=`disable-generic`；这不产生`primaryColor/secondaryColor/...` QuickJS DTO | artwork+colors同generation、全部公开color DTO、immutable Vec3/字符串转换、callback/update ordering、clear/missing/stale、failed activation/teardown、live producer与独立ROI；native oracle对照后切`generic-only`并退役bounded runtime。typed provider不等于完整SceneScript API；见 [E-V4-MEDIA-COLOR-OWNER-PARTITION](runtime-evidence-index.md#e-v4-media-color-owner-partition) |
