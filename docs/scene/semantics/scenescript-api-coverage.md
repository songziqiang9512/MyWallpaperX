...
| Generic object scalar alpha owner | `L3 bounded / S3 real owner-executed` | 非既有shared-alpha incumbent的exact `script/value`与primitive `script/scriptproperties/value`由同一QuickJS scalar owner执行；完整path/descriptor/bit definition守恒，成功target解除display suppression，失败保留current。真实`3509243656`有60个completion但整景黑，`2902406982`有8个且整幅非黑，`3768724269`的缺`getParent`与visible mutation各自局部失败。只证明bounded owner执行与whole-composition safety，不证明alpha ROI、shared初始化/global order、`visible`、跨属性mutation、旧shared-alpha owner迁移、完整API或parity |
