...
当前结论只有一条：项目已用同一 QuickJS-NG scene domain 执行 pass-owned numeric scalar 与 object-owned origin/scale Vec3 `init/update`，并开放 allowlisted `WEMath.smoothStep`、callback-scoped immutable `engine.timeOfDay/frametime/runtime` 以及 typed `engine.userProperties` snapshot；但**没有完整 SceneScript API/host runtime**。逐 API 表继续使用本专题定义的 `L0-L4` 表达接口覆盖；`S*` 只引用[运行证据索引](runtime-evidence-index.md)对精确 identity 的证据闭合等级。两轴正交，不能互相批量推导，也不能与其他专题的同名 `L` 横向比较。

| 能力面 | 当前结果 | 精确边界 |
|---|---|---|
| Generic ECMAScript VM/API | `L3 bounded scalar+Vec3+typed snapshots / S4 bounded visible` | QuickJS-NG 开放 inline scalar及object origin/scale Vec3 `init/update`、唯一 allowlisted `WEMath` module、三个 immutable `engine` frame fields与typed immutable user-property object；没有通用 file module graph、object/layer handle、event queue、timer/job scheduler、mutable `shared` 或完整 lifecycle |
...
| Generic Vec3 property owner | `L3 bounded / S4 bounded visible` | object `origin/scale` 的loss-preserving binding由同一QuickJS domain执行；真实`2802243144`两个origin owner完成callback并进入统一frame snapshot，旧四文件Swift source-shape owner已删除，route=`generic-only`、failure=`previous-current`。只证明两个真实origin consumer，不外推全部Vec3 target或完整Vec3 API |
