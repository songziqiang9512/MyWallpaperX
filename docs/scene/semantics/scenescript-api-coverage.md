...
| `ILayer` orientation | `getTransformMatrix`, `rotateObjectSpace`, `lookAt`, `lookAtYaw` | `L0` | 当前签名 App 在真实 `3238423642` 的 layer `918 / 920` 首先因 `getTransformMatrix` 缺失而失败，初始化共享布局未发布；后续 TextureAnimation frame 与向量错误是连锁结果 | 先定义矩阵布局、authored/local/world/parent 与 staged mutation 语义，再以 2D/3D、dynamic layer、奇异/非有限、retained handle 正反门接入既有 immutable snapshot；不新增坐标权威 |
