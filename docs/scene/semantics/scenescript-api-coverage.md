...
当前结论只有一条：项目保存了一部分 SceneScript 作者数据，并已用 QuickJS-NG 执行一个 pass-owned numeric scalar 的 bounded `init/update`；但**没有通用 SceneScript API/host runtime**。逐 API 表继续使用本专题定义的 `L0-L4` 表达接口覆盖；`S*` 只引用[运行证据索引](runtime-evidence-index.md)对精确 identity 的证据闭合等级。两轴正交，不能互相批量推导，也不能与其他专题的同名 `L` 横向比较。

| 能力面 | 当前结果 | 精确边界 |
|---|---|---|
| Generic ECMAScript VM/API | `L1 bounded scalar / S4 bounded visible` | QuickJS-NG 只开放 inline scalar `init/update`；没有通用 file/module loader、host object/typed handle、event queue、timer/job scheduler、mutable `shared` 或完整 lifecycle |
...
| Bounded Blend time-of-day AST | `L3 bounded / exact consumers S4 executable safety` | 严格 `multiply` / `{script,user:null,value}` / scalar grammar 直接从 descriptor 编译；显式 null provenance、全 descriptor/source 预算与 consumer-known-after-filter ownership 已闭合。当前四个 exact target 在 default/recovery 为 Program，rollback 只保留仍有 Program consumer 的两项并让另两项回到 previous-current；0 dedicated、0 failed frame。旧清晨/正午方向证据仍只属于相同 bounded grammar；本批没有独立 ROI，不创建 `engine`/`WEMath` module object，也不外推整个 SceneScript/Blend family |
...
表中的 `S4` 只引用运行证据索引登记的精确 App/fixture/ROI 身份；本批正反运行在当前 HEAD 的 Developer ID App 上完成，但 QuickJS scalar 只覆盖一个派生真实样本和一个 target，不把它扩张成 whole-scene、family、原始 authored corpus 或 parity 结论。
...
|---|---|---:|---|---|
| property-bound 实例 | 每份脚本绑定一个具体 property；通用逻辑通常绑定 layer visibility | `L1` | `I` 保真官方正式13处对应的五类 scene/object/effect/pass owner、完整target path、inline source/properties/authored fallback/JSON value type及可选`wrapperKeys`；独立source evidence又保真所有string-valued script wrapper，但nested/未知owner只提供拒绝证据，不提升为binding，`script + user`冲突fail-closed；IR/source evidence本身不执行脚本 | 补 file/module、schema-resolved generic Vec/value type、runtime handle；重复/缺失 target 与 owner teardown 门。官方13处与297 launch exact13是两项不同计数 |
| `init(value)` / `update(value)` 的 typed value | 入参是绑定 property 当前值；返回兼容值写回；无返回则保持原值 | text String / Blend、Opacity scalar / launch-origin Vec3 为 `L3 bounded`；QuickJS pass scalar 为 `L1 bounded scalar / S4 bounded visible`；generic 其他类型仍 `L0` | `D/F` 对唯一 exported text `update(value)` 的无循环 AST 写回 String；独立Blend子集只把exact time-of-day expression写回`multiply`；placeholder-fade子集只把严格media playback/update结构写回Opacity `alpha`；launch-origin只把完整initializer/master/follower结构投影成typed `object.origin`值；QuickJS-NG scalar owner 实际执行 ECMAScript `init/update`，输入是authored/user/Timeline合并后的current-frame preliminary value，成功结果才进入统一`sceneScript` channel。同target property/Timeline definition是有序contributor，不是重复publication owner；重复或已被bounded native producer占用的QuickJS candidate不形成VM owner。callback异常不提交Script值，最终snapshot保留本帧最新合法低优先级值，不承诺保留上一帧Script输出。正例派生 `3768724269` 记录 `0.3 -> 0.4`，异常负例按 typed `exception` 保留owner-entry current；原始隔离`2902406982`又执行三个跨优先级target而不再整scene硬停。各项都无sample/layer/hash dispatch，且QuickJS owner不把裸JS/Metal handle暴露给Swift | generic bool/Vec/string/mutation、无返回/错类型的完整 API 语义、block scope/coercion、更多 target、通用owner生命周期与官方行为对照；重复/excluded candidate仍缺typed conflict aggregation；`2902406982`只证明launch/producer组合，不证明整样本兼容 |
...
| source/module loader | 加载 inline 或 `.js` 源码及其 export/import | `L1 inline executable` | `I` binding IR保存已准入wrapper的inline source，独立source evidence遍历保存所有string-valued inline `script`；QuickJS wrapper 当前只执行 inline source 并提取 `init/update`，不开放文件/模块 host | `.js` file/module loader、规范化路径、UTF-8/大小限制、模块依赖图、循环/缺失/越界负向门 |
| ECMAScript VM | 受控 ECMAScript 环境，无 DOM/Web/Node/shell/任意文件系统 | `L1 bounded scalar / S4 bounded visible` | QuickJS-NG 固定 revision `bbe0480a68c664d7737eecde260fd47224b2c0d6`；每 scene 一个 runtime/context，每个 pass scalar binding 一个 generation-checked owner；正例在当前 Developer ID App 中完成 callback、graph、GPU/publication/compositor/next-frame；无 standard library、DOM、Node、filesystem、network 或 process host module | typed Vec/bool/string/object、host global allowlist/module loader、更多真实 producer/consumer、官方行为对照 |
...
| `update(value)` | 每个渲染帧调用；动画应乘 `engine.frametime` | `L3 bounded` for text/time-of-day/fade/launch-origin subsets; QuickJS scalar `L1 bounded / S4 bounded visible` | text与time-of-day子集按各自bounded AST求值；placeholder fade与launch-origin按scene/host typed state消费shared simulation delta，在surface loop外每host frame只推进一次并广播同帧snapshot；QuickJS scalar 正例在同一 frame order 中把 `0.3` 变为 `0.4` 并进入 layer 17 graph，负例只保留 previous current | 通用实例/owner、raw/effective delta parity、callback mutation、inverse4、任意frametime表达式与VM teardown |
...
| `engine.timeOfDay` | 24 小时归一化到 `[0,1]` | generic API `L0`；Blend subset `L3 bounded` | `G` 确认每帧从本地系统时间采样，与累计 scene runtime 分离；项目 `D/F` 把 exact property-bound update grammar 编译成 typed Blend `multiply`，按本地民用日秒逐帧求值。descriptor collector只接受显式`user:null`、完整wrapper和有限scalar，工作预算超限只停用该producer；实际MaterialProgram consumer形成后才取得ownership。213清晨/正午旧门与当前四target default/rollback/recovery共同限定该subset；没有 JS global/object | 时区/DST、跨午夜长稳、同帧 Date 一致性、其他 owner/target、事件顺序、通用 VM、独立当前ROI与 Windows 数值/像素 parity；bounded consumer 不替代 generic API |
| `engine.frametime` / `runtime` | 上帧秒数（重绘可为 0）和scene累计运行时间 | generic API `L0`; fade/origin delta consumers `L3 bounded` | `G`确认两者与engine tick/timer共用admitted/scaled/clamped effective delta；项目不建JS global，strict fade与launch-origin evaluator在surface loop外各推进一次，再把同一shared simulation delta所得值广播给所有surface | raw/effective数值parity、任意表达式、0 delta、switch/seek、inverse4、runtime getter与Windows同帧多屏golden |
...
| material property/function | effect 的 `setMaterialProperty` 和 `executeMaterialFunction` | `L0` | `G` 确认 ordered instance metadata lookup、scalar/Vec2/3/4 typed write、int/float 与 degree-to-radian metadata，以及 descriptor-defined ordered synchronous function execution；项目现有 bounded QuickJS-NG scalar owner 可把 mutation buffer 投影为 typed material-function request，并对 unknown/no-call/invalid target 做 layer-local fail-closed，但尚无通用 `IEffect` handle/API bridge | descriptor/pass identity、同帧 graph invalidation、通用 VM handle/API、function side effect、float/utility target 与 unsupported fail closed |
