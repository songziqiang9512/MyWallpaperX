...
| texture animation | `frameCount`, `duration`, `rate`, `play/pause/stop`, `isPlaying`, `get/setFrame`, `join` | `L0` | BC1/2/3 多 image TEX 已有严格 axis-aligned/integer/same-extent 的 bounded native autoplay，但不暴露 handle；默认维持 shared animation state。`3768903841` 五层可随 scene time 跨 image 换帧，但作者首次 `stop()` 后应 detach、再由各自 timer `play()`，当前未执行 | mutation 后的 instance-local detach、join、frame/rate/play/pause/stop/seek、owner timer、frame clamp、layer/asset 销毁与同帧冲突 |
