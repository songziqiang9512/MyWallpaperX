...
| `thisScene` layer mutation | `createLayer`, `destroyLayer`, `sortLayer`, `getLayerIndex`, `getInitialLayerConfig` | bounded text及package-literal image create/destroy/sort/index `L3 / S4 visible`；initial config `L0` | dynamic handle仍用domain/owner/generation隔离与负safe-integer identity。text config进入CoreText；image overload只接受全部调用均为安全直接字符串、model已launch-prepared且可投影为ordinary image。两者经同一Swift transaction与唯一Metal compositor；异常整批丢弃。真实`3792817546`创建63个bar | asset handle、动态路径、任意model/config/material、static sort、`getInitialLayerConfig`、复杂同轮顺序、多surface与官方parity |
