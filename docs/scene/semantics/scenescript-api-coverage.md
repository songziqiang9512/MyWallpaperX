...
| `thisScene.createLayer(asset)` | 由 path/config/asset handle/model data 创建动态 layer | text config及package-local literal image path `L3 bounded / S4 representative visible`；其余asset/model `L0` | text仍经CoreText；image只接受source中每个调用均为安全直接字符串字面量，并以作者`__workshopId`解析到同包launch-prepared ordinary model/image candidate。Swift transaction校验owner/generation/resource后在下一帧复用现有publication/compositor；真实`3792817546`创建63个bar | `engine.registerAsset` handle、动态表达式、其他model/material/config、资源卸载/设备重建、复杂teardown与官方parity |
