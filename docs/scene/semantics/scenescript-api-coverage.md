...
当前结论只有一条：项目已用同一 QuickJS-NG scene domain 执行 pass-owned numeric scalar `init/update`，并开放了 allowlisted `WEMath.smoothStep` 与 callback-scoped immutable `engine.timeOfDay/frametime/runtime` typed frame snapshot；但**没有完整 SceneScript API/host runtime**。逐 API 表继续使用本专题定义的 `L0-L4` 表达接口覆盖；`S*` 只引用[运行证据索引](runtime-evidence-index.md)对精确 identity 的证据闭合等级。两轴正交，不能互相批量推导，也不能与其他专题的同名 `L` 横向比较。

| 能力面 | 当前结果 | 精确边界 |
|---|---|---|
| Generic ECMAScript VM/API | `L3 bounded scalar+frame input / S4 bounded visible` | QuickJS-NG 开放 inline scalar `init/update`、唯一 allowlisted `WEMath` module 与三个 immutable `engine` frame fields；没有通用 file module graph、其他 typed handle、event queue、timer/job scheduler、mutable `shared` 或完整 lifecycle |
...
| ECMAScript VM | 受控 ECMAScript 环境，无 DOM/Web/Node/shell/任意文件系统 | `L3 bounded scalar+frame input / S4 bounded visible` | QuickJS-NG 固定 revision `bbe0480a68c664d7737eecde260fd47224b2c0d6`；每 scene 一个 runtime/context，每个 pass scalar binding 一个 generation-checked owner；真实 `2134765860` 的 11 owner 完成 callback，其中 6 个 time-of-day consumer完成日/夜差分、graph、GPU/publication/compositor/next-frame。callback 的 `engine` 是每次调用创建的不可写数据快照；无 DOM、Node、filesystem、network 或 process host module | typed Vec/bool/string/object、其他 host handle/module、更多真实 producer/consumer、官方行为对照 |
