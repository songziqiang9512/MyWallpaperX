...
| `applyUserProperties(changed)` | 首次加载调用，之后只含变化键；使用 `hasOwnProperty` | primitive/String/Vec2/Vec3 `L3 bounded`；Vec3 `S4 visible` | QuickJS在各typed owner先完成一次`init`，再于首轮派发全部typed properties，以后只派发值变化或删除为null的键；无变化不派发。Vec3真实`2974757317`的10个origin owner实际消费callback并产生可见卡片位移。scalar/String/Vec2/Vec3共用delta、异常隔离和init-before-properties顺序门；真实`3396722575`证明nested input更新后scalar `update`输出`-0.35`，但该作者脚本没有导出此callback，故不把它记作真实scalar callback消费 | scalar/String真实导出callback consumer、跨类型全局批量顺序、reload/多scene、通用JS heap rollback和官方行为对照 |
