...
当前结论只有一条：项目已用同一 QuickJS-NG scene domain 执行 pass-owned numeric scalar 与 object-owned origin/scale Vec3 `init/update`，并开放 allowlisted `WEMath.smoothStep`、callback-scoped immutable `engine.timeOfDay/frametime/runtime`、typed `engine.userProperties` snapshot、descriptor-bound `thisLayer` effect lookup、callback-scoped `thisScene` layer lookup与只读 `ILayer.id/name/origin`，以及 generation-deduplicated `mediaThumbnailChanged({hasThumbnail})`；但**没有完整 SceneScript API/host runtime**。逐 API 表继续使用本专题定义的 `L0-L4` 表达接口覆盖；`S*` 只引用[运行证据索引](runtime-evidence-index.md)对精确 identity 的证据闭合等级。两轴正交，不能互相批量推导，也不能与其他专题的同名 `L` 横向比较。

| 能力面 | 当前结果 | 精确边界 |
|---|---|---|
| Generic ECMAScript VM/API | `L3 bounded scalar+Vec3+typed snapshots+effect/layer/animation handles+one media event / S4 bounded visible` | QuickJS-NG 开放 inline scalar及object origin/scale Vec3 `init/update`、唯一 allowlisted `WEMath` module、immutable engine/user-property snapshots、descriptor-bound effect lookup、scene layer name/index/id/count lookup与只读origin，以及同一media inbox generation驱动的`mediaThumbnailChanged({hasThumbnail})`；没有通用 file module graph、scene/layer mutation、完整event queue、timer/job scheduler、mutable `shared` 或完整 lifecycle |
...
| 五个 media events | status/playback/properties/thumbnail/timeline 变化事件 | thumbnail `L3 bounded / S3 executed`; 其余generic `L0`; bounded playback/colors/text wiring `L2-L3` | 同一artwork inbox generation只向每个active property owner派发一次`mediaThumbnailChanged`，event只读暴露`hasThumbnail`，callback复用typed handles与原子mutation buffer；真实`2067939514`由事件提交`stop/play`并在下一帧推进current-property Timeline。没有通用event queue或live macOS producer | status/playback/properties/timeline JS事件、thumbnail color/previous、同帧事件顺序、重复/乱序、provider仲裁、reload/teardown与官方行为对照 |
