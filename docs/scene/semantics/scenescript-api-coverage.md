...
| Generic object Boolean visibility owner | `L3 bounded / S4 dependency-video + dynamic-image interactions` | exact root image/solid/text Bool owner共用同一QuickJS/snapshot。普通owner仍为value-only；dependency cohort开放own visibility与scene video command；dynamic-image cohort在全部`createLayer`参数为安全package literal且资源launch-prepared时开放owner及其动态实例的create/sort/transform/visibility。非value-only owner可对exact authored peer写`origin/scale/angles/visible`及真实text `text`，但value-only、foreign/stale、静态拓扑与其余字段仍拒绝。真实`2959875782`有17/17普通completion；`3775355045`可见X-Ray交互；`3792817546`创建63个动态bar；真实`3610154602`的layer 92初始化创建63个动态bar且不再发生visibility冲突，layer435的旧跨handle TypeError已消失、随后仅在真实缺失target处RangeError。callback失败、Swift preflight或surface submission拒绝会按原owner同时撤销primary、material/animation/video command、Swift plan与 C QuickJS dynamic-layer value/order/create journal；成功全surface barrier后才提交，失败创建 slot tombstone且不回退`layer_count`。同一 frame 的多个 owner 可追加 topology operation，拒绝某个 owner 时其操作从共享 baseline 中移除而保留已提交/仍待处理的 peer；teardown 只在 peer topology participant 存在时 fail closed，释放后可重试；不开放任意shared、asset handle/model config或动态代码。见[E-V4-SCENESCRIPT-DYNAMIC-IMAGE-LAYER](../../history/scene/runtime-evidence-index.md#e-v4-scenescript-dynamic-image-layer)、[E-V4-SCENESCRIPT-AUTHORED-PEER-LAYER-MUTATION](../../history/scene/runtime-evidence-index.md#e-v4-scenescript-authored-peer-layer-mutation)与[E-V4-SCENESCRIPT-DYNAMIC-LAYER-TRANSACTION](../../history/scene/runtime-evidence-index.md#e-v4-scenescript-dynamic-layer-transaction) |
...
| `thisScene` layer mutation | `createLayer`, `destroyLayer`, `sortLayer`, `getLayerIndex`, `getInitialLayerConfig` | bounded text及package-literal image create/destroy/sort/index `L3 / S4 visible`；initial config `L0` | dynamic handle仍用domain/owner/generation隔离与负safe-integer identity。text config进入CoreText；image overload只接受全部调用均为安全直接字符串、model已launch-prepared且可投影为ordinary image。C QuickJS record现在以owner-local value baseline 加 domain-level topology operation journal保存value/order/destroy/create slot，host在Swift admission与全surface submission barrier后按 owner commit/discard并从共享 baseline 重放剩余操作；失败创建slot保留tombstone且不回退`layer_count`。多个 owner 的 create/sort/destroy 可在同一 frame 依次观察 provisional order；teardown 只在 peer topology participant 存在时 fail closed，释放后可重试。两者仍经同一Swift transaction与唯一Metal compositor；异常整批丢弃。真实`3792817546`创建63个bar | asset handle、动态路径、任意model/config/material、static sort、`getInitialLayerConfig`、任意JS heap rollback、复杂同轮顺序、多surface与官方parity |
