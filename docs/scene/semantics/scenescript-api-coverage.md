...
当前结论只有一条：项目已用同一 QuickJS-NG scene domain 执行 pass-owned numeric scalar 与 object-owned origin/scale Vec3 `init/update`，并开放 allowlisted `WEMath.smoothStep`、callback-scoped immutable `engine.timeOfDay/frametime/runtime`、typed `engine.userProperties` snapshot、descriptor-bound `thisLayer` effect lookup、callback-scoped `thisScene` layer lookup与只读 `ILayer.id/name/origin`，以及分别按 typed generation 去重的 `mediaThumbnailChanged({hasThumbnail})` 与 `mediaPlaybackChanged({state})`；但**没有完整 SceneScript API/host runtime**。逐 API 表继续使用本专题定义的 `L0-L4` 表达接口覆盖；`S*` 只引用[运行证据索引](runtime-evidence-index.md)对精确 identity 的证据闭合等级。两轴正交，不能互相批量推导，也不能与其他专题的同名 `L` 横向比较。

| 能力面 | 当前结果 | 精确边界 |
|---|---|---|
| Generic ECMAScript VM/API | `L3 bounded scalar+Vec3+typed snapshots+effect/layer/animation handles+two media events / S4 bounded visible` | QuickJS-NG 开放 inline scalar及object origin/scale Vec3 `init/update`、唯一 allowlisted `WEMath` module、immutable engine/user-property snapshots、descriptor-bound effect lookup、scene layer name/index/id/count lookup与只读origin，以及同一media inbox驱动的thumbnail/playback typed events；没有通用 file module graph、scene/layer mutation、完整event queue、timer/job scheduler、mutable `shared` 或完整 lifecycle |
...
上述 bounded 子集的执行 identity 来自作者结构和 typed target，不以完整样本/source identity 选择视觉答案。精确证据分别见 [E-TEXT-SCRIPT](runtime-evidence-index.md#e-text-script)、[E-EFFECT-BLEND-TRANSFORM](runtime-evidence-index.md#e-effect-blend-transform)、[E-V2-SCENESCRIPT-MEDIA-PLAYBACK-EVENT](runtime-evidence-index.md#e-v2-scenescript-media-playback-event)、[E-BOUNDED-LAUNCH-ORIGIN-TRANSITION](runtime-evidence-index.md#e-bounded-launch-origin-transition)和[E-BOUNDED-INPUT-PROJECTION-WIRING](runtime-evidence-index.md#e-bounded-input-projection-wiring)。
...
| 五个 media events | status/playback/properties/thumbnail/timeline 变化事件 | thumbnail/playback `L3 bounded / S3 executed`; 其余generic `L0`; bounded colors/text wiring `L2-L3` | artwork与playback各自generation只向每个active property owner派发一次对应callback；`mediaPlaybackChanged({state})`在同帧`update`前执行，0/1/2常量由冻结的`MediaPlaybackEvent`提供，两类event均复用typed handles与原子mutation buffer。真实`2067939514`证明thumbnail animation命令；真实`2938612768`证明playback event接管10个新增scalar target且正式画面不回退。没有通用event queue或live macOS producer | status/properties/timeline JS事件、thumbnail colors/previous、跨类型完整event ordering、重复/乱序provider仲裁、reload/teardown与官方行为对照 |
