...
| Generic object Boolean visibility owner | `L3 bounded / S4 startup-destroy + cursor + dependency/dynamic-image interactions` | image/solid/text/container 的object `visible` Bool wrapper按descriptor identity、target、wrapper、content kind和预算进入同一effectful QuickJS/snapshot，不再扫描源码写法选择owner。dependency video command与安全package-local literal image create/sort/transform/visibility继续按typed host合同执行；exact authored handle可写`origin/scale/angles/visible`及真实text `text`。静态topology只开放owner销毁自身且无子层：destroy覆盖当帧Bool return，并在全surface barrier后持久移出render order/descriptor和下一帧snapshot；跨owner、带子层、stale、回生和static sort/create拒绝。真实`3768229922`的layer354退出开场，layer202点击展开作者面板；`2959875782`有17/17普通completion，`3775355045`可见X-Ray交互，`3792817546`创建63个动态bar。callback、Swift preflight或surface submission失败会同时撤销primary value和全部owner effects；不开放任意asset handle/model config。见[作者启动与点击闭环](runtime-evidence-current.md#e-2026-09-13-authored-startup-destroy-cursor)、[E-V4-SCENESCRIPT-DYNAMIC-IMAGE-LAYER](../../history/scene/runtime-evidence-index.md#e-v4-scenescript-dynamic-image-layer)与[E-V4-SCENESCRIPT-DYNAMIC-LAYER-TRANSACTION](../../history/scene/runtime-evidence-index.md#e-v4-scenescript-dynamic-layer-transaction) |
