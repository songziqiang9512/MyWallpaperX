...
| 能力面 | 当前结果 | 精确边界 |
|---|---|---|
| Generic ECMAScript VM/API | `L3 bounded scalar+String+Vec3+typed snapshots+effect/layer/animation/dynamic-text handles+audio buffers+media/cursor/property events+timers/jobs+lifecycle / S4 bounded visible` | QuickJS-NG 开放 inline scalar、object-owned text String及object origin/scale Vec3 `init/update`、allowlisted `WEMath.smoothStep/mix`、immutable engine/user-property snapshots、descriptor-bound effect/layer identity、scene layer lookup、typed layer setter与有界dynamic text create/destroy/sort、global-only 16/32/64 AudioBuffers、三个media event、bounded cursor enter/leave/down/up/click、Vec3 applyUserProperties、callback-scoped owner timer/Promise job scheduler及stop/scene-switch exact-once destroy；同一scene domain存在mutable `shared` carrier。仍无file module graph、asset/model/image layer创建、静态层sort publication、ordered sub-frame event queue、async module/top-level await、通用initializer或完整multi-surface lifecycle |
