...
| `MediaPropertiesEvent` | title/artist/subTitle/albumTitle/albumArtist/genres/contentType | API `L3 bounded seven-field String / S4 bounded visible` | inbox snapshot按properties generation向String owner派发冻结的完整七字段event并在同帧update前执行；真实`2938612768`的3个Text owner分别消费受控title/albumArtist等字段，输出9/16/9 UTF-8 bytes并发布动态文字texture。同generation去重、stale、错型、任一字段预算/控制字符与callback failure由自动门锁定；缺省optional字段在新generation按项目策略置空，不保留上一曲值 | live producer、真实原子曲目切换/clear、跨事件ordering、multi-surface与官方行为对照；19个非String origin/alpha consumer等待统一有序媒体事件批次。见[E-V4-MEDIA-PROPERTIES-SEVEN-FIELD-STRING](runtime-evidence-index.md#e-v4-media-properties-seven-field-string) |
