...
当前结论只有一条：项目已用同一 QuickJS-NG scene domain 执行 pass-owned numeric scalar、object-owned text String、object-owned origin/scale Vec3与object `instanceoverride.rate` scalar `init/update`，并开放 allowlisted `WEMath.smoothStep/mix`、callback-scoped immutable `engine.timeOfDay/frametime/runtime`、typed `engine.userProperties` snapshot、descriptor-bound `thisLayer` effect lookup、callback-scoped `thisScene` layer lookup，以及有界text layer `createLayer/destroyLayer/sortLayer/getLayerIndex`与typed layer setter；动态拓扑通过launch-scoped mutation transaction在下一帧进入现有CoreText publication和唯一Metal compositor。现役还包括global-only `engine.registerAudioBuffers`及retained 16/32/64 `AudioBuffers`、三个typed media event、bounded object `cursorEnter/cursorLeave/cursorDown/cursorUp/cursorClick`、per-scene mutable `shared` carrier、Vec3 owner首次全量/后续delta `applyUserProperties`，以及callback-scoped、owner-isolated的bounded timer与Promise job drain；但**没有完整 SceneScript API/host runtime**。逐 API 表继续使用本专题定义的 `L0-L4` 表达接口覆盖；`S*` 只引用[运行证据索引](runtime-evidence-index.md)对精确 identity 的证据闭合等级。两轴正交，不能互相批量推导，也不能与其他专题的同名 `L` 横向比较。

| 能力面 | 当前结果 | 精确边界 |
|---|---|---|
| Generic ECMAScript VM/API | `L3 bounded scalar+String+Vec3+typed snapshots+effect/layer/animation/dynamic-text handles+audio buffers+media/cursor/property events+timers/jobs / S4 bounded visible` | QuickJS-NG 开放 inline scalar、object-owned text String及object origin/scale Vec3 `init/update`、allowlisted `WEMath.smoothStep/mix`、immutable engine/user-property snapshots、descriptor-bound effect/layer identity、scene layer lookup、typed layer setter与有界dynamic text create/destroy/sort、global-only 16/32/64 AudioBuffers、三个media event、bounded cursor enter/leave/down/up/click、Vec3 applyUserProperties及callback-scoped owner timer/Promise job scheduler；同一scene domain存在mutable `shared` carrier，但未证明通用initializer/module lifecycle。仍无file module graph、asset/model/image layer创建、静态层sort publication、ordered sub-frame event queue、async module/top-level await或完整reload/teardown |
...
| `311115e3` 其余 bounded native projections | `L2 / S2 wired / visible unknown` | native hover、primary-button click、launch-origin与audio-scaled value已分别被generic cursor/Vec3/AudioBuffers owner取代并删除；shared alpha、identity display与media colors仍只有有界产品接线。它们不是通用shared property owner或live media API |
...
上述 bounded 子集的执行 identity 来自作者结构和 typed target，不以完整样本/source identity 选择视觉答案。精确证据分别见 [E-TEXT-SCRIPT](runtime-evidence-index.md#e-text-script)、[E-EFFECT-BLEND-TRANSFORM](runtime-evidence-index.md#e-effect-blend-transform)、[E-V2-SCENESCRIPT-MEDIA-PLAYBACK-EVENT](runtime-evidence-index.md#e-v2-scenescript-media-playback-event)、[E-V2-SCENESCRIPT-MEDIA-PROPERTIES-EVENT](runtime-evidence-index.md#e-v2-scenescript-media-properties-event)、[E-V2-SCENESCRIPT-AUDIO-BUFFERS](runtime-evidence-index.md#e-v2-scenescript-audio-buffers)、[E-V2-SCENESCRIPT-CURSOR-ENTER-LEAVE](runtime-evidence-index.md#e-v2-scenescript-cursor-enter-leave)、[E-V2-SCENESCRIPT-CURSOR-CLICK-LAUNCH-OWNER-RETIREMENT](runtime-evidence-index.md#e-v2-scenescript-cursor-click-launch-owner-retirement)、[E-V2-SCENESCRIPT-OWNER-TIMERS](runtime-evidence-index.md#e-v2-scenescript-owner-timers)、[E-V2-SCENESCRIPT-PROMISE-JOBS](runtime-evidence-index.md#e-v2-scenescript-promise-jobs)和[E-V2-SCENESCRIPT-DYNAMIC-TEXT-LAYER](runtime-evidence-index.md#e-v2-scenescript-dynamic-text-layer)。
...
| `init(value)` / `update(value)` 的 typed value | 入参是绑定 property 当前值；返回兼容值写回；无返回则保持原值 | QuickJS pass scalar、object-owned text String与object origin/scale Vec3为 `L3 bounded / S4 bounded visible`；其他generic类型仍 `L0` | QuickJS-NG在同一scene domain实际执行scalar/String/Vec3 ECMAScript `init/update`，输入是authored/user/Timeline合并后的current-frame preliminary value，成功结果才进入统一`sceneScript` channel；Vec3允许返回对象、原位修改或scalar splat，String只接受exact JS string。callback异常/错型不提交Script值，保留owner-entry current。真实`2802243144`两个origin owner完成callback；真实`2938612768`的3个String owner从旧Text AST取得执行权并可见更新title。各项都无sample/layer/hash dispatch，且VM owner不把裸JS/Metal handle暴露给Swift | generic bool、Vec2/Vec4/matrix、完整mutation/coercion/API、更多target、通用handle与官方行为对照；重复/excluded candidate仍缺typed conflict aggregation；单个真实样本不证明整样本兼容 |
...
| source/module loader | 加载 inline 或 `.js` 源码及其 export/import | `L3 inline + WEMath bounded` | `I` binding IR保存已准入wrapper的inline source；QuickJS 执行 inline module，并只允许 import 项目自有 `WEMath.smoothStep/mix`。unknown import 在 owner create 阶段 typed fail-closed，不暴露文件系统或 native module | `.js` file graph、规范化路径、UTF-8/大小限制、其他 allowlisted module、依赖图、循环/缺失/越界负向门 |
| ECMAScript VM | 受控 ECMAScript 环境，无 DOM/Web/Node/shell/任意文件系统 | `L3 bounded scalar+String+Vec3+typed snapshots / S4 bounded visible` | QuickJS-NG基于固定upstream revision `bbe0480a68c664d7737eecde260fd47224b2c0d6`，仅增加释放pending-job持有值的宿主API；每scene一个runtime/context，scalar/String/Vec3 binding共用generation-checked owner模型。真实`2134765860`闭合scalar日夜consumer，真实`2802243144`闭合两个object-origin Vec3 callback，真实`2938612768`闭合3个String callback。callback的`engine`与user-property object均为每次调用创建的深冻结数据快照；无DOM、Node、filesystem、network或process host module | bool、Vec2/Vec4/matrix、其他host handle/module、更多真实producer/consumer、官方行为对照 |
| budget/error boundary | 每实例/每帧时间、指令、内存和 timer 有界；脚本错误不终止 renderer | `L1 bounded scalar / S4 bounded visible` | QuickJS domain设置 heap/stack/interrupt budget；timer另有每owner 32槽与0…86400000 ms范围，单callback job上限64。contract harness覆盖 compile/callback/timer/job exception、disabled fuse、bad return、infinite loop interrupt、timer/job overflow、stale owner与owner isolation；App 负 fixture记录`exception / previous-current`，后续graph/compositor/next-frame仍成功；本地budget不冒充官方watchdog parity | OOM/recursion、每帧跨callback聚合job/timer量、wall-time watchdog、diagnostic aggregation、更多renderer/App failure injection |
| Promise/job drain | callback产生的microtask必须在同一owner host和预算内排空；残留不得交给下一owner | `L3 bounded / S3 executed` | 每callback最多执行64个pending job；handled rejection可继续，unhandled rejection、job exception、context mismatch与预算超限熔断当前owner并清空scene context队列。timer resolve的Promise在同一timer callback host内排空；module顶层job/top-level await保持拒绝 | 当前真实corpus无Promise/async/await/queueMicrotask调用，未取得App可见consumer；async module、跨帧外部Promise source、reload/stop residue和官方动态对照仍缺 |
...
| `update(value)` | 每个渲染帧调用；动画应乘 `engine.frametime` | `L3 bounded scalar+String+Vec3 / S4 bounded visible` | generic callback从authored/user/Timeline preliminary current取值，并读取同一host frame的immutable `timeOfDay/frametime/runtime`；失败只保留该owner的previous-current。真实`2134765860`闭合scalar日夜差分，`2802243144`闭合Vec3，`2938612768`闭合String title；旧bounded Text AST仅保留通用VM尚未证明的target | bool/Vec2/Vec4/matrix、raw/effective delta官方parity、更广event mutation与完整teardown |
...
| `applyUserProperties(changed)` | 首次加载调用，之后只含变化键；使用 `hasOwnProperty` | Vec3 owner `L3 bounded / S4 visible`；其他generic owner `L0` | QuickJS在Vec3 owner首轮派发全部typed `scriptProperties`，以后只派发值变化或删除为null的键；无变化不派发。payload在callback前成为`engine.userProperties`，异常仅禁用该owner并保留previous-current。真实`2974757317`的10个origin owner完成并产生可见卡片位移 | scalar/String/其他owner、跨类型批量顺序、reload/多scene、callback mutation冲突和官方行为对照 |
...
| `cursorDown` / `cursorUp` / `cursorClick` | 对象上按下、释放和同对象完整点击 | generic `L3 bounded / S4 click-visible` | host-frame primary edge命中作者object时向QuickJS owner派发down并捕获该owner；release向capture派发up，同一object仍命中才派发click。真实`2974757317`记录exact一次press/release/`cursorClick`且卡片由展开折叠；旧native launch/click owner已删除 | 当前没有ordered sub-frame event snapshot，快速点击可能漏失；drag-out/capture真实门、候选传播/parent mutation、puppet hitBox、多按钮、多surface与官方对照仍缺 |
| 五个 media events | status/playback/properties/thumbnail/timeline 变化事件 | thumbnail/playback/properties `L3 bounded`；properties为`S4 bounded visible`、前两项为`S3 executed`；其余generic `L0` | artwork、playback与properties各自generation只向每个active property owner派发一次对应callback；playback/properties在同帧`update`前执行，均复用typed handles与原子mutation buffer。真实`2067939514`证明thumbnail animation命令；真实`2938612768`证明playback接管10个scalar target，并证明3个String owner接管旧Text AST且title可见更新。没有通用event queue或live macOS producer | status/timeline JS事件、properties除title/artist外字段、thumbnail colors/previous、跨类型完整event ordering、重复/乱序provider仲裁、reload/teardown与官方行为对照 |
...
| `ILayer` transform | `origin`, `angles`, `scale`, `parallaxDepth`, `name`, `visible` | `id/name/origin getter L3 bounded / S4 bounded visible`；owned dynamic `origin/angles/scale/visible setter L3 bounded / S4 representative visible`；其余`L0` | descriptor catalog开放safe-integer `id`、`name`与actual `Vec3 origin`；typed setter只允许当前owner创建的dynamic layer，并在active callback写入owner-local finite mutation，Swift帧边界原子提交。动态text layer的origin/sort已在代表内容可见 | authored/static layer setter、parallaxDepth、angles/scale/visible getter、local/world与parent语义、asset/model/image layer及官方重名行为仍缺 |
...
| `thisScene` layer mutation | `createLayer`, `destroyLayer`, `sortLayer`, `getLayerIndex`, `getInitialLayerConfig` | bounded text create/destroy/sort/index `L3 / S4 representative visible`；initial config `L0` | `createLayer`只接finite、budgeted text config，分配domain内负safe-integer identity；每owner最多64、scene动态最多256、总catalog最多4096。opaque handle含domain/owner identity，跨owner、stale、伪造对象拒绝；sort只接受当前owner动态layer。C mutation snapshot经launch-scoped Swift transaction原子校验，并在下一帧复用CoreText publication与唯一Metal compositor；exception整批丢弃并保留authored previous-current | asset/model/image layer、name保真到render descriptor、static layer setter/sort、`getInitialLayerConfig`、destroy/update同轮精确顺序、reload/teardown归零、多surface与官方parity |
...
| `MediaPropertiesEvent` | title/artist/subTitle/albumTitle/albumArtist/genres/contentType | API `L3 bounded / S4 bounded visible`；只含title/artist | inbox snapshot按properties generation向String owner派发`mediaPropertiesChanged({title, artist})`并在同帧update前执行；真实`2938612768`把可见title从`Wallpaper Music`更新为`春日歌`。同generation去重、stale、错型与callback failure由自动门锁定；没有live producer | 其余字段、缺字段语义、live producer、原子曲目切换、跨事件ordering与官方行为对照；作者读取`albumArtist`时当前安全保持原值 |
...
| `engine.setTimeout` | 毫秒延迟，一次性 callback；返回函数可提前取消 | `L3 bounded / S3 executed` | `D/F` 在callback-scoped `engine`开放0…86400000 ms一次性timer；每owner与timer identity绑定，返回函数幂等取消，轮前snapshot保证callback中新建timer不同轮执行。同一runtime只推进一次，pause的零frame-time不累计；执行门覆盖取消、callback throw、跨owner取消拒绝、Promise resolve和peer存活 | 当前真实corpus扫描无作者调用，未取得App可见consumer；scene seek、reload/switch/stop归零和官方动态对照仍缺 |
| `engine.setInterval` | 毫秒周期 callback；返回函数用于停止 | `L3 bounded / S3 executed` | `D/F` 每owner固定最多32个timer，周期每帧至多执行一次、重置完整周期、不追赶也不保留overshoot；执行门覆盖长帧、自取消、预算溢出、同runtime不重复推进、job drain和owner销毁释放 | 当前真实corpus扫描无作者调用，未取得App可见consumer；pause/resume、scene seek、reload/stop residue与callback/帧总预算聚合仍缺 |
...
|---|---|---:|---|---|
| property input union | `Number | Boolean | String | Vec2 | Vec3 | Vec4 | Mat3 | Mat4` | Number、object-owned text String与object-origin/scale Vec3 `L3 bounded`；其余`L0` | C/Swift门锁定finite/UTF-8 bounded input、boxing、immutable snapshots与owner generation；真实`2802243144`执行两个Vec3 input，真实`2938612768`执行3个String input | bool、Vec2/Vec4/matrix与完整schema |
| property return union | v2.8 声明返回 `Number | Boolean | String | Vec2 | Vec3 | Vec4`，没有 Mat3/Mat4 | Number、String与object-origin/scale Vec3 `L3 bounded`；其余`L0` | String只接受exact JS string或undefined保持输入；Vec3支持对象return、mutation和finite scalar splat；错型/非有限值只禁用该owner且不发布 | bool/Vec2/Vec4、完整coercion与官方返回语义对照 |
| `AnimationEvent` | `name`, `frame` | `L0` | `N` | Timeline/puppet source、crossing 和 frame 数值测试 |
| media/cursor DTO | 本文 Input/Media 表列出的 typed event object | media thumbnail/playback/properties与cursor enter/leave `L3 bounded`；其余`L0` | media DTO按typed generation派发；cursor enter/leave object不可写/不可配置，含独立不可变world/local Vec3，owner generation与异常隔离有C/Swift门及真实运行 | cursor move/down/up/click detail/capture、完整坐标空间、其余media字段/事件与跨event排序 |
