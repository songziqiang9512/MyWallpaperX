...
| `cursorMove` | 指针移动时传 `CursorEvent` | generic `L3 bounded single-surface inside-hit / S4 isolated-visible` | surface ordered sample的位置真实变化时，只向current admitted authored owner派发move，复用现有world/local hit和QuickJS mutation transaction；省略authored scale按schema identity准入。真实`3122339805`的layer 207用原作者脚本把`thisLayer.origin`更新为`(1482.804932,800,0)`；完整样本证明generic-only主链，保留原脚本的13-layer派生隔离fixture证明窗口位移后稳定。[E-V4-SCENESCRIPT-CURSOR-MOVE-DRAG](runtime-evidence-index.md#e-v4-scenescript-cursor-move-drag) | move只在当前hit内派发；drag-out/capture move、同帧多move写同target、跨owner传播、parent/puppet/perspective、多按钮、失焦及官方坐标/顺序仍缺 |
| `cursorDown` / `cursorUp` / `cursorClick` | 对象上按下、释放和同对象完整点击 | generic `L3 bounded single-surface / S4 click-visible` | AppKit local/global primary event进入surface-local有界ordered batch，每个sample以同一current camera/world/model inverse命中作者object；down捕获owner，release向capture派发up，同一object仍命中才派发click。真实`2974757317`在同一main callback内记录press/release，下一VM frame exact一次`cursorClick`且卡片由展开折叠；512-event预算溢出整批拒绝并重同步，旧native launch/click owner已删除。[E-V4-SCENESCRIPT-ORDERED-POINTER-EDGES](runtime-evidence-index.md#e-v4-scenescript-ordered-pointer-edges) | multi-surface仍按frame snapshot；同帧多个callback写同一target、drag-out/capture真实门、候选传播/parent mutation、puppet hitBox、多按钮、失焦与官方顺序/坐标对照仍缺 |
