...
|---|---|---:|---|---|
| `Vec2` | `x/y`；构造 number/Vec3/string；完整声明方法族 | `L3 bounded constructor/basic arithmetic` | QuickJS bootstrap提供finite number、string及x/y object copy构造，`x/y`、`copy/add/subtract/multiply/divide/isFinite/toString`；`input.cursorScreenPosition`与`engine.canvasSize`现在是frozen Vec2，pass return只发布x/y | 其余官方方法、Vec3 overload的完整官方coercion、equals/length/dot/normalize等golden、零长度和除零官方结果 |
