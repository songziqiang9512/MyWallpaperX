...
当前结论只有一条：项目已用同一 QuickJS-NG scene domain 执行 pass-owned numeric scalar 与 object-owned origin/scale Vec3 `init/update`，并开放 allowlisted `WEMath.smoothStep`、callback-scoped immutable `engine.timeOfDay/frametime/runtime`、typed `engine.userProperties` snapshot，以及 descriptor-bound callback-scoped `thisLayer.getEffect(name|index)` / `getEffectCount()` 和 bounded `IEffect.executeMaterialFunction`；但**没有完整 SceneScript API/host runtime**。逐 API 表继续使用本专题定义的 `L0-L4` 表达接口覆盖；`S*` 只引用[运行证据索引](runtime-evidence-index.md)对精确 identity 的证据闭合等级。两轴正交，不能互相批量推导，也不能与其他专题的同名 `L` 横向比较。

| 能力面 | 当前结果 | 精确边界 |
|---|---|---|
| Generic ECMAScript VM/API | `L3 bounded scalar+Vec3+typed snapshots+effect handle / S4 bounded visible` | QuickJS-NG 开放 inline scalar及object origin/scale Vec3 `init/update`、唯一 allowlisted `WEMath` module、三个 immutable `engine` frame fields、typed immutable user-property object与descriptor-bound effect name/index/count lookup；没有通用 file module graph、scene/layer mutation handle、event queue、timer/job scheduler、mutable `shared` 或完整 lifecycle |
...
| `thisLayer: ILayer` | 当前脚本 owner 的 layer handle | `L3 bounded effect-layer handle / S3 contract-executable` | callback 期间只读开放 descriptor-bound `getEffect(name|index)` 和 `getEffectCount()`；scalar/Vec3 owner 共用同一 catalog bridge，脚本不能伪造 effect identity | 其他 layer 字段/方法、跨层访问、通用 stale wrapper 与销毁语义仍缺；新 name lookup 尚无真实 authored consumer 可见正证 |
