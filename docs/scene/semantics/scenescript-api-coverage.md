...
| `ILayer` parenting | 两个 `setParent` overload、`getParent`, `getChildren` | `getParent L3 bounded / S3 real owner-executed`；其余`L0` | immutable authored parent identity进入callback-scoped catalog；`getParent()`返回同domain persistent handle或`null`，duplicate/self/missing/non-safe-integer parent在建域前拒绝。真实`3768724269`不再出现原parent缺失失败；未实现parent mutation与children snapshot | `setParent`、`getChildren`、动态parent、attachment、effective visibility/transform传播、销毁门和官方行为对照 |
