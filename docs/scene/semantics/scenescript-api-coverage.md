...
| Generic object Boolean visibility owner | `L3 bounded / S4 dependency-video + dynamic-image interactions` | exact root image/solid/text Bool owner共用同一QuickJS/snapshot。普通owner仍为value-only；dependency cohort开放own visibility与scene video command；dynamic-image cohort在全部`createLayer`参数为安全package literal且资源launch-prepared时开放owner及其动态实例的create/sort/transform/visibility；精确typed target identity允许通过`getLayer`/`enumerateLayers`取得的authored visibility句柄写入，非target peer仍只读。真实`2959875782`有17/17普通completion；`3775355045`可见X-Ray交互；`3792817546`创建63个动态bar；真实`3610154602`的layer 92初始化创建63个动态bar且不再发生visibility冲突。callback失败撤销同批value/command/mutation；不开放任意shared、任意跨owner target、asset handle/model config或动态代码。见[E-V4-SCENESCRIPT-DYNAMIC-IMAGE-LAYER](runtime-evidence-index.md#e-v4-scenescript-dynamic-image-layer) |
...
| `init(value)` / `update(value)` 的 typed value | 入参是绑定property当前值；返回兼容值写回；无返回则保持原值 | QuickJS scalar/Bool/String/Vec2/Vec3为`L3 bounded` | typed owner从同一preliminary snapshot取值；Vec2使用bounded prototype并只发布x/y。effectful Bool callback的返回、video command或dynamic-image mutation统一在Swift preflight后提交；异常/错型保留owner-entry current | Vec4/matrix、Vec2完整coercion/API、更多target及官方行为对照 |
...
| `ILayer` transform | `origin`, `angles`, `scale`, `parallaxDepth`, `name`, `visible` | getter与owner-bound authored/dynamic `origin/angles/scale/visible` setter `L3 bounded / S4 visible` | descriptor catalog开放safe-integer identity与同一frame snapshot；owner-local finite mutation成功后从下一帧进入唯一dynamic snapshot。dynamic text与prepared package-image复用同一topology transaction；真实3D Clock与`3792817546`均可见 | parallaxDepth、authored static text/content、local/world与parent语义、任意asset/model、跨owner冲突顺序及官方对照 |
...
| `thisScene` lookup | `getLayer(name|index)`, `getLayerByID`, `getLayerCount`, `enumerateLayers` | `L3 bounded / S4 bounded visible` | callback-scoped catalog按当前authored+dynamic render order查找；`enumerateLayers()`返回该回调时点的persistent handle snapshot，动态层保持transaction render-order tail且不覆盖authored ordinal。最多4096层，safe-integer id、有限origin，缺失/非整数/越界/超额/stale均拒绝。真实`3122339805`消费name lookup；真实`3610154602`经枚举取得精确visibility target并完成初始化动态层事务 | 字符串ID与官方duplicate-name行为；当前重名取首个catalog ordinal只属项目策略；大规模动态增删、跨callback retained array和官方枚举快照语义仍缺 |
...
| `ITextLayer` | `text`, `color`, `alpha`, `opaquebackground`, `backgroundcolor`, `pointsize`, `font`, `padding`, `horizontalalign`, `verticalalign`, `anchor`, row/width limits | property-bound color/pointsize `L3 bounded`；其余handle setter `L0` | exact object binding可写color/pointsize；动态值经同一snapshot、CoreText generation与last-ready source atom进入普通effect/compositor。真实`3655958892`闭合3条带Precise Blur文字颜色；这不授权任意`thisLayer` setter | text/content及其余字段setter、live media producer、复杂layout/effect invalidation、Windows text golden与官方parity |
...
|---|---|---:|---|---|
| `thisLayer` / `thisScene` | 当前owner和scene的typed handles | `L3 bounded` | descriptor-bound static lookup已执行；owned dynamic text与package-literal image支持typed setter/create/destroy/sort，generation/owner/伪造handle拒绝 | authored static setter/sort、任意asset handle/model config、完整parent/topology、跨scene/screen泄漏负门与官方parity |
