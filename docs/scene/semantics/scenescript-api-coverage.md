...
| Generic object Boolean visibility owner | `L3 bounded / S3 real owners-executed` | exact `objects[index].visible`、Bool authored definition、root image/solid leaf及无topology/dependency/named-reference角色的`script/value`或primitive `script/scriptproperties/value`进入同一QuickJS value-only owner；effect-bearing leaf可准入。真正Bool返回进入同一snapshot与parent-aware visibility，`undefined`保留输入，bad return只熔断本owner；`shared`、handle、timer、audio、event、destroy与动态代码不开放。真实`2959875782`有17/17首帧completion、0 VM failure、17个owner exact quiescent teardown，多个true target实际编码source passthrough。整景仍被后序composition/cross-layer startup/shared/event状态覆盖且layer1478/1485局部effect失败，故不证明目标最终可见、完整display/shared API、整样本或parity；见[E-V4-SCENESCRIPT-LAYER-VISIBILITY-BOOLEAN](runtime-evidence-index.md#e-v4-scenescript-layer-visibility-boolean) |
