...
`d5bff76e` 再新增一个 **L3 bounded native texture-animation profile**：只在 raw source SHA-256、`visible` host、完整 `script/scriptproperties/user/value` wrapper 和三个 bounded delay 数值全部匹配时，为多 image playback 建立实例级 scene-time 状态机。它执行首轮 stop、initial delay、末帧 reset 与独立随机等待，不创建系统 timer，也没有样本/layer/resource-name 分支；未知或变异脚本继续走原生 autoplay。该子集不开放通用 `ITextureAnimation` handle 或任意 JavaScript API。
...
| texture animation | `frameCount`, `duration`, `rate`, `play/pause/stop`, `isPlaying`, `get/setFrame`, `join` | `L3 bounded` / generic `L0` | BC1/2/3 多 image TEX 保持严格 axis-aligned/integer/same-extent 的 native autoplay；另以 source SHA + 完整 wrapper/property shape 只准入一个 delayed-loop profile。`3768903841` 五层各自执行首轮 stop/initial delay、末帧 reset 与独立随机等待，scene-time 状态机无系统 timer 残留；未知/变异脚本继续 autoplay | 通用 instance-local detach/join、frame/rate/play/pause/stop/seek handle、owner timer/callback、同帧冲突与 Windows timing golden |
