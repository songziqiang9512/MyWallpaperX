...
| Generic object Boolean visibility owner | `L3 bounded / S4 dependency-video + dynamic-image interactions` | exact root image/solid/text Bool owner共用同一QuickJS/snapshot。普通owner仍严格value-only；dependency cohort只开放own visibility与scene video command；dynamic-image cohort只在全部`createLayer`参数为安全package literal且资源launch-prepared时开放owner及其动态实例的create/sort/transform/visibility。真实`2959875782`有17/17普通completion；`3775355045`可见X-Ray交互；`3792817546`创建63个动态bar，另有text layers `96/100` Bool completion但无文字ROI。callback失败撤销同批value/command/mutation；不开放任意shared、跨owner target、asset handle/model config或动态代码。见[E-V4-SCENESCRIPT-DYNAMIC-IMAGE-LAYER](runtime-evidence-index.md#e-v4-scenescript-dynamic-image-layer) |
