...
本轮跨专题执行顺序不另建平行计划：App/helper 定位回归和 ready 公开类型目录发布均已闭合实现批。用户在 2026-09-20 报告普通产品路径又出现远程数据为空、无法登录和找不到壁纸；当前签名候选已重新执行 `App -> SteamServiceClient -> bundled helper -> SteamKit`，直接协议与普通 UI 均未复现列表／搜索缺失，主动登录已到二维码 challenge 后由取证取消。该证据只说明本轮当前候选远程读取复验通过，不关闭账号认证终态，也不把本机旧安装包或另一个不同 identity 的 Debug 包确认为报告根因；相同当前 identity 再现时必须重新定位。下一批回到 [Scene 当前断点队列 Q1.4–Q1.6](scene-open-breakpoint-queue-2026-09-09.md#q14-音频频谱与采集生命周期2026-09-20-当前态)，依次做音频样本全集与普通 App 采集、频谱分布／冻结、丁达尔射线、鼠标点击及粒子／轨迹／X-Ray。每项先核对公共 owner 与首断点；旧分支、重复 owner 或错误版本目录不能因已有测试而保留。
...
2. **物理入库与暂存回收（实现批已闭合，真实门保留）：**新 v2 内容经受管 incoming 验证后原子进入 `创意工坊/Video|Web|Scene/<WorkshopID>-<generationUUID>`，marker 只声明 ownership，metadata rename 仍是唯一 ready 点；legacy scanner 对整个保留命名空间 fail-closed，避免 marker 损坏后产生第二 ready owner，库根使用符号链接时也按物理父目录识别。v1 隐藏内容只读复制，复制前进入下载共用的容量队列/预留，发布前重核旧 metadata，旧播放路径不移动；失败有界退避且后续 reload 可重试。lease 与回收使用永不复用、含布局/类型/generation 的 storage identity，跨 actor admission 后重验原 inode 与 marker。正常成功、验证失败和取消不遗留本次 incoming UUID；helper descriptor identity 在首个进度事件落入独立 `jobs-v4.json`，terminal 不得首次绑定。恢复时 expected device/inode 随请求进入 helper，并在任何内容操作前对已打开根 descriptor 校验；v1/v2/v3 无 identity partial 清空恢复路径并创建新 lease，同名替换不会被写入或删除。v3 sidecar 存在但损坏时不回退旧 `jobs.json`。v1/incoming/v2 retained bytes 与本次新增共同受 16GiB 门约束。隔离事务 26/26、Steam 维护测试 101/101、服务级迁移四场景和 Developer ID Debug build通过；未运行真实账号下载、真实库迁移、迁移后播放、跨卷/断电，故 SK7.1 仍开放。
3. **最新报告复验（当前候选读取通过，报告根因与登录终态仍开放）：**2.0.9 (277) 当前候选 `CDHash=b710ba9ce08c9910ef1a32d7aedaf5eaa9ffa344` 的 bundled helper 直接查询返回 30 项、远端总量 3,209,722；普通 App 首屏、`Night vibe` 搜索和二维码 challenge 均可见。本机 `/Applications/MyWallpaperX.app` 是 2.0.8 (269)，`CDHash=fcf5ead265d79bcc551379a560234023a50c8087` 且 helper 缺失，必然无法沿产品主链接入 SteamKit；默认 Xcode DerivedData 中另一个 2.0.9 (277) 的 `CDHash=ae8b77823385bedc16efde77e75699cb37522539`、helper SHA-256 `9478c5e06f109c78caa71ea626d0b955a7312eb5e642da7d101eebd9432271f9` 也不是当前候选。两者只构成候选解释；由于用户报告时的实际 bundle identity 未留存，本批不把其中任一包武断指定为根因，也不通过未授权安装覆盖制造“修复”。若相同当前 identity 再现，则重新核对网络、helper session、query generation 与 UI 投影；真实扫码／密码／Guard terminal 与 token 恢复继续留在 SK7.1。下一工程项为 Q1.4 音频全集 census 与普通 App 采集基线。
...
| 路径 | 次数 | 当前观测 | 证据边界 |
|---|---:|---|---|
| 最新 2.0.9 (277) Developer ID Debug 候选 → bundled helper 直接查询／普通 App UI | 1+1 | App `CDHash=b710ba9ce08c9910ef1a32d7aedaf5eaa9ffa344`，helper SHA-256 `8fddd20d0c214cc5b6d9ea41019dfbefefc3cf7e374afefd7f38088aadb72016`。helper `queryBrowse` page 1 返回 30 项、`total=3,209,722`、`hasMore=true`、wrong-app 0、partial 0；普通 App 首屏显示远程卡片，搜索 `Night vibe` 返回目标项，账号入口生成 QR 并显示“等待扫码确认…”，随后主动关窗取消 | 证明**当前候选**的 helper spawn、匿名读取、列表／搜索投影和认证 challenge 发起；未扫码、未提交凭据，不能称登录成功。隔离根 `/private/tmp/mwx-steam-current.enOjnt` 及一次无效 defaults-suite 尝试 `/private/tmp/mwx-steam-current.WZEYU4` 已逐项移入废纸篓，无 helper/App 进程残留，真实库未写入 |
| 本机可启动 App identity 盘点 | 3 | 当前隔离候选见上；`/Applications/MyWallpaperX.app` 为 2.0.8 (269)、`CDHash=fcf5ead265d79bcc551379a560234023a50c8087`、helper 缺失；默认 Xcode DerivedData App 为 2.0.9 (277)、`CDHash=ae8b77823385bedc16efde77e75699cb37522539`、helper SHA-256 `9478c5e06f109c78caa71ea626d0b955a7312eb5e642da7d101eebd9432271f9` | 只证明本机存在会导致结果分歧的旧／不同 bundle；用户报告时的 bundle identity 未冻结，故不能把来源精确归到某一个包。未覆盖、删除或移动任何 App；安装当前候选不在本批授权内 |
