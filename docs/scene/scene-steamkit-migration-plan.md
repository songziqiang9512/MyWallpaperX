...
> 状态：现役专项计划；SK1–SK4.2 已有实现并完成本轮离线审查修复，SK4.3 已把最终版本 owner 从隐藏 `.mywallpaperx-steam-versions` 迁到公开 `Video/Web/Scene`：每个 v2 generation 都使用不可复用的 `<WorkshopID>-<UUID>` 直接子项，强 marker 证明 ownership，metadata rename 仍是唯一 ready 点；marker 缺失或损坏的保留命名空间也不会被 legacy scanner 重新发布。旧 v1 只读兼容并在启动时后台复制、核对旧指针后发布，迁移复用下载的唯一复制容量 owner并有界重试，播放 lease/当前引擎引用继续保护 previous-current。正常成功或失败均精确清除本次 `.mywallpaperx-steam-incoming/<UUID>`；helper 从已打开 lease 的文件描述符发布 device+inode，App 在首个进度事件即校验并写入独立 `jobs-v4.json`，随后以绑定同一 account/job/path/manifest/identity 的控制帧解除 helper 写入门；恢复请求必须把该 identity 送回 helper，helper 在任何目录/文件收养或写入前比较已打开根句柄，terminal 只能复核既有身份，cleanup 只删除同一物理 lease。`jobs-v3.json` 存在时是唯一前代权威，损坏则失败关闭；只有它不存在才只读导入更旧 `jobs.json`，两者均保持原字节。崩溃遗留和已退役版本按 marker、generation identity、龄期和完整保留集回收，跨等待后还会复核原 inode/marker。SK4.4 工程切片已闭合物理排空、manifest-bound 恢复、精确 lease 清理、同 job 重试、两活动作业/共享 chunk 上限及跨作业磁盘预留；SK5.1 工程切片已接通 item/attempt 精确数字进度、独立裁剪 fill 与卡片/详情增量投影；SK5.2 已接通账号隔离的工具栏 badge、单一瞬态任务 popover 及当前队列交互；SK5.3 已由同一 JobStore 接通持久 terminal history、attempt 汇总、有界保留/清理与已下载定位；SK6.1 已完成默认结构化路由和 sidecar 回退边界，SK6.2 已撤旧获取源码、产品引用、专用状态与 78.97 MiB 随包 runtime。SK7.1 已闭合 App/helper bundle 定位回归：构建、发布验证、产品定位和实际产物现统一为 `Contents/Resources/SteamService/SteamService`，不保留 `Contents/PlugIns` fallback。用户 2026-09-21 再次要求优先复核远程数据、搜索和登录后，基线 `d4d4d5bf` 的 fresh Developer ID Debug 候选重新通过 bundled helper 匿名列表／搜索及普通 App 首屏／搜索／二维码 challenge；当前源码主链仍未复现“远程数据为空／找不到壁纸／不能发起登录”。本机 `/Applications/MyWallpaperX.app` 同日仍是 2.0.8 (269) 且没有 bundled helper；取证中按同名 App 选择时它与当前候选曾同时运行，证明本机确有可产生不同结果的 bundle 身份歧义，但原报告运行身份未冻结，不能把旧包或瞬态网络武断定为根因。未获授权不覆盖 `/Applications`。随后基线 `2eacca1b` 又完成真实库只读盘点和隔离产品 App 启动迁移：真实隐藏版本根没有受管 generation，九个当前受管版本均已位于公开 `Scene/<WorkshopID>-<UUID>`；隔离 v1 在启动时发布为 v2，metadata 与 marker 同一，incoming/staging 都为零。真实扫码／密码／Guard 登录终态、token 恢复、已授权真下载、迁移后播放、24 小时回收终态、其余异常与完整性能矩阵仍待继续；current-HEAD 无账号审计已闭合复制中取消遗留 incoming generation 与 staging durable acknowledgement 两个窗口，下一工程批按队列贯穿 birth-time identity，再关闭 current-v4 fail-closed 等公共 owner 偏差；真账号门保留，随后继续音频剩余 no-demand 集合、光线、粒子和鼠标簇。SK7.2 的真实发行公证、最低支持机与完整账号链路仍未验收。
>
...
5. **原子入库：**Swift 校验 staged receipt（job/account/version/path/manifest）→复制或移动至库内临时版本→验证→原子发布记录；持久化提交阶段/receipt，覆盖文件提交与记录更新之间的崩溃恢复，不假设二者天然是同一原子操作。跨卷不能假定 rename 原子。磁盘满/取消/重复完成不覆盖旧 ready。更新使用版本目录或等价安全替换，正在播放的旧版本保留到消费端释放；属性/依赖/预览映射继续按稳定 workshopID 关联。
   ...
   更新保留旧版本原路径；移除受管下载写 tombstone，避免旧目录被扫描复活。版本回收只处理超过 24 小时的 v1 直接 UUID、incoming 直接 UUID 与 marker-backed v2 generation；保留集由当前 ready、JobStore 未决 commit、Scene/Web 活跃消费 token、Video 库持久路径及 daemon 当前／pending intent 共同组成，依赖型 Web 的同一个 token 同时租住壳与依赖宿主的具体版本。删除过程基于目录描述符、不跟随链接；跨 actor admission 后按最初 device/inode 和 exact marker 复核，同名普通目录替换只释放 retiring reservation并跳过。公开未知目录、链接和失效 marker 均不被回收 owner 收养。JobStore v4 给 receipt-bound staging 持久化 device/inode；成功、取消或不可恢复 manifest 失配只在 helper 物理排空后清理同一 lease，同名替换与旧版无 identity 记录失败关闭。新复制前把 v1、incoming 与所有保留命名空间 v2 generation 连同本次新增字节计入 16GiB retained cap；普通下载与 v1 迁移共用一个串行 copy-capacity owner、跨作业 reservation 与 256MiB 安全余量，不以独立 free-space 快照超卖。
...
Envelope：`v/type/requestId/processEpoch/accountEpoch`；认证另有 authAttemptId，查询有 queryGeneration/cursor，作业有 jobId/attempt/sequence。ready 返回版本/capabilities；下载必须包含 `download-staging-ack-v1`，不能只因同为协议 v1 就假定具备写入门。accepted 不等于完成；每 request 最多一个 terminal。进度可合并，terminal 不得被节流丢失。helper callback pump 不阻塞 stdin；关闭/取消能终止正在等 Guard 或 QR 的异步等待。
...
| 08 | SK3.2 | 浏览分页、过滤与详情 | 结构化 route 保留同 key 原始已加载集合，整集合投影排序、失败保留页。2026-09-16 实测后 `updated`（query_type 21）与分级筛选经 `taggroups` 开放：分面筛选（类型/分级/分辨率/分类）面内 OR、跨面 AND，discovery 服务端执行，author/personal 客户端后筛，分面组合参与 QueryKey。SK6.2 后 discovery/ID/作者/个人列表只有同一 store，作者页返回还原 store 原始页、nextPage、hasMore 与 partial errors；详情 children 映射为严格十进制 dependency IDs，不再从 HTML 补依赖。真实分页/权限仍不能称完整验收 |
...
| 10 | SK4.1 | JobStore、队列与持久化 | 先在线准入；staged/committing 非终态；cancelAll 一次保存、失败不假推进并提示。任务 envelope 在 SK5.3 升至 v3，本批以向后兼容的 optional staging device/inode 升至 v4；当前 owner 只写 `jobs-v4.json`。v3 文件存在即为唯一前代权威：有效则只读导入，损坏则原位失败关闭；只有 v3 不存在才尝试更旧 `jobs.json`，旧文件保持原字节且不隔离。v1/v2/v3 partial 没有 descriptor identity 时只保留任务意图并清空恢复路径，完整 App 操作仍待 SK7 |
...
| 13 | SK4.4 | 取消、恢复、有限并发与重试 | 工程切片已闭合：每个取消都等待原 startDownload terminal 后释放本地槽，账号 epoch 改变不丢弃物理排空 waiter。下载前 helper ready 必须声明 `download-staging-ack-v1`；随后只允许 `allocated(path/manifest/device/inode) → App 校验 account/job/descriptor 并原子保存 → exact acknowledge → create/write`。identity 在 helper 的同一锁内一次发布；wrong/missing epoch、保存失败、错误／重复／迟到 ack 及 30 秒超时均不放行。超时删除新空 lease并返回 typed integrity，App 清空 recovery；显式 retry 先重开持久 lease，缺失／失配时同次持久清空并 fresh start。未确认空 lease 在私有 staging 根内 unlink 前再次 reopen/复核；确定性空 replacement swap 保留 replacement，但不把 POSIX name unlink 表述成原子 compare-delete。terminal receipt 只能复核已持久身份；恢复成组携带 path/manifest/device/inode，helper 在任何内容操作前打开并核对根 descriptor；旧 identityless partial 走 fresh。App/helper 各至多 2 个活动作业、每 job 4 worker、跨 job 共享 4 个 chunk 槽；下载、入库复制与 v1 迁移共用容量 owner、256MiB 余量与同卷保守预留。fake-wire 覆盖 mandatory capability、save-before-ack、保存失败、wrong/missing epoch、timeout invalidation、missing-lease same-retry fresh、A/B 隔离、同名替换与旧 partial；helper 自检覆盖同锁 publication、错误 identity/epoch、重复 ack、取消／超时等待及两次 removal identity check。helper-only 改动现由 checkpoint 强制触发产品 build。真实双下载／续传／崩溃／disk-full 与可见 UI 仍并入 SK7，不以离线绿色代替 |
...
| 16 | SK5.3 | 任务历史、保留策略与跨视图一致性 | 工程切片已闭合：JobStore 持久 envelope 在历史切片升级为 v3，本批以 optional staging device/inode 升为 v4，仍读取 v1/v2/v3；jobs 与 terminal history 同一次原子写入，完成/失败/取消按 `jobID-attempt` 幂等记录，retry 沿用逻辑 job 并由面板汇总 attempts。保留配置冻结为最近 100 条且最长 30 天，每次写入/加载及打开历史面板时裁剪；历史按 SteamID 隔离，清空仅删当前账号终结记录，保存失败不假清除且不改变队列/失败重试意图/本地库。完成历史只保存既有库 recordID 引用；点击时重新通过当前 managed metadata/文件可用性判定，存在则导航已下载并定位/打开 inspector，不存在则明确提示并可转作品详情。popover 顶部已接“进行中｜历史”，切换/关闭解绑当前行高频 observer，历史只消费 JobStore 低频发布，无第二累计源。可执行测试覆盖旧 envelope/history 迁移、完成重启不重复、两次失败 attempt 汇总、账号隔离、100/30 天裁剪、清理原子失败与任务保留；AppKit arm64 无签名 Debug build 通过。真实 20 次跨页下载/取消/重试、可见定位、observer/task 计数及键盘/辅助功能仍归 SK7，未以离线门宣称交互验收完成 |
| 17 | SK6.1 | 老用户数据迁移与新路切换 | 工程切片已闭合：该批 Release 固定由 SteamKit route 接管、DEBUG 曾保留一次整版本回退参数；discovery、严格 ID、作者作品、在线个人列表与离线登录指引均先由同一 `SteamKitBrowseStore` 路由，creator SteamID 支持作者查询。App 启动和浏览入口不再读取旧密码、runtime 状态或私人 HTML cache，只允许此前授权的新 token 静默恢复。当前任务 owner 写 `jobs-v4.json`；v3 sidecar 存在时只读导入它或在损坏时失败关闭，只有 v3 不存在才兼容更旧 `jobs.json`，旧文件均保持原位且三者互不回写。既有 Video/Web/Scene 目录、managed/legacy metadata、属性与选择 owner 未改，启动仍先 reload 本地库。该批的 DEBUG 回退与保持原字节旧状态已在 SK6.2 按精确清单退役；SK6.1 的隔离 fixture 与构建证据不替代 SK7 真实升级、账号和播放门 |
...
2. 当前版本只写 `Application Support/MyWallpaperX/SteamJobs/jobs-v4.json`。首次没有该 sidecar 时，若 `jobs-v3.json` 存在，它就是唯一前代权威：有效则只读导入，损坏则保留原位并失败关闭，不得回退重放更旧任务；只有 v3 完全不存在才尝试 `jobs.json`。导入成功、失败、后续执行都不改名、不删除、不隔离旧文件。已有 v4 sidecar 时不再次合并旧文件。
3. 回退前先停止新版活动任务。上一版本仍只读取保留的 `jobs-v3.json`，更旧版本仍只读取 `jobs.json`；它们都看不到也不会破坏 `jobs-v4.json`。回退版本不得创建需要再向前合并的新任务；再次升级以既有 v4 sidecar 为准。可执行 fixture 冻结 v3 原始字节，证明 v4 导入、保存和重启均不回写回滚源。本地库 ready/legacy 目录继续由原库 owner 读取，不随任务 schema 回退。
...
本轮跨专题执行顺序不另建平行计划：App/helper 定位回归和 ready 公开类型目录发布均已闭合实现批。用户再次报告远程数据为空、无法登录和找不到壁纸后，基线 `d4d4d5bf` 的 fresh 签名候选已重新执行 `App -> SteamServiceClient -> bundled helper -> SteamKit`：直接协议与普通 UI 均未复现列表／搜索缺失，主动登录到达二维码 challenge 后由取证取消。随后基线 `2eacca1b` 的真实库只读 census 证明隐藏版本根没有受管 generation，隔离产品 App 启动把真实形状 v1 复制并发布为公开 `Scene/<WorkshopID>-<UUID>`，incoming/staging 无残留。音频条件 consumer 批提交为 `22e6835f` 后，又用该 current-HEAD 签名候选的 bundled helper 复核匿名首屏与 QR challenge 发起／取消；结果仍正常，且本机 `/Applications` 旧 2.0.8 包继续缺 helper。三类证据分别只覆盖当前候选公开读取／认证发起、Scene 启动迁移与安装身份分歧；账号终态、已授权真下载、迁移后播放及到龄回收仍开放。current-HEAD 无账号审计又复现并闭合复制中取消遗留本批 incoming UUID，以及 helper 在 App 持久化 lease identity 前即可写内容的窗口；下一工程批把 helper 已读取的 birth seconds/nanoseconds 贯穿 event/ack/receipt/resume/JobStore/cleanup identity，再关闭 current-v4 fail-closed。真账号门不足时保留外部门而不重复已闭合发布审计。之后回到[音频剩余 no-demand 集合](scene-open-breakpoint-queue-2026-09-09.md#q14-音频频谱与采集生命周期2026-09-20-当前态)，继续丁达尔射线、鼠标点击及粒子／轨迹／X-Ray。每项先核对公共 owner 与首断点，旧分支、重复 owner 或错误版本目录不能因已有测试而保留。
...
2. **物理入库与暂存回收（实现批已闭合，当前产品启动迁移复验通过，真实下载门保留）：**新 v2 内容经受管 incoming 验证后原子进入 `创意工坊/Video|Web|Scene/<WorkshopID>-<generationUUID>`，marker 只声明 ownership，metadata rename 仍是唯一 ready 点；legacy scanner 对整个保留命名空间 fail-closed，避免 marker 损坏后产生第二 ready owner，库根使用符号链接时也按物理父目录识别。v1 隐藏内容只读复制，复制前进入下载共用的容量队列/预留，发布前重核旧 metadata，旧播放路径不移动；失败有界退避且后续 reload 可重试。lease 与回收使用永不复用、含布局/类型/generation 的 storage identity，跨 actor admission 后重验原 inode 与 marker。正常成功、验证失败和取消不遗留本次 incoming UUID；helper descriptor identity 在首个进度事件落入独立 `jobs-v4.json`，terminal 不得首次绑定。恢复时 expected device/inode 随请求进入 helper，并在任何内容操作前对已打开根 descriptor 校验；v1/v2/v3 无 identity partial 清空恢复路径并创建新 lease，同名替换不会被写入或删除。v3 sidecar 存在但损坏时不回退旧 `jobs.json`。v1/incoming/v2 retained bytes 与本次新增共同受 16GiB 门约束。2026-09-21 真实库只读 census 中，`.mywallpaperx-steam-versions` 只有 `.DS_Store`、没有 UUID generation，`.mywallpaperx-steam-incoming` 缺席；116 个 metadata 中九个 v2 受管提交都指向带 marker 的公开 Scene 目录，另外 107 个无 managed commit，v1/other/parse-failure 均为零。真实 Application Support staging 由两次紧邻的运行后只读快照确认当前为空，不把它抬成 App before/after 证据。fresh App 两次把隔离的真实形状 v1 发布到 `Scene/3801984224-<UUID>`，metadata/marker 相等且隔离 incoming/staging 为零；第二次逐文件真实 Workshop 快照 before/after 相等。旧 v1 仍按 previous-current 与至少 24 小时回收门保留，不把它误称下载缓存。当前 26 项 transaction、17 项执行、7 项迁移共 50 个测试方法 ALL OK；未运行真实账号 helper 下载、Video/Web 产品迁移、迁移后播放、到龄回收、跨卷／断电，故 SK7.1 仍开放。
3. **最新报告复验（current HEAD 读取／认证入口通过，报告根因与登录终态仍开放）：**基线 `22e6835f` 的 2.0.9 (277) Developer ID Debug 候选 `CDHash=08fc8db98ce5dbb124a89569f47a16844e944c44`，launcher/debug dylib/helper SHA-256=`b4495f2c0383e9b06af889a710f165cf014123b388d085734c9084c447133a94 / e03989f2144b478202fc4dd5eb39e257f0a71a704b0ba8d1184ff3c378faa638 / ff380595e2446bd4e7559b21ed0c305e50d2fc98054ca491d4a102771e5dc672`，deep/strict 验签通过。helper 直接匿名 `queryBrowse` 观察到 30 项、远端总量 3,210,563、wrong-app 0。首个 `loginQR` 负向探针沿用 account epoch 0，被 typed `cancelled / stale account epoch` 正确失败关闭；产品 `SteamAuthRoute.beginAccountIntent()` 的真实合同会先把 epoch 从 0 递增为 1。按该合同发出的第二个登录请求绑定 process epoch 1、account epoch 1 与唯一 attempt，到达有序 `connecting -> qrChallenge`，随后用同一 attempt 主动取消；原登录 request 只收到 1 个 cancelled terminal，未保留临时 challenge URL。此前 `d4d4d5bf` 的同版本候选普通 App 首屏／搜索／账号面板证据仍是 UI 上限，不由本次直接 helper 复验冒充重跑。本机 `/Applications/MyWallpaperX.app` 仍是 2.0.8 (269)、`CDHash=fcf5ead265d79bcc551379a560234023a50c8087` 且 helper 缺失，不能沿现役产品主链接入 SteamKit；这只证明身份分歧存在，不证明用户报告来自旧包。没有 current-HEAD 源码失败可供合理修补，本批不新增 fallback、第二 client 或宽松错误处理。ignored summary manifest SHA-256=`a05670d7162a25df100ad820f184e4e6eef27ac75e8869521cf45bd5e1704bea`；raw helper 输出未留存。真实扫码／密码／Guard terminal、token 恢复与已授权真下载继续留在 SK7.1。
4. **current-HEAD 无账号下载链审计（`11c3bd34` 后续批，按职责逐项关闭）：**公开发布目录与 `<WorkshopID>-<UUID>` 自动命名本身保持单一 owner；当前真实库再查仍为隐藏 versions 仅 `.DS_Store`、公开 `Scene` 9 个受管 generation、incoming/staging 各 0，20 个 Steam 维护模块 ALL OK，但这不替代真下载。第一批复现并修正“复制中取消”窗口：旧实现已在 `.mywallpaperx-steam-incoming/<UUID>/content` 复制时收到 cancellation，defer 又调用会读取 sticky cancellation 的 `removeOwnedTree`，因此 rollback 在第一个子项前再次取消并静默留下 UUID；新反例在 64 MiB 受控副本出现受管 incoming 后取消，修复只让该 descriptor-bound rollback 忽略 task cancellation，普通 staging/reclamation 仍可取消，并把非取消型 rollback 失败写入诊断。第二批关闭 staging durable acknowledgement：旧 helper 在 `downloadProgress` 公布 lease 后立即预分配／写 chunk，App 的 JobStore 保存不是写入前置；现役链改为 `mandatory ready capability -> allocated -> App 校验 account/job/descriptor 并原子保存 identity -> exact acknowledge -> helper create/write`，回执绑定 account/job/path/manifest/device/inode，错误、重复、迟到、30 秒超时或保存失败均不放行。identity 四元组由 worker 在 helper 的同一 `downloadGate` 内一次发布后才发事件；wrong/missing account epoch 事件不保存、不 ack，并取消原 start。ack 超时删除新空 lease后回 typed integrity，App 同批清空 recovery；重启或显式重试若发现持久 lease 缺失／失配，也在同一次点击先持久清空后走 fresh lease。未确认新空 lease 在 unlink 前再 reopen/复核；空 replacement 的确定性检查间 swap 已覆盖并保留 replacement，但 POSIX name unlink 的最后窗口不冒充原子 compare-delete。fake-wire 还确认缺 capability 的旧 helper 不收到 start，jobs 文件已写入后才收到 ack，保存失败则不 ack；helper 行为门覆盖同锁 publication、错误 identity/epoch、重复、取消/超时等待与两次 identity 检查。checkpoint 选择器也已把 helper-only product path 纳入标准 build，并把 C#/protocol fixture/Swift download control 映射到既有最近门。其余 findings 保持后续独立批：①C# 已读取的 birth seconds/nanoseconds 未进入 event/ack/receipt/resume/Swift identity，device+inode 仍不足以抵抗 inode 复用；②当前权威 `jobs-v4.json` 损坏会被移走并以空 jobs fail-open；③删除已有 ready、同时 active update 的项目只改 JobStore 而未立即取消 helper 物理任务；④全部 v1 generation 退役后隐藏根及 `.DS_Store` 的最终退役门尚未实现。前两项为 identity/persistence P1，第三项为资源释放 P2，第四项为退役 P3；下一代码批先贯穿 birth identity，再闭合 current-v4 fail-closed，避免扩大未知 orphan。
...
2026-09-15 的开发 helper 行和 2026-09-20 的候选行均是历史基线。首行是复用音频条件 consumer 批已生成并验签的 `22e6835f` current-HEAD Developer ID Debug 候选做 direct-helper 复核，不是本 Steam 文档批新建的 fresh build；`d4d4d5bf` 与 `2eacca1b` 行分别继续承载既有 fresh 普通 UI 和公开迁移 sentinel。这里都不是低配机、发行公证包或完整账号/下载性能门。
...
用户交互补充约束：卡片心形及详情标签不显示悬停 tooltip；订阅与下载的主动操作可打开登录，纯浏览与后台流程不得弹出登录。浏览默认时间范围为一周；显式时间筛选作用于已加载作品的发布时间。分面结果采用服务端过滤及客户端一致性校验，局部筛空不能将远端 hasMore 置否；自动续载暂停后保留手动继续入口。
