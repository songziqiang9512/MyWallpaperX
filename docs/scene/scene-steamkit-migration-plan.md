...
| 13 | SK4.4 | 取消、恢复、有限并发与重试 | 工程切片已闭合：取消后的本地队列等待各自 helper 原 startDownload terminal（物理 I/O 排空）再推进，账号 epoch 改变不提前丢弃 waiter；helper 在第一次内容写入前从已打开 staging lease 句柄发布 path+manifestID+device+inode，App 首个进度事件即核对当前描述符并写入 JobStore v4；terminal receipt 只能匹配已持久身份，不能凭同名路径首次认领。恢复请求必须成组携带 path/manifest/device/inode，helper 打开根 descriptor 后先核对预期 device/inode，再执行任何 manifest 目录/文件收养、创建或 chunk 写入；无 identity 的旧 partial 不发送旧路径，只创建全新 lease。崩溃重启只显示显式恢复，重试沿用同一逻辑 job 并递增 attempt、不跨账号。manifest 变化、取消与成功在物理 terminal 后只清理同一 inode，同名真实目录替换、未知兄弟和链接目标不受影响。App/helper 同为 2 个活动作业，各 job 最多 4 worker、跨 job 共享 4 个 CDN chunk 槽；A/B 假 wire消融证明取消 A 不撤 B 的 task/running/ready。helper 缺失块、普通 App 版本复制与 v1 启动迁移共用跨作业磁盘预留、256MiB 安全余量及同卷跨进程保守额度，disk-full 保留可恢复暂存并给出重试动作；双成功假 wire证明版本复制串行且两个 ready 均发布。假 wire覆盖首进度前同名替换与 v3 identityless partial 不再发送 resume，真实 helper staging 自检覆盖 persisted identity 与同名替换不匹配时在内容操作前拒绝、replacement sentinel 不变。离线事务 26 项、假 wire执行、admission 两槽反例与 Helper protocol/auth/manifest/staging 41/download 129/query 门通过；真实双下载/续传/崩溃/disk-full 恢复及可见 UI 仍并入 SK7，不以离线绿色代替 |
...
4. **current-HEAD 无账号下载链审计（`11c3bd34` 后续批，按职责逐项关闭）：**公开发布目录与 `<WorkshopID>-<UUID>` 自动命名本身保持单一 owner；当前真实库再查仍为隐藏 versions 仅 `.DS_Store`、公开 `Scene` 9 个受管 generation、incoming/staging 各 0，20 个 Steam 维护模块 ALL OK，但这不替代真下载。审计首先复现并修正“复制中取消”窗口：旧实现已在 `.mywallpaperx-steam-incoming/<UUID>/content` 复制时收到 cancellation，defer 又调用会读取 sticky cancellation 的 `removeOwnedTree`，因此 rollback 在第一个子项前再次取消并静默留下 UUID；新反例在 64 MiB 受控副本出现受管 incoming 后取消，修复只让该 descriptor-bound rollback 忽略 task cancellation，普通 staging/reclamation 仍可取消，并把非取消型 rollback 失败写入诊断。其余 findings 保持后续独立批、不得因本项通过而消失：①helper 创建 staging lease 后尚无 App durable acknowledgement barrier 就可能开始写入；②C# 已读取的 birth seconds/nanoseconds 未进入 event/receipt/resume/Swift identity，device+inode 仍不足以抵抗 inode 复用；③当前权威 `jobs-v4.json` 损坏会被移走并以空 jobs fail-open；④删除已有 ready、同时 active update 的项目只改 JobStore 而未立即取消 helper 物理任务；⑤全部 v1 generation 退役后隐藏根及 `.DS_Store` 的最终退役门尚未实现。前三项为 identity/persistence P1，第四项为资源释放 P2，第五项为退役 P3；下一代码批先关闭 staging durable acknowledgement，再闭合 birth identity 与 current-v4 fail-closed，避免扩大未知 orphan。
