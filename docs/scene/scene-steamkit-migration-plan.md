...
> 状态：现役专项计划。SteamKit 已有唯一 App→client→helper 查询／认证／下载链；公开内容发布 owner 已从隐藏 `.mywallpaperx-steam-versions` 迁到 `Video/Web/Scene/<WorkshopID>-<UUID>`，下载使用 descriptor identity、`jobs-v5.json` 与 `download-staging-ack-v2` 保护持久化先于写入，旧 v1 只读迁移及 previous-current 回收边界仍保留。2026-09-21 已复核 current helper 匿名列表／搜索与 QR challenge 发起，并闭合 helper 重启预算耗尽后的显式匿名读取恢复；默认 featured/week 空页的错误 Swift `timeCreated` 后过滤也已移除，把 1...365 日 `trendDays` 经唯一查询链交给远端排名，并以 `query-trend-days-v1` 阻止新 App 对旧 helper 静默退化。产品源 `3fbd7c5a` 的 2.0.9 (277) 候选又经隔离普通 App 复验：匿名首屏、`Night vibe` 搜索和 QR challenge 均可见，未复现“无远程数据／不能发起登录”；本机 `/Applications/MyWallpaperX.app` 仍是缺 helper 的 2.0.8 (269)，但用户故障发生时没有冻结 bundle identity，故只能列为明确分歧而不能独占归因。当前发布/清理矩阵又冻结 Video/Web/Scene 三类成功路径均只写公开 `<WorkshopID>-<UUID>` generation，成功后 incoming、helper staging 与 transient job 均释放，现役源码没有向隐藏 versions 新写内容的 owner；真实库也为 v1 commit=0、隐藏受管项=0、公开 Scene v2 generation=9、staging/jobs 空。真实扫码／密码／Guard 终态、token 恢复、已授权真下载、迁移后播放、长期回收、发行公证、最低支持机与完整性能矩阵仍待继续；未获授权不覆盖安装包。2026-09-22 的 current HEAD 隔离构建确认 2.0.9 (277) 会嵌入完整 SteamService，匿名 `Night vibe` 趋势查询仍成功；因此可复现的已安装 2.0.8 identity bundled route 首断点是缺 helper，用户报告本身因当时没有冻结 bundle identity 不能由此独占归因，下一动作是取得授权后安装/启动签名候选并冻结普通 App 的 child identity，再继续真实 QR／授权下载／三引擎播放验收。频谱参考算法批次 `8f4dc46d` 已提交，不再重复重做；其后按用户顺序进入音频全样本长稳、丁达尔射线、粒子和鼠标点击／轨迹／X-Ray；隐藏 v1 根最终退役仍开放。

> 复核（2026-09-22 installed bundle identity 批）：基线 `8f4dc46d`，tracked owned paths 为本文与断点队列，ignored observer manifest 为 `docs/scene/evidence/2026-09-22-steam-installed-bundle-recheck/manifest.md`。只读审计在 `env -u MWX_STEAM_HELPER_COMMAND` 下冻结 `/Applications/MyWallpaperX.app` `2.0.8 (269)`、CDHash `fcf5ead265d79bcc551379a560234023a50c8087`，`Contents/Resources/SteamService` 文件数为 0；因此该安装 identity 的 bundled route 首断点是 `bundle helper missing → executableMissing → RequestError.notReady`，不会到 Steam 网络层，不能独占归因用户报告当时的故障。current HEAD 隔离 `xcodebuild` Debug 构建 `BUILD SUCCEEDED`，候选 `2.0.9 (277)` 嵌入 helper（SHA-256 `4b33427572485571e56cb905d11c31a121ec60f97fc9b541cab38a7cfeb69ed3`），`validate_apple_silicon_release.py --skip-codesign` 通过（26 个 arm64 Mach-O、19 个 owned arm64-only、7 个 third-party arm64）；同一 helper 的匿名 `queryBrowse(sort=trend, search=Night vibe, days=30)` 返回 `ok=true`、30 项、total=43941、wrong-app=0，shutdown/exit=0。Steam 定向静态/protocol/release 37/37 通过。该批不覆盖扫码、账号终态、授权真下载、播放、发行签名，也未覆盖 `/Applications`；结论是安装身份分歧而非现役源码定位/查询 owner 缺陷，下一动作是用户授权的签名候选安装与普通 App→client→child helper 复验。
>
> 复核（2026-09-21 Steam 公开入库/成功清理矩阵）：基线 `25f9aa96`，tracked owned paths 为本文与两个既有 compiled fixture，ignored manifest 为 `docs/scene/evidence/2026-09-21-steam-public-layout-cleanup-matrix/manifest.md`。首轮冻结 tracked diff/manifest SHA-256=`436ba59ab08e16ae673daff0efe1552390edc2652a275f47346b58628879a0ab / cd6812184b8fe278c61d675a3abfa83350f38a5db0ceeb6fa184113705c16671`；独立只读首审以一项 P3 指出新增正则只约束长度/字符集，不能证明 canonical `<WorkshopID>-<UUID>`。现役修正以共享 helper 分离精确 workshop 前缀、解析 `uuid.UUID` 并要求 lowercase canonical serialization；修正冻结 tracked diff/manifest SHA-256=`c9c65493aa72ae0f06c36b5f6dff05678c8cca9179c13aa873c6d52ed6c23728 / a33f054756ff9203abdf67115ea62ad61096e69e32360b256db5e4c45076d5ad` 经同一审查者复审 **APPROVE / P0–P3 无遗留 finding**。transaction 30项、execution 37项、Steam 全集125项和 helper 27/26/32/49/130/27 均通过，文档 role/governance/code-health/semantics 13/12/12/7 项及 `git diff --check` 通过。批准只覆盖当前源码的公开类型发布/命名/成功清理机器矩阵与真实库只读 census；fake wire 不等于已授权真下载，Video/Web 真 generation、下载后播放、跨卷/断电、Release/性能与长期回收仍未验证。
>
> 复核（2026-09-21 current-product 普通 Steam UI 身份批）：基线 task HEAD `e2a32b5e`，复用产品源 `3fbd7c5a` 的 Developer ID Debug 候选；tracked owned path 只有本文，ignored observer manifest 为 `docs/scene/evidence/2026-09-21-steam-current-product-ui-recheck/manifest.md`。首轮冻结 tracked diff／manifest SHA-256=`6b3a05cf3e62b088782d2d7d5b58fe73bb85963af14697b32f9573b0ac32142a / 1e59b10ef0a4c2c806f71ad0552a3b4f5f6005a99944b574df035a11d88e5b43`；独立只读首审以一项 P2 阻断：UI 成功与单独 bundle-helper probe 被合并归因，但没有冻结 `MWX_STEAM_HELPER_COMMAND` 或真实 child executable identity。第二轮显式 `env -u` 重跑，冻结 App PID、唯一 SteamService child 的 command／realpath／SHA／Team／CDHash并在同一 live child 下重现首屏、搜索和 QR challenge；旧安装包表述同时收窄为“正常 bundled route 缺 helper”。修正冻结 tracked diff／manifest SHA-256=`d740d545f1403a5bf502b61979dd6d6e3dd9ff1fc8f56f7ef267ac78b2af75ca / ca7d91ec6096d7a62e951a5c400c02a2a287ff1b056612c26cec83195806d4d6` 经同一审查者复审 **APPROVE / P0–P3 无遗留 finding**。文档 role/governance/semantics 13/12/7 项及 `git diff --check` 通过；最初误用三项旧 module 名产生 import error，语义脚本第一次又因未设 `PYTHONPATH=.` 失败，均按仓库现有入口纠正重跑，不冒充产品失败。批准不覆盖扫码、账号 terminal/token、订阅、真下载、安装覆盖、发行或性能；manifest 是 observer summary，不是 raw AX／截图档案。
>
...
<a id="91-当前-helper-与产品-接缝观测-2026-09-21"></a>

### 9.1 当前 helper 与产品接缝观测（2026-09-21）

2026-09-15 的开发 helper 行和 2026-09-20 的候选行均是历史基线。首行冻结当前产品源 `3fbd7c5a` 的复用候选及其直接 helper／隔离普通 UI 复核；`3b8daa7a`、`65ee316b`、`22e6835f`、`d4d4d5bf` 与 `2eacca1b` 行继续承载各自实现批的趋势、读取恢复、helper、普通 UI 和公开迁移 sentinel。这里都不是低配机、发行公证包或完整账号/下载性能门。

| 路径 | 次数 | 当前观测 | 证据边界 |
|---|---:|---|---|
| 2026-09-21 current source → 三类型公开发布／transient 清理矩阵 + 真实库只读 census | 30 + 37 + 125 tests；1 次 census | 基线 `25f9aa96`；产品 owner 未改，两个现有 compiled fixture 收紧。真实 Swift service/transaction success 对 authored Video/Web/Scene 均发布 v2 `<WorkshopID>-<UUID>` 到对应公开类型目录，不创建隐藏 versions，incoming owned generation、helper staging lease 与 resourcesReleased 后 transient job 均为空；30 项 transaction、37 项 execution 和 Steam 维护全集 125 tests 全通过。真实库 116 metadata 全可解码：107 commit-less legacy、9 v2 scene、0 v1；隐藏 versions 受管子项 0、公开 incoming 不存在、Application Support staging 子项 0、`jobs-v5` 为 jobs/history 空；9 个公开受管目录均在 Scene 且采用 `<WorkshopID>-<UUID>` | 支持现役公共发布/自动命名/成功清理 owner 及当前真实磁盘状态，不是已授权网络真下载。矩阵使用 fake transport 和隔离 descriptor-backed 文件；真实库只读，没有 Video/Web v2 generation，不能外推三类真内容播放。disk-full 保留完整 staging 供显式 retry 是设计恢复态，不得为“清缓存”无条件删除；v1 read-only migration/reclamation 仍服务其他升级用户。ignored manifest=`2026-09-21-steam-public-layout-cleanup-matrix` |
| 2026-09-21 产品源 `3fbd7c5a` 的复用 2.0.9 (277) Developer ID Debug 候选 → bundled helper 三页搜索／隔离普通 App UI | 1+2 | task HEAD 为其后的两笔 docs-only Scene 音频提交 `e2a32b5e`。App `CDHash=4a51a532475cb47902d8f9513a7616c297e813f6`，launcher/debug dylib/helper SHA-256=`0d94a5e5c1e056289e85ea862420103e84d5f5d90bec5a90168cd1c270653a71 / 4c8b68213db80e0712bae37d1cd94d51f752b5425b2e6e42da3e55c76ccb486c / 858b8a590377c3f69bb197200d4905c99787772f89c283b2897c2e5c24c5ac5c`，deep/strict 验签通过。helper 对 Video／week／`Night vibe` 三页各收 30 项、跨页唯一 90、total=19,072，首项依次 `Night vibe / music / Assetto Corsa`、`Hatsune Miku night vibe`、`car racing at night on track`。首轮按显示名连接曾另启 2.0.8 安装包；该 UI 证据已拒绝，重绑候选绝对路径后才计入。针对独立首审 P2，又以全新隔离根及显式 `env -u MWX_STEAM_HELPER_COMMAND` 重跑：候选 App PID 66442 的唯一 SteamService child PID 66458，command realpath 指向该候选 `Contents/Resources/SteamService/SteamService`，SHA/CDHash 与上述 bundle helper 相同；同一 live child 下空隔离库显示 20 个首屏卡片，`Night vibe` 显示 20 个结果且首项与 helper 一致，唯一登录窗默认 QR 模式并显示“等待扫码确认…”，随后主动关闭 | 支持 current product source 的普通 App→client→bundled helper 匿名列表／搜索投影与 QR challenge 发起，未复现这些成功路线的普遍故障。未扫码、未取得账号 terminal/token、未订阅／下载，也未证明用户报告时运行的 bundle identity；安装包 2.0.8 的正常 bundled route 缺 helper，是具体分歧但不是可独占归因。observer summary（非 raw AX／截图）保存在 ignored evidence `2026-09-21-steam-current-product-ui-recheck`；未保存 expiring QR payload。两轮隔离 workshop 运行后均为空，取证 App/helper 进程均已停止，未覆盖安装包或写真实壁纸库 |
