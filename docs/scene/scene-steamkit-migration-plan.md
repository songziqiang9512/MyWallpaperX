...
本轮跨专题执行顺序不另建平行计划：App/helper 定位回归和 ready 公开类型目录发布均已闭合实现批。用户再次报告远程数据为空、无法登录和找不到壁纸后，基线 `d4d4d5bf` 的 fresh 签名候选已重新执行 `App -> SteamServiceClient -> bundled helper -> SteamKit`：直接协议与普通 UI 均未复现列表／搜索缺失，主动登录到达二维码 challenge 后由取证取消。随后基线 `2eacca1b` 的真实库只读 census 证明隐藏版本根没有受管 generation，隔离产品 App 启动把真实形状 v1 复制并发布为公开 `Scene/<WorkshopID>-<UUID>`，incoming/staging 无残留。两项证据分别只覆盖当前候选公开读取／认证发起和 Scene 启动迁移；账号终态、已授权真下载、迁移后播放及到龄回收仍开放。下一批按用户顺序进入[音频采集与全频谱样本](scene-open-breakpoint-queue-2026-09-09.md#q14-音频频谱与采集生命周期2026-09-20-当前态)，随后推进丁达尔射线、鼠标点击及粒子／轨迹／X-Ray；每项先核对公共 owner 与首断点，旧分支、重复 owner 或错误版本目录不能因已有测试而保留。
...
3. **最新报告复验（当前候选读取通过，报告根因与登录终态仍开放）：**2.0.9 (277) fresh 候选 `CDHash=e6ab49628b88573bc3a3f259c093a30b9465c025`，bundled helper SHA-256 `9d7ad02317ec172a0333a64f3985f7e6355ecf076ffde6a6c92b2c3f3df9a272`。helper 直接匿名查询观察到 30 项、远端总量 3,210,234、wrong-app 0，并以 `Night vibe` 观察到 30 项／远端总量 43,918；普通 App 首屏通过辅助树观察到远程卡片，搜索出现 `Night vibe / music / Assetto Corsa` 与 `Couple vibe / sound`，账号入口生成二维码并显示“等待扫码确认…”。本机 `/Applications/MyWallpaperX.app` 同日仍是 2.0.8 (269)、`CDHash=fcf5ead265d79bcc551379a560234023a50c8087` 且 helper 缺失，不能沿产品主链接入 SteamKit；取证曾同时观察到旧安装包和当前候选进程，但这只证明身份分歧存在，不证明用户报告来自旧包。20 个 Steam 维护模块 ALL OK、fresh Developer ID Debug build 与 deep/strict 验签通过；没有源码失败可供合理修补，本批不新增 fallback、第二 client 或宽松错误处理。本次 ignored 缓存只保留 summary manifest，raw helper/test/build 输出与脱敏 AX snapshot 未留存，因此这些运行数值是有身份约束的观察者记录，不是可独立重放的原始证据。真实扫码／密码／Guard terminal 与 token 恢复继续留在 SK7.1；当前顺序转入音频采集／全频谱样本。
...
2026-09-15 的开发 helper 行和 2026-09-20 的候选行均是历史基线；首行已替换为 2026-09-21 同一工作树的 fresh 自包含 Developer ID Debug 候选。这里都不是低配机、发行公证包或完整账号/下载性能门。
