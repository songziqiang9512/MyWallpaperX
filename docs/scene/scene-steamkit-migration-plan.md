...
| 10 | SK4.1 | JobStore、队列与持久化 | 先在线准入；staged/committing 非终态；cancelAll 一次保存、失败不假推进并提示。任务 envelope 在 SK5.3 升至 v3、device/inode 升至 v4；完整 device/inode/birth identity 是不兼容恢复边界，因此当前 owner 只写 `jobs-v5.json`。v5 缺席时依次检查 v4/v3/jobs.json，只导入最新存在且可读的前代并保持其原字节；最新前代损坏时停止更旧回退。缺 birth 的 v4 partial 清空 path/manifest/receipt/cleanup authority：running 回 queued，staged 变 retryable failed，failed 保留原失败意图，completed/cancelled 不再持有 staging 清理权；committing 保留 preparedCommit，使已发布 metadata 仍可结算。当前 v5 自身损坏后的持久 fail-closed admission 仍是下一批，完整 App 操作仍待 SK7 |
