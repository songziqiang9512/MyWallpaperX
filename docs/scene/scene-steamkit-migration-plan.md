...
本轮跨专题执行顺序不另建平行计划：App/helper 定位回归和 ready 公开类型目录发布均已闭合实现批。用户在 2026-09-20 报告当前普通产品路径又出现远程数据为空、无法登录和找不到壁纸；本批提交后先以当前签名候选重新复现 `App -> SteamServiceClient -> bundled helper -> SteamKit`，核对启动、协议、匿名查询、认证 attempt/epoch、页面投影与错误可见性，旧的列表／二维码成功证据不能冒充当前验证。该回归闭合后回到 [Scene 当前断点队列 Q1.4–Q1.6](scene-open-breakpoint-queue-2026-09-09.md#q14-音频频谱与采集生命周期2026-09-20-当前态)，依次做音频样本全集与普通 App 采集、频谱分布／冻结、丁达尔射线、鼠标点击及粒子／轨迹／X-Ray。每项先核对公共 owner 与首断点；旧分支、重复 owner 或错误版本目录不能因已有测试而保留。
