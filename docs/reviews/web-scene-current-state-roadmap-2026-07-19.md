...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v22；current implementation `94aebc5` 的 renderer 消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow、exact stock Opacity、Shake、Water Waves 与 Water Flow 八个严格注册 backend。Shake、Water Waves 与 Water Flow 都按完整 definition/material/shader fingerprint 准入，并在同一 ordered scheduler 保持作者顺序；Water Flow 可与 Opacity 组成真实 chain，重复 Water Waves 也不再压进单组 inline uniforms。默认 96 MiB pool 只保证 6 个全尺寸 BGRA texture unit，超预算链在规划阶段整链拒绝。同帧 graph command、受限 unique history seed/clear、Precise Blur interleave 与 exact legacy compose 继续生效；generic material pass、authored shader preprocessing/translation、真实 history consumer、generic compose/scene-background、condition/function、SceneScript/time/media 仍未实现。

当前运行证据分两层：`.codex/scene-waterflow-20260724/full26-final/report.json` 对当前真实目录的 26 个隔离样本 **26/26 通过**；`.codex/scene-waterflow-20260724/fixed13-final/report.json` 固定回归门 **13/13 通过**。26 门验证 image `330/330`、text `212/212`、solid `126/126`、particle `47/68`，named capture `8/8`、binding `9/9`、strict stage 38、chain 5、Water Flow 2、Water Waves 4、Shake 3、failed 0；固定门保护 20 stage、1 条 Blur/Shadow chain、Water Waves 6、Opacity 4、Workshop Shadow 1、failed 0。完整 Scene suite 为 **301/301、3 项跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `a37d51608519fed9d5ae86307654f9aba590357c`、可执行文件 SHA-256 `f6ae1d08a294e0e19221537b5864d23157032f3ebd763dbc94dc9db49624ffe4`。preview 与 WaifuX SceneBake MP4 都只支持同一样本方向性对照，不能解释为 WE 视觉一致。
...
`<isolated-scene-sample-root>` 必须包含测试副本 `Scene/<id>`；不得直接传入真实 `~/Movies/MyWallpaperX/创意工坊/Scene`。当前完整快照门为 `.codex/scene-waterflow-20260724/full26-final/report.json`，当前固定回归门为 `.codex/scene-waterflow-20260724/fixed13-final/report.json`，四关键样本门为 `.codex/scene-waterflow-20260724/followup4-final/report.json`，视觉校准门为 `.codex/scene-preview-visual-20260724/calibration6-final/report.json`；其他 Water Waves/Shake、legacy-compose、Workshop Shadow、effect-chain、Local Contrast、ShaderContract、frame-context、file-property、utility、particle-trail、static-fallback 和旧 IR 报告只保留为对应阶段证据，不能反向覆盖当前结论。
...
- 当前 26 样本完整快照门验证 image `330/330`、solid `126/126`、text `212/212`、particle `47/68`，named capture `8/8`、binding `9/9`；strict graph 合计 38 stage、5 chain、Water Flow 2、Water Waves 4、Shake 3、0 failed，仍有 13 个 named-target gap、63 个 route-only effect、16 个 legacy blocked layer 和 9 个缺失资源。固定 13 样本门另行保护 20 stage、1 条 Blur/Shadow chain、Water Waves 6、Opacity 4 与 Workshop Shadow 1。preview/MP4 只作同样本方向性对照，不设绝对阈值；没有 Windows WE 同配置录屏，不能把 26/26、13/13 或 SSIM 等同于动态/像素 parity。
