...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v22：继承 v21 exact stock Opacity、ShaderContract、provider metadata 与 authored graph，并保存 direct text content/point-size/color binding program；exact Shake 没有新增 wire 字段，启动时从 authored graph/ShaderContract 规划。当前实现基线 `1766c76` 的 renderer 仍消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow、exact stock Opacity 与 exact stock Shake 六个严格注册 backend；Shake 以 RG8 flow、可选 R8 phase/white fallback 和统一 scene time 在 Precise Blur 前后按作者顺序进入 chain。同帧 graph command foundation、受限 unique history seed/clear、Precise Blur material-command interleave 与 exact legacy compose 继续生效；Water Waves legacy inline 只在 owner layer 唯一可见 Effect 时准入，mixed/repeated declarations fail closed。benchmark 的非阻断 preview 方向性证据不改变 renderer、viewport 或 cover 投影。其他 topology、dynamic Shake、真实 history consumer、generic compose/scene-background、condition/function、SceneScript/time/media 继续 fail closed 或整场重建；这不是任意 graph、material、shader 或完整 SceneScript/media executor。

当前运行证据分两层：上一构建的 `.codex/scene-shake-20260724/full26-final/report.json` 对当前真实目录的 26 个隔离样本 **26/26 通过**；当前 `1766c76` 签名 App 的 `.codex/scene-waterwaves-ordered-chain-fixed13-final-20260724/report.json` 固定回归门 **13/13 通过**。26 门验证 image `330/330`、text `212/212`、solid `126/126`、particle `47/68`，named capture `8/8`、binding `9/9`、strict stage 30、chain 3、Shake 3、failed 0；固定门继续保护 14 stage、1 条 Blur/Shadow chain、Opacity 4、Workshop Shadow 1、failed 0，并精确锁定 Water Waves legacy count `[4,2,0,0,0]` 的五个正反样本。当前提交相关测试 **40/40**；上一实现基线完整 Scene suite 为 **297 项，其中 294 项通过、3 项按环境/fixture 跳过**。当前固定门 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `d37ecaceedea6b82daadeac90e17159c63d93e73`、可执行文件 SHA-256 `95b667f4fc6a54865b1a78f207af9258d16a1dfa0785fd68ded09b5630792896`。preview 证据只支持同一样本跨提交对照，293 的几何改善仍以人工 montage 为判定依据，不能解释为 WE 视觉一致。
...
`<isolated-scene-sample-root>` 必须包含测试副本 `Scene/<id>`；不得直接传入真实 `~/Movies/MyWallpaperX/创意工坊/Scene`。当前完整快照门为上一构建的 `.codex/scene-shake-20260724/full26-final/report.json`，当前固定回归门为 `.codex/scene-waterwaves-ordered-chain-fixed13-final-20260724/report.json`，Water Waves 定向门为 `.codex/scene-waterwaves-ordered-chain-targeted-20260724/report.json`，视觉校准门为 `.codex/scene-preview-visual-20260724/calibration6-final/report.json`，Shake 定向门为 `.codex/scene-shake-20260724/targeted-contract-final/report.json`；其他 legacy-compose、Workshop Shadow、effect-chain、Local Contrast、ShaderContract、frame-context、file-property、utility、particle-trail、static-fallback 和旧 IR 报告只保留为对应阶段证据，不能反向覆盖当前结论。
...
- material resolver 保留 8 个 nullable slot，并按低到高优先级保存 material asset -> material usertexture -> instance asset -> instance usertexture -> explicit bind；现有 strict backend 取末项，尚未通用接入 frame registry 的 ready/fallback 选择。六个 strict backend 共同使用 graph/effect identity、node order、RT、shader fingerprint、render state、combo、constant 和 binding 判定，不按样本 ID 或 layer 名称启用。Local Contrast、Workshop Shadow、Opacity 与 Shake 都只用 exact shader fingerprint 准入，仍执行项目内手写 MSL，不执行 authored source。
...
- 已有 Metal textured-quad、共享白纹理程序化 solid、静态与 direct-property 动态 text texture、作者定义 2D sprite 粒子 instancing、层级 transform/visibility、基础混合、六个 strict effect backend、Bloom、cover 相机和声明驱动的 parallax。exact stock Shake 已用 scene time、RG8 flow 与可选 phase 在未修改样本执行；动态文本只在对应 direct property 变化时异步更新，作者未启用的文本层不登记为活动 consumer。这不包含 SceneScript/time/media producer 或 dynamic Shake variants。
...
- benchmark 通过签名 App 身份、隔离样本/HOME、Metal ready/after 双帧、语义字段、utility/named/image-blend/particle planned 与 finished layer ID、authored graph canonical SHA、precise/standard/Local Contrast/Workshop Shadow backend exact succeeded/failed/legacy-blocked layer ID、ordered chain/stage 精确计数、live identity/visible-change，以及 stop 后 surface=0 验证当前 13 样本；七个 effect-graph 代表样本锁定 chain/stage 正负门，五个 Water Waves 样本同时锁定 legacy runtime 最小/最大精确计数。`3194ac5` 另把 `project.json.preview` 的 GIF 首帧/JPG 静帧转换为 reference，将运行截图中心裁到相同比例并生成分项指标和并排图；该路径明确非阻断。
...
- `2938612768`：image `44/44`、solid `11/11`、particle `0/1`；providers `141/1340` capture、consumers `299/322` binding 和 layers `239/657/775/875/1509` 静态 image blend 均成功，中央播放器/静态封面已出现，missing resource 为 0。可见 4/5-effect mixed chains 的 strict graph count 保持 0；`1766c76` 又把 layer 1509 的 Water Waves legacy count 从 1 降为 0，因为它位于 Blend/Blend Gradient 后且无法由共享 inline pass 保持作者顺序。当前 montage 中全屏夸张扭曲已消失、人物主构图恢复。它仍没有执行 authored Water Waves，媒体动态封面/文字、音频、粒子、named-target gap 1 和 18 个 route-only effect 尚未闭合。
...
descriptor 能识别 image、solid、particle、text、container 与 typed utility；v22 authored graph 中，六个 strict backend 已进入 GPU，ordered strict effect-chain scheduler、typed frame registry、property authored fallback 和受限 PNG/JPEG property source 也已落地。exact stock Shake 可在 Precise Blur 前后消费 flow/phase/scene time；同帧 copy/swap、受限 unique history seed/clear 和 Precise Blur 两种 material-command topology 继续生效。当前仍不是通用 GPU DAG executor：dynamic Shake、真实 history consumer、跨帧 logical swap、compose/condition/function、其他 topology 与通用 material/shader 尚未执行；system/media source、文件纹理的任意 material consumer、effectful provider、child/nested target、utility mask、复杂 blend、Texture Variants、child/world-space/其余粒子 renderer、puppet 和脚本驱动内容仍会缺失或降级。

#### shader/effect 不是兼容实现

当前 MSL effect 是对少量视觉特征的手写实现，不是 Wallpaper Engine HLSL、材质和 pass 语义的通用转换或执行；Local Contrast 虽精确核对 source fingerprint、pass 顺序、RT、combo、常量与 binding，fingerprint 仍只用于准入。ordered strict scheduler 已保留 effect chain、输入连续性、作者顺序和逐 stage snapshot，但它不会让缺失 backend 或 authored shader 自动可执行。下一步应按真实链依赖补 backend/primitive，不能重新把多个 effect 压成按名称选取的近似分支。
...
- 没有 SceneScript、系统时间/日期/媒体驱动文字、音频响应、puppet/mesh 和完整视频纹理生命周期；direct property 动态文字已完成首个 provider generation 子集。粒子现有作者包内 sprite、9 个精确 built-in 程序纹理与 sprite trail 子集，其余 visible candidate 仍有缺口。
- 当前 26 样本完整快照门验证 image `330/330`、solid `126/126`、text `212/212`、particle `47/68`，named capture `8/8`、binding `9/9`；strict graph 合计 30 stage、3 chain、Shake 3、0 failed，仍有 13 个 named-target gap、64 个 route-only effect、16 个 legacy blocked layer 和 9 个缺失资源。固定 13 样本门另行保护 14 stage、1 条 Blur/Shadow chain、Opacity 4 与 Workshop Shadow 1，并已生成 13/13 preview 并排图。六样本人工校准证明视觉门能暴露构图、过度形变和字体差异，也证明不同 preview 裁切会使跨样本分数失真；因此当前只作同样本对照，不设绝对阈值。没有完整性能预算，也没有 Windows Wallpaper Engine 同配置录屏；不能把封面或 26/26、13/13 等同于动态/像素 parity。

## 6. Scene 演进方向

### 阶段 0：产品边界与执行门（已确定方向，进行中）

当前仍选择“可审计的兼容 Runtime”方向。Effect-definition IR、authored graph planner、ShaderContract v1、六个 strict backend、ordered strict chain、resource identity/property fallback、file-backed `sceneTexture`、Frame Context、B0 live program、direct dynamic text、同帧 copy/swap、受限 unique history、Precise Blur interleave/legacy compose 与 exact stock Shake 已完成。`e505a9e` 证明按完整链缺失 backend 选择工作可以让未修改样本新增 6 stage、3 chain并改善动态像素；`1766c76` 证明当 inline profile 无法保持 effect 顺序和独立参数时，fail closed 比错误执行更接近作者主构图。后续优先补 Water Waves authored-chain 及其高命中前序 stage，再按完整 chain 解锁数、未修改样本新增 stage、同一样本拼图/分项变化和单位实现成本重排 SceneScript、history、shader 与 provider；route-only 和跨样本视觉分数都不能单独决定优先级。
...
2. B2 ordered strict effect-chain、target allocation、pass ordering、同帧 copy/swap command foundation、受限 unique FBO history seed/clear、Precise Blur 两种 material-command interleave、exact legacy compose 归一化、最终合成原子性、首条真实 chain 与 stock Opacity `MASK=0` live alpha 已完成；generic compose 和真实 history consumer 未完成，但是否紧接着实现由本轮可见收益复盘决定，不能复制各 profile 的调度代码。
...
- 六个 strict backend、ordered strict chain、Bloom、程序化 solid、静态与 direct-property 动态 text、camera cover/显式 parallax、受限水波/合成、单层 built-in UV foliage、9 个 built-in 粒子纹理/sprite trail、utility/named target、静态 image blend、file-backed `sceneTexture`、Frame Context、B0 live 主链、受限 unique history、Precise Blur interleave/legacy compose 与 exact stock Shake 已落地；v22 记录 direct text 合同。非阻断 preview 方向性门已生成固定 13 样本和六样本校准拼图。下一批以 [覆盖台账](../scene/semantics/coverage-ledger.md) 和 [Scene 播放能力开发计划](../scene/scene-capability-development-plan-2026-07-22.md) 的完整链与同样本视觉变化结论为准。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有签名 App、26 个当前真实目录隔离样本加 13 个固定回归样本、Metal 双帧/语义/dependency/strict graph/释放门，以及固定 13 样本和六样本校准的 preview 并排证据。v22 已把 direct text 三类属性接入 B0 snapshot，并以异步 per-layer generation/stale cancellation 闭合首个动态 provider；B2 已增加同帧 copy/swap、受限 unique history、Precise Blur interleave/legacy compose 和 exact stock Shake。六个 strict backend、当前完整门 3 条 Blur/Shake chain、固定门 1 条 Blur/Shadow chain、静态 blend、direct text 和 ShaderContract source 保留仍不等价于 authored shader execution、真实 history consumer、SceneScript/media 或完整属性系统。当前可以声明“受限能力可用、direct dynamic text property 可无重建更新、exact Shake 在未修改样本有动态 GPU 正门、同样本视觉变化可审查”，不能声明达到或接近 WE parity。后续不再用内部 blocker 减少、route-only 或跨样本视觉分数作为主要进度指标，而以未修改样本完整链解锁和同样本拼图/分项变化作为近期筛选门。
