...
- **执行子集是受限白名单，不是通用引擎**：十一类 strict effect backend、puppet bind-pose、15 个程序化 built-in 粒子纹理和受限 provider/dependency 链都按完整 fingerprint 准入、fail closed；generic material/shader executor、真实 history consumer、generic compose、condition/function 未实现。
