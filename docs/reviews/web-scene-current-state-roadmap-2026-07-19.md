...
- 已有 Scene 用户属性解析、受支持 target 的默认值/override 应用、按 wallpaper 持久化、条件显示和独立属性编辑窗口；实际可执行的 `sceneTexture` key 可选择 PNG/JPEG，bookmark 授权只覆盖同步解码上传，UI 不显示当前 renderer 无法执行的属性 target。layer alpha 已通过 v18 binding program 和 per-surface snapshot 无重建更新；新 target 只有 compiler mapping 与真实 consumer 同时存在才允许 live。
...
当前选择“可审计的兼容 Runtime”方向：不承诺私有格式 100% 复刻，但按官方行为合同、真实样本频率和 [Scene 语义手册](../scene/semantics/README.md) 推进。Effect-definition IR、authored graph planner、两个严格 Blur 子图、resource identity/property authored-fallback、file-backed `sceneTexture`、统一 Frame Context 第一阶段、首批 built-in 粒子纹理，以及 B0 binding program/per-surface snapshot/atomic layer-alpha live consumer 已完成。下一主线进入 B1 Provider Core 与 B2 Graph Resource Runtime；Timeline、SceneScript、动态文字、audio/media 以后横向接入同一 target/frame 合同。既有 bounded executor 在新路径取得 GPU/golden 等价证据前保留，但不再扩展名称分支。完整等级以 [覆盖台账](../scene/semantics/coverage-ledger.md) 为准。

### 阶段 1：正确性、安全和性能基础

1. B0 property 主链已建立 typed DynamicValue/target registry、format 18 binding program、per-surface snapshot、原子 live state 与 alpha consumer。新增 live target 必须同时注册 compiler target、真实 consumer、fallback 与 identity 门；dynamic text 和 visibility 不得仅靠现有 alpha 路径取消整场重建。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有“签名 App + 13 个隔离真实样本 + Metal 双帧/语义/dependency/authored-Blur GPU/释放门”的可持续开发链。v18 在 v17 provider metadata、逐帧 typed texture registry 与 authored fallback 上加入 binding program/effective values；B0 已把 property producer、per-surface snapshot、atomic routing 和 layer-alpha consumer 合龙。strict precise、standard-default、file-backed `sceneTexture` 静态 consumer 子集和统一 Frame Context 第一阶段已执行，实际画面仍主要来自 bounded renderer；固定 precise kernel、受限 standard profile、5/5 静态 blend 和 alpha live 子集都不等价于 authored shader、动态 sceneTexture 或完整属性系统。当前可以声明“受限能力可用、官方能力归属和逐项缺口可查、通用图合同已建立、首个文件型 property provider 与 layer-alpha live 主链已闭环”，不能声明达到或接近 Wallpaper Engine parity。下一步按 coverage-first 进入 B1 Provider Core 与 B2 Graph Resource Runtime，再接 B3/B4 的 Timeline/SceneScript、动态 text、audio/media、Particle 与更广 backend；增加样本硬编码、默认套用效果或把 compiler-only target 写成 live，仍不会形成兼容闭环。
