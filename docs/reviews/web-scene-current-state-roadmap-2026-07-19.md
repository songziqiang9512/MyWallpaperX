...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v22；当前 HEAD（前置安全整数提交 `10401d7`）的 renderer 消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow、exact stock Opacity、Shake、Water Waves 与 Water Flow 八个严格注册 backend。Shake、Water Waves 与 Water Flow 都按完整 definition/material/shader fingerprint 准入，并在同一 ordered scheduler 保持作者顺序；Water Flow 可与 Opacity 组成真实 chain，重复 Water Waves 也不再压进单组 inline uniforms。默认 96 MiB pool 只保证 6 个全尺寸 BGRA texture unit，超预算链在规划阶段整链拒绝。同帧 graph command、受限 unique history seed/clear、Precise Blur interleave 与 exact legacy compose 继续生效；generic material pass、authored shader preprocessing/translation、真实 history consumer、generic compose/scene-background、condition/function、SceneScript/time/media 仍未实现。

当前运行证据分两层：`.codex/scene-chromaticdot-20260724/full26-final/report.json` 对当前真实目录的 26 个隔离样本 **26/26 通过**；`.codex/scene-chromaticdot-20260724/fixed13-final/report.json` 固定回归门 **13/13 通过**。26 门验证 image `330/330`、text `212/212`、solid `126/126`、particle `52/68`，named capture `8/8`、binding `9/9`、strict stage 38、chain 5、Water Flow 2、Water Waves 4、Shake 3、failed 0；固定门保护 particle `15/27`、20 stage、1 条 Blur/Shadow chain、Water Waves 6、Opacity 4、Workshop Shadow 1、failed 0。完整 Scene suite 为 **302 项：299 通过、3 项跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `4d78ee2b61532fe6a7d3872e80707e8d9e074ecf`、可执行文件 SHA-256 `162b260a61600543f2212f6dd8ad11d92a7f860806e18ea56b4984c9abd9cadc`。样本自带 preview 是当前第一视觉依据；WaifuX SceneBake MP4 只作辅助动态参考，二者均不能解释为 WE 视觉一致。
...
- 用户真实 Scene 目录当前已有 26 个已进入完整快照门的样本；运行和属性注入均只使用 `.codex` 隔离副本，真实目录未修改。新增 5 个样本已经完成结构、运行和证据门，但本机没有 Windows Wallpaper Engine 同配置逐帧录屏，因此仍不能给出逐像素兼容结论。
...
`<isolated-scene-sample-root>` 必须包含测试副本 `Scene/<id>`；不得直接传入真实 `~/Movies/MyWallpaperX/创意工坊/Scene`。当前完整快照门为 `.codex/scene-chromaticdot-20260724/full26-final/report.json`，当前固定回归门为 `.codex/scene-chromaticdot-20260724/fixed13-final/report.json`，本批四样本门为 `.codex/scene-chromaticdot-20260724/targeted4-final/report.json`，视觉校准门为 `.codex/scene-preview-visual-20260724/calibration6-final/report.json`；其他 Water Flow/Water Waves/Shake、legacy-compose、Workshop Shadow、effect-chain、Local Contrast、ShaderContract、frame-context、file-property、utility、particle-trail、static-fallback 和旧 IR 报告只保留为对应阶段证据，不能反向覆盖当前结论。
...
- 已有 Metal textured-quad、共享白纹理程序化 solid、静态与 direct-property 动态 text texture、作者定义 2D sprite 粒子 instancing、层级 transform/visibility、基础混合、八个 strict effect backend、Bloom、cover 相机和声明驱动的 parallax。exact stock Shake、Water Waves 与 Water Flow 已在未修改样本执行；动态文本只在对应 direct property 变化时异步更新，作者未启用的文本层不登记为活动 consumer。这不包含 SceneScript/time/media producer 或动态 effect variants。
...
- benchmark 通过签名 App 身份、隔离样本/HOME、Metal ready/after 双帧、语义字段、utility/named/image-blend/particle planned 与 finished layer ID、authored graph canonical SHA、strict backend exact count、ordered chain/stage、live identity/visible-change，以及 stop 后 surface=0 验证固定 13 与完整 26 样本；矩阵现在精确锁定 Water Flow/Water Waves count、超预算长链拒绝和 dormant Opacity slot。`3194ac5` 的 preview 并排图与 WaifuX SceneBake MP4 路径都明确非阻断。
...
descriptor 能识别 image、solid、particle、text、container 与 typed utility；v22 authored graph 中，八个 strict backend 已进入 GPU，ordered strict effect-chain scheduler、typed frame registry、property authored fallback 和受限 PNG/JPEG property source 也已落地。exact stock Shake、Water Waves 与 Water Flow 已消费真实资源和 scene time，并按作者顺序进入 chain；同帧 copy/swap、受限 unique history seed/clear 和 Precise Blur 两种 material-command topology 继续生效。当前仍不是通用 GPU DAG executor：动态 effect、真实 history consumer、跨帧 logical swap、compose/condition/function、其他 topology 与通用 material/shader 尚未执行；system/media source、文件纹理的任意 material consumer、effectful provider、child/nested target、utility mask、复杂 blend、Texture Variants、child/world-space/其余粒子 renderer、puppet 和脚本驱动内容仍会缺失或降级。
...
- 没有 SceneScript、系统时间/日期/媒体驱动文字、音频响应、puppet/mesh 和完整视频纹理生命周期；direct property 动态文字已完成首个 provider generation 子集。粒子现有作者包内 sprite、10 个精确 built-in 程序纹理与 sprite trail 子集，其余 visible candidate 仍有缺口。
- 当前 26 样本完整快照门验证 image `330/330`、solid `126/126`、text `212/212`、particle `52/68`，named capture `8/8`、binding `9/9`；strict graph 合计 38 stage、5 chain、Water Flow 2、Water Waves 4、Shake 3、0 failed，仍有 13 个 named-target gap、63 个 route-only effect、16 个 legacy blocked layer 和 9 个缺失资源。固定 13 样本门另行保护 particle `15/27`、20 stage、1 条 Blur/Shadow chain、Water Waves 6、Opacity 4 与 Workshop Shadow 1。样本 preview 优先、WaifuX MP4 只作辅助动态参考，均不设绝对阈值；没有 Windows WE 同配置录屏，不能把 26/26、13/13 或 SSIM 等同于动态/像素 parity。

## 6. Scene 演进方向

### 阶段 0：产品边界与执行门（已确定方向，进行中）

当前仍选择“可审计的兼容 Runtime”方向。Effect-definition IR、authored graph planner、ShaderContract v1、八个 strict backend、ordered strict chain、resource identity/property fallback、file-backed `sceneTexture`、Frame Context、B0 live program、direct dynamic text、同帧 copy/swap、受限 unique history、Precise Blur interleave/legacy compose、exact stock Shake/Water Waves/Water Flow 已完成。下一批优先提取现有 backend 重复的 texture/state/target 执行职责，建立共享 material pass executor 的最小切入点，并用 SceneBakes 同时间帧继续做方向性视觉门；route-only、内部 blocker 数和跨样本视觉分数都不能单独决定优先级。
...
2. B2 ordered strict effect-chain、target allocation、pass ordering、同帧 copy/swap、受限 unique FBO history seed/clear、Precise Blur interleave、exact legacy compose、最终合成原子性、八个 strict backend 与当前 Water/Blur 真实 chain 已完成；generic compose 和真实 history consumer 未完成。下一步先共享 material pass 执行基础，不能继续复制各 profile 的 texture/state/target 调度代码。
...
- 八个 strict backend、ordered strict chain、Bloom、程序化 solid、静态与 direct-property 动态 text、camera cover/显式 parallax、受限水波/合成、单层 built-in UV foliage、9 个 built-in 粒子纹理/sprite trail、utility/named target、静态 image blend、file-backed `sceneTexture`、Frame Context、B0 live 主链、受限 unique history、Precise Blur interleave/legacy compose 与 exact stock Shake/Water Waves/Water Flow 已落地；v22 记录 direct text 合同。非阻断 preview 和 SceneBake MP4 方向性门已建立。下一批以 [覆盖台账](../scene/semantics/coverage-ledger.md) 和 [Scene 播放能力开发计划](../scene/scene-capability-development-plan-2026-07-22.md) 的共享 pass executor 路线为准。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有签名 App、26 个当前真实目录隔离样本加 13 个固定回归样本、Metal 双帧/语义/dependency/strict graph/释放门，以及 preview/SceneBake 同样本方向性证据。v22 已闭合 direct text 动态 provider；B2 已增加同帧 copy/swap、受限 unique history、Precise Blur interleave/legacy compose 和 exact stock Shake/Water Waves/Water Flow。八个 strict backend、当前完整门 5 条 chain、固定门 1 条 Blur/Shadow chain、静态 blend、direct text 和 ShaderContract source 保留仍不等价于 authored shader execution、真实 history consumer、SceneScript/media 或完整属性系统。当前可以声明“受限能力可用、Water Flow/Water Waves 在精确 profile 下进入真实 ordered GPU chain、同样本视觉变化可审查”，不能声明达到或接近 WE parity。后续主线转向共享 material pass executor、shader preprocessing IR 和 resource lifecycle，避免继续堆叠孤立 effect renderer。
