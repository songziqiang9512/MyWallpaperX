...
- 桌面宿主拥有单一 60 Hz frame driver；同一帧的 frame index、host/scene/wall time 向所有屏幕广播，shader、视频、粒子与 parallax 已迁移到 `SceneFrameContext`。pause/resume、delta clamp、fixed timestep 和 live provider snapshot 仍是后续合同。
...
当前选择“可审计的兼容 Runtime”方向：不承诺私有格式 100% 复刻，但按官方行为合同、真实样本频率和 [Scene 语义手册](../scene/semantics/README.md) 推进。Effect-definition IR、authored graph planner、两个严格 Blur 子图、resource identity/property authored-fallback、file-backed `sceneTexture`、统一 Frame Context 第一阶段、首批 built-in 粒子纹理和 typed DynamicValue/target 空 snapshot 已完成。当前下一独立切片是编译并持久化 property binding program，先补 layer alpha/color 分类，同时保留未迁移 target 的旧 resolver/rebuild fallback；Timeline、SceneScript、动态文字、audio/media 再横向接入同一合同。既有 bounded executor 在新路径取得 GPU/golden 等价证据前保留，但不再扩展名称分支。完整等级以 [覆盖台账](../scene/semantics/coverage-ledger.md) 为准。

### 阶段 1：正确性、安全和性能基础

1. 在已建立的 typed DynamicValue/target registry、同帧不可变空 snapshot 和 `authored -> user -> Timeline -> SceneScript` 优先级上，编译 property binding program；再让受支持属性、动态文字和 visibility 无需整场重建。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有“签名 App + 13 个隔离真实样本 + Metal 双帧/语义/dependency/authored-Blur GPU/释放门”的可持续开发链。v17 在可审计的 v16 authored graph 上加入 provider metadata、逐帧 typed texture registry 和空属性 authored fallback；`36bfef0` 又建立 typed dynamic target/value 与空 immutable snapshot，但没有 producer 或 consumer。strict precise、standard-default、file-backed `sceneTexture` 静态 consumer 子集和统一 Frame Context 第一阶段已执行，实际画面仍主要来自 bounded renderer；固定 precise kernel、受限 standard profile 和 5/5 静态 blend 都不等价于 authored shader 或动态 sceneTexture。当前可以声明“受限能力可用、官方能力归属和逐项缺口可查、通用图合同已建立、首个文件型 property provider 已闭环、现有动态消费者已统一帧时钟”，不能声明达到或接近 Wallpaper Engine parity。下一步按 coverage-first 先编译 live binding program，再横向接通 Timeline/SceneScript core、动态 text、audio/media/provider、Particle 与更广 backend；增加样本硬编码、默认套用效果或把结构无 blocker 写成视觉支持，仍不会形成兼容闭环。
