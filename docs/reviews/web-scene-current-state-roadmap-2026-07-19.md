...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v22：继承 v21 exact stock Opacity、ShaderContract、provider metadata 与 authored graph，并保存 direct text content/point-size/color binding program。当前实现基线 `4f13daf` 的 renderer 消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow 与 exact stock Opacity 五个严格注册 backend；同帧 graph command foundation 已按作者顺序计划 `copy/swap`，并以 Metal blit/逻辑身份交换执行；显式 `unique:true` FBO 已在读前写时进入受限 history seed/clear 生命周期；Precise Blur 两种完整白名单拓扑可按 authored nodeIndex 在两个 material node 之间执行一个 copy 或 swap，exact `KERNEL=0` legacy `compose:true` 两遍语法也可归一到同一 strict executor。其他 topology、真实 history consumer、generic compose/scene-background、condition/function、SceneScript/time/media 继续 fail closed 或整场重建，也没有让真实 Fluid/Motion Blur 进入 strict stage；这不是任意 graph、material、shader 或完整 SceneScript/media executor。

当前运行证据分两层：`.codex/scene-legacy-compose-20260724/full26-results-pass3/report.json` 对当前真实目录的 26 个隔离样本 **26/26 通过**，同一签名 App 的 `.codex/scene-legacy-compose-20260724/final13-results/report.json` 固定回归门 **13/13 通过**；两门有 6 个重叠、合计 33 个唯一样本快照。当前 26 门验证 image `330/330`、text `212/212`、solid `126/126`、particle `47/68`，named capture `8/8`、binding `9/9`、strict stage 24、failed 0；仍有 13 个 named-target gap、67 个 route-only effect、19 个 legacy blocked layer 和 9 个缺失资源，graph target 为 153、graph blocker 已从 38 降为 0。该门没有真实 multi-effect chain 或 Workshop Shadow，因此固定 13 门继续保护 14 stage、1 条真实 chain、Opacity 4、Workshop Shadow 1、failed 0。当前 Scene 全量测试共 **286 项，其中 284 项通过、2 项按环境跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `b82bb2a7ac697b34acf4ec491b0a9841224027c4`、可执行文件 SHA-256 `cf477e7b10b26e67144d0bc80a79f62de748229486742b2936422d4e4e2411bb`。本批消除了错误 blocker，但原始样本 strict stage 和 legacy blocked layer 均未改善；两门不能解释为 WE 视觉一致，下一实现批次暂停，先讨论并重排可见效果路线。
...
`<isolated-scene-sample-root>` 必须包含测试副本 `Scene/<id>`；不得直接传入真实 `~/Movies/MyWallpaperX/创意工坊/Scene`。当前正式复核使用 `.codex/scene-matrix13-samples-20260722`，最新矩阵为 `.codex/scene-workshop-shadow-final13-20260723-1540/report.json`，对应定向门为 `.codex/scene-workshop-shadow-targeted-20260723-1536/report.json`；`.codex/scene-effect-chain-gated-final13-20260723/report.json`、`.codex/scene-local-contrast-targeted-20260723/report.json`、`.codex/scene-local-contrast-final13-20260723/report.json`、`.codex/scene-shader-contract-final13-v2-20260723/report.json`、`.codex/scene-live-solid-color-final13-20260723-1015/report.json`、`.codex/scene-particle-builtins-final13-r2-20260723/report.json` 与 frame-context、file-property、v16 canonical graph、utility、particle-trail、static-fallback、v15 IR 报告只保留为对应阶段证据，不能反向覆盖当前结论。
...
- material resolver 保留 8 个 nullable slot，并按低到高优先级保存 material asset -> material usertexture -> instance asset -> instance usertexture -> explicit bind；现有 strict backend 取末项，尚未通用接入 frame registry 的 ready/fallback 选择。严格 precise Blur、standard-default Blur、stock Local Contrast 与 exact Workshop Shadow backend 共同使用 graph/effect identity、node order、RT、shader fingerprint、render state、combo、constant 和 binding 判定，不按样本 ID 或 layer 名称启用。Local Contrast 与 Workshop Shadow 都只用 exact shader fingerprint 准入，仍执行项目内手写 MSL，不执行 authored source。
...
- 已有 Metal textured-quad、共享白纹理程序化 solid、静态与 direct-property 动态 text texture、作者定义 2D sprite 粒子 instancing、层级 transform/visibility、基础混合、五个 strict effect backend、Bloom、cover 相机和声明驱动的 parallax。动态文本只在对应 direct property 变化时异步更新，作者未启用的文本层不登记为活动 consumer；这不包含 SceneScript/time/media producer。
...
- 当前 26 样本完整快照门验证 image `330/330`、solid `126/126`、text `212/212`、particle `47/68`，named capture `8/8`、binding `9/9`；strict graph 合计仍为 24 stage、0 failed，仍有 13 个 named-target gap、67 个 route-only effect、19 个 legacy blocked layer 和 9 个缺失资源。legacy compose 归一化把 graph blocker 从 38 降为 0、target 提升到 153，但没有让原始样本新增 strict stage。固定 13 样本门另行保护当前 26 门没有覆盖的真实 strict chain 与 Workshop Shadow。没有完整性能预算，也没有与 Windows Wallpaper Engine 同配置录屏的图像差异门，因此不能把 26/26 或 13/13 等同于完整兼容或 WE parity。

## 6. Scene 演进方向

### 阶段 0：产品边界与执行门（已确定方向，进行中）

当前仍选择“可审计的兼容 Runtime”方向。Effect-definition IR、authored graph planner、ShaderContract v1、五个 strict backend、ordered strict chain、resource identity/property fallback、file-backed `sceneTexture`、Frame Context、B0 live program、direct dynamic text、同帧 copy/swap command foundation、受限 unique FBO history seed/clear、Precise Blur 两种 material-command interleave 与 exact legacy compose 归一化已完成。由于最近批次多次改善内部合同却没有增加原始样本 strict stage，现暂停自动进入 history/compose 或其他语义项；先按完整 effect chain 解锁数、原始样本新增 stage、Windows WE 对照画面改善和单位实现成本重排 SceneScript、常见 backend、history、shader 与 provider 路线。route-only 只作布局诊断。
...
- 五个 strict backend、ordered strict chain、Bloom、程序化 solid、静态与 direct-property 动态 text、camera cover/显式 parallax、受限水波/合成、单层 built-in UV foliage、9 个 built-in 粒子纹理/sprite trail、utility/named target、静态 image blend、file-backed `sceneTexture`、Frame Context、B0 live 主链、显式 unique FBO 的受限 history seed/clear、Precise Blur 两种 material-command interleave 与 exact legacy compose 归一化已落地；v22 记录 direct text 合同。下一实现批次暂停，以 [覆盖台账](../scene/semantics/coverage-ledger.md) 和 [Scene 播放能力开发计划](../scene/scene-capability-development-plan-2026-07-22.md) 的可见收益复盘结论为准重排。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有签名 App、26 个当前真实目录隔离样本加 13 个固定回归样本、Metal 双帧/语义/dependency/strict graph/释放门。v22 已把 direct text 三类属性接入 B0 snapshot，并以异步 per-layer generation/stale cancellation 闭合首个动态 provider；B2 已增加严格的同帧 copy/swap plan/runtime foundation、显式 unique FBO 的受限 history seed/clear 生命周期、Precise Blur 两种 material-command interleave 和 exact legacy compose 归一化。五个 strict backend、1 条真实 chain、静态 blend、direct text、copy/swap/history foundation、受限 interleave/compose 和 ShaderContract source 保留仍不等价于 authored shader execution、真实 history consumer、SceneScript/media 或完整属性系统。当前可以声明“受限能力可用、direct dynamic text property 可无重建更新、精确 legacy Blur executor 有隔离 GPU 正门”，不能声明达到或接近 WE parity。更重要的是，最近一批把当前 26 样本 graph blocker 降到 0，却没有让 strict stage 超过 24 或 legacy blocked layer 低于 19；因此继续按语义表逐项填充已不足以证明产品进步，下一步必须先完成可见效果路线复盘再恢复开发。
