...
当前运行证据仍分完整快照与固定回归两层：legacy 指纹批次以真实目录重建 45 样本隔离副本，`.codex/scene-legacy-fingerprints-20260727/full45-report.json` 为 **43/45**、particle `110/131`、strict stage 142、chain 19、graph failed 0；唯二失败 `2131872317`/`3028090166` 均为多指纹白名单（Pulse/Water Waves/Water Ripple/Tint/Shake 的 stock + legacy 指纹）解锁后的正向计数撞 v29 旧合同，完整矩阵刷新到 v30 后二样本定向复跑 PASS（`full45-contract-confirm.json`）；`3770500543` 因缺少 `scene.pkg` 排除。`.codex/scene-legacy-fingerprints-20260727/fixed13-report.json` 固定门首跑 **12/13**（唯一 mismatch 同源，合同刷新后定向复跑 PASS）。`2131872317` 现为五层整链：19 stage、4 chain、Water Waves 7、Shake 3、Water Ripple 2、Pulse 7（首批真实语料执行）。历史 static-origin 定向门 `.codex/scene-static-origin-targeted-20260725/report.json` 按当时口径保留 1/1、`17/19`，`3088601835` 当前为 `19/19`，Snow root layers `513/534` 可见且未过曝，`snowstormfog` child 在两层生成实例且未洗白。完整 Scene suite **90+ 模块全部通过**（音频频谱线并行推进中），代码健康为 **472 Swift files、44 locked legacy files、400-line limit**。样本自带 preview 是当前第一视觉依据（证据基线 `3194ac5`）；WaifuX SceneBake MP4 只作辅助动态参考，二者均不能解释为 WE 视觉一致。
...
`<isolated-scene-sample-root>` 必须包含测试副本 `Scene/<id>`；不得直接传入真实 `~/Movies/MyWallpaperX/创意工坊/Scene`。当前完整快照门为 `.codex/scene-legacy-fingerprints-20260727/full45-report.json`，当前源码固定回归门为 `.codex/scene-legacy-fingerprints-20260727/fixed13-report.json`，历史 static-origin 定向门为 `.codex/scene-static-origin-targeted-20260725/report.json`；Snow/Smoke texture、static child、Flare、continuous-child、Event Follow、Turbulent Velocity、Light Shafts 0、Fire、BC、Puppet animation/attachment、`particle/halo_4`、X-Ray 和视觉校准报告只保留为对应阶段证据，不能反向覆盖当前结论。
