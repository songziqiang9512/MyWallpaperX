...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v20：v15 保真保存 EffectDefinition，v16 把可见 layer/effect 编译为 authored graph，v17 加入 provider/resource metadata，v18 持久化 property binding program 与 effective values，v19 加入 loss-preserving `ShaderContract`，v20 再把通过 strict execution catalog 的 Local Contrast strength scalar target 写入同一 binding program。`8474ace` 已安全保留 13 个隔离样本中 173 份 shader contract（143 authored、30 host built-in）、286 个 stage、155 个 include、1523 个 annotation 和 2660 个 declaration，原始 source/hash 与 canonical identity 均进入合同，诊断为 0；这些合同本身仍只是 L1 识别/保留。当前实现基线 `809b75e` 的 renderer 消费 precise Blur、standard-default Blur、stock Local Contrast 和 exact Workshop Shadow 四个严格注册的 backend，并沿用 `b541867` 的 ordered strict effect-chain scheduler：只有整条链无 graph blocker，且 effect/node/target identity、输入连续性和每个 stage 的 strict profile 全部成立时才进入 catalog，同一 command buffer 按作者顺序执行，只有最终 stage 合成到主画面。首段应用原 layer uniforms/mask/UV/alpha，后续段以 neutral capture 消费前段输出，每个 Local Contrast stage 从同一 per-surface snapshot 独立取值；任一后段失败时不会泄漏前段画面。exact Workshop Shadow 只匹配本地资产核验过的完整 definition/material/ShaderContract/render-state/combo/static-parameter 合同，不按样本 ID 分派，也不代表官方 45 项 Effect 表的通用 Shadow 或 generic shader。target pool 会先完成整链 plan/table/预算/LRU 候选，再原子提交 cache，失败不改 resident bytes、access counter 或 LRU。typed texture registry 也已支持 layer/named/property/system identity、provider 状态、静态 resource generation、named frame epoch 和属性缺失时的 authored fallback。文件型 `sceneTexture` 的首个产品切片已经接通 PNG/JPEG 选择、按壁纸 bookmark、限时 security scope、逐屏 Metal 解码上传和失败回退，但只开放给当前实际可执行的静态 image-blend property consumer。其他 current-prefix、named dependency、静态 blend 和手写 effect 仍是 bounded GPU 路径。这不是任意 graph、material、shader 或完整 sceneTexture/media executor。

当前正式语义矩阵 `.codex/scene-workshop-shadow-final13-20260723-1540/report.json` 为 **13/13 通过**，定向矩阵 `.codex/scene-workshop-shadow-targeted-20260723-1536/report.json` 为 **1/1 通过**。definition/graph 结构基线仍为 106 definitions、182 passes、179 material passes、50 FBO，以及 197 layer plans、300 effects、411 nodes、76 RT；5 个 blocker 仍集中在 `3723344874` fluid layer。四个 strict backend 合计 10 个 authored graph stage、1 条完整真实 multi-effect strict chain、1 个 Workshop Shadow、0 failed、2 个 legacy blocked、34 个 route-only effect。`3724289844` succeeded `[20,28,36]`、failed/blocked `[]`、stages 4、chains 1；`2938612768` 保持 stages/chains 0，没有局部执行可见 4/5-effect mixed chains。当前 Scene 全量测试共运行 **258 项、257 项通过、1 项跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `e4c25d85cb1b878019ec6a7b9e334fd6caaef098`、可执行文件 SHA-256 `bd02f455de2e5e11cbc365dabee5063afa4c32ab28d959abcd8c8bb77746a8c0`，运行前后签名均验证。Shadow 的 `common_blending` mode 0 没有官方像素 oracle，当前路径没有 Windows golden，不能声称 WE 像素等价。pause/resume、delta clamp、fixed timestep、copy/swap/compose/history/condition/function、generic material/shader execution 仍未完成。下一主切片是 stock Opacity `MASK=0` strict profile + binding program / per-surface snapshot live alpha；B1 Provider Core 的 dynamic generation、metadata 与 cancellation 并行推进。逐项等级与缺口见 [Scene 官方语义与实现覆盖台账](../scene/semantics/coverage-ledger.md)，执行顺序见 [Scene 播放能力开发计划](../scene/scene-capability-development-plan-2026-07-22.md)。
...
- 已有 Metal textured-quad、共享白纹理程序化 solid、静态 text texture、作者定义 2D sprite 粒子 instancing、层级 transform/visibility、基础混合、legacy coarse/precise Gaussian blur、standard Blur 默认 4-pass quarter-RT、stock Local Contrast 4-pass quarter-RGBA、exact Workshop Shadow 单 pass、Bloom threshold/blur/composite、cover 相机和声明驱动的 parallax。四个 strict backend 可在整条链所有 stage 都严格受支持时按作者顺序连续执行；任一 stage 不支持则整链不进入 renderer。单层 built-in UV foliage 读取作者参数；粒子可解析包内纹理和 9 个精确 built-in key 的程序化纹理，`spritetrail` 按速度方向和作者 stretch 参数拉伸。
...
- 已有 13 个真实 Scene 样本的语义自动门，当前验证 image `180/181`、solid `54/54`、text `79/108`、particle `14/27`，named capture `8/8`、binding `9/9`、static image blend `5/5`；四个 strict backend 合计 10 个 authored graph stage、1 条真实 strict multi-effect chain、0 failed，unsupported mixed chain 保持整链关闭。本轮仍观测到 34 个 route-only effect、2 个 legacy blocked layer、1 个 named-target gap，没有完整性能预算，也没有与 Windows Wallpaper Engine 同配置录屏的图像差异门，因此不能把 13/13 等同于完整兼容或 WE parity。

## 6. Scene 演进方向

### 阶段 0：产品边界与执行门（已确定方向，进行中）

当前选择“可审计的兼容 Runtime”方向：不承诺私有格式 100% 复刻，但按官方行为合同、真实样本频率和 [Scene 语义手册](../scene/semantics/README.md) 推进。Effect-definition IR、authored graph planner、ShaderContract v1、BGRA/RGBA graph target、四个 strict backend、ordered strict effect-chain scheduler、首条真实 chain、resource identity/property authored-fallback、file-backed `sceneTexture`、统一 Frame Context 第一阶段、首批 built-in 粒子纹理，以及 B0 binding program/per-surface snapshot/atomic live consumer 已完成。下一主切片是 stock Opacity `MASK=0` strict profile + binding program / per-surface snapshot live alpha；B1 Provider Core 的 dynamic generation、metadata 与 cancellation 并行推进。Timeline、SceneScript、动态文字、audio/media 以后横向接入同一 target/frame 合同。既有 bounded executor 在新路径取得 GPU/golden 等价证据前保留，但不再扩展名称分支。完整等级以 [覆盖台账](../scene/semantics/coverage-ledger.md) 为准。
...
2. B2 ordered strict effect-chain、target allocation、pass ordering、最终合成原子性与首条真实 chain 已完成；下一步在同一合同上接 stock Opacity `MASK=0` strict profile 及其 per-surface snapshot live alpha，再扩 copy/swap/compose、RT history，不能复制各 profile 的调度代码。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有“签名 App + 13 个隔离真实样本 + Metal 双帧/语义/dependency/strict graph GPU/释放门”的可持续开发链。v20 继承 v18 binding program/effective values 与 v19 loss-preserving ShaderContract；B0 已把 property producer、per-surface snapshot、atomic routing，以及 layer alpha、solid-only color、exact strict Local Contrast strength consumer 合龙。strict precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow、ordered strict effect-chain、file-backed `sceneTexture` 静态 consumer 子集和统一 Frame Context 第一阶段已执行，实际画面仍主要来自 bounded renderer；四个 strict backend、1 条真实 fully-supported multi-effect chain、5/5 静态 blend、三个 live consumer 子集和 ShaderContract source 保留都不等价于 authored shader execution、动态 sceneTexture 或完整属性系统。当前可以声明“受限能力可用、官方能力归属和逐项缺口可查、graph/target 与 shader source 合同已建立、首个文件型 property provider、B0 live 主链及首条真实 strict chain 已闭环”，不能声明达到或接近 Wallpaper Engine parity。下一步实现 stock Opacity `MASK=0` strict profile，并把 alpha 绑定接入同一 per-surface snapshot live 路径；B1 Provider Core 的 dynamic generation、metadata、cancellation 并行，随后再接 copy/swap/compose/history 与 B3/B4 的 Timeline/SceneScript、动态 text、audio/media、Particle 和更广 backend。增加样本硬编码、默认套用效果或把 compiler-only target 写成 live，仍不会形成兼容闭环。
