...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v22：继承 v21 exact stock Opacity、ShaderContract、provider metadata 与 authored graph，并保存 direct text content/point-size/color binding program。当前实现基线 `f1c6a10` 的 renderer 仍消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow 与 exact stock Opacity 五个严格注册 backend；同帧 graph command foundation 已按作者顺序计划 `copy/swap`，并以 Metal blit/逻辑身份交换执行，但尚未接进通用 material scheduler，也没有让真实 Fluid/Motion Blur 进入 strict stage。动态文本仍按 per-surface snapshot 只重栅格化变化层。history、compose、condition/function、SceneScript/time/media 继续 fail closed 或整场重建；这不是任意 graph、material、shader 或完整 SceneScript/media executor。

当前正式语义矩阵 `.codex/scene-copy-swap-final13-20260723-1955/report.json` 为 **13/13 通过**，报告 SHA-256 为 `1399d3ea4a4c00bb5529dc6db7b0fd80e810003c945e04d6a26200944f18fe74`，矩阵 SHA-256 为 `fbb252a64018cf785e20b2200db5966a300e9351a994b4a36f1fe9565c5b354c`。五个 strict backend 指标保持 14 stage、1 条真实 chain、Opacity 4、Local Contrast 2、Workshop Shadow 1、0 failed、2 个 legacy blocked、30 个 route-only effect；定义侧观察到 copy 1、swap 2，graph 中 swap node 2。定向样本 `3723344874` 仍为 stage/chain 0，并因 1 个 function 与 4 个 condition blocker 失败关闭，证明本批没有把 Fluid/Motion Blur 误报为可执行。当前 Scene 全量测试共 **276 项，其中 274 项通过、2 项按环境跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `c0940f2da38e653e338d829663ad3a0596c73b50`、可执行文件 SHA-256 `fe940c3d6706655d0a07556ef7570ab581c9ee000c5ec62344bd0df92c6d9827`。13/13 仍不能解释为 WE 视觉一致；下一主线是 B2 persistent/history 生命周期、material-command interleave 与 compose，并把文本 provider lifecycle 推广到 system/media/video。
...
- 已有 13 个真实 Scene 样本的语义自动门，当前验证 image `180/181`、solid `54/54`、text `79/108`、particle `14/27`，named capture `8/8`、binding `9/9`、static image blend `5/5`；五个 strict backend 合计 14 个 authored graph stage、1 条真实 strict multi-effect chain、0 failed，unsupported mixed chain 保持整链关闭。本轮仍观测到 30 个 route-only effect、2 个 legacy blocked layer、1 个 named-target gap，没有完整性能预算，也没有与 Windows Wallpaper Engine 同配置录屏的图像差异门，因此不能把 13/13 等同于完整兼容或 WE parity。
