...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v20：v15 保真保存 EffectDefinition，v16 把可见 layer/effect 编译为 authored graph，v17 加入 provider/resource metadata，v18 持久化 property binding program 与 effective values，v19 加入 loss-preserving `ShaderContract`，v20 再把通过 strict execution catalog 的 Local Contrast strength scalar target 写入同一 binding program。`8474ace` 已安全保留 13 个隔离样本中 173 份 shader contract（143 authored、30 host built-in）、286 个 stage、155 个 include、1523 个 annotation 和 2660 个 declaration，原始 source/hash 与 canonical identity 均进入合同，诊断为 0；这些合同本身仍只是 L1 识别/保留。renderer 当前消费 precise Blur、standard-default Blur 和 stock Local Contrast 三个严格注册的 graph profile；`b541867` 在这些 backend 上补齐 ordered strict effect-chain scheduler：只有整条链无 graph blocker，且 effect/node/target identity、输入连续性和每个 stage 的 strict profile 全部成立时才进入 catalog，同一 command buffer 按作者顺序执行，只有最终 stage 合成到主画面。首段应用原 layer uniforms/mask/UV/alpha，后续段以 neutral capture 消费前段输出，每个 Local Contrast stage 从同一 per-surface snapshot 独立取值；任一后段失败时不会泄漏前段画面。target pool 会先完成整链 plan/table/预算/LRU 候选，再原子提交 cache，失败不改 resident bytes、access counter 或 LRU。typed texture registry 也已支持 layer/named/property/system identity、provider 状态、静态 resource generation、named frame epoch 和属性缺失时的 authored fallback。文件型 `sceneTexture` 的首个产品切片已经接通 PNG/JPEG 选择、按壁纸 bookmark、限时 security scope、逐屏 Metal 解码上传和失败回退，但只开放给当前实际可执行的静态 image-blend property consumer。其他 current-prefix、named dependency、静态 blend 和手写 effect 仍是 bounded GPU 路径。这不是任意 graph、material、shader 或完整 sceneTexture/media executor。

当前正式语义矩阵 `.codex/scene-effect-chain-gated-final13-20260723/report.json` 为 **13/13 通过**。definition/graph 结构基线仍为 106 definitions、182 passes、179 material passes、50 FBO，以及 197 layer plans、300 effects、411 nodes、76 RT；5 个 blocker 仍集中在 `3723344874` fluid layer。正式矩阵合计 8 个 authored graph stage、0 个完整 multi-effect strict chain、0 failed、3 个 legacy blocked、35 个 route-only effect；当前 13 个真实样本没有一条 multi-effect chain 的每个 stage 都属于上述三个 strict backend，因此 chain count 0 是正确的全链失败关闭结果。synthetic precise -> precise GPU 门证明两段真实顺序、stage0 output -> stage1 input 像素差不超过 1、final -> main 差不超过 2；故意让第二段失败时第一段虽已产像素，main 仍全透明。`3724289844` 保持 stages 2、chains 0、blocked layer `[20]`，`2938612768` 保持 stages/chains 0，没有局部执行可见 4/5-effect mixed chains。当前 Scene 全量测试共运行 **251 项、250 项通过、1 项跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `61fc420b6dc3c014d1e18f2cdf16fef1d127d5ec`、可执行文件 SHA-256 `629ae6daf23a6e62f2d9502042dc94b69a54419749e801b26c4743651bc12758`，运行前后签名均验证。pause/resume、delta clamp、fixed timestep、copy/swap/compose/history/condition/function、generic material/shader execution 仍未完成。下一主切片是 `3724289844` layer `20` 的 exact Workshop single-pass shadow profile，使现有 `Blur Precise -> Shadow` 成为首条真实 strict chain；它不是官方 45 项 Shadow 或 generic authored shader。B1 Provider Core 的 dynamic generation、metadata 与 cancellation 并行推进；逐项等级与缺口见 [Scene 官方语义与实现覆盖台账](../scene/semantics/coverage-ledger.md)，执行顺序见 [Scene 播放能力开发计划](../scene/scene-capability-development-plan-2026-07-22.md)。
...
- benchmark 通过签名 App 身份、隔离样本/HOME、Metal ready/after 双帧、语义字段、utility/named/image-blend/particle planned 与 finished layer ID、authored graph canonical SHA、precise/standard/Local Contrast backend exact succeeded/failed/legacy-blocked layer ID、ordered chain/stage 精确计数、live identity/visible-change，以及 stop 后 surface=0 验证当前 13 样本；七个 effect-graph 代表样本锁定 chain/stage 正负门。
...
- 已有 13 个真实 Scene 样本的语义自动门，当前验证 image `180/181`、solid `54/54`、text `79/108`、particle `14/27`，named capture `8/8`、binding `9/9`、static image blend `5/5`；三个 strict backend 合计 8 个 authored graph stage、0 failed，当前真实样本完整 strict multi-effect chain 为 0，unsupported mixed chain 保持整链关闭。但本轮仍观测到 35 个 route-only effect、1 个 named-target gap，没有完整性能预算，也没有与 Windows Wallpaper Engine 同配置录屏的图像差异门，因此不能把 13/13 等同于完整兼容或 WE parity。

## 6. Scene 演进方向

### 阶段 0：产品边界与执行门（已确定方向，进行中）

当前选择“可审计的兼容 Runtime”方向：不承诺私有格式 100% 复刻，但按官方行为合同、真实样本频率和 [Scene 语义手册](../scene/semantics/README.md) 推进。Effect-definition IR、authored graph planner、ShaderContract v1、BGRA/RGBA graph target、三个 strict profile、ordered strict effect-chain scheduler、resource identity/property authored-fallback、file-backed `sceneTexture`、统一 Frame Context 第一阶段、首批 built-in 粒子纹理，以及 B0 binding program/per-surface snapshot/atomic live consumer 已完成。下一主切片是 `3724289844` layer `20` 的 exact Workshop single-pass shadow profile，用完整 definition/material/ShaderContract fingerprint 解锁首条真实 strict chain；B1 Provider Core 的 dynamic generation、metadata 与 cancellation 并行推进。Timeline、SceneScript、动态文字、audio/media 以后横向接入同一 target/frame 合同。既有 bounded executor 在新路径取得 GPU/golden 等价证据前保留，但不再扩展名称分支。完整等级以 [覆盖台账](../scene/semantics/coverage-ledger.md) 为准。
...
2. B2 ordered strict effect-chain、target allocation、pass ordering 与最终合成原子性骨架已完成；下一步在同一合同上补能解锁真实链的 strict backend，再扩 copy/swap/compose、RT history，不能复制各 profile 的调度代码。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有“签名 App + 13 个隔离真实样本 + Metal 双帧/语义/dependency/strict graph GPU/释放门”的可持续开发链。v20 继承 v18 binding program/effective values 与 v19 loss-preserving ShaderContract；B0 已把 property producer、per-surface snapshot、atomic routing，以及 layer alpha、solid-only color、exact strict Local Contrast strength consumer 合龙。strict precise Blur、standard-default Blur、stock Local Contrast、ordered strict effect-chain、file-backed `sceneTexture` 静态 consumer 子集和统一 Frame Context 第一阶段已执行，实际画面仍主要来自 bounded renderer；三个 strict profile、0 条真实 fully-supported multi-effect chain、5/5 静态 blend、三个 live consumer 子集和 ShaderContract source 保留都不等价于 authored shader execution、动态 sceneTexture 或完整属性系统。当前可以声明“受限能力可用、官方能力归属和逐项缺口可查、graph/target 与 shader source 合同已建立、首个文件型 property provider 及 B0 live 主链已闭环”，不能声明达到或接近 Wallpaper Engine parity。下一步实现 `3724289844:20` exact Workshop single-pass shadow profile，取得首条真实 `Blur Precise -> Shadow` 正门；B1 Provider Core 的 dynamic generation、metadata、cancellation 并行，随后再接 copy/swap/compose/history 与 B3/B4 的 Timeline/SceneScript、动态 text、audio/media、Particle 和更广 backend。增加样本硬编码、默认套用效果或把 compiler-only target 写成 live，仍不会形成兼容闭环。
