...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v21：v15-v20 依次保留 EffectDefinition、authored graph、provider metadata、property binding program、ShaderContract 与 Local Contrast live target；v21 再保存 exact stock Opacity 的候选层诊断和执行合同。当前实现基线 `b8842d8` 的 renderer 消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow 与 exact stock Opacity 五个严格注册的 backend，并沿用 ordered strict effect-chain scheduler。Opacity 只接受官方 stock definition/material/shader 的精确 fingerprint、单 effect/单 material stage、`MASK=0`、无额外纹理/命令/condition，以及有限静态 alpha 或直接 user-property binding；动态值从既有 binding program 和 per-surface snapshot 进入 GPU，预乘宿主边界按同一 alpha 缩放 RGBA。SceneScript 计算值、MASK1、未知字段/hash/combo/state 和 unsupported mixed chain 均 fail closed，不按样本 ID、名称或路径子串启用。target pool、typed texture registry 与 file-backed `sceneTexture` 仍保持既有受限边界；这不是任意 graph、material、shader 或完整 sceneTexture/media executor。

当前正式语义矩阵 `.codex/scene-opacity-final13-20260723-1730/report.json` 为 **13/13 通过**，矩阵 SHA-256 为 `47d01b05a60368cddc679393fb5dd11d562cd4b69cc9d4bd1f559c179fff4e23`。五个 strict backend 合计 14 个 authored graph stage、1 条完整真实 multi-effect strict chain、Opacity 4、Local Contrast 2、Workshop Shadow 1、0 failed、2 个 legacy blocked、30 个 route-only effect。`2902406982` 精确执行 Opacity layers `[365,372,647,664]`，`newproperty50=0.2` 通过 snapshot live 消费且 surface/window identity 不变；`2938612768` 的 `[165,454,626,629,924]` 只是 SceneScript 候选，Opacity/stages 均为 0，继续作为 fail-closed 负门。当前 Scene 全量测试共 **269 项，其中 267 项通过、2 项按环境跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `6e70e547f61dcc6821009a4ebbd97af156fb5916`、可执行文件 SHA-256 `0cbaff3bd989e5cb9f8807236fd270feec19ca1f18548c122ca3ccb74033774a`，运行前后签名均验证。293 的主画面仍被 unsupported effect 明显扭曲，13/13 不能解释为 WE 视觉一致。pause/resume、delta clamp、fixed timestep、copy/swap/compose/history/condition/function、SceneScript 和 generic material/shader execution 仍未完成；下一优先级按覆盖台账选择最低公共依赖，不回退到按名称近似。逐项等级与缺口见 [Scene 官方语义与实现覆盖台账](../scene/semantics/coverage-ledger.md)，执行顺序见 [Scene 播放能力开发计划](../scene/scene-capability-development-plan-2026-07-22.md)。
...
- 使用 versioned `.mywallpaperx-scene-interpretation.json` 作为稳定中间层；当前 format 为 v21。
...
- 已有 Metal textured-quad、共享白纹理程序化 solid、静态 text texture、作者定义 2D sprite 粒子 instancing、层级 transform/visibility、基础混合、legacy coarse/precise Gaussian blur、standard Blur 默认 4-pass quarter-RT、stock Local Contrast 4-pass quarter-RGBA、exact Workshop Shadow 与 exact stock Opacity 单 pass、Bloom threshold/blur/composite、cover 相机和声明驱动的 parallax。五个 strict backend 可在整条链所有 stage 都严格受支持时按作者顺序连续执行；任一 stage 不支持则整链不进入 renderer。单层 built-in UV foliage 读取作者参数；粒子可解析包内纹理和 9 个精确 built-in key 的程序化纹理，`spritetrail` 按速度方向和作者 stretch 参数拉伸。
...
- 已有 Scene 用户属性解析、受支持 target 的默认值/override 应用、按 wallpaper 持久化、条件显示和独立属性编辑窗口；实际可执行的 `sceneTexture` key 可选择 PNG/JPEG，bookmark 授权只覆盖同步解码上传，UI 不显示当前 renderer 无法执行的属性 target。layer alpha、solid-only layer color、exact stock Local Contrast strength 与 exact stock Opacity alpha 已通过 v21 binding program 和 per-surface snapshot 无重建更新；新 target 只有 compiler mapping 与真实 consumer 同时存在才允许 live。
...
descriptor 能识别 image、solid、particle、text、container 与 typed utility；v20 继承的 authored graph 中，严格 precise Blur、standard-default Blur、stock Local Contrast 与 exact Workshop Shadow 四个 backend 已进入 GPU，ordered strict effect-chain scheduler、typed frame registry、property authored fallback 和受限 PNG/JPEG property source 也已落地。当前仍不是通用 GPU DAG executor：scheduler 只连接已严格准入的 material-only stage，copy/swap/compose/history/condition/function 与通用 material/shader 尚未执行；system/media source、文件纹理的任意 material consumer、effectful provider、child/nested target、utility mask、复杂 blend、Texture Variants、child/world-space/其余粒子 renderer、puppet 和脚本驱动内容仍会缺失或降级。
...
- Scene 用户属性已有独立编辑窗口、条件显示、持久化、活动壁纸 fallback 重建，以及 `texture`/`scenetexture` 内部类型归一；当前可执行的静态 image-blend texture key 已支持 PNG/JPEG picker、bookmark、解码上传、真实 override 和 authored fallback。layer alpha、纯 solid layer color、exact strict Local Contrast strength 与 exact stock Opacity alpha 已 live；non-solid/mixed color、SceneScript opacity、Texture Variants、视频、system/media、通用 material consumer、transform、particle、puppet 和其他 effect uniform 等 target 仍隐藏或整场重建，因此不是完整属性兼容链。
...
当前选择“可审计的兼容 Runtime”方向：不承诺私有格式 100% 复刻，但按官方行为合同、真实样本频率和 [Scene 语义手册](../scene/semantics/README.md) 推进。Effect-definition IR、authored graph planner、ShaderContract v1、BGRA/RGBA graph target、五个 strict backend、ordered strict effect-chain scheduler、首条真实 chain、resource identity/property authored-fallback、file-backed `sceneTexture`、统一 Frame Context 第一阶段、首批 built-in 粒子纹理，以及 B0 binding program/per-surface snapshot/atomic live consumer 已完成。exact stock Opacity `MASK=0` 与 live alpha 已合龙；下一主线以 21 样本结构化 census 的高命中属性为依据推进动态文字/Provider Core，并继续准备 copy/swap/compose/history。Timeline、SceneScript、audio/media 以后横向接入同一 target/frame 合同。既有 bounded executor 在新路径取得 GPU/golden 等价证据前保留，但不再扩展名称分支；route-only 只作布局诊断。完整等级以 [覆盖台账](../scene/semantics/coverage-ledger.md) 为准。
...
2. B2 ordered strict effect-chain、target allocation、pass ordering、最终合成原子性、首条真实 chain 与 stock Opacity `MASK=0` live alpha 已完成；下一步在同一合同上扩 copy/swap/compose、RT history，不能复制各 profile 的调度代码。
...
- 受限 legacy coarse/precise Gaussian blur、strict standard Blur 默认 4-pass、strict stock Local Contrast 4-pass、exact Workshop Shadow、exact stock Opacity、ordered strict effect-chain scheduler、Bloom、程序化 solid、静态 text geometry/font、camera cover/显式 parallax、无 mask normal-map waterripple、perspective/opacity、mode 9 composite、单层 built-in UV foliage、9 个精确 built-in 粒子程序纹理/sprite trail、utility current-prefix、bounded named target、简单静态 image blend、typed provider fallback、file-backed `sceneTexture` 第一切片、统一 Frame Context 第一阶段，以及 B0 layer-alpha/solid-color/Local Contrast strength/Opacity alpha live 主链已落地；v21 记录 exact stock Opacity 候选与执行合同。下一步以 [覆盖台账](../scene/semantics/coverage-ledger.md) 和 [Scene 播放能力开发计划](../scene/scene-capability-development-plan-2026-07-22.md) 为准，按真实链缺口补 backend/primitive，B1 Provider Core 并行。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有“签名 App + 13 个隔离真实样本 + Metal 双帧/语义/dependency/strict graph GPU/释放门”的可持续开发链。v21 继承 binding program/effective values 与 loss-preserving ShaderContract，并记录 exact stock Opacity 候选；B0 已把 property producer、per-surface snapshot、atomic routing，以及 layer alpha、solid-only color、exact strict Local Contrast strength 与 exact stock Opacity alpha consumer 合龙。strict precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow、stock Opacity、ordered strict effect-chain、file-backed `sceneTexture` 静态 consumer 子集和统一 Frame Context 第一阶段已执行，实际画面仍主要来自 bounded renderer；五个 strict backend、1 条真实 fully-supported multi-effect chain、5/5 静态 blend、四类 live consumer 子集和 ShaderContract source 保留都不等价于 authored shader execution、动态 sceneTexture 或完整属性系统。当前可以声明“受限能力可用、官方能力归属和逐项缺口可查、graph/target 与 shader source 合同已建立、首个文件型 property provider、B0 live 主链及首条真实 strict chain 已闭环”，不能声明达到或接近 Wallpaper Engine parity。下一步用结构化 census 推进高命中动态文字/Provider Core，再接 copy/swap/compose/history、Timeline/SceneScript、audio/media、Particle 和更广 backend。增加样本硬编码、默认套用效果或把 compiler-only target 写成 live，仍不会形成兼容闭环。
