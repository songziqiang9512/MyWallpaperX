...
- **动态系统尚未起步**：Timeline 无 IR、SceneScript 无运行时、音频/媒体桥未接、puppet 动画（MDLA）与 attachment（MDAT）只解析边界；这些决定大量样本的"画面会动"体验。
...
当前仍选择"可审计的兼容 Runtime"方向。Effect-definition IR、authored graph planner、ShaderContract v1、十一类 strict backend、ordered strict chain、resource identity/property fallback、file-backed `sceneTexture`、Frame Context、B0 live program、direct dynamic text、同帧 copy/swap、受限 unique history、Precise Blur interleave/legacy compose、exact Shake/Foliage Sway/Water Ripple/Water Waves/Water Flow/X-Ray，以及 `3769688830` 暴露的公共合成缺口（puppet bind-pose、BC premultiply、REFRACT fail-closed）已完成。当前批次优先级以 [开发计划的当前批次优先级](../scene/scene-capability-development-plan-2026-07-22.md) 为准：puppet MDAT attachment 定位、MDLA 动画、粒子 child/eventdeath、超预算 BC 合成，随后是 Fire effect 与全局比例/裁切。route-only、内部 blocker 数和跨样本视觉分数都不能单独决定优先级。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有签名 App、45 个当前可运行真实目录隔离样本加 13 个固定回归样本、Metal 双帧/语义/dependency/strict graph/释放门，以及 preview/SceneBake 同样本方向性证据；真实目录中的 `3770500543` 因缺少 package 未进入运行矩阵。v23 已闭合 direct text 动态 provider 与 puppet bind-pose mesh；B2 已有同帧 copy/swap、受限 unique history、Precise Blur interleave/legacy compose 和 exact stock Shake/Foliage Sway/Water Ripple/Water Waves/Water Flow/X-Ray；`3769688830` 一类 puppet 样本的主体构图已由 bind-pose 重组恢复，BC 颜色纹理的白 matte 与折射粒子白块假阳性已消除。十一类 strict backend、当前完整门 15 条 chain、固定门 2 条 chain、静态 blend、direct text、11 个 built-in 粒子纹理和 ShaderContract source 保留仍不等价于 authored shader execution、真实 history consumer、SceneScript/media、puppet 动画、粒子 child/event 生命周期或完整属性系统。当前可以声明"受限能力可用、精确 profile 进入真实 ordered GPU chain、puppet 主体按 bind pose 正确组装、三个 X-Ray 样本在桌面路径可交互、同样本视觉变化可审查"，不能声明达到或接近 WE parity。后续批次以开发计划的当前批次优先级为准：MDAT attachment、MDLA 动画、粒子 child/eventdeath、超预算 BC 合成，再到 Fire effect 与比例/裁切。
