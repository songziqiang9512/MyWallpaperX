...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v22；实现基线 `3baf1fc` 的 renderer 消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow、exact stock Opacity、Shake、Water Waves、Water Flow、Foliage Sway、Water Ripple 与 X-Ray 十一类严格注册 backend。动态 profile 按完整 definition/material/shader/resource fingerprint 准入，并在同一 ordered scheduler 保持作者顺序；X-Ray 另有明确受限的前缀执行边界。默认 96 MiB pool 只保证 6 个全尺寸 BGRA texture unit，超预算链在规划阶段整链拒绝。同帧 graph command、受限 unique history seed/clear、Precise Blur interleave 与 exact legacy compose 继续生效；generic material pass、authored shader preprocessing/translation、真实 history consumer、generic compose/scene-background、condition/function、SceneScript/time/media 仍未实现。

当前运行证据分两层：`.codex/scene-xray-final-20260724/full26-duration7/report.json` 对当前真实目录的 26 个隔离样本 **26/26 通过**；`.codex/scene-xray-final-20260724/fixed13-duration7-final/report.json` 固定回归门 **13/13 通过**。26 门验证 image `330/330`、text `212/212`、solid `126/126`、particle `52/68`，named capture `8/8`、binding `9/9`、strict stage 73、chain 12、Water Flow 7、Water Waves 10、Shake 21、failed 0；固定门保护 particle `15/27`、24 stage、2 chain、Water Flow 1、Water Waves 6、Shake 1、Opacity 4、Workshop Shadow 1、failed 0。完整 Scene suite 为 **318 项：315 通过、3 项跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `9951f4c741e3484894be717b68019f55f10a800f`、可执行文件 SHA-256 `61bfdfb5c9215ea5ceaab8bfa6d995be38f0ce2e5fc8629d0299326295e3832c`。样本自带 preview 是当前第一视觉依据；WaifuX SceneBake MP4 只作辅助动态参考，二者均不能解释为 WE 视觉一致。
...
- benchmark 通过签名 App 身份、隔离样本/HOME、Metal ready/after 双帧、语义字段、utility/named/image-blend/particle planned 与 finished layer ID、authored graph canonical SHA、strict backend exact count、ordered chain/stage、live identity/visible-change，以及 stop 后 surface=0 验证固定 13 与完整 26 样本；矩阵现在精确锁定 Water/Shake/Foliage/Ripple/X-Ray chain、超预算长链拒绝和 dormant Opacity slot。`3194ac5` 的 preview 并排图与 WaifuX SceneBake MP4 路径都明确非阻断。
...
- 当前 26 样本完整快照门验证 image `330/330`、solid `126/126`、text `212/212`、particle `52/68`，named capture `8/8`、binding `9/9`；strict graph 合计 73 stage、12 chain、Water Flow 7、Water Waves 10、Shake 21、0 failed，仍有 13 个 named-target gap、66 个 route-only effect 和 16 个 legacy blocked layer。固定 13 样本门另行保护 particle `15/27`、24 stage、2 chain、Water Flow 1、Water Waves 6、Shake 1、Opacity 4 与 Workshop Shadow 1。样本 preview 优先、WaifuX MP4 只作辅助动态参考，均不设绝对阈值；没有 Windows WE 同配置录屏，不能把 26/26、13/13 或 SSIM 等同于动态/像素 parity。

## 6. Scene 演进方向

### 阶段 0：产品边界与执行门（已确定方向，进行中）

当前仍选择“可审计的兼容 Runtime”方向。Effect-definition IR、authored graph planner、ShaderContract v1、十一类 strict backend、ordered strict chain、resource identity/property fallback、file-backed `sceneTexture`、Frame Context、B0 live program、direct dynamic text、同帧 copy/swap、受限 unique history、Precise Blur interleave/legacy compose，以及 exact Shake/Foliage Sway/Water Ripple/Water Waves/Water Flow/X-Ray 已完成。下一批从新增 `3769688830` 的公共 blend/composition 与 `3769364482` 的 Fire effect 开始，随后处理 299 多余粒子和全局比例/裁切；route-only、内部 blocker 数和跨样本视觉分数都不能单独决定优先级。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有签名 App、26 个当前真实目录隔离样本加 13 个固定回归样本、Metal 双帧/语义/dependency/strict graph/释放门，以及 preview/SceneBake 同样本方向性证据。v22 已闭合 direct text 动态 provider；B2 已增加同帧 copy/swap、受限 unique history、Precise Blur interleave/legacy compose 和 exact stock Shake/Foliage Sway/Water Ripple/Water Waves/Water Flow/X-Ray。十一类 strict backend、当前完整门 12 条 chain、固定门 2 条 chain、静态 blend、direct text 和 ShaderContract source 保留仍不等价于 authored shader execution、真实 history consumer、SceneScript/media 或完整属性系统。当前可以声明“受限能力可用、精确 profile 进入真实 ordered GPU chain、三个 X-Ray 样本在桌面路径可交互、同样本视觉变化可审查”，不能声明达到或接近 WE parity。后续从新增样本暴露的公共 composition/blend 与 Fire effect 开始，再处理粒子和比例问题。
