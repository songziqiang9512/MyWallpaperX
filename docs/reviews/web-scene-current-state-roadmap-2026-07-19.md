...
`<isolated-scene-sample-root>` 必须包含测试副本 `Scene/<id>`；不得直接传入真实 `~/Movies/MyWallpaperX/创意工坊/Scene`。当前正式复核使用 `.codex/scene-matrix13-samples-20260722`，最新报告为 `.codex/scene-frame-context-final13-20260723/report.json`；file-property、v16 canonical graph 与更早 utility、particle-trail、static-fallback、v15 IR 报告只保留为阶段性证据，不能反向覆盖当前结论。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有“签名 App + 13 个隔离真实样本 + Metal 双帧/语义/dependency/authored-Blur GPU/释放门”的可持续开发链。v17 在可审计的 v16 authored graph 上加入 provider metadata、逐帧 typed registry 和空属性 authored fallback；strict precise、standard-default、file-backed `sceneTexture` 静态 consumer 子集和统一 Frame Context 第一阶段已执行，但实际画面仍主要来自 bounded renderer，固定 precise kernel、受限 standard profile 和 5/5 静态 blend 都不等价于 authored shader 或动态 sceneTexture。当前可以声明“受限能力可用、通用图合同已建立、首个文件型 property provider 已闭环、现有动态消费者已统一帧时钟”，不能声明达到或接近 Wallpaper Engine parity。下一步按 coverage-first 先横向接通 built-in particle、typed live targets、Timeline/SceneScript core、动态 text、audio/media/provider 与更广 backend，再用矩阵暴露的共享根因纵向校准；增加样本硬编码、默认套用效果或把结构无 blocker 写成视觉支持，仍不会形成兼容闭环。
