...
45. 2026-07-23 的 Scene file-backed `sceneTexture` 第一切片：属性窗口只为实际可执行的 texture property key 提供 PNG/JPEG picker；书签按 wallpaper/property 隔离，安全作用域只覆盖同步通知、逐屏解码/Metal 上传和后续屏幕重建，完成即释放。`2938612768` 隔离副本向 `newproperty25/26` 注入 200×200 PNG 后两张纹理均加载，static image blend 仍为 5/5、image 44/44；空值、坏图和不支持扩展名 fail closed 并保留 authored fallback。最终报告 `.codex/scene-user-texture-final13-r2-20260723/report.json` 为 13/13，Scene 测试共运行 134 项、133 项通过、1 项跳过；签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `0256234220b2b02ba1aaf551ef27eb4d75bb95cc`、可执行文件 SHA-256 `4c294ff88338c64f0862a7b719134e39a51dbc05fe28362c5bc6819ab697475c`。这不证明 system/media、Texture Variants、视频、任意 material property consumer、动态 alpha 或 SceneScript 已可用；真实 NSOpenPanel 点击后跨重启恢复、沙盒构建和物理显示器热插拔仍保留为产品回归项。
...
- 已有 Scene 用户属性解析、受支持 target 的默认值/override 应用、按 wallpaper 持久化、条件显示和独立属性编辑窗口；实际可执行的 `sceneTexture` key 可选择 PNG/JPEG，bookmark 授权只覆盖同步解码上传，UI 不显示当前 renderer 无法执行的属性 target。
...
- Scene 用户属性已有独立编辑窗口、条件显示、持久化、活动壁纸受控重建，以及 `texture`/`scenetexture` 内部类型归一；当前可执行的静态 image-blend texture key 已支持 PNG/JPEG picker、bookmark、解码上传、真实 override 和 authored fallback。Texture Variants、视频、system/media、通用 material consumer、transform、particle、puppet 和未支持 effect uniform 等 target 仍不开放，因此不是完整属性兼容链。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有“签名 App + 13 个隔离真实样本 + Metal 双帧/语义/dependency/authored-Blur GPU/释放门”的可持续开发链。v17 在可审计的 v16 authored graph 上加入 provider metadata、逐帧 typed registry 和空属性 authored fallback；strict precise、standard-default 与 file-backed `sceneTexture` 静态 consumer 子集已执行，但实际画面仍主要来自 bounded renderer，固定 precise kernel、受限 standard profile 和 5/5 静态 blend 都不等价于 authored shader 或动态 sceneTexture。当前可以声明“受限能力可用、通用图合同已建立、首个文件型 property provider 已闭环”，不能声明达到或接近 Wallpaper Engine parity。下一步是 system/media/variant/video/effectful/nested provider、通用 material consumer 与更广 backend；增加样本硬编码、默认套用效果或把结构无 blocker 写成视觉支持，仍不会形成兼容闭环。
