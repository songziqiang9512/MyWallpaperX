...
> 评估日期：2026-07-19，Web 状态更新至 2026-07-22，Scene 状态更新至 2026-07-24
...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v23；实现基线 `dd85dcf` 的 renderer 消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow、exact stock Opacity、Shake、Water Waves、Water Flow、Foliage Sway、Water Ripple 与 X-Ray 十一类严格注册 backend，并生成 11 个精确登记的项目自有 built-in 粒子纹理。`8bac86e` 新增 puppet bind-pose mesh 重组（MDLV mesh block 把图集重组为 bind-pose 纹理；warp 动画/attachment 未实现）、BC1/2/3 颜色纹理的 premultiplied CPU 解码与裁剪（`dd85dcf` 限定单帧 16M 像素预算内，超限/多帧容器保持直通），以及 REFRACT 粒子材质 fail-closed。动态 profile 按完整 definition/material/shader/resource fingerprint 准入，并在同一 ordered scheduler 保持作者顺序；X-Ray 另有明确受限的前缀执行边界。默认 96 MiB pool 只保证 6 个全尺寸 BGRA texture unit，超预算链在规划阶段整链拒绝。generic material pass、authored shader preprocessing/translation、真实 history consumer、generic compose/scene-background、condition/function、SceneScript/time/media、puppet 动画与粒子 child/event 完整生命周期仍未实现。

当前运行证据分两层：`.codex/scene-puppet-full45-20260725/results-v2/report.json` 对当前真实目录中 45 个可运行隔离样本 **45/45 通过**；`3770500543` 因源目录缺少 `scene.pkg` 而明确排除。`.codex/scene-puppet-fixed13-20260725/final-v2/report.json` 固定回归门 **13/13 通过**。45 门验证 image `449/451`、text `244/244`、solid `140/140`、particle `79/131`（`8bac86e` 的 REFRACT fail-closed 新口径），strict stage 91、chain 15、Water Flow 10、Water Waves 11、Shake 24、failed 0；固定门保护 particle `13/27`、24 stage、2 chain、Water Flow 1、Water Waves 6、Shake 1、Opacity 4、Workshop Shadow 1、failed 0。完整 Scene suite 为 **415 项：412 通过、3 项跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `50db8b27546c3d6c2a1adff4a45991f61a94c7cf`、可执行文件 SHA-256 `973f43b1ef58267c5678a1e6478c38a0cefd5d893b0267243b7a430a01dfcc7d`。样本自带 preview 是当前第一视觉依据（基线 `3194ac5`）；WaifuX SceneBake MP4 只作辅助动态参考，二者均不能解释为 WE 视觉一致。
...
58. 2026-07-23 的 Scene exact Workshop Shadow 与首条真实 strict chain：`b541867` 先建立 ordered all-supported scheduler，`809b75e` 再以完整 definition/material/ShaderContract/render-state/combo/static-parameter 合同准入单 pass Workshop Shadow。定向报告 `.codex/scene-workshop-shadow-targeted-20260723-1536/report.json` 为 **1/1**，正式报告 `.codex/scene-workshop-shadow-final13-20260723-1540/report.json` 为 **13/13**；四个 strict backend 合计 stages 10、chains 1、Workshop Shadow 1、failed 0、legacy blocked 2、route-only 34。`3724289844` succeeded `[20,28,36]`、failed/blocked `[]`、stages 4、chains 1。全量 Scene **258 项、257 项通过、1 项跳过**；签名 CDHash 为 `e4c25d85cb1b878019ec6a7b9e334fd6caaef098`，executable SHA-256 为 `bd02f455de2e5e11cbc365dabee5063afa4c32ab28d959abcd8c8bb77746a8c0`。该 profile 只记为 exact Workshop `L3 executed-degraded`；`common_blending` mode 0 无官方像素 oracle，不能升级官方通用 Shadow、generic shader 或 WE pixel parity。
59. 2026-07-23 的 Scene exact stock Opacity 与 live alpha：提交 `b8842d8` 将 interpretation 升至 v21，只接受 stock `effects/opacity/effect.json`、精确 material/raw shader fingerprint、单 material stage、`MASK=0`、无额外纹理/命令/condition，以及有限静态 alpha 或直接 user binding。`2902406982` 的 layers `[365,372,647,664]` 精确执行，`newproperty50=0.2` 经 binding program/per-surface snapshot 改变 GPU 像素且不重建 surface/window；`2938612768` 的 `[165,454,626,629,924]` 因 SceneScript 值严格拒绝。正式报告 `.codex/scene-opacity-final13-20260723-1730/report.json` 为 **13/13**，五个 strict backend 合计 stages 14、chains 1、Opacity 4、failed 0、legacy blocked 2、route-only 30；全量 Scene **269 项、267 项通过、2 项跳过**。签名 CDHash 为 `6e70e547f61dcc6821009a4ebbd97af156fb5916`，executable SHA-256 为 `0cbaff3bd989e5cb9f8807236fd270feec19ca1f18548c122ca3ccb74033774a`。这只提升 exact `MASK=0` profile，不代表 MASK1、SceneScript、generic shader 或 WE pixel parity。
60. 2026-07-23 的 Scene direct dynamic text：提交 `1762743` 将 interpretation 升至 v22，把 `textinput` content、slider point-size 和 color 接入既有 binding program/per-surface snapshot；`SceneDynamicTextTextureStore` 提供 per-layer signature 去重、串行异步栅格化、stale cancellation 与 last-ready fallback。定向报告 `.codex/scene-dynamic-text-targeted-213-final-20260723-1907/report.json` 在隔离 `2134765860` 先按作者属性开启 Custom 模式，再同时更新三类文本 target，surface/window 不变；正式 `.codex/scene-dynamic-text-final13-20260723-1915/report.json` 13/13，Scene tests 273 total / 271 pass / 2 skip。SceneScript clock/date/media 和 Windows 字体像素一致仍未完成。
61. 2026-07-23 的 Scene 同帧 copy/swap command foundation：提交 `f1c6a10` 让 target plan 按作者 node 顺序编译 command，并要求 source/target 已声明、已写入、descriptor 相同；Metal runtime 以 blit copy 完整纹理，以 logical identity 映射交换实现 swap，拒绝物理 alias、condition/compose 和未知 command。定向 `.codex/scene-copy-swap-targeted-372-20260723-1945/results-pass/report.json` 为 1/1；正式 `.codex/scene-copy-swap-final13-20260723-1955/report.json` 为 13/13，Scene tests 275 total / 273 pass / 2 skip。`3723344874` 仍有 function 1/condition 4 blocker、stage/chain 0；本批不宣称 Fluid/Motion Blur 或通用 material-command graph 可执行。
62. 2026-07-23 的 Scene 当前样本全量刷新：当前真实目录 26 个样本复制到隔离 root 并以 source manifest 校验零差异；新增仓库矩阵 `script/scene_wallpaper_full_sample_matrix.json`。首轮探测发现 utility consumer 被 dependency planner 误计为可执行，以及 ready/after 截图单槽覆盖；分别由 `f73a939`、`8f144da` 修复并补回归测试。最终 `.codex/scene-history-full26-final-20260723-2332/results-pass/report.json` 为 26/26，同一 App 的 `.codex/scene-final13-after-full26-20260723/report.json` 为 13/13；两门合计 33 个唯一样本快照。Scene tests 279 total / 277 pass / 2 skip。完整聚合缺口与哈希以 [运行证据索引](../scene/semantics/runtime-evidence-index.md) 为唯一入口。

63. 2026-07-23 的 Scene persistent history target seed/clear：提交 `dcedc2e` 允许显式 `unique:true` FBO 在读前写时通过 plan，并在 target table 首次消费时编码一次透明 GPU clear；非 unique 读前写仍 `historyRequired`，clear 仅在 command buffer 成功完成后提交初始化状态，失败可重试。定向 graph/Metal 测试 14 项通过；同一签名 App 的当前真实 26 样本隔离门仍 26/26、residue 0。没有合法正向 history consumer，因此该批只证明生命周期初始化无回归，能力仍为 `L2 wired/routed`，不计为 Motion Blur/Fluid 或跨帧 logical swap 执行。
64. 2026-07-24 的 Scene material-command interleave：提交 `ebf44a9` 新增统一 `SceneGraphNodeScheduler`，按 authored nodeIndex 交错编码 material/copy/swap，并向 material encoder 传递当前 logical texture mapping。Precise Blur 只接受 `material0 -> copy/swap -> material1` 两种完整白名单 topology；late command、compose、资源不匹配和既有 blocker 继续失败关闭，其他四个 strict backend 未扩宽。定向 graph/runtime/Metal/semantics 72/72，Scene 全量 282/280/2；当前真实 26 样本门与固定 13 样本门分别 26/26、13/13。真实集合没有正向 interleave sample，`3723344874` 仍因 function/condition 保持 stage/chain 0，因此只把 Precise Blur 两种 synthetic GPU profile 记为 `L3`，generic graph 保持 `L2`。
65. 2026-07-24 的 exact Blur Precise legacy compose 归一化：提交 `4f13daf` 只接受 `KERNEL=0` 与两遍 Gaussian 的精确语法，为第一遍 `compose:true` 合成 `_rt_FullCompoBuffer1`，不把它解释成 scene background。定向 planner/GPU 34/34，Scene 全量 286/284/2；当前 26 样本和固定 13 样本分别 26/26、13/13。`2067939514` 与 `2802243144` graph blocker 分别 `32 -> 0`、`6 -> 0`，但原始样本 strict stage 均仍为 0，完整链继续被 unsupported sibling 阻断；只有关闭 `2802243144:41` 无关 Shake sibling 的隔离副本取得 precise Blur stage 1。generic compose、Refraction、scene-background capture 与任意 kernel 继续 fail closed。本批结束后暂停实现，先按原始样本可见增益复盘路线。
66. 2026-07-24 的 exact stock Shake 与未修改样本 Blur/Shake chain：提交 `e505a9e` 只接受精确 stock definition/material/raw shader fingerprint、单 material、`MASK=0/AUDIOPROCESSING=0/NOISETEXTURE=0`、RG8 flow 与 R8 phase 或 authored white fallback，并把统一 scene time 传入 ordered chain。定向 `.codex/scene-shake-20260724/targeted-contract-final/report.json` 为 2/2；`2802243144:[41,64,115]` succeeded、failed `[]`、Shake 3、chains 3、stages 6，覆盖 Blur 前后两种作者顺序；`2134765860` dynamic audio/speed 保持 Shake/stage 0。完整 26 门和固定 13 门均通过，Scene 全量 292/289/3，签名身份更新为 CDHash `0d7efcabf4b99a101a3341e64fb10d80ef8a4020`、executable SHA-256 `7cdb04292a891e7787d0fa489c8fbeb5337332e436bc0ea2de1b9827a135cbca`。该 profile 是 `L3 executed-degraded`，不是 dynamic Shake、authored shader 或 WE pixel parity。
67. 2026-07-24 的 exact stock Water Waves：提交 `31ae557` 把精确 stock definition/material/shader fingerprint、authored mask 和 direction/speed/scale/exponent/strength 放入 ordered strict backend；重复声明保留独立参数与作者顺序，非 exact profile 继续受 legacy inline 边界约束。当前完整门 `3768903841` 为 succeeded `[33,37,41,181,188,202,211,220,232]`、Water Waves 4、chains 1、stages 10。
68. 2026-07-24 的 exact stock Water Flow：提交 `94aebc5` 只接受精确 definition/material/shader/slot/constant/render-state 合同，消费 flow + phase，并仅为精确缺失的 `particle/normal_ring_smooth` 生成项目自有 64x64 R8 phase。`3028090166` 为 Water Flow 2、Opacity 1、chains 1、stages 3；`3768229922` 的六段长链因超过默认 96 MiB pool 的 6 texture-unit 保证而在规划阶段保持 stages 0。完整/固定门分别 26/26、13/13，Scene suite 301/301、3 skipped；WaifuX MP4 只作方向性视觉参考，不是 WE golden。
69. 2026-07-24 的 `particle/chromaticdot` 白色雪粒子与 Color Random 纠偏：新增精确 64x64 premultiplied 白色软点，固定门 particle 提升至 15/27、完整门提升至 52/68；`2802243144:[20,25]`、`2938612768:[173]`、`3747492842:[269]`、`3750342273:[52]` 共五层进入运行。`3750342273` 自带 preview 明确显示白色雪粒子，WaifuX 彩色点与之冲突，故不作为目标；颜色初始化改为在两个作者颜色间使用单一随机系数插值。程序纹理、密度/尺寸、色彩空间和随机分布仍无 Windows WE pixel golden。
70. 2026-07-24 的 `particle/halo_4` 校准：提交 `c06b0fb` 新增精确 64x64 premultiplied 紧凑白色光点；定向三样本门 3/3，`2131872317` 从 2/9 提升到 5/9、`2419444134` 从 3/4 提升到 4/4，`3768903841` 保持 5/5。当前 45 个可运行真实样本完整快照门 45/45，particle 为 84/131；固定门 13/13、15/27。`2131872317` 的烟花炸开仍依赖样本定义的 `eventdeath` child systems、瞬时 burst、trail/star/distort 等生命周期，日志继续明确 `childSystemsUnsupported`，本批只修复上升头部纹理，不把它写成烟花效果完成。
...
- 使用 versioned `.mywallpaperx-scene-interpretation.json` 作为稳定中间层；当前 format 为 v22。
...
- 已有 Metal textured-quad、共享白纹理程序化 solid、静态与 direct-property 动态 text texture、作者定义 2D sprite 粒子 instancing、层级 transform/visibility、基础混合、十一类 strict effect backend、Bloom、cover 相机和声明驱动的 parallax。exact stock Shake、Foliage Sway、Water Ripple、Water Waves、Water Flow 与 X-Ray 已在未修改样本执行；动态文本只在对应 direct property 变化时异步更新，作者未启用的文本层不登记为活动 consumer。这不包含 SceneScript/time/media producer 或动态 effect variants。
...
- 已有 Scene 用户属性解析、受支持 target 的默认值/override 应用、按 wallpaper 持久化、条件显示和独立属性编辑窗口；实际可执行的 `sceneTexture` key 可选择 PNG/JPEG。layer alpha、solid-only layer color、direct text content/point-size/color、exact stock Local Contrast strength 与 exact stock Opacity alpha 已通过 v22 binding program 和 per-surface snapshot 无重建更新；新 target 只有 compiler mapping 与真实 consumer 同时存在才允许 live。
- benchmark 通过签名 App 身份、隔离样本/HOME、Metal ready/after 双帧、语义字段、utility/named/image-blend/particle planned 与 finished layer ID、authored graph canonical SHA、strict backend exact count、ordered chain/stage、live identity/visible-change，以及 stop 后 surface=0 验证固定 13 与完整 45 样本；矩阵现在精确锁定 Water/Shake/Foliage/Ripple/X-Ray chain、超预算长链拒绝、dormant Opacity slot 和 11 个 built-in 粒子纹理中的真实消费子集。`3194ac5` 的 preview 并排图与 WaifuX SceneBake MP4 路径都明确非阻断。
- 代码入口：[SceneDiagnostics.swift](../../MyWallpaperX/Core/SteamWorkshopScene/Runtime/SceneDiagnostics.swift)、[SceneRenderDescriptor.swift](../../MyWallpaperX/Core/SteamWorkshopScene/Runtime/SceneRenderDescriptor.swift)、[SceneMetalRenderer.swift](../../MyWallpaperX/Core/SteamWorkshopScene/Rendering/SceneMetalRenderer.swift)、[SceneDesktopWallpaperHost.swift](../../MyWallpaperX/Core/SteamWorkshopScene/Runtime/SceneDesktopWallpaperHost.swift)。

### 5.2 用户重点样本现状

- `3724289844`：当前 authored graph succeeded `[20,28,36]`、failed/blocked `[]`、stages 4、chains 1；layer `20` 的 `Blur Precise -> Shadow` 已成为首条真实 fully-supported strict chain，layers `28/36` 保持 precise Blur singleton。Shadow count 为 1，准入不读取样本 ID；该结果只证明 exact Workshop profile 的受限 GPU 路径，不证明官方通用 Shadow 或 WE 像素等价。
- `2902406982`：image `30/30`、solid `9/9`、text `44/57`、particle `1/1`；6 个 named provider capture 与 7 个 consumer binding 成功。当前 authored graph GPU 成功层为 `[167,177,530]`：`530` 是 stock standard Blur，`167/177` 是 exact stock Local Contrast 4-pass；Local Contrast count 为 2、failed 为空，`brcontraststrength=3.0` live accepted 且不替换 surface/window。白三角和旧 coarse blur 导致的重复背景已消除，missing resource 为 0；仍有 7 个 route-only effect，以及动态内容、文字和剩余 effect 未闭合。
- `2938612768`：image `44/44`、solid `11/11`、particle `0/1`；providers `141/1340` capture、consumers `299/322` binding 和 layers `239/657/775/875/1509` 静态 image blend 均成功，中央播放器/静态封面已出现，missing resource 为 0。可见 4/5-effect mixed chains 的 strict graph count 保持 0；虽然 `31ae557` 已有 exact Water Waves backend，该样本仍因 Blend/Blend Gradient 等 unsupported 前序 stage 整链拒绝，legacy inline 也保持 0。媒体动态封面/文字、音频、粒子、named-target gap 1 和 18 个 route-only effect 尚未闭合。
...
descriptor 能识别 image、solid、particle、text、container 与 typed utility；v22 authored graph 中，十一类 strict backend 已进入 GPU，ordered strict effect-chain scheduler、typed frame registry、property authored fallback 和受限 PNG/JPEG property source 也已落地。exact stock Shake、Foliage Sway、Water Ripple、Water Waves、Water Flow 与 X-Ray 已消费真实资源、scene time 或 pointer，并按作者顺序进入 chain；同帧 copy/swap、受限 unique history seed/clear 和 Precise Blur 两种 material-command topology 继续生效。当前仍不是通用 GPU DAG executor：动态 effect、真实 history consumer、跨帧 logical swap、compose/condition/function、其他 topology 与通用 material/shader 尚未执行；system/media source、文件纹理的任意 material consumer、effectful provider、child/nested target、utility mask、复杂 blend、Texture Variants、child/world-space/其余粒子 renderer、puppet 和脚本驱动内容仍会缺失或降级。

#### shader/effect 不是兼容实现

当前 MSL effect 是对少量视觉特征的手写实现，不是 Wallpaper Engine HLSL、材质和 pass 语义的通用转换或执行；fingerprint 只用于准入。ordered strict scheduler 已保留 effect chain、输入连续性、作者顺序和逐 stage snapshot，但每新增一个独立 backend 仍重复 texture/state/target 基础。下一步应提取共享 material pass executor、stock shader registry 和 shader preprocessing IR，再让现有 strict profile 逐步复用，不能继续无限复制每个 effect 的调度代码。

#### 存在应先修复的正确性和健壮性问题

- 已移除旧的无作者声明 ripple/capability fallback 和 composition 缺省视差；precise Blur、standard-default Blur、stock Local Contrast 与 exact Workshop Shadow 严格子图已迁到 graph 选路，规划失败后不再从旧文件名路径复活。其他受限执行器仍按 visibility/mask/combo/参数限界；禁止继续扩展名称分支，待对应 graph backend 取得证据后逐项迁移。
...
- 普通 image 与 composition 都只有显式非零 `parallaxDepth` 才产生逐层位移；缺失和零值不移动。位移归一化与 delay 曲线仍待 Windows golden 校准，zoom、camera shake 和更完整 interaction 语义也未闭合；effect Shake 只有 exact 静态 stock profile，不代表这些 camera/input 语义。
...
- Scene 用户属性已有独立编辑窗口、条件显示、持久化、活动壁纸 fallback 重建，以及 `texture`/`scenetexture` 内部类型归一；静态 image-blend texture key 已支持 PNG/JPEG picker/bookmark/decode。layer alpha、纯 solid color、direct text content/point-size/color、exact Local Contrast strength 与 stock Opacity alpha 已 live；hidden/no-consumer text、SceneScript/time/media、non-solid/mixed color、Texture Variants、通用 material、transform、particle、puppet 和其他 effect uniform 仍重建或隐藏，因此不是完整属性兼容链。
- 没有按屏 pause/resume、FPS、音量、画质策略和独立 Scene 状态。
- 没有 SceneScript、系统时间/日期/媒体驱动文字、音频响应、puppet/mesh 和完整视频纹理生命周期；direct property 动态文字已完成首个 provider generation 子集。粒子现有作者包内 sprite、11 个精确 built-in 程序纹理与 sprite trail 子集，其余 visible candidate 以及 child/event 生命周期仍有缺口。
- 当前 45 样本完整快照门验证 image `449/451`、solid `140/140`、text `244/244`、particle `84/131`，named capture `8/8`、binding `9/9`；strict graph 合计 91 stage、15 chain、Water Flow 10、Water Waves 11、Shake 24、0 failed，仍有 13 个 named-target gap、113 个 route-only effect 和 22 个 legacy blocked layer。固定 13 样本门另行保护 particle `15/27`、24 stage、2 chain、Water Flow 1、Water Waves 6、Shake 1、Opacity 4 与 Workshop Shadow 1。样本 preview 优先、WaifuX MP4 只作辅助动态参考，均不设绝对阈值；没有 Windows WE 同配置录屏，不能把 45/45、13/13 或 SSIM 等同于动态/像素 parity。

## 6. Scene 演进方向

### 阶段 0：产品边界与执行门（已确定方向，进行中）

当前仍选择“可审计的兼容 Runtime”方向。Effect-definition IR、authored graph planner、ShaderContract v1、十一类 strict backend、ordered strict chain、resource identity/property fallback、file-backed `sceneTexture`、Frame Context、B0 live program、direct dynamic text、同帧 copy/swap、受限 unique history、Precise Blur interleave/legacy compose，以及 exact Shake/Foliage Sway/Water Ripple/Water Waves/Water Flow/X-Ray 已完成；`particle/halo_4` 已补齐上升光点，但烟花炸开所需 child/event 生命周期仍未执行。下一批先按公共依赖处理 `3769688830` 的 blend/composition，再进入粒子 child/event death；`3769364482` 的 Fire effect、299 多余粒子和全局比例/裁切随后按隔离样本收益排序。route-only、内部 blocker 数和跨样本视觉分数都不能单独决定优先级。
...
2. B2 ordered strict effect-chain、target allocation、pass ordering、同帧 copy/swap、受限 unique FBO history seed/clear、Precise Blur interleave、exact legacy compose、最终合成原子性、十一类 strict backend 与当前 Water/Blur/X-Ray 真实 chain 已完成；generic compose 和真实 history consumer 未完成。下一步先验证新增样本暴露的公共 composition/blend 和 Fire topology，再决定共享 material pass 执行基础的最小切入点。
...
- 十一类 strict backend、ordered strict chain、Bloom、程序化 solid、静态与 direct-property 动态 text、camera cover/显式 parallax、受限水波/合成、pointer-driven X-Ray、built-in 粒子纹理/sprite trail、utility/named target、静态 image blend、file-backed `sceneTexture`、Frame Context、B0 live 主链、受限 unique history、Precise Blur interleave/legacy compose 与 exact stock Shake/Foliage Sway/Water Ripple/Water Waves/Water Flow 已落地；v22 记录 direct text 合同。非阻断 preview 和 SceneBake MP4 方向性门已建立。下一批以 [覆盖台账](../scene/semantics/coverage-ledger.md) 和 [Scene 播放能力开发计划](../scene/scene-capability-development-plan-2026-07-22.md) 的新增样本与公共能力顺序为准。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有签名 App、45 个当前可运行真实目录隔离样本加 13 个固定回归样本、Metal 双帧/语义/dependency/strict graph/释放门，以及 preview/SceneBake 同样本方向性证据；真实目录中的 `3770500543` 因缺少 package 未进入运行矩阵。v22 已闭合 direct text 动态 provider；B2 已增加同帧 copy/swap、受限 unique history、Precise Blur interleave/legacy compose 和 exact stock Shake/Foliage Sway/Water Ripple/Water Waves/Water Flow/X-Ray。十一类 strict backend、当前完整门 15 条 chain、固定门 2 条 chain、静态 blend、direct text、11 个 built-in 粒子纹理和 ShaderContract source 保留仍不等价于 authored shader execution、真实 history consumer、SceneScript/media、粒子 child/event 生命周期或完整属性系统。当前可以声明“受限能力可用、精确 profile 进入真实 ordered GPU chain、三个 X-Ray 样本在桌面路径可交互、同样本视觉变化可审查”，不能声明达到或接近 WE parity。后续先处理新增样本暴露的公共 composition/blend，再处理烟花所需粒子 child/event death；Fire effect、299 多余粒子和比例问题随后按隔离样本收益推进。
