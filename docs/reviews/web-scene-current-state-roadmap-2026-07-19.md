...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v22：继承 v21 exact stock Opacity、ShaderContract、provider metadata 与 authored graph，并保存 direct text content/point-size/color binding program。当前实现基线 `1762743` 的 renderer 仍消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow 与 exact stock Opacity 五个严格注册 backend；同时动态文本按 per-surface snapshot 只重栅格化变化层，重复值去重，stale/failed generation 不覆盖 last-ready texture。隐藏文本、conditional binding、静态 `text` label、SceneScript/time/media 继续 fail closed 或整场重建。target pool、typed texture registry 与 file-backed `sceneTexture` 仍保持受限边界；这不是任意 graph、material、shader 或完整 SceneScript/media executor。

当前正式语义矩阵 `.codex/scene-dynamic-text-final13-20260723-1915/report.json` 为 **13/13 通过**，矩阵 SHA-256 为 `fbb252a64018cf785e20b2200db5966a300e9351a994b4a36f1fe9565c5b354c`。五个 strict backend 指标保持 14 stage、1 条真实 chain、Opacity 4、Local Contrast 2、Workshop Shadow 1、0 failed、2 个 legacy blocked、30 个 route-only effect。动态文字定向报告 `.codex/scene-dynamic-text-targeted-213-final-20260723-1907/report.json` 在作者 Custom 模式下同时更新 `customtext/textcolor/textsize`，surface/window identity 不变、changed ratio `0.003541478640192539`、text `6/6`。当前 Scene 全量测试共 **273 项，其中 271 项通过、2 项按环境跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `25f1e84ab81ee23d270ea1e436de0e47ccd34489`、可执行文件 SHA-256 `85634666c237a9026ac89b04bc33815963f24fd77b339c1520f6cb20ae29f5d6`。13/13 仍不能解释为 WE 视觉一致；下一主线是 B2 copy/swap/compose/history，并把文本 provider lifecycle 推广到 system/media/video。
...
当前选择“可审计的兼容 Runtime”方向。Effect-definition IR、authored graph planner、ShaderContract v1、五个 strict backend、ordered strict chain、resource identity/property fallback、file-backed `sceneTexture`、Frame Context、B0 live program，以及 direct dynamic text 的首个 generation/stale-cancellation consumer 已完成。下一主线推进 B2 copy/swap/compose/history；B1 只把已经验证的文本 lifecycle 抽到 system/media/video 共用合同。Timeline、SceneScript、audio/media 以后横向接入同一 target/frame 合同；route-only 只作布局诊断。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有签名 App、13 个隔离样本、Metal 双帧/语义/dependency/strict graph/释放门。v22 已把 direct text 三类属性接入 B0 snapshot，并以异步 per-layer generation/stale cancellation 闭合首个动态 provider；五个 strict backend、1 条真实 chain、静态 blend、direct text 和 ShaderContract source 保留仍不等价于 authored shader execution、SceneScript/media 或完整属性系统。当前可以声明“受限能力可用、direct dynamic text property 可无重建更新”，不能声明达到或接近 WE parity。下一步是 copy/swap/compose/history 和通用 provider lifecycle，再接 Timeline/SceneScript、audio/media、Particle 与更多 backend。
