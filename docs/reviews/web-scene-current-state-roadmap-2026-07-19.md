...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v19：v15 保真保存 EffectDefinition，v16 把可见 layer/effect 编译为 authored graph，v17 加入 provider/resource metadata，v18 持久化 property binding program 与 effective values，v19 再加入 loss-preserving `ShaderContract`，不改变既有 graph identity。`8474ace` 已安全保留 13 个隔离样本中 173 份 shader contract（143 authored、30 host built-in）、286 个 stage、155 个 include、1523 个 annotation 和 2660 个 declaration，原始 source/hash 与 canonical identity 均进入合同，诊断为 0；这只是 L1 识别/保留，不包含 include expansion、预处理、翻译、编译、uniform upload 或任意 GPU 执行。renderer 现已消费两个严格注册的 Blur 子集；`73f415b` 已把 precise/standard strict Blur 迁入 effect-instance logical target/lifetime plan 与不可变 Metal target table，`228cdde` 又让 table 保真分配 `rgba8888 -> .rgba8Unorm`，同时保持 input/output 与 `rgba_backbuffer -> .bgra8Unorm`。target table 按完整 effect identity、plan、extent 与 format 复用，跨 effect 隔离，resize/format 变化采用候选成功后再替换的事务；96 MiB 只表示事务提交后的 cache residency 上限，不是瞬时 VRAM 硬上限。该格式基础尚未接入 Local Contrast pipeline，不新增可见执行层。typed texture registry 也已支持 layer/named/property/system identity、provider 状态、静态 resource generation、named frame epoch 和属性缺失时的 authored fallback。文件型 `sceneTexture` 的首个产品切片已经接通 PNG/JPEG 选择、按壁纸 bookmark、限时 security scope、逐屏 Metal 解码上传和失败回退，但只开放给当前实际可执行的静态 image-blend property consumer。其他 current-prefix、named dependency、静态 blend 和手写 effect 仍是 bounded GPU 路径。这不是任意 graph、material、shader 或完整 sceneTexture/media executor。

当前正式语义矩阵 `.codex/scene-shader-contract-final13-v2-20260723/report.json` 为 **13/13 通过**；`228cdde` 没有新增可见执行层，因此不制造无信息视觉报告。definition/graph 结构基线仍为 106 definitions、182 passes、179 material passes、50 FBO，以及 197 layer plans、300 effects、411 nodes、76 RT；5 个 blocker 仍集中在 `3723344874` fluid layer。`73f415b` 后的正向 graph-target 报告 `.codex/scene-graph-target-positive-final-20260723/report.json` 为 **3/3**，GPU 成功执行 `2902406982:[530]`、`3724289844:[28,36]` 与 `3765760121:[68,76,82]`，失败为 0；负向报告 `.codex/scene-graph-target-negative-final-20260723/report.json` 也为 **3/3**，history shape 未误执行，layer `20/348` 保持 fail closed，`3723344874` 的 blocker 仍为 `unsupportedFunctions:1`、`unsupportedCondition:4`。`2938612768` 的 static image blend 保持 **5/5**，并已通过双代修复避免静态 provider 每帧误失效；`2902406982` 的 named target 仍按帧换代。宿主级 `SceneFrameTiming` / `SceneFrameContext` 已统一所有屏幕的 frame index、host/scene/wall time；B0 live-property 主链也已完成 v19 继承的 v18 binding program、per-surface transaction、atomic state、Host/Service/UI 路由，以及 layer alpha 与 solid-only layer color consumer。alpha 的 `2902406982:newproperty11`、`2938612768:newproperty17` 和 color 的 `3122339805:accentcolourdefault800080`、`2902406982:newproperty33` 均 live accepted，更新前后 surface/window identity 不变。当前 Scene 全量测试为 **225 项、224 项通过、1 项跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `6fc75d86b05d665138f8d7080cdf7e149522a10e`、可执行文件 SHA-256 `d237711dbd6f92628303431db10c5ecf8b8fdf0088789e1118b98048d518410d`。pause/resume、delta clamp、fixed timestep、generic scheduler、history/copy/swap/compose/condition/function，以及 ShaderContract 的预处理与执行仍未完成。后续采用 coverage-first：下一项实现 stock Local Contrast 的严格默认单效果 profile，以 `2902406982` layers `167/177` 为正向门；`2938612768` 已核验的可见 mixed-effect Local Contrast 继续 fail closed。逐项等级与缺口见 [Scene 官方语义与实现覆盖台账](../scene/semantics/coverage-ledger.md)，执行顺序见 [Scene 播放能力开发计划](../scene/scene-capability-development-plan-2026-07-22.md)。
...
- 已有 Metal textured-quad、共享白纹理程序化 solid、静态 text texture、作者定义 2D sprite 粒子 instancing、层级 transform/visibility、基础混合、legacy coarse/precise Gaussian blur、standard Blur 默认 4-pass quarter-RT、Bloom threshold/blur/composite、cover 相机和声明驱动的 parallax。legacy coarse blur 仍是全尺寸 RT + 4 倍 texel step 近似，只服务尚未迁移路径；严格 default standard graph 已使用真实 quarter RT。单层 built-in UV foliage 读取作者参数；粒子可解析包内纹理和 9 个精确 built-in key 的程序化纹理，`spritetrail` 按速度方向和作者 stretch 参数拉伸。
...
- 已有 Scene 用户属性解析、受支持 target 的默认值/override 应用、按 wallpaper 持久化、条件显示和独立属性编辑窗口；实际可执行的 `sceneTexture` key 可选择 PNG/JPEG，bookmark 授权只覆盖同步解码上传，UI 不显示当前 renderer 无法执行的属性 target。layer alpha 已通过 v19 继承的 v18 binding program 和 per-surface snapshot 无重建更新；新 target 只有 compiler mapping 与真实 consumer 同时存在才允许 live。
- benchmark 通过签名 App 身份、隔离样本/HOME、Metal ready/after 双帧、语义字段、utility/named/image-blend/particle planned 与 finished layer ID、authored graph canonical SHA、precise/standard backend exact succeeded/failed/legacy-blocked layer ID，以及 stop 后 surface=0 验证当前 13 样本；最新门另锁定 293 的五个 property blend layer，定向注入证明两张用户文件纹理进入同一 registry/renderer 链。
...
- `2902406982`：image `30/30`、solid `9/9`、text `44/57`、particle `0/1`；6 个 named provider capture 与 7 个 consumer binding 成功，layer `530` 的 stock standard Blur 默认图已走真实 4-pass quarter-RT，白三角和旧 coarse blur 导致的重复背景已消除。missing resource 为 0；本轮截图与上一 standard-Blur 报告相同，仍有 9 个 route-only effect，以及动态内容、文字和剩余 effect 未闭合。
- `2938612768`：image `44/44`、solid `11/11`、text `8/24`、particle `0/1`；providers `141/1340` capture、consumers `299/322` binding 和 layers `239/657/775/875/1509` 静态 image blend 均成功，中央播放器/静态封面已出现，missing resource 为 0。隔离注入已证明 `newproperty25/26` 的实际 PNG 可替换对应静态 provider；它不等于系统媒体封面或 current/previous thumbnail 时序。仍有 named-target gap 1、18 个 route-only effect；authored waterwaves 当前由 legacy 算法近似，UV/幅度/采样/局部作用严重失真，导致全屏夸张扭曲。媒体动态封面/文字、音频和粒子尚未闭合。
...
descriptor 能识别 image、solid、particle、text、container 与 typed utility；v17 继承的 v16 graph 中，严格 precise 与 standard-default Blur 子集已进入 GPU，typed frame registry、property authored fallback 和受限 PNG/JPEG property source 也已落地，但其余节点仍由 bounded executor 处理或保持未支持。当前不是通用 GPU DAG executor：system/media source、文件纹理的任意 material consumer、effectful provider、child/nested target、utility mask、复杂 blend、Texture Variants、child/world-space/其余粒子 renderer、puppet 和脚本驱动内容仍会缺失或降级。

#### shader/effect 不是兼容实现

当前 MSL effect 是对少量视觉特征的手写近似，不是 Wallpaper Engine HLSL、材质和 pass 语义的转换或执行；utility、foliage 和 blend 也只覆盖经过正反样本约束的子集。下一步必须保留作者 pass 顺序、输入纹理和 live uniform 的合同，不能继续把多个 effect 压成一个按名称选取的近似分支。

#### 存在应先修复的正确性和健壮性问题

- 已移除旧的无作者声明 ripple/capability fallback 和 composition 缺省视差；precise 与 standard-default Blur 严格子图已迁到 graph 选路，规划失败后不再从旧文件名路径复活。其他受限执行器仍按 visibility/mask/combo/参数限界；禁止继续扩展名称分支，待对应 graph backend 取得证据后逐项迁移。
- legacy coarse blur 虽已按作者像素单位并消除早期重复背景，但仍是全尺寸 RT + 4 倍 texel step 近似，只能留在尚未迁移的受限路径。stock standard Blur 默认图已使用真实 4-pass/quarter-RT；非默认 kernel/composite/mask/alpha 变体继续 fail closed。单层 built-in UV foliage 已服从作者 mask/参数，多 stack 和 vertex/workshop 变体继续 fail closed。
...
- Scene 用户属性已有独立编辑窗口、条件显示、持久化、活动壁纸 fallback 重建，以及 `texture`/`scenetexture` 内部类型归一；当前可执行的静态 image-blend texture key 已支持 PNG/JPEG picker、bookmark、解码上传、真实 override 和 authored fallback。layer alpha 与纯 solid layer color 已 live；non-solid/mixed color、Texture Variants、视频、system/media、通用 material consumer、transform、particle、puppet 和未支持 effect uniform 等 target 仍隐藏或整场重建，因此不是完整属性兼容链。
...
- 已有 13 个真实 Scene 样本的语义自动门，当前验证 image `180/181`、solid `54/54`、text `79/108`、particle `14/27`，named capture `8/8`、binding `9/9`、static image blend `5/5` 和 strict authored Blur graph `6/6`，并记录成功 6 层、失败 0、legacy blocked 3 层；但本轮仍观测到 37 个 route-only effect、1 个 named-target gap，没有完整性能预算，也没有与 Windows Wallpaper Engine 同配置录屏的图像差异门，因此不能把 13/13 等同于完整兼容或 WE parity。

## 6. Scene 演进方向

### 阶段 0：产品边界与执行门（已确定方向，进行中）

当前选择“可审计的兼容 Runtime”方向：不承诺私有格式 100% 复刻，但按官方行为合同、真实样本频率和 [Scene 语义手册](../scene/semantics/README.md) 推进。Effect-definition IR、authored graph planner、ShaderContract v1、`rgba8888` target format、两个严格 Blur 子图、resource identity/property authored-fallback、file-backed `sceneTexture`、统一 Frame Context 第一阶段、首批 built-in 粒子纹理，以及 B0 binding program/per-surface snapshot/atomic layer-alpha live consumer 已完成。下一切片是 stock Local Contrast 的严格默认单效果 profile，随后继续 B1 Provider Core 与 B2 generic scheduler；Timeline、SceneScript、动态文字、audio/media 以后横向接入同一 target/frame 合同。既有 bounded executor 在新路径取得 GPU/golden 等价证据前保留，但不再扩展名称分支。完整等级以 [覆盖台账](../scene/semantics/coverage-ledger.md) 为准。

### 阶段 1：正确性、安全和性能基础

1. B0 property 主链已建立 typed DynamicValue/target registry、v19 继承的 v18 binding program、per-surface snapshot、原子 live state 与 alpha consumer。新增 live target 必须同时注册 compiler target、真实 consumer、fallback 与 identity 门；dynamic text 和 visibility 不得仅靠现有 alpha 路径取消整场重建。
...
5. 在上述视觉主链之外继续补 PKG header/entry/解包边界、索引与纹理复用、按屏刷新率/FPS 和性能预算。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有“签名 App + 13 个隔离真实样本 + Metal 双帧/语义/dependency/authored-Blur GPU/释放门”的可持续开发链。v19 在 v18 binding program/effective values 上加入 loss-preserving ShaderContract；B0 已把 property producer、per-surface snapshot、atomic routing 和 layer-alpha consumer 合龙。strict precise、standard-default、file-backed `sceneTexture` 静态 consumer 子集和统一 Frame Context 第一阶段已执行，实际画面仍主要来自 bounded renderer；固定 precise kernel、受限 standard profile、5/5 静态 blend、alpha live 子集和 ShaderContract source 保留都不等价于 authored shader execution、动态 sceneTexture 或完整属性系统。当前可以声明“受限能力可用、官方能力归属和逐项缺口可查、通用图与 shader source 合同已建立、首个文件型 property provider 与 layer-alpha live 主链已闭环”，不能声明达到或接近 Wallpaper Engine parity。下一步先以 `2902406982` layers `167/177` 闭合 stock Local Contrast 严格 profile，再按 coverage-first 进入 B1 Provider Core 与 B2 generic scheduler，随后接 B3/B4 的 Timeline/SceneScript、动态 text、audio/media、Particle 与更广 backend；增加样本硬编码、默认套用效果或把 compiler-only target 写成 live，仍不会形成兼容闭环。
