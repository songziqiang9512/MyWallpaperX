...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v22：继承 v21 exact stock Opacity、ShaderContract、provider metadata 与 authored graph，并保存 direct text content/point-size/color binding program。当前实现基线 `dcedc2e` 的 renderer 仍消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow 与 exact stock Opacity 五个严格注册 backend；同帧 graph command foundation 已按作者顺序计划 `copy/swap`，并以 Metal blit/逻辑身份交换执行；显式 `unique:true` FBO 已在读前写时进入受限 history seed/clear 生命周期，但两者尚未接进通用 material scheduler，也没有让真实 Fluid/Motion Blur 进入 strict stage。dependency planner 现拒绝不可绘制 utility consumer，调试证据截图改为队列，避免同帧后续请求覆盖 ready 帧。动态文本仍按 per-surface snapshot 只重栅格化变化层。真实 history consumer、compose、condition/function、SceneScript/time/media 继续 fail closed 或整场重建；这不是任意 graph、material、shader 或完整 SceneScript/media executor。

当前运行证据分两层：`.codex/scene-history-full26-final-20260723-2332/results-pass/report.json` 对当前真实目录的 26 个隔离样本 **26/26 通过**，同一签名 App 随后运行 `.codex/scene-final13-after-full26-20260723/report.json`，固定回归门 **13/13 通过**；两门有 6 个重叠、合计 33 个唯一样本快照。当前 26 门验证 image `330/330`、text `212/212`、solid `126/126`、particle `47/68`，named capture `8/8`、binding `9/9`、strict stage 24、failed 0；仍有 13 个 named-target gap、67 个 route-only effect、19 个 legacy blocked layer、9 个缺失资源和 38 个 graph blocker。该门没有真实 multi-effect chain 或 Workshop Shadow，因此固定 13 门继续保护 14 stage、1 条真实 chain、Opacity 4、Workshop Shadow 1、failed 0。当前 Scene 全量测试共 **279 项，其中 277 项通过、2 项按环境跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `557e57e3c6f55ee73a147787f84251e099092256`、可执行文件 SHA-256 `337327019eff96866a2bb493ff1cf379dfaee14f849a4fb6dcf0e7b4b3415193`。两门均不能解释为 WE 视觉一致；下一主线不变。
...
descriptor 能识别 image、solid、particle、text、container 与 typed utility；v22 authored graph 中，五个 strict backend 已进入 GPU，ordered strict effect-chain scheduler、typed frame registry、property authored fallback 和受限 PNG/JPEG property source 也已落地。同帧 copy/swap 已有独立、严格的 plan/runtime foundation，显式 unique FBO 也有受限 history seed/clear 生命周期，但尚未与任意 material node 交错调度。当前仍不是通用 GPU DAG executor：真实 history consumer、跨帧 logical swap、compose/condition/function 与通用 material/shader 尚未执行；system/media source、文件纹理的任意 material consumer、effectful provider、child/nested target、utility mask、复杂 blend、Texture Variants、child/world-space/其余粒子 renderer、puppet 和脚本驱动内容仍会缺失或降级。
...
当前选择“可审计的兼容 Runtime”方向。Effect-definition IR、authored graph planner、ShaderContract v1、五个 strict backend、ordered strict chain、resource identity/property fallback、file-backed `sceneTexture`、Frame Context、B0 live program、direct dynamic text、同帧 copy/swap command foundation，以及受限 unique FBO history seed/clear 已完成。下一主线推进 B2 material-command interleave、真实 history consumer 与 compose；B1 只把已经验证的文本 lifecycle 抽到 system/media/video 共用合同。Timeline、SceneScript、audio/media 以后横向接入同一 target/frame 合同；route-only 只作布局诊断。
...
2. B2 ordered strict effect-chain、target allocation、pass ordering、同帧 copy/swap command foundation、受限 unique FBO history seed/clear、最终合成原子性、首条真实 chain 与 stock Opacity `MASK=0` live alpha 已完成；下一步在同一合同上扩 material-command interleave、真实 history consumer 与 compose，不能复制各 profile 的调度代码。
...
- 五个 strict backend、ordered strict chain、Bloom、程序化 solid、静态与 direct-property 动态 text、camera cover/显式 parallax、受限水波/合成、单层 built-in UV foliage、9 个 built-in 粒子纹理/sprite trail、utility/named target、静态 image blend、file-backed `sceneTexture`、Frame Context、B0 live 主链，以及显式 unique FBO 的受限 history seed/clear 已落地；v22 记录 direct text 合同。下一步以 [覆盖台账](../scene/semantics/coverage-ledger.md) 和 [Scene 播放能力开发计划](../scene/scene-capability-development-plan-2026-07-22.md) 为准推进 material-command interleave、真实 history consumer、compose 与通用 provider lifecycle。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有签名 App、26 个当前真实目录隔离样本加 13 个固定回归样本、Metal 双帧/语义/dependency/strict graph/释放门。v22 已把 direct text 三类属性接入 B0 snapshot，并以异步 per-layer generation/stale cancellation 闭合首个动态 provider；B2 已增加严格的同帧 copy/swap plan/runtime foundation，并为显式 unique FBO 建立受限 history seed/clear 生命周期。五个 strict backend、1 条真实 chain、静态 blend、direct text、copy/swap foundation、history seed/clear 和 ShaderContract source 保留仍不等价于 authored shader execution、真实 history consumer、SceneScript/media 或完整属性系统。当前可以声明“受限能力可用、direct dynamic text property 可无重建更新、同帧 graph command 与显式 history seed 有自动门”，不能声明达到或接近 WE parity。下一步是 material-command interleave、真实 history consumer、compose 和通用 provider lifecycle，再接 Timeline/SceneScript、audio/media、Particle 与更多 backend。
