...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v18：v15 保真保存 EffectDefinition，v16 把可见 layer/effect 编译为 authored graph，v17 加入 provider/resource metadata，v18 再持久化 property binding program 与 effective values，不改变既有 graph identity。renderer 现已消费两个严格注册的 Blur 子集；`38e238d` 另建立了 effect-instance logical target/lifetime plan 与预算原子的不可变 Metal target table，但尚未把 Blur 从旧尺寸池迁入该表，因此本阶段只算 B2 `L2` 基础设施。typed texture registry 也已支持 layer/named/property/system identity、provider 状态、静态 resource generation、named frame epoch 和属性缺失时的 authored fallback。文件型 `sceneTexture` 的首个产品切片已经接通 PNG/JPEG 选择、按壁纸 bookmark、限时 security scope、逐屏 Metal 解码上传和失败回退，但只开放给当前实际可执行的静态 image-blend property consumer。其他 current-prefix、named dependency、静态 blend 和手写 effect 仍是 bounded GPU 路径。这不是任意 graph、material、shader 或完整 sceneTexture/media executor。

当前正式语义矩阵为 **13/13 通过**。definition/graph 结构基线仍为 106 definitions、182 passes、179 material passes、50 FBO，以及 197 layer plans、300 effects、411 nodes、76 RT；5 个 blocker 仍集中在 `3723344874` fluid layer。graph GPU 门成功执行 `2902406982:[530]`、`3724289844:[28,36]` 与 `3765760121:[68,76,82]`，失败为 0；不满足严格合同的 layers `20/348/358` 已拒绝并阻断 legacy blur fallback。`2938612768` 的 static image blend 保持 **5/5**，并已通过双代修复避免静态 provider 每帧误失效；`2902406982` 的 named target 仍按帧换代。宿主级 `SceneFrameTiming` / `SceneFrameContext` 已统一所有屏幕的 frame index、host/scene/wall time；B0 live-property 主链也已完成 binding program、format 18、per-surface transaction、atomic state、Host/Service/UI 路由，以及 layer alpha 与 solid-only layer color consumer。alpha 的 `2902406982:newproperty11`、`2938612768:newproperty17` 和 color 的 `3122339805:accentcolourdefault800080`、`2902406982:newproperty33` 均 live accepted，更新前后 surface/window identity 不变。当前 color census 的 73 条指令中 25 条属于纯 solid key 并可 live；`3122339805:basecolor` 的 48 条 solid 指令因同键还含未支持目标，必须整键重建。particle、container、non-solid color、mixed、unsupported 或无 consumer 的 key 同样整场重建。当前 Scene 全量测试为 211 项、210 项通过、1 项跳过。pause/resume、delta clamp 和 fixed timestep 仍未完成。后续采用 coverage-first：先把 strict Blur 迁入新的 effect target table 并补 resize/reset/stop 运行门，再继续 B1 Provider Core 与 B2 Graph Resource Runtime；逐项等级与缺口见 [Scene 官方语义与实现覆盖台账](../scene/semantics/coverage-ledger.md)，执行顺序见 [Scene 播放能力开发计划](../scene/scene-capability-development-plan-2026-07-22.md)。
