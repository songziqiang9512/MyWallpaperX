...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v25；实现基线 `f02f41d` 的 renderer 消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow、exact stock Opacity、Shake、Water Waves、Water Flow、Foliage Sway、Water Ripple 与 X-Ray 十一类严格注册 backend，并生成 14 个精确登记的项目自有 built-in 粒子纹理，其中 `particle/fire/fire1` 是低能量程序化火焰遮罩。Puppet 已有 bind-pose mesh、静态 MDAT attachment，以及只接受 MDLV0023/MDLS0004/MDLA0006、单个静态可见 loop clip、non-additive、blend/rate=1 的 source-FPS 离散 CPU LBS 子集；粒子已有 strict depth-one eventspawn/natural-eventdeath child、Sprite Trail 和 1,024/system 预算，`2131872317` 延迟 App 门可见烟花。BC1/2/3 颜色纹理在预算内走 CPU premultiply，超预算或多 image 载荷走 GPU premultiply；`3768903841` 的跨 image sprite 当前只裁 authored 首帧作静态 fallback，完整 5-image/140-frame 动画未实现。其他 profile 保持 fail closed。动态 effect profile 按完整 definition/material/shader/resource fingerprint 准入，并在同一 ordered scheduler 保持作者顺序；X-Ray 另有明确受限的前缀执行边界。默认 96 MiB pool 只保证 6 个全尺寸 BGRA texture unit，超预算链在规划阶段整链拒绝。generic material pass、authored shader preprocessing/translation、真实 history consumer、generic compose/scene-background、condition/function、SceneScript/time/media、Puppet 插值/mixing/动画 attachment follow/constraints，以及 static/follow/collision/delete/nested child 与 event transform/inheritance 仍未实现。

当前运行证据分两层：`.codex/scene-fire1-full45-20260725/report.json` 对当前真实目录中 45 个可运行隔离样本 **45/45 通过**；`3770500543` 因源目录缺少 `scene.pkg` 而明确排除。`.codex/scene-fire1-fixed13-20260725/report.json` 固定回归门 **13/13 通过**。45 门验证 image `449/451`、text `244/244`、solid `140/140`、particle `86/131`，strict stage 91、chain 15、Water Flow 10、Water Waves 11、Shake 24、failed 0；固定门保护 particle `18/27`、24 stage、2 chain、Water Flow 1、Water Waves 6、Shake 1、Opacity 4、Workshop Shadow 1、failed 0。完整 Scene suite 为 **372 项：369 通过、3 项跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `8400b69a9dfb69aee168f6a8f57200a3da2ac94e`、可执行文件 SHA-256 `c20abc4f14ef80d7291b6ccb98ed3bc28d3db65e8d6859f4f5f17ad5eec91dd6`；13/45 个样本均为 interpretation v25、退出 surface 归零、sample root residue 0。样本自带 preview 是当前第一视觉依据（证据基线 `3194ac5`）；WaifuX SceneBake MP4 只作辅助动态参考，二者均不能解释为 WE 视觉一致。
...
`<isolated-scene-sample-root>` 必须包含测试副本 `Scene/<id>`；不得直接传入真实 `~/Movies/MyWallpaperX/创意工坊/Scene`。当前完整快照门为 `.codex/scene-fire1-full45-20260725/report.json`，当前固定回归门为 `.codex/scene-fire1-fixed13-20260725/report.json`，Fire 定向门为 `.codex/scene-fire1-targeted-v2-20260725/report.json`，BC 定向门为 `.codex/scene-bc-targeted-20260725/report.json`，Puppet animation 定向门为 `.codex/scene-puppet-animation-20260725/targeted-v1/report.json`；Puppet attachment、`particle/halo_4`、X-Ray 和视觉校准报告只保留为对应阶段证据，不能反向覆盖当前结论。
...
- **执行子集是受限白名单，不是通用引擎**：十一类 strict effect backend、puppet bind-pose、14 个程序化 built-in 粒子纹理和受限 provider/dependency 链都按完整 fingerprint 准入、fail closed；generic material/shader executor、真实 history consumer、generic compose、condition/function 未实现。
...
- **动态系统仍不完整**：Timeline 无 IR、SceneScript 无运行时、音频/媒体桥未接；Puppet 已有严格单 clip MDLA/full-TRS/CPU LBS，但没有插值、mixing、动态 attachment follow、constraint/IK/physics。粒子只有 strict natural-eventdeath 子集，static/follow/collision/delete/nested child、动态文字 producer 和大量 effect 时间语义仍决定许多样本的核心动态差距。
...
当前仍选择"可审计的兼容 Runtime"方向。Effect-definition IR、authored graph planner、ShaderContract v1、十一类 strict backend、ordered strict chain、resource identity/property fallback、file-backed `sceneTexture`、Frame Context、B0 live program、direct dynamic text、同帧 copy/swap、受限 unique history、Precise Blur interleave/legacy compose、exact Shake/Foliage Sway/Water Ripple/Water Waves/Water Flow/X-Ray、Puppet 严格单 clip LBS、strict depth-one eventspawn/natural-eventdeath child、`rosepetals`/`beam_1`/`particle/fire/fire1` 高频 built-in，以及 `3769688830`/`3768903841` 暴露的公共合成缺口（puppet bind-pose、静态 MDAT attachment、预算内 CPU 与超预算/多 image GPU BC premultiply、静态首帧 fallback、REFRACT fail-closed）已完成。当前批次优先级以 [开发计划的当前批次优先级](../scene/scene-capability-development-plan-2026-07-22.md) 为准：下一步定位 `2998757800` 的多余/缺失粒子与全局比例/裁切。route-only、内部 blocker 数和跨样本视觉分数都不能单独决定优先级。
...
2. B2 ordered strict effect-chain、target allocation、pass ordering、同帧 copy/swap、受限 unique FBO history seed/clear、Precise Blur interleave、exact legacy compose、最终合成原子性、十一类 strict backend 与当前 Water/Blur/X-Ray 真实 chain 已完成；generic compose 和真实 history consumer 未完成。下一步先从 `2998757800` 区分粒子资源/child、坐标空间、camera cover 与 layer transform 根因，再决定共享 composition 或 particle space 合同的最小切入点。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有签名 App、45 个当前可运行真实目录隔离样本加 13 个固定回归样本、Metal 双帧/语义/dependency/strict graph/释放门，以及 preview/SceneBake 同样本方向性证据；真实目录中的 `3770500543` 因缺少 package 未进入运行矩阵。v25 已闭合 direct text 动态 provider、Puppet bind-pose/静态 MDAT/严格单 clip LBS，以及 strict depth-one eventspawn/natural-eventdeath child；B2 已有同帧 copy/swap、受限 unique history、Precise Blur interleave/legacy compose 和 exact stock Shake/Foliage Sway/Water Ripple/Water Waves/Water Flow/X-Ray；`3769688830` 的主体构图、arm/shoulder 与 orb 静态挂点已恢复，BC 白 matte 与折射粒子白块假阳性已消除，受限樱花/光束/火焰粒子也已进入 runtime；`3768903841` 的五层 5-image BC3 不再绘制整张 4x7 atlas 和 straight-alpha matte，而是显示各自 `1920x1080` authored 首帧。该资源的 140 帧跨-image 动画仍未实现。十一类 strict backend、当前完整门 15 条 chain、固定门 2 条 chain、静态 blend、direct text、14 个 built-in 粒子纹理和 ShaderContract source 保留仍不等价于 authored shader execution、真实 history consumer、SceneScript/media、完整 Puppet、完整 child/event 或属性系统。当前可以声明"受限能力可用、精确 profile 进入真实 ordered GPU chain、Puppet 严格子集可播放、`2131872317` natural-eventdeath 烟花在延迟桌面门可见、三个 X-Ray 样本在桌面路径可交互、同样本视觉变化可审查"，不能声明达到或接近 WE parity。后续批次以开发计划的当前批次优先级为准：下一步定位 `2998757800` 的多余/缺失粒子与全局比例/裁切。
