...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v25；实现基线 `18d0056` 的 renderer 消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow、exact stock Opacity、Shake、Water Waves、Water Flow、Foliage Sway、Water Ripple 与 X-Ray 十一类严格注册 backend，并生成 13 个精确登记的项目自有 built-in 粒子纹理。Puppet 已有 bind-pose mesh、静态 MDAT attachment，以及只接受 MDLV0023/MDLS0004/MDLA0006、单个静态可见 loop clip、non-additive、blend/rate=1 的 source-FPS 离散 CPU LBS 子集；粒子已有 strict depth-one eventspawn/natural-eventdeath child、Sprite Trail 和 1,024/system 预算，`2131872317` 延迟 App 门可见烟花。BC1/2/3 颜色纹理在预算内走 CPU premultiply，超预算或多 image 载荷走 GPU premultiply；`3768903841` 的跨 image sprite 当前只裁 authored 首帧作静态 fallback，完整 5-image/140-frame 动画未实现。其他 profile 保持 fail closed。动态 effect profile 按完整 definition/material/shader/resource fingerprint 准入，并在同一 ordered scheduler 保持作者顺序；X-Ray 另有明确受限的前缀执行边界。默认 96 MiB pool 只保证 6 个全尺寸 BGRA texture unit，超预算链在规划阶段整链拒绝。generic material pass、authored shader preprocessing/translation、真实 history consumer、generic compose/scene-background、condition/function、SceneScript/time/media、Puppet 插值/mixing/动画 attachment follow/constraints，以及 static/follow/collision/delete/nested child 与 event transform/inheritance 仍未实现。

当前运行证据分两层：`.codex/scene-bc-full45-20260725/report.json` 对当前真实目录中 45 个可运行隔离样本 **45/45 通过**；`3770500543` 因源目录缺少 `scene.pkg` 而明确排除。`.codex/scene-bc-fixed13-20260725/report.json` 固定回归门 **13/13 通过**。45 门验证 image `449/451`、text `244/244`、solid `140/140`、particle `83/131`，strict stage 91、chain 15、Water Flow 10、Water Waves 11、Shake 24、failed 0；固定门保护 particle `18/27`、24 stage、2 chain、Water Flow 1、Water Waves 6、Shake 1、Opacity 4、Workshop Shadow 1、failed 0。完整 Scene suite 为 **372 项：369 通过、3 项跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `d14c652a6fedcd979a8aba61fa3a91f9906f67af`、可执行文件 SHA-256 `42257016bb831ebd6af6b93896af2c00a992a5c9805e230d02b234d390914c6d`；13/45 个样本均为 interpretation v25、退出 surface 归零、sample root residue 0。样本自带 preview 是当前第一视觉依据（证据基线 `3194ac5`）；WaifuX SceneBake MP4 只作辅助动态参考，二者均不能解释为 WE 视觉一致。
...
`<isolated-scene-sample-root>` 必须包含测试副本 `Scene/<id>`；不得直接传入真实 `~/Movies/MyWallpaperX/创意工坊/Scene`。当前完整快照门为 `.codex/scene-bc-full45-20260725/report.json`，当前固定回归门为 `.codex/scene-bc-fixed13-20260725/report.json`，BC 定向门为 `.codex/scene-bc-targeted-20260725/report.json`，Puppet animation 定向门为 `.codex/scene-puppet-animation-20260725/targeted-v1/report.json`；Puppet attachment、`particle/halo_4`、X-Ray 和视觉校准报告只保留为对应阶段证据，不能反向覆盖当前结论。
...
- **执行子集是受限白名单，不是通用引擎**：十一类 strict effect backend、puppet bind-pose、13 个程序化 built-in 粒子纹理和受限 provider/dependency 链都按完整 fingerprint 准入、fail closed；generic material/shader executor、真实 history consumer、generic compose、condition/function 未实现。
