...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v22：继承 v21 exact stock Opacity、ShaderContract、provider metadata 与 authored graph，并保存 direct text content/point-size/color binding program；exact Shake 没有新增 wire 字段，启动时从 authored graph/ShaderContract 规划。当前实现基线 `e505a9e` 的 renderer 消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow、exact stock Opacity 与 exact stock Shake 六个严格注册 backend；Shake 以 RG8 flow、可选 R8 phase/white fallback 和统一 scene time 在 Precise Blur 前后按作者顺序进入 chain。同帧 graph command foundation、受限 unique history seed/clear、Precise Blur material-command interleave 与 exact legacy compose 继续生效。其他 topology、dynamic Shake、真实 history consumer、generic compose/scene-background、condition/function、SceneScript/time/media 继续 fail closed 或整场重建；这不是任意 graph、material、shader 或完整 SceneScript/media executor。

当前运行证据分两层：`.codex/scene-shake-20260724/full26-final/report.json` 对当前真实目录的 26 个隔离样本 **26/26 通过**，同一签名 App 的 `.codex/scene-shake-20260724/fixed13-final/report.json` 固定回归门 **13/13 通过**。当前 26 门验证 image `330/330`、text `212/212`、solid `126/126`、particle `47/68`，named capture `8/8`、binding `9/9`、strict stage 30、chain 3、Shake 3、failed 0；仍有 13 个 named-target gap、64 个 route-only effect、16 个 legacy blocked layer 和 9 个缺失资源，graph target 为 153、blocker 0。未修改 `2802243144` 的 layers `[41,64,115]` 首次执行三条 Blur/Shake chain；固定 13 门继续保护 14 stage、1 条 Blur/Shadow chain、Opacity 4、Workshop Shadow 1、failed 0。当前 Scene 全量测试共 **292 项，其中 289 项通过、3 项按环境/fixture 跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `0d7efcabf4b99a101a3341e64fb10d80ef8a4020`、可执行文件 SHA-256 `7cdb04292a891e7787d0fa489c8fbeb5337332e436bc0ea2de1b9827a135cbca`。这是近期首次让未修改样本的 strict stage、chain、blocked 与动态像素同时改善；仍不能解释为 WE 视觉一致。
...
- benchmark 通过签名 App 身份、隔离样本/HOME、Metal ready/after 双帧、语义字段、utility/named/image-blend/particle planned 与 finished layer ID、authored graph canonical SHA、precise/standard/Local Contrast/Workshop Shadow backend exact succeeded/failed/legacy-blocked layer ID、ordered chain/stage 精确计数、live identity/visible-change，以及 stop 后 surface=0 验证当前 13 样本；七个 effect-graph 代表样本锁定 chain/stage 正负门。
...
- 当前 26 样本完整快照门验证 image `330/330`、solid `126/126`、text `212/212`、particle `47/68`，named capture `8/8`、binding `9/9`；strict graph 合计 30 stage、3 chain、Shake 3、0 failed，仍有 13 个 named-target gap、64 个 route-only effect、16 个 legacy blocked layer 和 9 个缺失资源。固定 13 样本门另行保护 14 stage、1 条 Blur/Shadow chain、Opacity 4 与 Workshop Shadow 1。没有完整性能预算，也没有 Windows Wallpaper Engine 同配置录屏；下一视觉门使用样本封面在固定画布上检查主构图、主体位置、色调/亮度和明显效果范围，但不能把封面或 26/26、13/13 等同于动态/像素 parity。

## 6. Scene 演进方向

### 阶段 0：产品边界与执行门（已确定方向，进行中）

当前仍选择“可审计的兼容 Runtime”方向。Effect-definition IR、authored graph planner、ShaderContract v1、六个 strict backend、ordered strict chain、resource identity/property fallback、file-backed `sceneTexture`、Frame Context、B0 live program、direct dynamic text、同帧 copy/swap、受限 unique history、Precise Blur interleave/legacy compose 与 exact stock Shake 已完成。`e505a9e` 证明按完整链缺失 backend 选择工作可以让未修改样本新增 6 stage、3 chain，并改善真实动态像素。后续按完整 chain 解锁数、未修改样本新增 stage、封面方向性视觉差异和单位实现成本重排 SceneScript、常见 backend、history、shader 与 provider；route-only 只作布局诊断。
...
- 六个 strict backend、ordered strict chain、Bloom、程序化 solid、静态与 direct-property 动态 text、camera cover/显式 parallax、受限水波/合成、单层 built-in UV foliage、9 个 built-in 粒子纹理/sprite trail、utility/named target、静态 image blend、file-backed `sceneTexture`、Frame Context、B0 live 主链、受限 unique history、Precise Blur interleave/legacy compose 与 exact stock Shake 已落地；v22 记录 direct text 合同。下一批以 [覆盖台账](../scene/semantics/coverage-ledger.md) 和 [Scene 播放能力开发计划](../scene/scene-capability-development-plan-2026-07-22.md) 的完整链/封面方向性视觉门结论为准。
