...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v22；实现基线 `c06b0fb` 的 renderer 消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow、exact stock Opacity、Shake、Water Waves、Water Flow、Foliage Sway、Water Ripple 与 X-Ray 十一类严格注册 backend，并生成 11 个精确登记的项目自有 built-in 粒子纹理。动态 profile 按完整 definition/material/shader/resource fingerprint 准入，并在同一 ordered scheduler 保持作者顺序；X-Ray 另有明确受限的前缀执行边界。默认 96 MiB pool 只保证 6 个全尺寸 BGRA texture unit，超预算链在规划阶段整链拒绝。同帧 graph command、受限 unique history seed/clear、Precise Blur interleave 与 exact legacy compose 继续生效；generic material pass、authored shader preprocessing/translation、真实 history consumer、generic compose/scene-background、condition/function、SceneScript/time/media、粒子 child/event 生命周期仍未实现。

当前运行证据分两层：`.codex/scene-full45-halo4-final-20260724/report.json` 对当前真实目录中 45 个可运行隔离样本 **45/45 通过**；`3770500543` 因源目录缺少 `scene.pkg` 而明确排除。`.codex/scene-halo4-fixed13-20260724/report.json` 固定回归门 **13/13 通过**。45 门验证 image `449/451`、text `244/244`、solid `140/140`、particle `84/131`，named capture `8/8`、binding `9/9`、strict stage 91、chain 15、Water Flow 10、Water Waves 11、Shake 24、failed 0；固定门保护 particle `15/27`、24 stage、2 chain、Water Flow 1、Water Waves 6、Shake 1、Opacity 4、Workshop Shadow 1、failed 0。完整 Scene suite 为 **318 项：315 通过、3 项跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `74e844dbcfc942ea5b28e4467e8c7d6f6684da06`、可执行文件 SHA-256 `12d13ebd57102fa0ce8f9f3a8a95fe7672d3f9e514da7cf6e702e3c707263ab9`。样本自带 preview 是当前第一视觉依据；WaifuX SceneBake MP4 只作辅助动态参考，二者均不能解释为 WE 视觉一致。
