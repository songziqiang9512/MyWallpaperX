...
`<isolated-scene-sample-root>` 必须包含测试副本 `Scene/<id>`；不得直接传入真实 `~/Movies/MyWallpaperX/创意工坊/Scene`。当前完整快照门为 `.codex/scene-static-origin-full45-20260725/report.json`，当前源码固定回归门为 `.codex/scene-static-origin-fixed13-20260725/report.json`，static-origin 定向门为 `.codex/scene-static-origin-targeted-20260725/report.json`；Snow/Smoke texture、static child、Flare、continuous-child、Event Follow、Turbulent Velocity、Light Shafts 0、Fire、BC、Puppet animation/attachment、`particle/halo_4`、X-Ray 和视觉校准报告只保留为对应阶段证据，不能反向覆盖当前结论。
