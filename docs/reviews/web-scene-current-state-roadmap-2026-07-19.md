...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v22：继承 v21 exact stock Opacity、ShaderContract、provider metadata 与 authored graph，并保存 direct text content/point-size/color binding program。当前实现基线 `ebf44a9` 的 renderer 消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow 与 exact stock Opacity 五个严格注册 backend；同帧 graph command foundation 已按作者顺序计划 `copy/swap`，并以 Metal blit/逻辑身份交换执行；显式 `unique:true` FBO 已在读前写时进入受限 history seed/clear 生命周期；Precise Blur 两种完整白名单拓扑还可按 authored nodeIndex 在两个 material node 之间执行一个 copy 或 swap。其他 topology、真实 history consumer、compose、condition/function、SceneScript/time/media 继续 fail closed 或整场重建，也没有让真实 Fluid/Motion Blur 进入 strict stage；这不是任意 graph、material、shader 或完整 SceneScript/media executor。

当前运行证据分两层：`.codex/scene-command-interleave-20260724-003051/full26-results-pass2/report.json` 对当前真实目录的 26 个隔离样本 **26/26 通过**，同一签名 App 的 `.codex/scene-command-interleave-20260724-003051/final13-results-pass/report.json` 固定回归门 **13/13 通过**；两门有 6 个重叠、合计 33 个唯一样本快照。当前 26 门验证 image `330/330`、text `212/212`、solid `126/126`、particle `47/68`，named capture `8/8`、binding `9/9`、strict stage 24、failed 0；仍有 13 个 named-target gap、67 个 route-only effect、19 个 legacy blocked layer、9 个缺失资源和 38 个 graph blocker。该门没有真实 multi-effect chain、Workshop Shadow 或正向 interleave sample，因此固定 13 门继续保护 14 stage、1 条真实 chain、Opacity 4、Workshop Shadow 1、failed 0，interleave 的正反 GPU 证据由 synthetic fixture 提供。当前 Scene 全量测试共 **282 项，其中 280 项通过、2 项按环境跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `9dbff1cfcdff7bba12d0c8b69d5f58722a938337`、可执行文件 SHA-256 `8ccf1804bf11c3f0933abf9f55dc94295b2fc618378e19efb53cf72d2bf87e31`。两门均不能解释为 WE 视觉一致；下一主线推进真实 history consumer 与 compose。
...
- 当前 26 样本完整快照门验证 image `330/330`、solid `126/126`、text `212/212`、particle `47/68`，named capture `8/8`、binding `9/9`；strict graph 合计 24 stage、0 failed，但仍有 13 个 named-target gap、67 个 route-only effect、19 个 legacy blocked layer、9 个缺失资源和 38 个 graph blocker。固定 13 样本门另行保护当前 26 门没有覆盖的真实 strict chain 与 Workshop Shadow。没有完整性能预算，也没有与 Windows Wallpaper Engine 同配置录屏的图像差异门，因此不能把 26/26 或 13/13 等同于完整兼容或 WE parity。

## 6. Scene 演进方向

### 阶段 0：产品边界与执行门（已确定方向，进行中）

当前选择“可审计的兼容 Runtime”方向。Effect-definition IR、authored graph planner、ShaderContract v1、五个 strict backend、ordered strict chain、resource identity/property fallback、file-backed `sceneTexture`、Frame Context、B0 live program、direct dynamic text、同帧 copy/swap command foundation、受限 unique FBO history seed/clear，以及 Precise Blur 两种 material-command interleave 已完成。下一主线推进 B2 真实 history consumer 与 compose；B1 只把已经验证的文本 lifecycle 抽到 system/media/video 共用合同。Timeline、SceneScript、audio/media 以后横向接入同一 target/frame 合同；route-only 只作布局诊断。
...
2. B2 ordered strict effect-chain、target allocation、pass ordering、同帧 copy/swap command foundation、受限 unique FBO history seed/clear、Precise Blur 两种 material-command interleave、最终合成原子性、首条真实 chain 与 stock Opacity `MASK=0` live alpha 已完成；下一步在同一合同上扩真实 history consumer 与 compose，不能复制各 profile 的调度代码。
...
- 五个 strict backend、ordered strict chain、Bloom、程序化 solid、静态与 direct-property 动态 text、camera cover/显式 parallax、受限水波/合成、单层 built-in UV foliage、9 个 built-in 粒子纹理/sprite trail、utility/named target、静态 image blend、file-backed `sceneTexture`、Frame Context、B0 live 主链、显式 unique FBO 的受限 history seed/clear，以及 Precise Blur 两种 material-command interleave 已落地；v22 记录 direct text 合同。下一步以 [覆盖台账](../scene/semantics/coverage-ledger.md) 和 [Scene 播放能力开发计划](../scene/scene-capability-development-plan-2026-07-22.md) 为准推进真实 history consumer、compose 与通用 provider lifecycle。
...
Scene 的基础架构成立，运行能力仍是明确子集，并已有签名 App、26 个当前真实目录隔离样本加 13 个固定回归样本、Metal 双帧/语义/dependency/strict graph/释放门。v22 已把 direct text 三类属性接入 B0 snapshot，并以异步 per-layer generation/stale cancellation 闭合首个动态 provider；B2 已增加严格的同帧 copy/swap plan/runtime foundation、显式 unique FBO 的受限 history seed/clear 生命周期和 Precise Blur 两种 material-command interleave。五个 strict backend、1 条真实 chain、静态 blend、direct text、copy/swap/history foundation、受限 interleave 和 ShaderContract source 保留仍不等价于 authored shader execution、真实 history consumer、SceneScript/media 或完整属性系统。当前可以声明“受限能力可用、direct dynamic text property 可无重建更新、两种 Precise Blur interleave 有 GPU 正反门”，不能声明达到或接近 WE parity。下一步是真实 history consumer、compose 和通用 provider lifecycle，再接 Timeline/SceneScript、audio/media、Particle 与更多 backend。
