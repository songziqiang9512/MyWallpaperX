...
Scene 已建立独立模块、PKGV 读取、受控缓存、typed interpretation、纹理解码、Metal 渲染和桌面宿主。当前链路为 interpretation v22：继承 v21 exact stock Opacity、ShaderContract、provider metadata 与 authored graph，并保存 direct text content/point-size/color binding program；exact Shake 没有新增 wire 字段，启动时从 authored graph/ShaderContract 规划。当前实现基线 `3194ac5` 的 renderer 仍消费 precise Blur、standard-default Blur、stock Local Contrast、exact Workshop Shadow、exact stock Opacity 与 exact stock Shake 六个严格注册 backend；Shake 以 RG8 flow、可选 R8 phase/white fallback 和统一 scene time 在 Precise Blur 前后按作者顺序进入 chain。同帧 graph command foundation、受限 unique history seed/clear、Precise Blur material-command interleave 与 exact legacy compose 继续生效；benchmark 新增非阻断 preview 方向性证据，不改变 renderer、viewport 或 cover 投影。其他 topology、dynamic Shake、真实 history consumer、generic compose/scene-background、condition/function、SceneScript/time/media 继续 fail closed 或整场重建；这不是任意 graph、material、shader 或完整 SceneScript/media executor。

当前运行证据分两层：`.codex/scene-shake-20260724/full26-final/report.json` 对当前真实目录的 26 个隔离样本 **26/26 通过**，同一签名 App 的 `.codex/scene-preview-visual-20260724/fixed13-final/report.json` 固定回归门 **13/13 通过**。当前 26 门验证 image `330/330`、text `212/212`、solid `126/126`、particle `47/68`，named capture `8/8`、binding `9/9`、strict stage 30、chain 3、Shake 3、failed 0；仍有 13 个 named-target gap、64 个 route-only effect、16 个 legacy blocked layer 和 9 个缺失资源，graph target 为 153、blocker 0。未修改 `2802243144` 的 layers `[41,64,115]` 首次执行三条 Blur/Shake chain；固定 13 门继续保护 14 stage、1 条 Blur/Shadow chain、Opacity 4、Workshop Shadow 1、failed 0，并为 13/13 生成 preview reference 与中心裁切并排图。当前 Scene 全量测试共 **297 项，其中 294 项通过、3 项按环境/fixture 跳过**。签名 App 为 `2.0.8 (268)`、Team `H9QWU9XN8R`、CDHash `0d7efcabf4b99a101a3341e64fb10d80ef8a4020`、可执行文件 SHA-256 `7cdb04292a891e7787d0fa489c8fbeb5337332e436bc0ea2de1b9827a135cbca`。这是近期首次让未修改样本的 strict stage、chain、blocked 与动态像素同时改善；preview 证据只支持同一样本跨提交对照，仍不能解释为 WE 视觉一致。
...
`<isolated-scene-sample-root>` 必须包含测试副本 `Scene/<id>`；不得直接传入真实 `~/Movies/MyWallpaperX/创意工坊/Scene`。当前现役完整门为 `.codex/scene-shake-20260724/full26-final/report.json`，固定回归门为 `.codex/scene-preview-visual-20260724/fixed13-final/report.json`，对应视觉校准门为 `.codex/scene-preview-visual-20260724/calibration6-final/report.json`，Shake 定向门为 `.codex/scene-shake-20260724/targeted-contract-final/report.json`；`.codex/scene-legacy-compose-20260724/full26-results-pass3/report.json`、`.codex/scene-workshop-shadow-final13-20260723-1540/report.json`、`.codex/scene-effect-chain-gated-final13-20260723/report.json`、`.codex/scene-local-contrast-final13-20260723/report.json`、`.codex/scene-shader-contract-final13-v2-20260723/report.json` 与其他 frame-context、file-property、v16 canonical graph、utility、particle-trail、static-fallback、v15 IR 报告只保留为对应阶段证据，不能反向覆盖当前结论。
...
- benchmark 通过签名 App 身份、隔离样本/HOME、Metal ready/after 双帧、语义字段、utility/named/image-blend/particle planned 与 finished layer ID、authored graph canonical SHA、precise/standard/Local Contrast/Workshop Shadow backend exact succeeded/failed/legacy-blocked layer ID、ordered chain/stage 精确计数、live identity/visible-change，以及 stop 后 surface=0 验证当前 13 样本；七个 effect-graph 代表样本锁定 chain/stage 正负门。`3194ac5` 另把 `project.json.preview` 的 GIF 首帧/JPG 静帧转换为 reference，将运行截图中心裁到相同比例并生成分项指标和并排图；该路径明确非阻断。
...
当前仍选择“可审计的兼容 Runtime”方向。Effect-definition IR、authored graph planner、ShaderContract v1、六个 strict backend、ordered strict chain、resource identity/property fallback、file-backed `sceneTexture`、Frame Context、B0 live program、direct dynamic text、同帧 copy/swap、受限 unique history、Precise Blur interleave/legacy compose 与 exact stock Shake 已完成。`e505a9e` 证明按完整链缺失 backend 选择工作可以让未修改样本新增 6 stage、3 chain，并改善真实动态像素；`3194ac5` 建立了同样本 preview 并排证据，避免继续只看运行计数。后续按完整 chain 解锁数、未修改样本新增 stage、同一样本拼图/分项变化和单位实现成本重排 SceneScript、常见 backend、history、shader 与 provider；route-only 和跨样本视觉分数都不能单独决定优先级。
