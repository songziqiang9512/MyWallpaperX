...
def effect_runtime_disposition_preview() -> str:
    return "\n".join([
        ...
        "effectStageAdmission: layer=2 effect=0 descriptor=2%23effect%230 activity=active admission=admitted-dedicated coverage=complete backend=standard-blur profile=- reason=- path=effects/blur/effect.json",
        ...
        "effectStageRuntimeDisposition: layer=2 effect=0 descriptor=2%23effect%230 kind=dedicated attribution=exact-key family=standard-blur group=2 role=owner reason=- path=effects/blur/effect.json",
...
"""
    ...
    def test_effect_stage_admission_metrics_are_structured_and_conserved(self) -> None:
        ...
        preview = "\n".join([
            ...
            "effectStageAdmission: layer=1 effect=1 descriptor=1%23effect%231 activity=active admission=admitted-dedicated coverage=complete backend=standard-blur profile=- reason=- path=effects/blur/effect.json",
        ...
        invalid_combination = preview.replace(
            ...
            "admission=not-admitted coverage=complete backend=standard-blur profile=- reason=unified-capability-unavailable",
        )
