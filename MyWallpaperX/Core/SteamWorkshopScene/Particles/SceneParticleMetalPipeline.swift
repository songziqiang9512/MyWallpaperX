...
fragment float4 sceneParticleFrag(
    ...
    float4 first = texture.sample(sampler2d, in.uv0 * colorUVScale);
    float4 second = texture.sample(sampler2d, in.uv1 * colorUVScale);
...
fragment float4 sceneParticleRefractFrag(
    ...
    constexpr sampler sampler2d(filter::linear, address::clamp_to_edge);
    float4 colorFirst = colorTexture.sample(sampler2d, in.uv0 * uvScales.xy);
    float4 colorSecond = colorTexture.sample(sampler2d, in.uv1 * uvScales.xy);
    ...
    float4 normalFirst = normalTexture.sample(sampler2d, normalUV0);
    float4 normalSecond = normalTexture.sample(sampler2d, normalUV1);
    ...
    float3 background = backgroundTexture.sample(sampler2d, backgroundUV).rgb;
