...
extension SceneAuthoredGodraysPlanner: SceneEffectStageGraphCandidatePlanner {
    ...
    /// Stock radial God Rays is owned by the source-proven MaterialProgram
    /// profiles. Keep the exact planner only as an oracle and as the retained
    /// legacy Directional owner; a failed stock Program must preserve the
    /// effect-entry current instead of reviving the old fixed renderer.
    nonisolated static func compile(
        _ input: SceneEffectStageCompileInput
    ) -> SceneEffectStageBackendCompileResult<DedicatedPlan> {
        let result = SceneEffectStageDedicatedCompilerAdapter.compile(
        ...
        guard case let .accepted(plan) = result,
              plan.direction == nil,
              !plan.usesDirectionalGaussianKernel else {
            return result
        }
        return .rejected(.init(
            backend: compilerBackend,
            phase: .compatibility,
            code: .dedicatedProfileRejected,
            details: ["stock-radial-owner-revoked-to-material-program"]
        ))
    }
}
