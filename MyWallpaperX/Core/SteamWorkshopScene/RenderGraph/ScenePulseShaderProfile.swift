...
/// - `directPhaseSaturateV1`：历史版，`sin(t*speed + phase)` 无偏移、noise UV (1, 0.333)、
///   noisespeed 默认 0.1 / range [0,0.5]、输出 `saturate(rgba)`；
/// - `directPhaseMaxClampV1`：主体同上，输出改 `max(0, rgb)`（两个 fragment 写法变体，
///   `CAST3(0)` 与字面 `0`，各自独立指纹）。
/// 另一种 `#ifndef AUDIOPROCESSING` 变体的生效公式取决于 WE combo
/// 注入行为，无证据判定 vert/frag 哪条路径生效，继续 fail closed。
nonisolated enum ScenePulseShaderProfile: Equatable {
    case stock2842
    case directPhaseSaturateV1
    case directPhaseMaxClampV1
    ...
    var phaseOffset: Float {
        ...
        case .directPhaseSaturateV1, .directPhaseMaxClampV1: 0
        }
    }
    ...
    var noiseUVScale: SIMD2<Float> {
        ...
        case .directPhaseSaturateV1, .directPhaseMaxClampV1: SIMD2(1, 0.333)
        }
    }
    ...
    var noiseSpeedDefault: Double {
        switch self {
        case .stock2842: 0.5
        case .directPhaseSaturateV1, .directPhaseMaxClampV1: 0.1
        }
    }
    ...
    var noiseSpeedRange: ClosedRange<Double> {
        switch self {
        case .stock2842: 0 ... 1
        case .directPhaseSaturateV1, .directPhaseMaxClampV1: 0 ... 0.5
        }
    }

    /// 输出行：`saturate(albedo)` 或 `vec4(max(0, rgb), a)`。
    var saturatesOutput: Bool {
        self == .directPhaseSaturateV1
    }

    /// 旧编辑器定义省略了 `replacementkey`；只对已登记的 v1 shader 指纹兼容。
    var acceptsMissingReplacementKey: Bool {
        switch self {
        case .stock2842: false
        case .directPhaseSaturateV1, .directPhaseMaxClampV1: true
        }
    }
    ...
    private static let directPhaseVertexSHA256 =
