...
/// Physical layout for one admitted layer chain. Pair rotation is owned only
/// by `SceneLayerFullFramePairPlan`; this type assigns its two members and the
/// authored FBOs to non-overlapping resident slots.
nonisolated struct SceneGraphRenderTargetChainPlan: Equatable {
    ...
    /// cache scope remains this layer chain until cross-owner aliasing is
