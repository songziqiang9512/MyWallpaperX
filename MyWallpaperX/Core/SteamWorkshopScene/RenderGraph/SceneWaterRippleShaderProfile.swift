...
///
/// 已验证的 authored variants 携带 3 种 waterripple shader 源，核心数学
...
/// - `invertedScrollV1`：无 PERSPECTIVE/MASK
...
nonisolated enum SceneWaterRippleShaderProfile: Equatable {
    ...
    var scrollDirectionOffset: Float {
        self == .invertedScrollV1 ? .pi : 0
    }

    /// effect.json 是否要求 stock 的 PERSPECTIVE gizmos 声明（v1 variants 均无）。
    var expectsPerspectiveGizmos: Bool {
        self == .stock2842
    }

    /// 更早的 effect.json 变体没有 replacementkey。
    var acceptsMissingReplacementKey: Bool {
        self == .invertedScrollV1
    }

    /// v1 语料实例只写非默认常量键，准入放宽为白名单键的子集。
