...
`Fonts/` 下的原版保持原始文件名（解析器按作者写的 basename 匹配），替代字体用自己的
真实文件名，由 `SceneTextFontResolver` 做名字映射，不伪装成原版。
...
| 原版引用 | 替代文件 | 选择依据 |
|---|---|---|
| `Atami-Regular` | `Poppins-Medium.ttf` | 同为几何无衬线，单层 a、圆形 O、字重与比例吻合 |
| `Alcubierre` | `Poppins-ExtraLight.ttf` | 同为极细几何无衬线，线宽与圆形结构接近 |
| `spincycle_3d_ot` | `BungeeShade-Regular.ttf` | 同为宽扁立体描边大写字 |
| `summer85` | `PermanentMarker-Regular.ttf` | 同为粗马克笔 display，笔触质感与字重接近 |
| `Lazer84` | `PermanentMarker-Regular.ttf` | 同为粗笔刷全大写 display |
| `kust` | `Bangers-Regular.ttf` | 同为粗大写手绘 display |
| `CursedTimerUlil-Aznm` | `Segment7Standard.otf` | 同为七段数码管字体，直接复用已随包的原版 |
