...
现有 RenderGraph 二级职责：`AuthoredGraph` 负责 authored graph IR、结构规划与 graph/condition admission，不包含 execution backend、target 生命周期、dependency 调度或 GPU encoding；`EffectExecution` 负责统一 GraphExecutor 的 stage preparation/encoding/execution；`GraphTargets` 负责 authored graph target 的计划、状态、分配、驻留、资源命令与 publication；`LayerDependencies` 负责跨 layer dependency plan、按帧 named target reservation/publication 与 named texture pool，其 named target 不属于 authored effect FBO/history；`MaterialProgram` 负责 resolved material schema、Template compilation、Program identity/derivation、ingress/script/loop 分类与 host uniform encoding，M1 后 frame/resource finalization 与 texture resolution 暂留 RenderGraph 根目录，RuntimeCatalog 不并入，GPU execution 仍属于 `EffectExecution`；`ShaderFrontend` 负责有界作者 shader 的词法、语法、语义/颜色证明、翻译与 Metal source emission；`ShaderPreparation` 负责 directive/macro 预处理、variant 环境与解析、prepared source 产生及畸形 metadata 准入。`SceneShaderContract`、`SceneShaderSourceGraph`、FrameInputs、`ShaderFrontend` 和 resolved Program 均属于其他生命周期，不并入 `ShaderPreparation`。
