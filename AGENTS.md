...
现有 RenderGraph 二级职责：`AuthoredGraph` 负责 authored graph IR、结构规划与 graph/condition admission，不包含 execution backend、target 生命周期、dependency 调度或 GPU encoding；`EffectCompilation` 负责从 authored stage graph 切片、dedicated candidate/backend 编译、stage Program/ExecutionPlan 到全链 admission catalog/reporting 的编译与准入生命周期，不包含 effect-specific planner 的完整准入实现、MaterialProgram 生产、GPU encoding 或 target/resource 生命周期；`EffectExecution` 负责 launch-scoped effect pipeline view 以及统一 GraphExecutor 的 stage preparation/encoding/execution；`GraphTargets` 负责 authored graph target 的计划、状态、分配、驻留、资源命令与 publication；`LayerDependencies` 负责跨 layer dependency plan、按帧 named target reservation/publication 与 named texture pool，其 named target 不属于 authored effect FBO/history；`MaterialProgram` 负责从 ingress、Template compilation、schema/variant preparation 到 Program identity/derivation、texture selection、frame finalization 与 uniform encoding 的完整 producer lifecycle，RuntimeCatalog 不并入，GPU execution consumer 仍属于 `EffectExecution`；`ShaderContract` 负责 loss-preserving shader contract/schema、stage metadata 解析、immutable source-graph contract 与 root/VFS contract loading/projection，不包含具体 VFS/resource resolution、preparation、frontend、MaterialProgram 或 GPU execution；`ShaderFrontend` 负责有界作者 shader 的词法、语法、语义/颜色证明、翻译与 Metal source emission；`ShaderPreparation` 负责 directive/macro 预处理、variant 环境与解析、prepared source 产生及畸形 metadata 准入。`ShaderContract`、FrameInputs、`ShaderFrontend` 和 resolved Program 均属于其他生命周期，不并入 `ShaderPreparation`。

现有 Runtime 二级职责：`ResolvedMaterialExecution` 负责 surface-scoped runtime façade，以及从 claim/preflight、target commit、ordered encode/compositor ticket consumption 到 command-buffer completion、rollback、epoch invalidation 和 terminal graph observation 的原子提交生命周期；scene-lifetime RuntimeCatalog、通用 graph telemetry、RenderGraph GPU executor 与 Rendering consumer 不并入。
